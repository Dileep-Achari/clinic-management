...
const _notifications = require("../../../schedule-jobs/cm-notifications")
...
async function generateToken(_data) {
    if (_data.expireToken && _data.expireToken.length > 0) {
        let expireTokenIn = `${_data.expireToken * 60 * 1000}ms`;
        return await _token.createTokenWithExpire(_data, expireTokenIn);
    }
    else {
...
async function insertDefaultData_bkp(_data, _idx, _output, _dbType, _orgData, _sessionId) {
    try {
...
async function insertDefaultData_bkp1(_data, _idx, _output, _dbType, _orgData, _sessionId) {
    try {
        if (_data.length > _idx) {
            // _.each(_data[_idx].data, (_o) => {
            //     _o.orgId = _orgData._id;
            ...
            if (_data[_idx].depColl && _data[_idx].depColl.length == 0) {
                ...
                if (_data[_idx].type === "ICD_MASTERS" || _data[_idx].type === "PINCODES") {
                    let _filter = {
                        filter: [
                            {
                                $project: {
                    ...
                    let _mResp = await _mUtils.commonMonogoCall(_data[_idx].coll, "aggregation", _filter, "", "", "", "rh")
                    if (!(_mResp && _mResp.success)) {
                        _output.push({ success: false, type: _data[_idx].type, desc: _mResp.desc || "", data: [] });
                    }
                    else {
                        _data[_idx].data = _mResp.data
                    }
                }
...
const addOptionalSpacesRegex = (str) => {
    return str.split('').join('\\s*');
}

async function processGrnEntriesOld(grnData, req, results = { success: [], failed: [] }) {
    if (grnData.length === 0) {
        if (results.failed.length > 0) {
            return { success: false, status: 400, desc: "some updates Failed", data: results }
        }
        return { success: true, status: 200, data: results.success };
    }

    const grnItem = grnData[0];
    let _filter = {
        "filter": { 'batchNumber': grnItem.batchNumber, '_id': mongoose.Types.ObjectId(grnItem._id), 'recStatus': true },
        "selectors": "-audit -history"
    };
    try {
        const grnResult = await mongoMapper('cm_grn_entries', 'find', _filter, req.tokenData.dbType);
        if (grnResult.data && grnResult.data.length > 0) {
            let decreaseQuantity = grnItem.quantity;
            let updatedQuantity = grnResult.data[0].quantityPerBatch - decreaseQuantity
            let updateRecStatus = updatedQuantity <= 0 ? false : true;

            let _uParams = {
                params: {
                    _id: grnResult.data[0]._id,
                    quantityPerBatch: updatedQuantity,
                    recStatus: updateRecStatus
                }
            }
            let pLoadResp = { payload: {} };
            pLoadResp = await _mUtils.preparePayload('U', _uParams);
            if (!pLoadResp.success) {
                results.failed.push({ _id: grnItem._id, desc: pLoadResp.desc });
                return processGrnEntries(grnData.slice(1), req, results);
            }
            try {
                await mongoMapper('cm_grn_entries', 'findOneAndUpdate', pLoadResp.payload, req.tokenData.dbType);
                results.success.push({ _id: grnData._id, updatedQuantity, recStatus: updateRecStatus });
            } catch (error) {
                results.failed.push({ _id: grnItem._id, desc: error });
            }
        } else {
            results.failed.push({ _id: grnItem._id, desc: "No record found in grnEntry against batchNumber and _id" });
        }
...
async function processGrnEntries(serviceData, req, _idx, results = { success: [], failed: [] }) {
    try {
        if (serviceData.length > _idx) {
            if (serviceData[_idx].grnData && Array.isArray(serviceData[_idx].grnData) && serviceData[_idx].grnData.length > 0) {
                for (let grnItem of serviceData[_idx].grnData) {
                    let _filter = {
                        "filter": { 'batchNumber': grnItem.batchNumber, '_id': mongoose.Types.ObjectId(grnItem._id), 'recStatus': true },
                        "selectors": "-audit -history"
                    };
                    ...
                    if (grnResult.data && grnResult.data.length > 0) {
                        ...
                        try {
                            await mongoMapper('cm_grn_entries', 'findOneAndUpdate', pLoadResp.payload, req.tokenData.dbType);
                            results.success.push({ _id: grnItem._id, updatedQuantity, recStatus: updateRecStatus });
                        } catch (error) {
                            results.failed.push({ _id: grnItem._id, desc: error });
                        }
                    } else {
            ...
            _idx = _idx + 1;
            await processGrnEntries(serviceData, req, _idx, results);
        }
        else {
            return { success: true, data: results }
        }
        //let _final = _.filter(results, (_r) => { return !_r.success });
        return {
            "success": results.failed.length > 0 ? false : true,
            "data": results
        }
    }
    catch (error) {
        return { success: false, data: [], desc: error.message || error }
    }
}
            ...
            if (_data[_idx].depColl && _data[_idx].depColl.length == 0) {
                ...
                if (_data[_idx].type === "ROLES") {
                    ...
                    if (!(_mResp && _mResp.success)) {
                        ...
                        _.each(_data[_idx].data, (_role) => {
                            let _docs = [];
                            if (Array.isArray(_role.documents) && _role.documents.length > 0) {
                                _.each(_role.documents, (_doc) => {
                                    let document = _.find(_mResp.data, (_d) => { return _d.docmntName == _doc })
                                    _docs.push({
                                        documentId: document._id,
                                        documentCd: document.cd || "",
                                        groupCd: document.groupCd,
                                        groupName: document.groupName,
                                        iconClass: document.iconClass || "",
                                        documentName: document.docmntName,
                                        docmntUrl: document.docmntUrl,
                ...
                if (_data[_idx].type === "LABELSMAPS") {
                    ...
                    let _labels = await _mUtils.commonMonogoCall("cm_labels", "find", _labelFilter, "", "", "", _dbType)
        ...
        if (typeof media == "object" && Object.keys(media).length > 0) {
            for (let _med in media) {
                ...
                if (_resp.success !== true) {
                    ...
                    if (_type == "I") {
                        let _insrtMediaPrms = {
                            referenceId: _data[0]._id,
                            [`${_med}`]: _resp._bufferData,
                            [`${_med}MimeType`]: _resp._mimeType,
                            referenceType: `${coll.split("_")[1]}_${_med}`
                        }
                        if (_med == "orglogo" || _med == "favIcon") {
                            _insrtMediaPrms['referenceType'] = req.body.params.dbType || req.tokenData.dbType;
                        }
                        let _mediaResp = await _mUtils.commonMonogoCall("cm_media", "insertMany", _insrtMediaPrms, "", "", "", req.tokenData.dbType);
                        ...
                        let _mediaId = req.body.params[`${_med}MediaId`]
                        if (req.body && req.body.params && !(_mediaId && _mediaId.length > 0)) {
                            let _insrtMediaPrms = {
                            ...
                            if (req.body.params[`${_med}RecStatus`] == false) {
                                _updtMediaPrms.params['recStatus'] = false
                            }
                            ...
                            let _olResp = await _mUtils.commonMonogoCall("cm_media", "findOneAndUpdate", _updtMediapLoadResp.payload, "", "", "", req.tokenData.dbType)
...
async function bookAppointment(_type, _data, req, _docId, _flag) {
    try {
        //_data.reviewDt = `${_data.reviewDt.split('T')[0]}T00:00:00.000Z`
        let _filter = {
            "filter": {
                recStatus: true,
        ...
        if (_type == "U") {
            let _adata = await _mUtils.commonMonogoCall("cm_transactions", "findById", req.body.params._id, "", req.body.params._id, "", req.tokenData.dbType)

            if (!(_adata && _adata.success)) {
                _output.push({ success: false, status: 400, desc: _adata.desc || "", data: _adata.data || [] });
            }
            else {
                _filter.filter['UMR'] = _adata.data.UMR
                _docId = _adata.data.docDetails.docId
            }
        }

        let _patient = await _mUtils.commonMonogoCall("cm_patients", "find", _filter, "", req.body, "", req.tokenData.dbType)
        if (!(_patient && _patient.success)) {
            _output.push({ success: false, status: 400, desc: _patient.desc || "", data: _patient.data || [] });
        }

        if (_type == "U" && _data.apmntId) {

            let _mResp = await _mUtils.commonMonogoCall("cm_appointments", "findById", _data.apmntId, "", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                _output.push({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            if (_mResp.success) {
                if (Object.keys(_mResp.data).length == 0) {
                    _output.push({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
                }
                else if (Object.keys(_mResp.data).length > 0) {

                    if (_mResp.data.dateTime != _data.reviewDt) {
                        let _matchedVisit = _.filter(_patient.data[0].visits, (_visit) => { return _visit.visitId == _data.apmntId })
                        let _prms = {
                            params: {
                                _id: _patient.data[0]._id,
                                visits: [
                                    {
                                        _id: _matchedVisit[0]._id,
                                        status: "CANCELLED"
                                    }
                                ]
                            }
                        ...
                        if (!pLoadResp.success) {
                            _output.push({ success: false, status: 400, desc: pLoadResp.desc || "", data: [] });
                        }
                        let _pResp = await _mUtils.commonMonogoCall("cm_patients", 'findOneAndUpdate', pLoadResp.payload, "", "", "", req.tokenData.dbType);
                        if (!(_pResp && _pResp.success)) {
                            _output.push({ success: false, status: 400, desc: _pResp.desc || "", data: _pResp.data || [] });
                        }
                        ...
                        let _aptPLoadResp = await _mUtils.preparePayload('U', _aptPrms);
                        if (!_aptPLoadResp.success) {
                            _output.push({ success: false, status: 400, desc: _aptPLoadResp.desc || "", data: [] });
                        }
                        let _aResp = await _mUtils.commonMonogoCall("cm_appointments", 'findOneAndUpdate', _aptPLoadResp.payload, "", "", "", req.tokenData.dbType);
                        if (!(_aResp && _aResp.success)) {
                            _output.push({ success: false, status: 400, desc: _aResp.desc || "", data: _aResp.data || [] });
                        }
                    }
                }
            }
        ...
        let _output = []
        let _mResp = await _mUtils.commonMonogoCall("cm_doctors", "findById", _docId, "", req.body, "", req.tokenData.dbType)
        if (!(_mResp && _mResp.success)) {
        ...
        _data.reviewDt = new Date(_data.reviewDt);

        let shiftTimeId = ""
        let _docShift = _docLoc[0].shifts || [];
        for (let i = 0; i < _docShift.length; i++) {
            let shift = _docShift[i];
            let shiftFrom = moment(shift.from, "HH:mm").set({
                year: _data.reviewDt.getYear(),
                month: _data.reviewDt.getMonth(),
                date: _data.reviewDt.getDate()
            });
            let shiftTo = moment(shift.to, "HH:mm").set({
                year: _data.reviewDt.getYear(),
                month: _data.reviewDt.getMonth(),
                date: _data.reviewDt.getDate()
            });
            if (!_data.captureTm || _data.captureTm == "") {
                _docShift = []
                let __shift = _.reduce(_docLoc[0].shifts, (acc, current) => {
                    let curFrom = moment.utc(current.from, "HH:mm")
                    return !acc || curFrom.isBefore(moment.utc(acc.from, "HH:mm")) ? current : acc;
                })
                _docShift.push(__shift)
                _data.reviewDt = new Date(_data.reviewDt.setHours(parseInt(_docShift[0].from.split(':')[0]), parseInt(_docShift[0].from.split(':')[1])))
            }
            else if (!_data.captureTm != undefined && _data.captureTm.length > 0) {
                _data.reviewDt = new Date(_data.reviewDt.setHours(parseInt(_data.captureTm.split(':')[0]), parseInt(_data.captureTm.split(':')[1])))
                if (_data.reviewDt >= shiftFrom && _data.reviewDt <= shiftTo) {
                    _docShift = shift;
                    break;
                } else if (_data.reviewDt < shiftFrom) {
                    _data.reviewDt = shiftFrom;
                    shiftTimeId = shift._id;
                    break;
                } else if (_data.reviewDt > shiftTo && i + 1 < _docShift.length) {
                    let nextShift = _docShift[i + 1];
                    let nextShiftFrom = moment(nextShift.from, "HH:mm").toDate();
                    if (_data.reviewDt >= shiftTo && _data.reviewDt < nextShiftFrom) {
                        _data.reviewDt = nextShiftFrom;
                        shiftTimeId = nextShift._id;
                        break;
                    }

                } else if (_data.reviewDt > shiftTo && i === _docShift.length - 1) {
                    shiftTimeId = shift._id;
                    break;
                }
            }
        ...
        else if (_flag === "ONLINE") {
            _amount = parseFloat(_docLoc[0].fees.online);
        }
        else if (_flag === "REVISIT") {
            _amount = parseFloat(_docLoc[0].fees.reVisit);
        }
        else if (_flag === "REVIEW") {
            _amount = parseFloat(_docLoc[0].fees.reVisit);
        }
        else if (_flag === "EMERGENCY") {
        ...
        let _params = {
            ...
            "UMR": _patient.data[0].UMR || "",
            "patId": _patient.data[0]._id || "",
            "titleCd": _patient.data[0].titleCd || "",
            "titleName": _patient.data[0].titleName || "",
            "patName": _patient.data[0].dispName || "",
            "age": _patient.data[0].age || "",
            "admnNo": req.body.params.admnNo || "",
            "admnDt": req.body.params.admnDt || "",
            "gender": _patient.data[0].gender || "",
            "email": _patient.data[0].email || "",
            "isVIP": _patient.data[0].isVIP || false,
            "mobile": _patient.data[0].mobile || "",
            "address": _patient.data[0].homeAddress.address1 || "",
            "reasonForVisit": "",
        ...
        let _gPayload = await generateSlotsPayload(_params, _data.reviewDt, _docShift[0].duration, 1, 0, [], "", "", req);
        ...
        let _appointment = await _mUtils.commonMonogoCall("cm_appointments", "insertMany", _finalResp, "", req.body, "", req.tokenData.dbType)

        if (!(_appointment && _appointment.success)) {
            _output.push({ success: false, desc: _appointment.desc || "", data: _appointment.data || [] });
        }
        else {

            let _obj = {
                "visitId": _appointment.data[0]._id,
                "dateTime": _appointment.data[0].dateTime,
                "docName": _appointment.data[0].docDetails.name,
                "docId": _appointment.data[0].docDetails.docId,
                "locId": req.tokenData.locId,
                "visit": "OP",
                "visitNo": _appointment.data[0].code || "",
                "status": _appointment.data[0].status,
            };
            let _patResp = await updateVisitsOrBillData(_filter.filter.UMR, "VISITS", _obj, req);
...
async function checkIfSlotBooked_old(locId, docId, _apmntTime, req) {
    let _cParams = {
        "filter": {
            "locId": locId,
...
async function checkIfSlotBooked(locId, docId, _apmntTime, req) {
    if (!req.body.flag || (req.body.flag != "stillBookSlot" && req.body.flag.trim() === "")) {
        if (req.body.params.UMR) {
            ...
            let _mResp = await _mUtils.commonMonogoCall("cm_appointments", 'findOne', _cParams, "", "", "", req.tokenData.orgKey.toLowerCase());
            if (_mResp && _mResp.success == true && _mResp.data && _mResp.data.dateTime == _apmntTime) {
                //return true;//slot is already booked
                //return { success: true, desc: `This patient already has an appointment with Dr.${_mResp.data.docDetails.name} on ${_mResp.data.dateTime}. Do you still want to proceed?`, data: [] };
                let utcDateTime = _mResp.data.dateTime
                let istDateTime = moment.utc(utcDateTime).add(5, 'hours').add(30, 'minutes').format("DD-MM-YYYY hh:mm A");
                return { success: true, desc: `This patient already has an appointment with Dr.${_mResp.data.docDetails.name} on ${istDateTime}. Do you still want to proceed?`, data: [] };
            }
            else {
                // return false;// slot is not booked, allowing for booking
                let _dParams = {
                ...
                let _dResp = await _mUtils.commonMonogoCall("cm_appointments", 'findOne', _dParams, "", "", "", req.tokenData.orgKey.toLowerCase());
                if (_dResp && _dResp.success == true && _dResp.data && _dResp.data.dateTime == _apmntTime) {
                    return true;//slot is already booked
                }
                else {
                    return false;// slot is not booked, allowing for booking
    ...
    else if (req.body.flag.length > 0 && req.body.flag == "stillBookSlot") {
        let _cParams = {
            "filter": {
                "locId": locId,
                "docDetails.docId": mongoose.Types.ObjectId(docId),
...
async function generateSlotsPayload(_data, _time, _duration, _cnt, _idx, _output, _coll, _method, req) {
    try {
        if (_idx < _cnt) {
            ...
            else if (isSlotBooked && isSlotBooked.success === true) {
                return { success: false, desc: isSlotBooked.desc, data: [] };
            } 
            else {
    ...
    try {
        return new Promise(async (resolve, reject) => {
            ...
            if (_type && _type === "BILLS") {
                ...
                patData.params.visitTrans = visitTransUpd;
            }
...
async function completeTransactionBill(_umrNo, req, res) {
    try {
        return new Promise(async (resolve, reject) => {
            ...
            let _updateResp = await insertUpdateInMultiData(patData, 0, [], req.tokenData.dbType, 'cm_transactions', 'bulkWrite', 'BW')
    ...
    try {
        if (req.query && req.query.orgKey) {
            ...
            mongoMapper("cm_organization", "find", _filter, req.query.orgKey.toLowerCase()).then(async (result) => {
                ...
                if (_parseData[0].locations && _parseData[0].locations.length > 0) {
                    for (let location of _parseData[0].locations) {
                        if (location.nabhLogo && location.nabhLogo !== "" && location.nabhMimeType && location.nabhMimeType.length > 0) {
                            ...
                            location['nabhLogo'] = _base64nabh ? `${location.nabhMimeType},${_base64nabh}` : "";
                ...
                if (_parseData[0].locations && _parseData[0].locations.length > 0) {
                    for (let location of _parseData[0].locations) {
                        if (location._id && location._id.length > 0) {
                            let _cFilter = {
                                "filter": {
                                    "locId": location._id,
                                }
                            }
                            let _cntrDet = await _mUtils.commonMonogoCall("cm_counters", "find", _cFilter, "", "", "", req.query.orgKey)
                            if (_cntrDet.success && _cntrDet.data.length > 0) {
                                location['counters'] =  _cntrDet.data
                            }
                        }
                    }
                }
...
router.post("/auth-user", async (req, res) => {
    try {
        ...
        //syncchnages
        let locationDetails = (_mOrgResp.data.locations).find(loc => loc._id == (req.body.params.locId));
        if (!locationDetails) {
        ...
        let _user = {
            "orgKey": req.body.params.orgKey, "orgId": req.body.params.orgId, "locId": req.body.params.locId, "syncOrgId": _mOrgResp.data.syncOrgId, "syncLocId": locationDetails.syncLocId, "locName": _clnUser[0].locations[0].locName, "dbType": _mOrgResp.data.dbType, "createdDt": new Date(), "userId": _clnUser[0]._id, "userName": _clnUser[0].userName,
            "displayName": _clnUser[0].displayName, "roleId": _clnUser[0].locations[0].roleId, "roleName": _clnUser[0].locations[0].role, docAccess: _docString, "i_orgCd": locationDetails.i_orgCd, "i_locCd": locationDetails.i_locCd, "emrMigId": locationDetails.emrMigId, "emMigId": locationDetails.migId, "syncApiUrl": locationDetails.syncServUrl, "expireToken": locationDetails.sessionOut
        };
        //syncchanges
        // let _user = {
    ...
    try {
        if (req.body.params && req.body.params.userName && req.body.params.userName.length > 0 && req.headers.orgkey.length > 0) {
            let _filter = {
                ...
                "selectors": "-history -password  -email -otp"
            }
...
//     const excludeRoutes = ["/get-lab-result"];
        ...
        if (req.body && req.body.params && req.body.params.dbType && req.body.params.dbType.length > 0) {
            ...
            if (req.body.params.locations && req.body.params.locations.length > 0) {
                for (let location of req.body.params.locations) {
                    if (location.nabhLogo && location.nabhLogo.length > 0) {
            ...
            if (req.body.params.notificationTemplate && req.body.params.notificationTemplate.length > 0) {
                for (let _notifTemp of _cBody.params.notificationTemplate) {
                    let _seqResp = await _mUtils.getSequenceNextValue({ seqName: 'Notification', orgId: req.tokenData.orgId, }, "cm", req);
                    if (_seqResp && _seqResp.success) {
                        _notifTemp['notifCd'] = _seqResp.data;
                    }
                }
            }
            // if (_cBody.params.locations && _cBody.params.locations.length > 0 && req.body.params.locations[0].nabhLogo && req.body.params.locations[0].nabhLogo.length > 0) {
            //     req.body.params.locations[0]["nabhMimeType"] = req.body.params.locations[0].nabhLogo.split(',')[0];
            //     _nabhbase64data = req.body.params.locations[0].nabhLogo.replace(/^data:.*,/, '');
            //     req.body.params.locations[0]["nabhLogo"] = new Buffer.from(_nabhbase64data, 'base64');
            // }
                ...
                if (_roles[0].success && _roles[0].data.length > 0) {
                    _.each(_roles[0].data, (role) => {
                        let _user = {
                            ...
                            userName: role.cd == "SUPER_ADMIN" ? "superadmin" : `${result.data[0].orgKey.toLowerCase()}_${role.label.toLowerCase().replace(/\s+/g, "")}`,
                            password: role.cd == "SUPER_ADMIN" ? "softhealth" : `${result.data[0].orgKey.toLowerCase()}123`,
                            email: `${result.data[0].orgKey.toLowerCase()}@gmail.com`,
                            mobile: "98374867579",
...
function formatPayload(_obj) {
    if (_obj.constructor.name == 'Array') {
        return _obj.map(formatPayload)
    }
    if (_obj.constructor.name == 'Object' && _obj != null) {
        const _newObj = {}
        if ('_id' in _obj) {
            _newObj._id = _obj._id
        }
        for (const _key in _obj) {
            if (_key != '_id') {
                _newObj[_key] = formatPayload(_obj[_key])
            }
        }
        return _newObj
    }
...
router.post("/update-organization", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            ...
            //newly addeed
            let _locFilter = {  "filter": {"recStatus": true,_id: req.body.params._id},"selectors": "locations" }
            let locationLoc = await _mUtils.commonMonogoCall("cm_organization", "find", _locFilter, "", "", "", req.tokenData.dbType)
            if (!(locationLoc && locationLoc.success)) {
                return res.status(400).json({ success: false, status: 400, desc: locationLoc.desc || "", data: locationLoc.data || [] });
            }
            //newly added
            ...
            if (!(_hResp && _hResp.success)) {
                ...
                //newly added 
                if (locationLoc && locationLoc.data && locationLoc.data.length > 0 && locationLoc.data[0].locations.length > 1) {
                    if (_cBody.params.locations && _cBody.params.locations.length > 0) {
                        let idx = 0;
                        let lengthOfLocations = locationLoc.data[0].locations.length
                        for (let location of _cBody.params.locations) {
                            location["syncLocId"] = (Number(locationLoc.data[0].locations[0].syncLocId) + lengthOfLocations + idx).toString();
                            location["migId"] = (Number(locationLoc.data[0].locations[0].migId) + lengthOfLocations + idx).toString();
                            idx = idx+1
                        }
                    }
                }
                ...
                // if (_cBody.params.locations && _cBody.params.locations.length > 0 && _cBody.params.locations[0].nabhLogo && _cBody.params.locations[0].nabhLogo.length > 0) {
                //     _cBody.params.locations[0]["nabhMimeType"] = _cBody.params.locations[0].nabhLogo.split(',')[0];
                //     _nabhbase64data = _cBody.params.locations[0].nabhLogo.replace(/^data:.*,/, '');
                //     _cBody.params.locations[0]["nabhLogo"] = new Buffer.from(_nabhbase64data, 'base64');
                // }
                let _seqResp = []
                if(_cBody.seqValues && _cBody.seqValues.length > 0){
                    if (_cBody.params.locations && _cBody.params.locations.length > 0) {
                        for(let _seq of _cBody.seqValues){
                            for (let location of _cBody.params.locations) {
                                if(location._id && location._id.length > 0){
                                    let _cFilter = {
                                        "filter":{
                                            "locId": location._id,
                                            "seqName": _seq.seqName 
                                        }
                                    }
                                    let _cntrDet = await _mUtils.commonMonogoCall("cm_counters", "find", _cFilter, "", "", "", req.tokenData.dbType)
                                    if(_cntrDet.success && _cntrDet.data.length > 0){
                                        let _updtFilter = {
                                            "params":{
                                                "_id": _cntrDet.data[0]._id,
                                            }
                                        }
                                        _.each(_seq, (_sv, _sk) => {
                                            _updtFilter.params[`${_sk}`] = _sk == 'seqType' || _sk == 'format' ? `${_sv.toUpperCase()}` : _sv
                                        })
                                        _updtFilter.params['audit'] = req.body.params.audit;
                                        let _seqPLoadResp = await _mUtils.preparePayload("BW", _updtFilter);
                                        if (!_seqPLoadResp.success) {
                                            _seqResp.push({ success: false, status: 400, desc: _seqPLoadResp.desc || "", data: [] });
                                        }
                                        else{
                                            let _cntrs = await _mUtils.commonMonogoCall("cm_counters", "bulkWrite", _seqPLoadResp.payload, "", "", "", req.tokenData.dbType)
                                            if(!_cntrs.success){
                                                _seqResp.push({ success: false, status: 400, desc: "Error While Updating Counters", data: [] })
                                            }
                                        }
                                    }
                ...
                if(_seqResp.length > 0){
                    return res.status(400).json({ success: false, status: 400, desc: "Error While Updating Counters", data: _seqResp });
                }
                if (_cBody.params.notificationTemplate && _cBody.params.notificationTemplate.length > 0) {
                    for (let _notifTemp of _cBody.params.notificationTemplate) {
                        if (!_notifTemp._id) {
                            let _seqResp = await _mUtils.getSequenceNextValue({ seqName: 'Notification', orgId: req.tokenData.orgId, }, "cm", req);
                            if (_seqResp && _seqResp.success) {
                ...
                if (_cBody.params.locations && _cBody.params.locations.length > 0) {
                    for (let location of _cBody.params.locations) {
                        if(!(location._id && location._id.length > 0)){
                            ...
                            let _data = _.filter(_defaults, (_d) => { return _d.type == 'COUNTERS' }) 
                            let _orgDet = await _mUtils.commonMonogoCall("cm_organization", "findById", result.data._id, "", "", "", req.tokenData.dbType)
                            let _locDet = _.filter(_orgDet.data.locations, (_loc) => { return _loc.locKey == location.locKey })
                            console.log(_data)
                            let _counterData;
                            let _insrtCntrsData = []
                            if(Array.isArray(_data) && _data.length > 0){
                                _counterData = _data[0].data
                                if(Array.isArray(_counterData) && _counterData.length > 0){
                                    for(let _cData of _counterData){
                                        _cData['locId'] = _locDet[0]._id
                                        _cData['orgId'] = _orgDet.data._id
                                        _cData['locName'] = _locDet[0].locName
                                        if(_cBody.seqValues && Array.isArray(_cBody.seqValues) && _cBody.seqValues.length > 0){
                                            let _matchSeq = _.filter(_cBody.seqValues, (_seq) => { return _seq.seqName == _cData.seqName })
                                            if(Array.isArray(_matchSeq) && _matchSeq.length > 0){
                                                if(!_matchSeq[0].seqType){
                                                    _cData['seqType'] = `${_cData.seqType}${_locDet[0].locKey.toUpperCase()}`
                                                }
                                                _.each(_matchSeq[0], (_mv, _mk) => {
                                                    if(_cData[`${_mk}`] != ""){
                                                        _cData[`${_mk}`] = _mk == 'seqType' || _mk == 'format' ? `${_mv.toUpperCase()}` : _mv
                                                        }
                                                })
                                                _cData['audit'] = _audit
                                                _insrtCntrsData.push(_cData)
                                            }
                                            else{
                                                _cData['seqType'] = `${_cData.seqType}${_locDet[0].locKey.toUpperCase()}`
                                                _audit['documentedBy'] = "system"
                                                _cData['audit'] = _audit
                                                _insrtCntrsData.push(_cData)
                                            }
                                        }
                            ...
                            let _cntrs = await _mUtils.commonMonogoCall("cm_counters", "insertMany", _insrtCntrsData, "", "", "", req.tokenData.dbType)
                            if(!_cntrs.success){
                                _output.push({ success: false, status: 400, desc: "Error While Inserting Counters", data: [] })
                            }
                        }
                    }
                }
                if(_output.length > 0){
                    return res.status(400).json({ success: false, status: 400, desc: "Error While Inserting Counters", data: [] });
                }
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        ...
        if (_defaults && _defaults.length > 0) {
            _.each(_defaults, (_data) => {
                if (_data.type == "ROLES" && _data.tabsMap && _data.tabsMap.length > 0) {
                    req.body.params['tabsMap'] = _data.tabsMap
                }
            })
        }
...
router.post("/insert-employee", async (req, res) => {
    try {
        if (req.body && req.body.params && req.body.params.locations && req.body.params.locations.length > 0 && !req.body.params.locations[0]._id && req.body.params.mobile && req.body.params.mobile.length > 0 && req.body.params.emailID && req.body.params.emailID.length > 0) {
            req.body.params.orgId = req.tokenData.orgId;
            //  req.body.params.locations[0]["audit"] = req.body.params.audit;
...
router.post("/get-employee", async (req, res) => {
    try {
        ...
        if (req.body.params.locId && req.body.params.locId.length > 0) {
            _filter.filter.push({
                $match: { "locations.locId": { $eq: mongoose.Types.ObjectId(req.body.params.locId) } }
            })
        }
...
router.post("/insert-doctor", async (req, res) => {
    try {
        if (req.body && req.body.params && req.body.params.locations && req.body.params.locations.length > 0 && !req.body.params.locations[0]._id && req.body.params.mobile && req.body.params.mobile.length > 0 && req.body.params.emailID && req.body.params.emailID.length > 0) {
            ...
            //     req.body.params["photoMimeType"] = _cBody.photo.split(',')[0];
            //     _photobase64data = req.body.params.photo.replace(/^data:.*,/, '');
            //     req.body.params["photo"] = new Buffer.from(_photobase64data, 'base64');
            // }
            // if(req.body.params.signature && req.body.params.signature.length > 0){
            //     req.body.params["signatureMimeType"] = req.body.params.signature.split(',')[0];
            //     _signaturebase64data = req.body.params.signature.replace(/^data:.*,/, '');
            //     req.body.params["signature"] = new Buffer.from(_signaturebase64data, 'base64');
            // }
            if (req.body.params.photo && req.body.params.photo.length > 0) {
...
router.post("/get-doctor", async (req, res) => {
    try {
        ...
        if(req.body.params && req.body.params.locId && req.body.params.locId.length > 0){
        ...
        mongoMapper("cm_doctors", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
            if (result.data && result.data.length > 0) {
                let _resp = JSON.parse(JSON.stringify(result.data))
                let _finalResp = _.each(_resp, (doc) => {
                    if (doc.locations && doc.locations.length > 0) {
                        if(req.body.params.locId && req.body.params.locId.length > 0){
                            doc.locations = _.filter(doc.locations, (_loc) => { return _loc.locId == req.body.params.locId })
                        }
...
router.post("/upload-document", async (req, res) => {
    try {
        if (req.body.params._id && req.body.params.UMR && req.body.params.docs) {
            ...
            mongoMapper("cm_uploadDocuments", "insertMany", _prms, req.tokenData.dbType).then(async (result) => {
                if (!(result.data && result.data.length > 0)) {
                    return res.status(400).json({ status: 'FAIL', desc: "No Records are available.." || error, data: [] });
                }
                ...
                if (_clnDocs.docs.length > 0) {
                    for (let _doc of req.body.params.docs) {
                        ...
                        let _patUpdate = await _mUtils.commonMonogoCall("cm_patients", "findOneAndUpdate", pLoadResp.payload, "", "", "", req.tokenData.dbType)
                        if (!(_patUpdate && _patUpdate.success)) {
...
router.post("/get-upload-documents-by-umr", async (req, res) => {
    try {
        if (req.body.params.UMR) {
            ...
            let _params = {
                "filter": { "recStatus": { $eq: true }, ...req.body.params },
                "selectors": ""
            }
            mongoMapper("cm_uploadDocuments", "find", _params, req.tokenData.dbType).then(async (result) => {
...
router.post("/get-upload-documents-by-id", async (req, res) => {
    try {
        if (req.body.params._id) {
            ...
            mongoMapper("cm_uploadDocuments", "find", _filter, req.tokenData.dbType).then(async (result) => {
                if (!(result.data)) {
                    return res.status(400).json({ status: 'FAIL', desc: "No Records are available.." || error, data: [] });
                }
                let _base64 = "";
                ...
                if (result.data[0].docData && result.data[0].docData.length > 0) {
                    if (result.data[0].docData[0].docInfo && result.data[0].docData[0].docMimeType && result.data[0].docData[0].docMimeType.length > 0) {
                        _base64 = `${result.data[0].docData[0].docMimeType},${await result.data[0].docData[0].docInfo.toString("base64")}`;
                    }
                }
                let _resp = {
                    UMR: result.data[0].UMR,
                    admnNo: result.data[0].admnNo,
                    docName: result.data[0].docName,
                    docType: result.data[0].docType,
                    format: result.data[0].format,
                    remarks: result.data[0].remarks,
                    isImage: result.data[0].isImage,
                    path: result.data[0].path || "",
                    documentedBy: result.data[0].audit.documentedBy,
                    documentedId: result.data[0].audit.documentedId,
                    documentedDt: result.data[0].audit.documentedDt,
                    docDataBase64: _base64
                };
...
router.post('/update-upload-document', async (req, res) => {
    try {
        if (!(req.body.params || req.body.params.UMR || req.body._id || req.headers.recStatus)) {
        ...
        let _drResp = await _mUtils.commonMonogoCall("cm_uploadDocuments", "findOneAndUpdate", pLoadResp.payload, "", req.body, "", req.tokenData.dbType)
        if (!(_drResp && _drResp.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _drResp.desc || "", data: _drResp.data || [] });
        }
        return res.status(200).json({ success: true, status: 400, desc: "", data: _drResp.data || [] });
        ...
        if (req.body && req.body.flag && !(req.body.flag == "")) {
            if ((!req.body.params.source || req.body.params.source == "") && (req.body.flag === "G" || req.body.flag === "M")) {
                if (req.body.flag === "G") {
                    ...
                    mongoMapper("cm_medications", "aggregation", _filter, req.tokenData.dbType).then((result) => {
                        return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
                    }).catch((error) => {
                        return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
                    });
            ...
            else if (req.body.params.source == "PHARMA" && (req.body.flag === "G" || req.body.flag === "M")) {
                //let __filter = { filter: [] }
                if (req.body.flag == "G") {
                    _filter.filter.push(
                        {
                            $match: { STRING_3: { $regex: req.body.params.searchValue, $options: 'i' } }
                        },
                        {
                            $project: {
                                "_id": "$_id",
                                "cd": "$BRAND_PRODUCT_MAP_CD",
                                "genericName": "$STRING_3",
                                "genericCd": "$STRING_3_CD",
                                "medName": "$BRAND_STRING2",
                                "medFormTypeName": "$DOSE_FORM_NAME",
                                "medFormTypeCd": "$DOSE_FORM_CD"
                            }
                        }
                    )
                }
                if (req.body.flag === "M") {
                    _filter.filter.push(
                        {
                            $match: { BRAND_STRING2: { $regex: req.body.params.searchValue, $options: 'i' } }
                        },
                        {
                            $project: {
                                "_id": "$_id",
                ...
                mongoMapper("cm_pharmacy_med_masters", "aggregation", _filter, req.tokenData.dbType).then((result) => {
                    return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
                }).catch((error) => {
                    return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
                });
    ...
    try {
        ...
        if (req.body && req.body.flag && req.body.flag === "search") {
            if (req.body && req.body.params && req.body.params.name && req.body.params.name.length > 2) {
                ...
                mongoMapper("cm_investigations", req.body.query, _filter, req.tokenData.dbType).then(async (result) => {
                    ...
                    let withCode = result.data.filter(item => item.hasOwnProperty('i_cd') && item.i_cd);
                    let withoutCode = result.data.filter(item => !item.hasOwnProperty('i_cd') || !item.i_cd);
                    let processedData = withCode.length > 0 ? await findDuplicatesAndCombinedData(withCode, 'i_cd') : [];
                    let finalData = [...processedData, ...withoutCode];
                    return res.status(200).json({ success: true, status: 200, desc: '', data: finalData });
                    //newly added
                }).catch((error) => {
                    return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
                });
            ...
            mongoMapper("cm_investigations", req.body.query, _pGData.data, req.tokenData.dbType).then(async (result) => {
                // let uniqueData = await findDuplicatesAndCombinedData(result.data, 'i_cd');

                // let _finalResp = uniqueData.map(rec => {
                //     if (rec.ageGenderRanges && typeof rec.ageGenderRanges === 'object') {
                //         Object.keys(rec.ageGenderRanges).forEach(key => {
                //             if (Array.isArray(rec.ageGenderRanges[key])) {
                //                 rec.ageGenderRanges[key] = rec.ageGenderRanges[key].filter(data => data.recStatus === true)
                //             }
                //         })
                //     }
                //     return rec;
...
router.post("/get-allergy", async (req, res) => {
    try {
        ...
        mongoMapper("cm_allergies", req.body.query, _pGData.data, req.tokenData.dbType).then((result) => {
            ...
            let finalData = result.data.map(rec => {
                if (rec.labels && Array.isArray(rec.labels)) {
                    rec.labels = rec.labels.filter(label => label.recStatus == true)
                }
                return rec;
            })
            return res.status(200).json({ success: true, status: 200, desc: '', data: finalData });
    ...
    try {
        ...
        mongoMapper("cm_entity", req.body.query, _pGData.data, req.tokenData.dbType).then((result) => {
            if (result.data.length > 0) {
                ...
                _resultData = _.filter(_resultData, (_r) => { return _r.recStatus == true })
                for (let _data of _resultData) {
                    _data.child = _.filter(_data.child, (_ch) => { return _ch.recStatus == true })
                }
    ...
    try {
        ...
        if (req.body.params._id) {
            ...
            mongoMapper('cm_entity', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                ...
                let _output = []
                let _roles = await _mUtils.commonMonogoCall("cm_roles", "find", _rFilter, "", "", "", req.tokenData.dbType)
                if (_roles.success && _roles.data.length > 0) {
                    for (let _data of _roles.data) {
                        if (req.body.params.child && req.body.params.child.length > 0) {

                            let _entityChild = JSON.parse(JSON.stringify(req.body.params.child))
                            for (let _child of _entityChild) {
                                let _entityData = await _mUtils.commonMonogoCall("cm_entity", "findById", result.data._id, "", "", "", req.tokenData.dbType)
                                _entityData = JSON.parse(JSON.stringify(_entityData))
                                let _entChild = _.filter(_entityData.data.child, (_ch) => { return _ch._id == _child._id })
                                if (_entChild.length > 0) {
                                    let _tabs = _.filter(_data.tabsMap, (_tab) => { return _tab.cd == _entChild[0].cd })
                                    if (_tabs.length > 0) {
                                        let _uFilter = {
                                            "params": {
                                                "_id": _data._id,
                                        ...
                                        _.each(_child, (_cv, _ck) => {
                                            if (_ck != "_id" && _ck != "revNo") {
                                                _uFilter.params.tabsMap[0][`${_ck}`] = `${_cv}`
                                            }
                                        })
                                        let pLoadResp = await _mUtils.preparePayload('BW', _uFilter);
                                        if (!pLoadResp.success) {
                                            _output.push({ success: false, status: 400, desc: pLoadResp.desc, data: [] });
                                        }
                                        let _uResp = await _mUtils.commonMonogoCall("cm_roles", 'bulkWrite', pLoadResp.payload, "", "", "", req.tokenData.dbType);
                                        if (!(_uResp && _uResp.success && _uResp.data)) {
                                            _output.push({ success: false, status: 400, desc: _uResp.desc || 'Error occurred while updating role ..', data: [] });
                                        } else {
                ...
                if (_output.length > 0) {
                    let _final = _.filter(_output, (_r) => { return _r.success == false })
                    if (_final.length > 0) {
                        return res.status(400).json({ success: false, status: 400, desc: "Error While Updating TabsMap in Roles", data: [] });
                    }
                }
...
router.post("/get-counter", async (req, res) => {
    try {
        if(req.body.params && req.body.params.locId && req.body.params.locId.length > 0){
            ...
            mongoMapper('cm_counters', "find", _filter, req.tokenData.dbType).then((result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
    ...
    try {
        let _pFilter = {
            "filter":{
                ...
                "dispName": req.body.params.dispName,
                "mobile": req.body.params.mobile,
                "dob": req.body.params.dob,
            }
        }
        let _patResp = await _mUtils.commonMonogoCall("cm_patients", "find", _pFilter, "", req.body, "", req.tokenData.dbType)
        if (!(_patResp && _patResp.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _patResp.desc || "", data: _patResp.data || [] });
        }
        if (_patResp && _patResp.success && _patResp.data.length > 0) {
            return res.status(400).json({ success: false, status: 400, desc: "This Patient already exists.", data: [] });
        }
        ...
        req.body.params["abhaNo"] = req.body.params.abhaNo;
        req.body.params["abhaAddress"] = req.body.params.abhaAddress;
        req.body.params["abhaJson"] = req.body.params.abhaJson;
        let _cBody = JSON.parse(JSON.stringify(req.body))
        if (_cBody.params.abhaCard || _cBody.params.abhaCard == "") {
            delete _cBody.params['abhaCard']
        }
        if (_cBody.params.photo || _cBody.params.photo == "") {
            delete _cBody.params['photo']
        }
        if (_cBody.params.abhaJson && _cBody.params.abhaJson.length > 0) {

            let _mResp = await _mUtils.commonMonogoCall("cm_abha_json", "insertMany", _cBody.params.abhaJson, "", "", "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            else {
                _cBody.params.abhaJson = _mResp.data[0]._id
            }
        }
        mongoMapper('cm_patients', req.body.query, _cBody.params, req.tokenData.dbType).then(async (result) => {
            if ((req.body.params.abhaCard && req.body.params.abhaCard.length > 0) || (req.body.params.photo && req.body.params.photo.length > 0)) {
                let _media = {}
                if (req.body.params.abhaCard && req.body.params.abhaCard.length > 0) {
                    _media["abhaCard"] = req.body.params.abhaCard
                }
                if (req.body.params.photo && req.body.params.photo.length > 0) {
                    _media["photo"] = req.body.params.photo
                }
                let _mediaResp = await insertMedia("I", "cm_patients", _media, req, result.data)

                if (!_mediaResp.success) {
                    return res.status(400).json({ success: false, status: 400, desc: "Error While Inserting Media", data: _oResp.data || [] });
                }
            }
            ...
            let locRecord = await mongoMapper('cm_organization', 'find', _filter, req.tokenData.dbType);
            let locRec = JSON.parse(JSON.stringify(locRecord.data[0].locations))
            if (locRec && locRec[0].regFee) {
                let _insParams = {
                    "UMR": result.data[0].UMR || req.body.params.UMR || _seqResp.data,
                    "patientName": result.data[0].userName,
                    "amount": locRec[0].regFeeAmount,
                    "billAmount": locRec[0].regFeeAmount,
                    "concessionAmt": 0.00,
                    "outStandingDue": locRec[0].regFeeAmount,
                    //"paymentMode": "CASH",
                    "services": [
                        {
                            "type": "FEE",
                            "serviceCd": "REGFEE",
                            "serviceName": "REGESTRATION FEE AMOUNT",
                            "serviceType": "dummy",
                            "qty": 1,
                            "price": locRec[0].regFeeAmount,
                            "amount": locRec[0].regFeeAmount,
                            "totPrice": locRec[0].regFeeAmount,
                        }
                    ],
                    "audit": {
                        "documentedById": result.data[0].audit.documentedById,
                        "documentedBy": result.data[0].audit.documentedBy
                    }
                }
                ...
                if (!(invoiceNum && invoiceNum.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: invoiceNum.desc || "", data: invoiceNum.data || [] });
                }
                _insParams["invoiceNo"] = invoiceNum.data;
                _insParams['visit'] = "OP"
                _insParams["billNo"] = _seqResp.data;
                mongoMapper('cm_bills', "insertMany", _insParams, req.tokenData.dbType).then(async (_resp) => {
                    if (!(_resp && _resp.data)) {
                        return res.status(400).json({ success: false, status: 400, desc: `Error occurred while inserting reg fee bills ..`, data: [] });
                    }
                    let _fParams = {
                        params: {
                            _id: result.data[0]._id,
                            bills: [
                                {
                                    "billId": _resp.data[0]._id.toString(),
                                    "billNo": _resp.data[0].billNo,
                                    "dateTime": _resp.data[0].billDate,
                                    "visit": _resp.data[0].visit,
                                    "visitNo": ""
                                }
                            ]
                        }
                    ...
                    mongoMapper('cm_patients', "findOneAndUpdate", pLoadResp.payload, req.tokenData.dbType).then(async (_resp1) => {
                        if (!(_resp1 && _resp1.data)) {
                            return res.status(400).json({ success: false, status: 400, desc: `Error occurred while Updating reg fee bills in Patient ..`, data: [] });
                        }
                        return res.status(200).json({ success: true, status: 200, desc: '', data: _resp1.data });
                    }).catch((error) => {
                        return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
                    });
                }).catch((error) => {
    ...
    try {
        if (req.body && req.body.params && req.body.params.flag && (req.body.params.flag === 'NAME' || req.body.params.flag == 'MOB' || req.body.params.flag == 'UMR' || req.body.params.flag == 'VSTID' || req.body.params.flag === 'VSTNO') && req.body.params.searchValue && req.body.params.searchValue.length > 2) {
            let _filter = {
                ...
                "selectors": "-history",
                "populate": [
                    { 'path': 'staffReferId', 'select':'titleName dispName gender joinDt empTypeName' }
                ]
            };
            ...
            mongoMapper("cm_patients", req.body.query, _filter, req.tokenData.dbType).then(async (result) => {
...
router.post("/get-patients", async (req, res) => {
    try {
        let _filter = {
            ...
            "populate": [
                { 'path': 'staffReferId', 'select':'recStatus titleCd titleName fName mName lName dispName genderCd gender dob emailID userName adharNo passport phone mobile' }
            ]
        }
        if (req.body.params && req.body.params.populate && req.body.params.populate.length > 0) {
            _.each(req.body.params.populate.split('|'), (item) => {
                _filter.populate.push({ 'path': item, 'select': '' })
                _filter.selectors[`${item}`] = `$${item}`
            })
        }

        if(req.body.params && (req.body.params.dispName || req.body.params.UMR || req.body.params.abhaAddress )){
            const params = req.body.params;
            if(params.UMR){
                req.body.params.UMR = { $regex: addOptionalSpacesRegex(params.UMR), $options:"i" };
            }
            if(params.dispName){
                req.body.params.dispName = { $regex: addOptionalSpacesRegex(params.dispName), $options:"i" };
            }
            if(params.abhaAddress){
                _filter.filter.abhaAddress = { $regex: addOptionalSpacesRegex(params.abhaAddress), $options:"i" };
            }
        }
        ...
        mongoMapper("cm_patients", req.body.query, _pGData.data, req.tokenData.dbType).then(async (result) => {
            _.each(result.data, (obj, ind) => {
                ...
                if (obj.visits.length > 0) {
                    _.each(obj.visits, (visit, _ind) => {
                        visit.docName = `Dr. ${visit.docName}`
                    })
                }
                // if(obj.visitTrans.length > 0){
                //     _.each(obj.visitTrans, (rec, index)=>{
                //         rec.docName = `Dr. ${ rec.docName}`
                //     })
                // }
            })
            result.data = await findDuplicatesAndCombinedData(result.data, 'UMR');
            _.each(result.data, (_data) => {
                if (_data.policy && _data.policy.length > 0) {
                    _data.policy = _.filter(_data.policy, (_policy) => { return _policy.recStatus == true })
                }
            ...
            for (let resp of _finalResp) {
                ...
                if (resp.abhaCard && resp.abhaCard.abhaCard && resp.abhaCard.abhaCard !== "" && resp.abhaCard.abhaCardMimeType && resp.abhaCard.abhaCardMimeType.length > 0) {
                    let data = await getBase64Data(resp.abhaCard.abhaCardMimeType, resp.abhaCard.abhaCard)
                    resp['abhaCardMediaId'] = resp.abhaCard._id
                    resp['abhaCard'] = data
                }
                else {
                    resp['abhaCard'] = ""
                ...
                if (resp.photo && resp.photo.photo && resp.photo.photo !== "" && resp.photo.photoMimeType && resp.photo.photoMimeType.length > 0) {
                    let data = await getBase64Data(resp.photo.photoMimeType, resp.photo.photo)
                    resp['photoMediaId'] = resp.photo._id
                    resp['photo'] = data
                }
                else {
                    resp['photo'] = ""
    ...
    try {
        ...
        if (req.body.params._id) {
            ...
            if (!(_hResp && _hResp.success)) {
                ...
                _cBody.params.revNo = _mResp.data.params.revNo;
                let _media = {};
                if (_cBody.params.photo) {
                    _media["photo"] = _cBody.params.photo
                }
                if (_cBody.params.photoMediaId) {
                    _media["photoMediaId"] = _cBody.params.photoMediaId
                }
                if (_cBody.params.abhaCard) {
                    _media["abhaCard"] = _cBody.params.abhaCard
                }
                if (_cBody.params.abhaCardMediaId) {
                    _media["abhaCardMediaId"] = _cBody.params.abhaCardMediaId
                }

                let _mediaResp = await insertMedia("U", "cm_patients", _media, req, _mResp.data);
                if (!_mediaResp.success) {
                ...
                if (_cBody.params.abhaJson && _cBody.params.abhaJson.length > 0) {
                    if (_cBody.params.abhaJson[0]._id && _cBody.params.abhaJson[0]._id.length > 0) {
                        let _updtPrms = {
                            "params": _cBody.params.abhaJson[0]
                        }
                        let _abhapLoadResp = await _mUtils.preparePayload("BW", _updtPrms);
                        if (!_abhapLoadResp.success) {
                            return res.status(400).json({ success: false, status: 400, desc: _abhapLoadResp.desc || "", data: [] });
                        }
                        let _aResp = await _mUtils.commonMonogoCall("cm_abha_json", "bulkWrite", _abhapLoadResp.payload, "", "", "", req.tokenData.dbType)
                        if (!(_aResp && _aResp.success)) {
                            return res.status(400).json({ success: false, status: 400, desc: _aResp.desc || "", data: _aResp.data || [] });
                        }
                        else {
                            _cBody.params.abhaJson = _aResp.data._id
                        }
                    }
                    else {
                        let _mResp = await _mUtils.commonMonogoCall("cm_abha_json", "insertMany", _cBody.params.abhaJson, "", "", "", req.tokenData.dbType)
...
router.post("/get-label-map", async (req, res) => {
    try {
        ...
        let _mResp = await _mUtils.commonMonogoCall("cm_labelsmaps", "find", _pGetData.data, "", "", "", req.tokenData.dbType)
...
router.post("/insert-bill", async (req, res) => {
    try {
        ...
        mongoMapper('cm_bills', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            ...
            if (req.body.params.services && req.body.params.services.length > 0) {
                const grnUpdateResult = await processGrnEntries(req.body.params.services, req, 0);
                if (!grnUpdateResult.success || (grnUpdateResult.data && grnUpdateResult.data.failed && grnUpdateResult.data.failed.length > 0)) {
                    return res.status(400).json({ success: false, status: 400, desc: "Error While Inserting Bills", data: [] });
                }
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }
            else {
...
/**Appointment Booking */
router.post("/book-appointment-bkp", async (req, res) => {
    try {
        if (req.body.params && (req.body.params.apmntId || (req.body.params.locId && req.body.params.docId && req.body.params.shiftId)) && req.body.params.patName && req.body.params.apmntType) {
            ...
            if (!(_visitNoSeqResp && _visitNoSeqResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _visitNoSeqResp.desc || "", data: _visitNoSeqResp.data || [] });
            }
...
router.post("/book-appointment--", async (req, res) => {
    try {
        if (req.body.params && (req.body.params.apmntId || (req.body.params.locId && req.body.params.docId && req.body.params.shiftId)) && req.body.params.patName && req.body.params.apmntType) {
            ...
            if (!req.body.params.apmntId) {
                let _mResp = await _mUtils.commonMonogoCall("cm_doctors", "findById", req.body.params.docId, "", req.body, "", req.tokenData.dbType)
                if (!(completeTransactionBill && _mResp.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
                }
                let _docLoc = _.filter(_mResp.data.locations, (_o) => { return _o.locId === req.body.params.locId });
                if (!(_docLoc || _docLoc.length === 0 || _docLoc[0].shifts || _docLoc[0].shifts.length === 0 || _docLoc[0].fees || _docLoc[0].fees.reg)) {
                ...
                let _metaData = {
                    UMR: req.body.params.UMR,
                    patName: req.body.params.patName,
                    patMobile: req.body.params.mobile,
                    patEmail: req.body.params.email || "",
                    gender: req.body.params.gender,
                    docId: req.body.params.docId,
                    docMobile: _mResp.data.mobile,
                    docEmail: _mResp.data.emailID,
                    dateTime: req.body.params.apmntDtTime,
                    apptCd: _gPayload.data[0].data.code
                }

                // let _notifData = await _notifications.sendNotifications(_metaData, req, 'NTFC0021')
                // if (!_notifData || !_notifData.success) {
                //     //return res.status(400).json({ success: false, status: 400, desc: "Error while inserting into notifications", data: _notifData && _notifData.data ? _notifData.data : [] });
                // }
    ...
    try {
        if (req.body.params && (req.body.params.apmntId || (req.body.params.locId && req.body.params.docId && req.body.params.shiftId)) && req.body.params.patName && req.body.params.apmntType) {
            ...
            if (!req.body.params.apmntId) {
                ...
                let _notifData = await _notifications.sendNotifications(_metaData, req, 'NTFC0001')
                if (!_notifData || !_notifData.success) {
                    //return res.status(400).json({ success: false, status: 400, desc: "Error while inserting into notifications", data: _notifData && _notifData.data ? _notifData.data : [] });
                }
        ...
        if (req.body.params._id) {
            ...
            mongoMapper('cm_appointments', 'findOneAndUpdate', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                if (result.data.status === 'CANCELLED') {
                    ...
                    let visitPatRec = await mongoMapper('cm_patients', 'find', _filter, req.tokenData.dbType);
                    ...
                    mongoMapper('cm_patients', "findOneAndUpdate", pLoadResp.payload, req.tokenData.dbType).then(async (_resp) => {
                        if (!(_resp && _resp.data)) {
    ...
    try {
        if (req.body.params && req.body.params.locId && req.body.params.UMR && req.body.params.visit && req.body.params.data && req.body.params.docId && req.body.params.data.length > 0) {
            if (req.body.params.documentId && req.body.params.documentId.length > 0) {
                ...
                let _mResp = await _mUtils.commonMonogoCall("cm_drafts", 'find', _draftFilter, "", "", "", req.tokenData.orgKey.toLowerCase());
                if (_mResp.success == true && _mResp.data.length > 0) {
                    ...
                    let _dResp = await _mUtils.commonMonogoCall("cm_drafts", "findOneAndUpdate", pLoadResp.payload, "", req.body, "", req.tokenData.dbType)
                        ...
                        if (_tran.child && _tran.child.length > 0) {
                            for (let _child of _tran.child) {
                                ...
                                if (appointment.success) {
                                    ...
                                    let _uResp = await _mUtils.commonMonogoCall("cm_transactions", 'findOneAndUpdate', pLoadResp.payload, "", "", "", req.tokenData.dbType);
            ...
            if (Array.isArray(_parseResultData) && _parseResultData.length > 0) {
                for (let data of _parseResultData) {
                    if (data.docDetails.docId && data.docDetails.docId.length > 0) {
                        ...
                        let resp = await _mUtils.commonMonogoCall("cm_doctors", "find", _filter, "", "", "", req.tokenData.dbType)
                    ...
                    if (data.visitNo && data.visitNo.length > 0) {
                        ...
                        let _resp = await _mUtils.commonMonogoCall("cm_appointments", "aggregation", _aptfilter, "", "", "", req.tokenData.dbType)
...
router.post("/update-transaction", async (req, res) => {
    try {
        if (req.body.params._id) {
            ...
            let _tResp = await _mUtils.commonMonogoCall("cm_transactions", "findById", req.body.params._id, "", req.body, "", req.tokenData.dbType)
            if (!(_tResp && _tResp.success)) {
            ...
            let _dResp = await _mUtils.commonMonogoCall("cm_drafts", 'find', _draftFilter, "", "", "", req.tokenData.orgKey.toLowerCase());
            if (_dResp.success == true && _dResp.data.length > 0) {
                ...
                let _drPLoadResp = await _mUtils.preparePayload('U', _prms);
                if (!_drPLoadResp.success) {
                    return res.status(400).json({ success: false, status: 400, desc: _drPLoadResp.desc || "", data: [] });
                }
                let _drftResp = await _mUtils.commonMonogoCall("cm_drafts", "findOneAndUpdate", _drPLoadResp.payload, "", req.body, "", req.tokenData.dbType)
                if (!(_drftResp && _drftResp.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _drftResp.desc || "", data: _drftResp.data || [] });
                }
                
                let _aFilter = {
                    "filter": {
                        "code": _dResp.data[0].visitNo,
                    }
                }
                let _appt = await _mUtils.commonMonogoCall("cm_appointments", 'find', _aFilter, "", "", "", req.tokenData.orgKey.toLowerCase());
                let _apptPrms = {
                    "params": {
                        "_id": _appt.data[0]._id,
                        "isDraft": false
                    }
                }
                let _apptPLoadResp = { payload: {} };
                _apptPLoadResp = await _mUtils.preparePayload('BW', _apptPrms);
                if (!_apptPLoadResp.success) {
                    return res.status(400).json({ success: false, status: 400, desc: _apptPLoadResp.desc || "", data: [] });
                }
                mongoMapper('cm_appointments', 'bulkWrite', _apptPLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                    // return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
                }).catch((error) => {
                    return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
                });
            ...
            if (_cBody.params.recStatus === false) {
                ...
                if (!(_hResp && _hResp.success)) {
                    ...
                    if (!(_cBody.params.data)) {
                        ...
                        let _updateResp = await insertUpdateInMultiData(_finalPayloadObj, 0, [], req.tokenData.dbType, 'cm_transactions', 'bulkWrite', 'BW')
                        if (!_updateResp.success) {
                            return res.status(400).json({ success: false, status: 400, desc: `Error occurred while Updating Transaction data, Err:-${_updateResp.desc}`, data: [] });
                        }
                        else {
                            ...
                            let _mTransResp = await _mUtils.commonMonogoCall("cm_transactions", "find", _filter, "", "", "", req.tokenData.dbType)
...
//             };
//             let _collection = "";
//             let staticArrey = ["cm_investigations", "cm_medications"]//cm_pharmacy_med_masters
//             if (req.body.flag === "") {
...
//                             let grnResult = await mongoMapper("cm_grn_entries", req.body.query, _filter, req.tokenData.dbType)
        ...
        if (req.body && req.body.params) {
            ...
            for (const obj of getTableData.data) {
                if (obj.serviceTypeName || obj.serviceTypeCd) {
                    ...
                    let _filter = { filter: { productId: obj._id } };
                    let grnResult = await mongoMapper("cm_grn_entries", req.body.query, _filter, req.tokenData.dbType)
                    objectAssign.grnData = grnResult.data.map(grn => ({
                        expirationDate: grn.expirationDate,
                        batchNumber: grn.batchNumber,
                        quantity: grn.quantityPerBatch,
                        manufactureDate: grn.manufactureDate
                    }));
...
function grouplabResult(data, flag) {
    try {
        if (flag == "H") {
            return data.reduce((acc, record) => {
                const dateTime = dateFormat(record.REPORTDT);
                if (!dateTime) return null;

                let dateGrp = acc.find((grp) => grp.disName === dateTime)
                if (!dateGrp) {
                    dateGrp = { disName: dateTime, isShown: false, isHistoryShown: false, src: [] };
                    acc.push(dateGrp);
                }

                let srvcGrp = dateGrp.src.find((grp) => grp.invsName === record.SERVICE_NAME);

                if (!srvcGrp) {
                    srvcGrp = {
                        invsCd: record.SERVICE_CODE,
                        invsName: record.SERVICE_NAME,
                        srvGrpCd: record.SERVICEGROUPCD,
                        subChild: []
                    };
                    dateGrp.src.push(srvcGrp);
                }

                let subchildGroup = srvcGrp.subChild.find((grp) => grp.disName === record.PARAMETERDESC);
                if (!subchildGroup) {
                    subchildGroup = { disName: record.PARAMETERDESC,  normalValues: record.NORMALVALUES ? `${record.NORMALVALUES}` : "", supChild: [] };
                    srvcGrp.subChild.push(subchildGroup)
                }

                subchildGroup.supChild.push({
                    dateTime: dateTime,
                    value: record.RESULTVALUES ? `${record.RESULTVALUES}` : "",
                    units: record.RESULTUOM ? `${record.RESULTUOM}` : "",
                    normalValues: record.NORMALVALUES ? `${record.NORMALVALUES}` : "",
                    deviateType: record.DEVIATETYPE
                });

                return acc;
            }, []).map(dateGrp =>{
                dateGrp.src.forEach(srvcGrp=>{
                    const _deviations = srvcGrp.subChild.flatMap(sub => sub.supChild.map(child => child.deviateType));
                    srvcGrp.deviateType = _deviations.every(type => type === "NORMAL") ? "Normal" : "Abnormal"
                });
                return dateGrp; 
            })
        }
        else if (flag == "E") {
            let dateTime = data.dateTime ? data.dateTime : "";
            ...
            return data.data.reduce((acc, record) => {
                ...
                let srvcGrp = dateGrp.src.find((grp) => grp.invsName === record.invsName);

                if (!srvcGrp) {
                    srvcGrp = {
                        invsCd: record.invsCd,
                        invsName: record.invsName,
                        subChild: []
                    };
                    dateGrp.src.push(srvcGrp);
                }
                ...
                let subchildGroup = srvcGrp.subChild.find((grp) => grp.disName === record.PARAMETERDESC);
                if (!subchildGroup) {
                    subchildGroup = { disName: record.invsName ? record.invsName : "", normalValues: record.NORMALVALUES ? `${record.NORMALVALUES}` : "", supChild: [] };
                    srvcGrp.subChild.push(subchildGroup)
                }
                ...
                subchildGroup.supChild.push({
                    ...
                    units: record.RESULTUOM ? `${record.RESULTUOM}` : "",
                    normalValues: record.highVal ? `${record.highVal}` : "",
                    deviateType: record.status ? `${record.status}`: ""
                });
...
function dateFormat(dateStr){
    ...
    return match ? new Date(parseInt(match[1], 10)).toISOString().split('T')[0] : dateStr.split('T')[0];
}
...
router.post("/get-lab-result_bkp", async (req, res) => {
    try {
        if (req.body.params.flag == "HIS") {
            ...
            let location = await _mUtils.commonMonogoCall("cm_organization", "find", _locFilter, "", "", "", req.tokenData.dbType)

            let _loc = _.filter(location.data[0].locations, (_l) => { return _l._id == req.tokenData.locId })

            _loc = JSON.parse(JSON.stringify(_loc))

            const queryParams = new URLSearchParams({
                LkUpName: "HIST",
                LkUpPrms: JSON.stringify({
                    EMR: [{
                        UMR_NO: req.body.params.UMR,
                        TEST_ID: "",
                        FROM_DT: req.body.params.fromDt ? moment(req.body.params.fromDt).format("DD-MMM-YYYY") : new Date(new Date().setMonth(new Date().getMonth() - 3)).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).replace(/([a-zA-Z]{3})[a-zA-Z]*/, '$1').replace(/ /g, '-'),
                        TO_DT: req.body.params.toDt ? moment(req.body.params.toDt).format("DD-MMM-YYYY") : new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).replace(/([a-zA-Z]{3})[a-zA-Z]*/, '$1').replace(/ /g, '-'),
                        PARAMETER_ID: "",
                        PROFILE_ID: "",
                        REF_TYPE: "B",
                        ADMN_NO: "",
            ...
            let url = `${_loc[0].himsLabResUrl}syncApptData?${queryParams}`
            commonAxiosCall('GET', url).then(async (resp) => {
                let jsonString = resp.data.replace(/<\?xml.*\?>/g, '').replace(/<\/?string>/g, '').trim();
                jsonString = JSON.parse(jsonString)
                // if (!jsonString.length > 0) {
                //     return res.status(404).json({ status: 'FAIL', desc: 'No lab results found in selected date range', data: [] })
                // }
                ...
                let patient = await _mUtils.commonMonogoCall("cm_patients", "find", _patFilter, "", "", "", req.tokenData.dbType)
                patient = JSON.parse(JSON.stringify(patient.data));
                let patDet = {
                    "patId": patient[0]._id,
                ...
                _output.patDet = patDet

                if (req.body.params.isForceLoad == true) {


                    let _finalResp = req.body.flag == 'HIS_TREND' ? grouplabResult(jsonString) : transformLabResults(jsonString)
                    let groupedData = _.groupBy(_finalResp, "billNo")

                    for (let key in groupedData) {
                        _output.data = groupedData[key]
                        _output.dateTime = groupedData[key][0].billedDate
                        let _doctorFilter = {
                            "filter": [
                                {
                                    $match: {
                                        "recStatus": { $eq: true },
                                        i_docCd: groupedData[key][0].docCd
                                    }
                                }
                            ]
                        }

                        let _doctor = await _mUtils.commonMonogoCall("cm_doctors", "aggregation", _doctorFilter, "", "", "", req.tokenData.dbType)
                        _doctor = JSON.parse(JSON.stringify(_doctor.data));

                        let _docDetails = {
                            "docId": _doctor[0]._id,
                            "cd": _doctor[0].docCd,
                            "regNo": _doctor[0].regNo || "",
                            "name": _doctor[0].fName,
                            "degree": _doctor[0].degree || "",
                            "designation": _doctor[0].designation || "",
                            "specName": _doctor[0].specName || "",
                            "imgSign": _doctor[0].signature || ""
                        }
                        _output.docDetails = _docDetails
                        finalResponse.push(_output)
                    }
                    return res.status(200).json({ status: 'SUCCESS', desc: '', data: finalResponse })
                }
                ...
                let _filter = {
                    "filter": {
                        "recStatus": true,
                        UMR: req.body.params.UMR,
                        visitId: req.body.params.visitId,
                        // fromDt: req.body.params.fromDt? new Date(req.body.params.fromDt).toISOString() : "",
                        // toDt: req.body.params.toDt? new Date(req.body.params.toDt).toISOString() : ""
                    },
                    "selectors": "-audit -history "
                }
                let _dbType = req.headers.orgkey.toLowerCase() || req.tokenData.dbType;
                mongoMapper("cm_hims_lab_results", "find", _filter, _dbType).then(async (result) => {
                    let respId;
                    if (result.data && result.data.length > 0) {
                        let _prms = {
                            params: {
                        ...
                        let pLoadResp = await _mUtils.preparePayload('BW', _prms);
                        if (!pLoadResp.success) {
                            return res.status(400).json({ success: false, status: 400, desc: pLoadResp.desc || "", data: [] });
                        }
                        if (jsonString) {
                            pLoadResp.payload.pData[0].updateOne.update.$set["data"] = jsonString;
                        }
                        let _uResp = await _mUtils.commonMonogoCall("cm_hims_lab_results", 'bulkWrite', pLoadResp.payload, "", "", "", req.headers.orgkey.toLowerCase());
                        if (!(_uResp && _uResp.success)) {
                            return res.status(400).json({ success: false, status: 400, desc: _uResp.desc || "", data: _uResp.data || [] });
                        }
                        respId = JSON.parse(JSON.stringify(_uResp.data._id));

                        let _mResp = await _mUtils.commonMonogoCall("cm_hims_lab_results", "findById", respId, "", "", "", req.tokenData.dbType)
                        if (!(_mResp && _mResp.success)) {
                            return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
                        }

                        let _finalResp2 = req.body.flag == 'HIS_TREND' ? grouplabResult(JSON.parse(JSON.stringify(_mResp.data.data))) : transformLabResults(JSON.parse(JSON.stringify(_mResp.data.data)))
                        let groupedData = _.groupBy(_finalResp2, "billNo")
                        ...
                        let _pGData = await prepareGetPayload(_labFilter, _params);
                        if (!_pGData.success) {
                            return res.status(400).json({ success: false, status: 400, desc: _pGData.desc, data: [] });
                        }

                        let _resultResp = await _mUtils.commonMonogoCall("cm_labresults", "find", _pGData.data, "", "", "", req.tokenData.dbType)

                        if (_resultResp.data && _resultResp.data.length > 0) {
                            _.each(_resultResp.data, (obj, ind) => {
                                finalResponse.push(obj)
                            })
                        }


                        return res.status(200).json({ status: 'SUCCESS', desc: 'lab results fetched successfully', data: finalResponse });
                    }
                    else {
                        ...
                        mongoMapper("cm_hims_lab_results", "insertMany", _insrtPrms, req.tokenData.dbType.toLowerCase()).then(async (result) => {
                            if (!(result.status == 'SUCCESS' && result.data && result.data.length > 0)) {
                                return res.status(400).json({ success: false, status: 400, desc: `failed to insert hims lab results`, data: [] });
                            }


                            finalResp = req.body.flag == "HIS" ? transformLabResults(JSON.parse(JSON.stringify(result.data[0].data))) : grouplabResult(JSON.parse(JSON.stringify(result.data[0].data)))

                            let groupedData = _.groupBy(finalResp, "billNo")
                            ...
                            return res.status(200).json({ success: true, status: 200, desc: "SUCCESS", data: finalResponse || [] });
                        }).catch((error) => {
                            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
                        });
                    }
        ...
        else if (req.body.params.flag == "PID") {
            let _filter = {
                "filter": {
                    "recStatus": true,
...
router.post("/get-lab-result--bkp1", async (req, res) => {
    try {
        if (req.body.params.flag == "HIS") {
            ...
            commonAxiosCall('GET', url).then(async (resp) => {
                ...
                if (req.body.params.isForceLoad == true) {
                    ...
                    let _finalResp = req.body.flag == 'HIS_TREND' ? grouplabResult(jsonString, "H") : transformLabResults(jsonString)
                    let groupedData = _.groupBy(_finalResp, "billNo")
                ...
                mongoMapper("cm_hims_lab_results", "find", _filter, _dbType).then(async (result) => {
                    ...
                    if (result.data && result.data.length > 0) {
                        ...
                        let _finalResp2 = req.body.flag == 'HIS_TREND' ? grouplabResult(JSON.parse(JSON.stringify(_mResp.data.data)), "H") : transformLabResults(JSON.parse(JSON.stringify(_mResp.data.data)))
                        let groupedData = _.groupBy(_finalResp2, "billNo")
                        ...
                        // if (_resultResp.data && _resultResp.data.length > 0) {
                        //     _.each(_resultResp.data, (obj, ind) => {
                        //         if (req.body.flag == "HIS_TREND") {
                        //             obj = grouplabResult(obj, "E")
                        //         }
                        //         let _data = []
                        //         _.each(obj, (_o)=>{
                        //             _data.push(_o)
                        //         })
                        //         finalResponse.push({data:_data})
                        //     })
                        // }

                        _resultResp = JSON.parse(JSON.stringify(_resultResp))
                        if (_resultResp.data && _resultResp.data.length > 0) {
                            let _data = []
                            _.each(_resultResp.data, (obj, ind) => {
                                if (req.body.flag == "HIS_TREND") {
                        ...
                        mongoMapper("cm_hims_lab_results", "insertMany", _insrtPrms, req.tokenData.dbType.toLowerCase()).then(async (result) => {
                            ...
                            finalResp = req.body.flag == "HIS_TREND" ? grouplabResult(JSON.parse(JSON.stringify(result.data[0].data)), "H") : transformLabResults(JSON.parse(JSON.stringify(result.data[0].data)))
        ...
        if (req.body.flag == "PRINT") {
            ...
            mongoMapper("cm_hims_lab_results", "find", _filter, _dbType).then(async (result) => {
                ...
                if (result.data && result.data.length > 0) {
                    ...
                    _resp[0].data = _.filter(_resp[0].data, (_dt) => { return _dt.REPORTDT.split("T")[0] == req.body.params.fromDt.split("T")[0] })

                    let _finalResp2 = req.body.flag == 'HIS_TREND' ? grouplabResult(JSON.parse(JSON.stringify(_resp[0].data)), "H") : transformLabResults(JSON.parse(JSON.stringify(_resp[0].data)))
                    let groupedData = _.groupBy(_finalResp2, "billNo")
                    ...
                    _labFilter.filter.dateTime = {
                        $gte: `${fromDt}T00:00:00.000Z`,
                        $lte: `${fromDt}T23:59:59:999Z`
                    }

                    if (_params.hims_id || _params.hims_id == "") {
                        delete _params['hims_id']
                    }
                    let _pGData = await prepareGetPayload(_labFilter, _params);
                    if (!_pGData.success) {
                        return res.status(400).json({ success: false, status: 400, desc: _pGData.desc, data: [] });
            ...
            if (req.body.params.flag == "HIS") {
                ...
                commonAxiosCall('GET', url).then(async (resp) => {
                    ...
                    let _filter = {
                        "filter": {
                            ...
                            UMR: req.body.params.UMR,
                            //visitId: req.body.params.visitId,
                            fromDt: { $regex: `${req.body.params.fromDt}` }, //req.body.params.fromDt? new Date(req.body.params.fromDt).toISOString() : "",
                            toDt: { $regex: `${req.body.params.toDt}` }, //req.body.params.toDt? new Date(req.body.params.toDt).toISOString() : ""
                        },
                        "selectors": "-audit -history "
                    }
                    let _dbType = req.headers.orgkey.toLowerCase() || req.tokenData.dbType;
                    mongoMapper("cm_hims_lab_results", "find", _filter, _dbType).then(async (result) => {
                        ...
                        if (result.data && result.data.length > 0) {
                            ...
                            mongoMapper("cm_hims_lab_results", "insertMany", _insrtPrms, req.tokenData.dbType.toLowerCase()).then(async (result) => {
                                ...
                                let groupedData = _.groupBy(finalResp, "billNo")
                                _output['hims_id'] = result.data[0]._id
                                _output['source'] = "his"
                                for (let key in groupedData) {
                                    _output.data = groupedData[key]
                                    _output.dateTime = groupedData[key][0].billedDate
                ...
                mongoMapper("cm_labresults", "find", _pGData.data, req.tokenData.dbType).then((result) => {
                    let _resultResp = JSON.parse(JSON.stringify(result.data));
                    let _finalResp = []
                    let _data = []
                    if(req.body.flag == "HIS_TREND"){
                        if(_resultResp.length > 0){
                            _.each(_resultResp, (_resp) => {
                                _resp = grouplabResult(_resp, "E");
                                _data.push(_resp);
                            })
                            if(_data.length > 0){
                                _.each(_data, (_dt) => {
                                    _finalResp.push({ data: _dt });
                                })
                            }
                        }
                        return res.status(200).json({ success: true, status: 200, desc: '', data: _finalResp });
                    }
                    return res.status(200).json({ success: true, status: 200, desc: '', data: _resultResp });
                }).catch((error) => {
                    return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
                });
            }
    ...
    try {
        if (req.body && req.body.params && req.body.params.UMR && req.body.params.visit && req.body.params.visitNo) {
            ...
            mongoMapper('cm_diabetics_tran', req.body.query, req.body.params, req.tokenData.dbType).then(async result => {
                ...
                let updateAppointmentDoc = await updateVisitsOrBillData(req.body.params.UMR, "AAPT_UPDT", apptObj, req)
                if (!(updateAppointmentDoc && updateAppointmentDoc.success)) {
                    console.log("Error Occured while updating appointment details to Patient");
                }
                let _patResp = await updateVisitsOrBillData(req.body.params.UMR, "VISIT_TRAN", _obj, req);
                ...
                let _visitResp = await updateVisitsOrBillData(req.body.params.UMR, "VISIT_UPD", _visitObj, req);
                if (!(_visitResp && _visitResp.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: `Error Occured while updating visits to Patient ..`, data: [] });
                }

                let _finalPayload = await individualCollPayload("I", req.body.params.data, 0, [], req)
                let _finalData = [
                    {
                        name: "Medication",
                        type: "MEDICATION",
                ...
                if (_finalPayload.success && _finalPayload.data.length > 0) {
                    for (let _child of _finalPayload.data) {
                        for (let _main of _finalData) {
                            if (_child.type == _main.type) {
                                _main.child.push(_child)
                            }
                        }
                    }
                    let _updateResp = await insertIndividualColl("I", _finalData, 0, [], req, req.body.params);
                    if (!_updateResp.success) {
                        console.log("Error Occured while Insert data to Individual collection");
                        // return res.status(400).json({ success: false, status: 400, desc: `Error occurred while insert default documents`, data: [] });
                    }
...
router.post("/update-diabetic-transaction", async (req, res) => {
    try {
        ...
        if (_cBody.params.locId != undefined) {
            delete _cBody.params.locId
        }
        if (_cBody.params._id && _cBody.params.UMR && _cBody.params.visit && _cBody.params.visitNo) {
            ...
            mongoMapper('cm_diabetics_tran', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                ...
                if (req.body.params.recStatus == false) {
                    let _obj = {
                        "diabTranId": result.data._id,
                        "diabStatus": false
                    };
                    let _patResp = await updateVisitsOrBillData(req.body.params.UMR, "VISIT_TRAN_UPD", _obj, req);
                    if (!(_patResp && _patResp.success)) {
                        return res.status(400).json({ success: false, status: 400, desc: `Error Occured while updating visits transactions to Patient ..`, data: [] });
                    }
                    let _visitObj = {
...
router.post("/get-all-locations", async (req, res) => {
    try {
        ...
        mongoMapper('cm_organization', req.body.query, _pGData.data, req.tokenData.dbType).then((result) => {
            let _finalResp = JSON.parse(JSON.stringify(result.data))
...
router.post("/insert-grnEntryData", async (req, res) => {
    try {
        if (req.body && req.body.params && req.body.params.manufactureDate && req.body.params.quantityPerBatch && req.body.params.productName && req.body.params.batchNumber && req.body.params.expirationDate) {
            ...
            mongoMapper('cm_grn_entries', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
...
router.post("/get-grnEntryData", async (req, res) => {
    try {
        ...
        mongoMapper("cm_grn_entries", "find", _pGData.data, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/update-grnEntryData", async (req, res) => {
    try {
        let _cBody = JSON.parse(JSON.stringify((req.body)));
        if (req.body.params._id) {
            let _mResp = await _mUtils.commonMonogoCall("cm_grn_entries", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            let _hResp = await _mUtils.insertHistoryData('cm_grn_entries', _mResp.data.params, _cBody.params, req);
            let pLoadResp = { payload: {} };
            if (!(_hResp && _hResp.success)) {
                return
            }
            ...
            mongoMapper('cm_grn_entries', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
...
router.post("/insert-specialities", async (req, res) => {
    try {
        req.body.params['orgId'] = req.tokenData.orgId
        mongoMapper('cm_specialities', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/get-specialities", async (req, res) => {
    try {
        ...
        mongoMapper('cm_specialities', req.body.query, _pGData.data, req.tokenData.dbType).then((result) => {
            let _finalResp = JSON.parse(JSON.stringify(result.data))
...
router.post("/update-specialities", async (req, res) => {
    try {
        let _cBody = JSON.parse(JSON.stringify((req.body)));
        if (req.body.params._id) {
            let _mResp = await _mUtils.commonMonogoCall("cm_specialities", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            let _hResp = await _mUtils.insertHistoryData('cm_specialities', _mResp.data.params, _cBody.params, req);
            let pLoadResp = { payload: {} };
            if (!(_hResp && _hResp.success)) {
            ...
            mongoMapper('cm_specialities', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
...
router.post("/insert-new-field", async (req, res) => {
    try {
        let cBody = JSON.parse(JSON.stringify(req.body));
        let newField = cBody.params.newField;
        let _collection = cBody.params.collection
        //let { filter, update } = await prepareUpdtMany(cBody.params.newField)
        let _params = {
            "filter": { eventId: "700", recStatus: false },
            "update": { $set: { recStatus: true } }
        }
        //let _params = cBody.params
        console.log("_params", _params)
        //_params.update = { $set: _params.update }
        mongoMapper(`cm_mirtheventtrans`, cBody.query, _params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch(async (error) => {
...
router.post("/get-doc-dashboard-data/old", async (req, res) => {
    try {
        if (req.body.params && req.body.params.docId && req.body.params.fromDt) {
            ...
            mongoMapper("cm_doctors", "aggregation", _docFilter, req.tokenData.dbType).then(async (result) => {
                if (!result.data || result.data.length == 0) {
                    ...
                    mongoMapper("cm_appointments", "aggregation", _aptFilter, req.tokenData.dbType).then(async (_resp) => {
...
router.post("/get-doc-dashboard-data", async (req, res) => {
    try {
        if (req.body.params && req.body.params.docId && req.body.params.locId && req.body.params.fromDt) {
            ...
            let _dashBoardData = [{}]
            await mongoMapper("cm_doctors", "aggregation", _docFilter, req.tokenData.dbType).then(async (result) => {
                if (!result.data || result.data.length == 0) {
                    return res.status(400).json({ status: 'FAIL', desc: 'No Doctor Found', data: [] });
                }
                else {
                    ...
                    await mongoMapper("cm_appointments", "aggregation", _aptFilter, req.tokenData.dbType).then(async (_resp) => {
                        if (!_resp.data || _resp.data.length == 0) {
                            return res.status(400).json({ status: 'FAIL', desc: 'Error While Getting Appointments', data: [] });
                        }
                        else {
            ...
            if (req.body.params.flag) {
                ...
                const appointmentData = await _mUtils.commonMonogoCall("cm_appointments", "aggregation", _dayWiseFilter, "", "", "", req.tokenData.dbType)
...
router.post('/forgot-password', async (req, res) => {
    try {
        if (req.body.flag == "REQ_OTP") {
            ...
            let _dbType = req.headers.orgkey.toLowerCase() || req.tokenData.dbType;
            mongoMapper("cm_users", "find", _filter, _dbType).then(async (result) => {
                if (result.data.length == 0) {
                    ...
                    let _uResp = await _mUtils.commonMonogoCall("cm_users", 'findOneAndUpdate', pLoadResp.payload, "", "", "", req.headers.orgkey.toLowerCase());
                    if (!(_uResp && _uResp.success)) {
                        return res.status(400).json({ success: false, status: 400, desc: _uResp.desc || "", data: _uResp.data || [] });
                    }
...
router.post("/insert-draft--bkp", async (req, res) => {
    try {
        if (req.body.params.userId && req.body.params.userId.length > 0 && req.body.params.formId && req.body.params.formId.length > 0) {
            ...
            let _mResp = await _mUtils.commonMonogoCall("cm_drafts", 'find', _filter, "", "", "", req.tokenData.orgKey.toLowerCase());
            if (_mResp.success == true && _mResp.data.length > 0) {
                let _data = JSON.parse(JSON.stringify(_mResp.data))
                let _prms = {
                    "params": {
                ...
                mongoMapper('cm_drafts', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                    return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
                }).catch((error) => {
                    return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
                });
                ...
                req.body.params['orgId'] = req.tokenData.orgId
                req.body.params['locId'] = req.tokenData.locId
                mongoMapper('cm_drafts', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
                    return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
                }).catch((error) => {
                    return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
                });
    ...
    try {
        if (req.body.params.userId && req.body.params.userId.length > 0 && req.body.params.formId && req.body.params.formId.length > 0) {
            ...
            if (_mResp.success == true && _mResp.data.length > 0) {
                ...
                //--update isDraft in appointments
                let _aFilter = {
                    "filter": {
                        "code": _data[0].visitNo,
                    }
                }
                let _appt = await _mUtils.commonMonogoCall("cm_appointments", 'find', _aFilter, "", "", "", req.tokenData.orgKey.toLowerCase());
                let _apptPrms = {
                ...
                let pLoadResp = { payload: {} };
                pLoadResp = await _mUtils.preparePayload('BW', _apptPrms);
                if (!pLoadResp.success) {
                    return res.status(400).json({ success: false, status: 400, desc: pLoadResp.desc || "", data: [] });
                }
                mongoMapper('cm_appointments', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                    // return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
                }).catch((error) => {
                    return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
                });
                ...
                req.body.params['orgId'] = req.tokenData.orgId
                req.body.params['locId'] = req.tokenData.locId
                mongoMapper('cm_drafts', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {

                    result.data = JSON.parse(JSON.stringify(result.data))
                    //----update isDraft in appointments
                    let _aFilter = {
                        "filter": {
                            "code": result.data[0].visitNo,
                        }
                    }
                    let _appt = await _mUtils.commonMonogoCall("cm_appointments", 'find', _aFilter, "", "", "", req.tokenData.orgKey.toLowerCase());
                    let _apptPrms = {
...
router.post("/get-draft", async (req, res) => {
    try {
        ...
        mongoMapper("cm_drafts", "find", _pGData.data, req.tokenData.dbType).then((result) => {
...
router.post("/update-draft--bkp", async (req, res) => {
    try {
        let _cBody = JSON.parse(JSON.stringify((req.body)));
        ...
        if (req.body.params._id) {
            let _mResp = await _mUtils.commonMonogoCall("cm_drafts", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            _cBody.params.revNo = _mResp.data.params.revNo;
    ...
    try {
        ...
        if (req.body.params._id) {
            ...
            mongoMapper('cm_drafts', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                ...
                if (req.body.params.status == "SUBMITTED" || req.body.params.status == "REJECTED") {
                    let _dResp = await _mUtils.commonMonogoCall("cm_drafts", "findById", req.body.params._id, "", "", "", req.tokenData.dbType)
                    if (!(_dResp && _dResp.success)) {
                        return res.status(400).json({ success: false, status: 400, desc: _dResp.desc || "", data: _dResp.data || [] });
                    }
                    let _aFilter = {
                        "filter": {
                            "code": _dResp.data.visitNo,
                        }
                    }
                    let _appt = await _mUtils.commonMonogoCall("cm_appointments", 'find', _aFilter, "", "", "", req.tokenData.orgKey.toLowerCase());
                    let _apptPrms = {
...
router.post("/generate-report", async (req, res) => {
    try {
        if (req.body.params && req.body.params.toDt && req.body.params.type && req.body.params.fromDt) {
                    ...
                    "filter": [
                        ...
                        },
                        {
                            $project: {
                                ...
                                auditDocumentBy: "$overalDetails.audit.documentedBy",
                                auditDocumentDt: "$overalDetails.audit.documentedDt",
                                consolidateData: 1,
                            },
                        },
                        {
                            $group: {
                                ...
                                overalData: {
                                    $push: {
                                        ...
                                        patProof: "$patProof",
                                        //audit: "$audit",
                                        auditDocumentBy: "$auditDocumentBy",
                                        auditDocumentDt: "$auditDocumentDt",
                                    },
                                },
                                consolidateData: {
            ...
            else if (req.body.params.type === "ApmntReport") {
                _collectionName = "cm_appointments";
                _patRegReport = {
                    "filter": [
                        {
                            $lookup: {
                                from: "doctors",
                                localField: "docDetails.docId",
                                foreignField: "_id",
                                as: "doctorInfo"
                            }
                        },
                        ...
                        },
                        {
                            $match: {
                                dateTime: {
                                    // $gte: "2024-11-21T00:00:000Z", $lte: "2024-11-22T23:59:59Z"
                                    $gte: `${fromDt}T00:00:00.000Z`, $lte: `${toDt}T23:59:59:999Z`,
                                }
                            }
                        },
                        {
                            $group: {
                                ...
                                overalData: {
                                    $push: {
                                        ...
                                        department: "$doctorInfo.speclityName",
                                        status: "$status",
                                        appointmentType: "$apmntType",
                                        payment: { isPaid: "$isPayment", amount: "$appointmentFee" },
                                        audit: "$audit",
                                        locationFees: {
                                            $arrayElemAt: [
                                                {
                                                    $filter: {
                                                        input: "$doctorInfo.locations", as: "location",
                                                        cond: {
                                                            $eq: [
                                                                "$$location.locId", req.tokenData.locId //"673ad5042a36715fbc9284f9"
                                                            ]
                                                        }
                                                    }
                                                }, 0
                        ...
                        },
                        {
                            $addFields: {
                                paidAmount: {
                                    $reduce: {
                                        input: "$overalData", initialValue: 0,
                                        in: {
                                            $add: [
                                                "$$value",
                                                {
                                                    $cond: [
                                                        { $eq: ["$$this.payment.isPaid", true] },
                                                        {
                                                            $switch: {
                                                                branches: [
                                                                    {
                                                                        case: {
                                                                            $eq: ["$$this.appointmentType", "NORMAL"]
                                                                        },
                                                                        then: { $toDouble: "$$this.locationFees.fees.normal" }
                                                                    },
                                                                    {
                                                                        case: {
                                                                            $eq: ["$$this.appointmentType", "EMERGENCY"]
                                                                        },
                                                                        then: { $toDouble: "$$this.locationFees.fees.emergency" }
                                                                    },
                                                                    {
                                                                        case: {
                                                                            $eq: ["$$this.appointmentType", "ONLINE"]
                                                                        },
                                                                        then: { $toDouble: "$$this.locationFees.fees.online" }
                                                                    },
                                                                    {
                                                                        case: {
                                                                            $eq: ["$$this.appointmentType", "REVISIT"]
                                                                        },
                                                                        then: { $toDouble: "$$this.locationFees.fees.reVisit" }
                                                                    },
                                                                    {
                                                                        case: {
                                                                            $eq: ["$$this.appointmentType", "REGIST"]
                                                                        },
                                                                        then: { $toDouble: "$$this.locationFees.fees.reg" }
                                                                    }
                                                                ], default: 0
                                                            }
                                                        }, 0
                                                            ...
                                                            $switch: {
                                                                branches: [
                                                                    ...
                                                                    },
                                                                    {
                                                                        ...
                                                                        then: {
                                                                            $toDouble: "$$this.locationFees.fees.reg"
                                                                        }
                                                                    }
                                                                ],
                                                                default: 0
                        ...
                        {
                            $lookup: {
                                from: "appointments", let: { umrList: "$totalUMRs" },
                                pipeline: [
                                    {
                                        $match: {
                                            $expr: {
                                                $and: [
                                                    ...
                                                    {
                                                        $lt: [
                                                            "$dateTime", `${fromDt}T00:00:00.000Z`//"2024-11-21T00:00:00Z"
                                                        ]
                                                    }
                                                ]
                                            }
                        ...
                        {
                            $addFields: {
                                oldAppointments: { $size: "$oldData" },
                                newAppointments: {
                                    $subtract: ["$totalAppointments", { $size: "$oldData" }]
                                }
                            }
                        },
                        {
                            $project: {
                                _id: 0,
                                reportType: {
                                    $literal: "Appointment Summary Report"
                                },
                                overalData: {
                                    $map: {
                                        input: "$overalData",
                                        as: "entry",
                                        in: {
                                            appointmentNo: "$$entry.appointmentNo",
                                            ...
                                            payment: "$$entry.payment.isPaid",
                                            //audit: "$$entry.audit"
                                            auditDocumentBy: "$$entry.audit.documentedBy",
                                            auditDocumentDt: "$$entry.audit.documentedDt",
                                        }
                                    }
                                },
                                consolidateData: {
                                    ...
                                    paidAppointmentsCount: "$paidAppointments",
                                    paidAmount: "$paidAmount",
                                    unpaidAppointmentsCount: "$unpaidAppointments",
                                    unpaidAmount: "$unpaidAmount"
                                },
                                labelNames: [
                                    { header: "Apmnt No", order: 1, visible: true, key: "appointmentNo", width: 130 },
                                    { header: "UMR No", order: 2, visible: true, key: "umr", width: 100 },
                                    { header: "Consulatation Date", order: 3, visible: true, key: "dateTime", width: 150 },
                                    { header: "Doctor Name", order: 4, visible: true, key: "doctorName", width: 200 },
                                    { header: "Department", order: 5, visible: true, key: "department", width: 180 },
                                    { header: "Status", order: 6, visible: true, key: "status", width: 80 },
                                    { header: "Apmnt Type", order: 7, visible: true, key: "appointmentType", width: 90 },
                                    { header: "Payment", order: 8, visible: true, key: "payment", width: 60 },
                                    { header: "Patient Name", order: 9, visible: true, key: "patientName", width: 200 },
                                    { header: "Age", order: 10, visible: true, key: "age", width: 130, width: 50 },
                                    { header: "Gender", order: 11, visible: true, key: "gender", width: 40 },
                                    { header: "Mobile", order: 12, visible: true, key: "mobileNo", width: 90 },
                                    { header: "Address", order: 13, visible: true, key: "address", width: 200 },
                                    { header: "Created By", order: 14, visible: true, key: "auditDocumentBy", width: 180, },
                                    { header: "Created Date", order: 15, visible: true, key: "auditDocumentDt", width: 150, },
                                    { header: "Total Apmnt", order: 16, visible: true, key: "totalAppointments", width: 30 },
                                    { header: "Old Apmnt", order: 17, visible: true, key: "oldAppointments", width: 30 },
                                    { header: "New Apmnt", order: 18, visible: true, key: "newAppointments", width: 30 },
                                    { header: "Paid Apmnt Count", order: 19, visible: true, key: "paidAppointmentsCount", width: 30 },
                                    { header: "Paid Amount", order: 20, visible: true, key: "paidAmount", width: 40 },
                                    { header: "Unpaid Apmnt Count", order: 21, visible: true, key: "unpaidAppointmentsCount", width: 30 },
                                    { header: "Unpaid Amount", order: 22, visible: true, key: "unpaidAmount", width: 40 },
                                ]
            ...
            else if (req.body.params.type === "TATReport") {
                _collectionName = "cm_appointments";
                _patRegReport = {
                    "filter": [
                        {
                            $match: {
                                dateTime: { $exists: true, $ne: null, $ne: "", $gte: `${fromDt}T00:00:00.000Z`, $lte: `${toDt}T23:59:59:999Z`, },
                                "checkStatus.checkInDt": { $exists: true, $ne: null, $ne: "" }
                            }
                        },
                        {
                            $addFields: {
                                checkInStartDuration: {
                                    $divide: [
                                        { $subtract: [{ $toDate: "$dateTime" }, { $toDate: "$checkStatus.checkInDt" }] },
                                        60000
                                    ]
                                },
                                waitingTime: {
                                    $cond: {
                                        if: { $and: [{ $ne: ["$checkStatus.checkOutDt", null] }, { $ne: ["$checkStatus.checkOutDt", ""] }] },
                                        then: {
                                            $divide: [
                                                { $subtract: [{ $toDate: "$checkStatus.checkOutDt" }, { $toDate: "$checkStatus.checkInDt" }] },
                                                60000
                                            ]
                                        },
                                        else: null
                        ...
                        },
                        {
                            $group: {
                                _id: null,
                                averageCheckInStartDuration: { $avg: "$checkInStartDuration" },
                        ...
                        },
                        {
                            $lookup: {
                                ...
                                pipeline: [
                                    { $unwind: "$locations" },
                                    { $unwind: "$locations.shifts" },
                                    { $addFields: { "locations.shifts.duration": { $toDouble: "$locations.shifts.duration" } } },
                                    { $group: { _id: null, avgDuration: { $avg: "$locations.shifts.duration" } } }
                                ],
                                as: "consultationDuration"
                            }
                        },
                        {
                            $addFields: {
                                checkInStartDuration: { $concat: [{ $toString: { $round: ["$averageCheckInStartDuration", 0] } }, " Min"] },
                                averageWaitingTime: { $concat: [{ $toString: { $round: ["$averageWaitingTime", 0] } }, " Min"] },
                                consultationDurationFormatted: {
                                    $concat: [{ $toString: { $round: [{ $arrayElemAt: ["$consultationDuration.avgDuration", 0] }, 0] } }, " Min"]
                                }
                            }
                        },
                        {
                            $project: {
                                _id: 0,
                                reportType: { $literal: "Turn Around Time (TAT) Report" },
                                overalData: [
                                    {
                                        patientWaitingTime: "$checkInStartDuration",
                                        patientConsultTime: "$averageWaitingTime",
                                        DoctorConsultationDuration: "$consultationDurationFormatted"
                                    }
                                ],
                                "labelNames": [
                                    { "header": "Pat Waiting Time", "order": 1, "visible": true, "key": "patientWaitingTime", "width": 100 },
                                    { "header": "Pat Consultant Time", "order": 2, "visible": true, "key": "patientConsultTime", "width": 100 },
                                    { "header": "Doc Consultation Duration", "order": 3, "visible": true, "key": "DoctorConsultationDuration", "width": 100 }
                                ]
                            }
                        }
                    ]
            ...
            await mongoMapper(_collectionName, "aggregation", _patRegReport, req.tokenData.dbType).then(async (_resp) => {
                if (!_resp.data || _resp.data.length == 0) {
...
router.post("/get-potential-Matches", async (req, res) => {
    try {
        const { dispName, genderCd, dob, mobile } = req.body.params;
        let _filter = {
            ...
            "selectors": {
                ...
                "patient_png_card": "$patient_png_card",
                "abhaNo": "$abhaNo",
                "abhaAddress": "$abhaAddress",
                // "abhaJson": {
                //     $filter: {
        ...
        let potentialMatches = await _mUtils.commonMonogoCall("cm_patients", 'find', _filter, "", "", "", req.tokenData.dbType);
        ...
        const genderMatchedRecors = potentialMatches.data.filter(record => record.genderCd === genderCd);
        if (genderMatchedRecors.length === 0) {
            return res.status(200).json({ success: true, status: 200, desc: "no gender match.", data: [] });
        }

        const dobYear = new Date(dob).getFullYear();
        let dobMatchedRecords = genderMatchedRecors.filter(record => {
            const recordDobYear = new Date(record.dob).getFullYear();
        ...
        dobMatchedRecords = JSON.parse(JSON.stringify(dobMatchedRecords))
        for (let resp of dobMatchedRecords) {
            if (resp.abhaCard && resp.abhaCard.abhaCard && resp.abhaCard.abhaCard !== "" && resp.abhaCard.abhaCardMimeType && resp.abhaCard.abhaCardMimeType.length > 0) {
                let data = await getBase64Data(resp.abhaCard.abhaCardMimeType, resp.abhaCard.abhaCard)
                resp['abhaCardMediaId'] = resp.abhaCard._id
                resp['abhaCard'] = data
...
router.post("/get-employee-search", async (req, res) => {
    try {
        if (req.body.params && (req.body.params.mobile || req.body.params.userName || req.body.params.dispName || req.body.params.empCd)) {
            let _filter = {
                "filter": {
                    "recStatus": true
                },
            ...
            if (req.body.params.mobile) {
                _filter.filter["mobile"] = req.body.params.mobile
            }
            else if (req.body.params.empCd) {
                _filter.filter["cd"] = req.body.params.empCd
            }
            else if (req.body.params.userName) {
                _filter.filter["userName"] =  { $regex: req.body.params.userName, $options: 'i' }
            }
            else if (req.body.params.dispName) {
                _filter.filter["dispName"] = { $regex: req.body.params.dispName, $options: 'i' }
            }
            let potentialMatches = await _mUtils.commonMonogoCall("cm_employee", 'find', _filter, "", "", "", req.tokenData.dbType);
            if (!(potentialMatches && potentialMatches.success)) {
                return res.status(400).json({ success: false, status: 400, desc: potentialMatches.desc || "", data: potentialMatches.data || [] });
            }
            else if (potentialMatches.data.length > 0) {
                return res.status(200).json({ success: true, status: 200, desc: [], data: potentialMatches.data });
            }
        }
        else {
            return res.status(400).json({ success: false, status: 400, desc: "Required parameters are missing ..", data: [] });
