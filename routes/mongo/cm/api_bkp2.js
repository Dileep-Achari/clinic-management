            ...
            if (type === "I") {
                ...
                if (!(_mResp && _mResp.success)) {
                    ...
                    if (empData.loc._id && empData.loc.roleId) {
                        ...
                        _locations[0].audit["modifiedByBy"] = req.tokenData.userName;
                    ...
                    userData.params.audit["modifiedByBy"] = req.tokenData.userName;
...
async function updateVisitsOrBillData(_umrNo, _type, _data, req) {
    try {
        return new Promise(async (resolve, reject) => {
            ...
            patData.params.audit["modifiedByBy"] = req.tokenData.userName;
        ...
        if (_data.length > _idx) {
            if (!_data[_idx]._id) {
                ...
                if (!(_mResp && _mResp.success)) {
                    ...
                    _data[_idx].audit["modifiedByBy"] = req.tokenData.userName;
...
router.use(function paramValidation(req, res, next) {
    try {
        ...
        if (_query == 'findById') {
            ...
            req["cAudit"] = {
                documentedById: req.tokenData ? req.tokenData.userId : null,
                documentedBy: req.tokenData ? req.tokenData.userName : null
            }
        }
...
router.post("/get-patient-search", async (req, res) => {
    try {
        if (req.body && req.body.params && req.body.params.flag && (req.body.params.flag === 'NAME' || req.body.params.flag == 'UMR') && req.body.params.searchValue && req.body.params.searchValue.length > 2) {
            let _filter = {
                "filter": {
                    "recStatus": { $eq: true }
                },
            ...
            if (req.body.params.flag == 'UMR') {
                _filter.filter["UMR"] = new RegExp('.*' + req.body.params.searchValue + '.*');
            }
