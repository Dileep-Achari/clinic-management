...
async function updateVisitsOrBillData(_umrNo, _type, _data, req,_result) {
    try {
        return new Promise(async (resolve, reject) => {
            ...
            let _uResp;
            if (_type === "AAPT_UPDT"|| _type === "UPDATE_APPOINTMENT") {
    ...
    try {
        if (req.body.params && req.body.params.docId && req.body.params.locId) {
            ...
            if (req.body.params.fromDt) {
                let zeroDateFrmt = new Date(req.body.params.fromDt)
                req.body.params.fromDt = new Date(zeroDateFrmt.setHours(zeroDateFrmt.getHours() + 10))
                if (req.body.params.toDt) {
                    let toDt = req.body.params.toDt ? new Date(new Date(req.body.params.toDt).setHours(new Date(req.body.params.toDt).getHours() + 10)) : new Date();
                    ...
                    _filter.filter["dateTime"] = { $gte: new Date(new Date(req.body.params.fromDt).setUTCHours(0, 0, 0, 0)).toISOString(), $lt: new Date(new Date(req.body.params.fromDt).setUTCHours(23, 59, 59, 999)).toISOString() }
                }
