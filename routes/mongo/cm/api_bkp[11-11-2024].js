    ...
    try {
        if (req.body && req.body.params && req.body.params.flag && (req.body.params.flag === 'NAME' || req.body.params.flag == 'MOB' || req.body.params.flag == 'UMR' || req.body.params.flag == 'VSTID' || req.body.params.flag === 'VSTNO') && req.body.params.searchValue && req.body.params.searchValue.length > 2) {
            ...
            mongoMapper("cm_patients", req.body.query, _filter, req.tokenData.dbType).then(async (result) => {
                ...
                if (_finalResp.length > 0 && _finalResp[0].visits && Array.isArray(_finalResp[0].visits) && _finalResp[0].visits.length > 0) {
                    for (let visit of _finalResp[0].visits) {
                        let _filter = {
                            "filter": {
                                "_id": visit.visitId
                            },
                        ...
                        let resp = await _mUtils.commonMonogoCall("cm_appointments", "find", _filter, "", "", "", req.tokenData.dbType)
                        if (resp.data && resp.data.length > 0) {
                            if (resp.data[0].recStatus !== true) {
                                visit['recStatus'] = false
                            }
                            else {
                                visit['recStatus'] = true
                            }
                        }
                    }
                }
...
router.post("/billing-search", async (req, res) => {
    try {
        if (req.body && req.body.params.searchValue && req.body.params.searchValue.length > 2) {
            let _filter = { filter: { recStatus: { $eq: true } }, limit: 50 };
            let _collection = "";
                ...
                if (req.body.flag === "INV" || req.body.flag === "MED") {
                    ...
                    mongoMapper(_collection, req.body.query, _filter, req.tokenData.dbType).then((result) => {
                        let filteredResult = result.data.map(obj => ({
                            ...
                            serviceCd: obj.cd,
                            serviceId: obj._id,
                            serviceType: req.body.flag === "INV" ? obj.serviceTypeName : obj.medFormTypeName,
                            tarrif: obj.tarrif || []
                        }));
...
router.post("/insert-new-field", async (req, res) => {
    try {
        let cBody = JSON.parse(JSON.stringify(req.body));
        //let newFiled = cBody.params.newField;
        //let collection =cBody.params.collection
        //let {filter, update} = await preapreUpdtMany(cBody.params.newField)
        let _params = {
            "filter": { RECORD_STATUS: "A", ORG_ID: 1002 },
            "update": { $set: { mig_id: 1216, recStatus: true, eventId: 184 } }
        }
...
//module.exports = insertAndUpdateUser;