...
async function processGrnEntries(grnData, req, results = { success: [], failed: [] }) {
    if (grnData.length === 0) {
...
router.all('/*', (req, res, next) => {
    try {
        console.log("method", req.method, req.body);
...
router.post("/insert-bill", async (req, res) => {
    try {
        ...
        mongoMapper('cm_bills', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            ...
            if (req.body && req.body.params.grnData && req.body.params.grnData.length > 0) {
                const grnUpdateResult = await processGrnEntries(req.body.params.grnData, req);
                if (!grnUpdateResult.success) {
                    return res.status(400).json({ success: false, status: 400, desc: grnUpdateResult.desc, data: grnUpdateResult.data });
                }
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }
            else {
...
router.post("/billing-search", async (req, res) => {
    try {
        if (req.body && req.body.params.searchValue && req.body.params.searchValue.length > 2) {
            ...
            let _collection = "";
            let staticArrey = ["cm_investigations", "cm_pharmacy_med_masters"]
            if (req.body.flag === "") {
                let hh = await getBillingDataSearch(staticArrey, 0, [], req.tokenData.dbType)
                let finalArrey = []
                let objectAssign = {
                ...
                _.filter(hh.data, (obj, indx) => {
                    _.filter(obj.data, (obj1) => {
                        if (obj1.serviceTypeName) {
                            ...
                            if (obj1.BRAND_STRING2.toLowerCase().includes(req.body.params.searchValue.toLowerCase())) {
                                objectAssign = {
                                    serviceName: obj1.BRAND_STRING2,
                                    type: "MED",
                ...
                if (req.body.flag === "INV" || req.body.flag === "MED") {
                    ...
                    if (req.body.flag === "MED") {
                        let nameExp = { $regex: req.body.params.searchValue, $options: 'i' }
                        _filter.filter["BRAND_STRING2"] = nameExp;
                        _collection = "cm_pharmacy_med_masters"
                    }
                    mongoMapper(_collection, req.body.query, _filter, req.tokenData.dbType).then((result) => {
                        let filteredResult = result.data.map(obj => ({
                            serviceName: req.body.flag === "INV" ? obj.name : obj.BRAND_STRING2,
                            type: req.body.flag,
                            serviceType: req.body.flag === "INV" ? obj.name : obj.DOSE_FORM_NAME,
                            serviceCd: req.body.flag === "INV" ? obj.cd : obj.BRAND_PRODUCT_MAP_CD,
                            serviceId: obj._id,
                            tarrif: obj.tarrif ? obj.tarrif : []
                        }));
                        return res.status(200).json({ success: true, status: 200, desc: '', data: filteredResult });
