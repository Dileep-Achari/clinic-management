...
router.post("/insert-mirthdata", async (req, res) => {
    console.log("insert-mirthdata")
    console.log("insert-mirthdata", req);
    try {
        console.log("insert-mirthdata", req.body);
        if (req.body && req.body.params.data) {

            const records = Array.isArray(req.body.params.data) ? req.body.params.data : [req.body.params.data]
            let results = [];
            for (let record of records) {
                ...
                try {
                    ...
                    let dataToInsert = {
                        "migId": jsonResult.results.result.mig_id,
                        "host": req.body.params.host,
                        "eventId": jsonResult.results.result.event_id,
                        "eventTrackId": jsonResult.results.result.event_track_id,
                        "data": jsonResult.results,
                        "recStatus": req.body.params.recStatus,
                        "audit": req.body.params.audit || {}
                    };
                    let insertResult = await mongoMapper('dk_crush_mirtheventtrans', "insertMany", dataToInsert, "emr");
                    if (insertResult && insertResult.data) {
                        results.push(`${jsonResult.results.result.event_track_id}^Success`)
                    }
                    else {
                        results.push(`${jsonResult.results.result.event_track_id}^Fail`)
                    }
                }
                catch (error) {
                    results.push(`${jsonResult.results.result.event_track_id}^Fail`)
            ...
            return res.status(200).json({ success: true, status: 200, desc: '', data: results.join('|') });
        }
        else {
...
router.post("/validate-user", async (req, res) => {
    try {
        if (req.body.params && req.body.params.userName && req.body.params.userName.length > 0) {
            ...
            mongoMapper("dk_crush_users", "find", _filter, _dbType).then(async (result) => {
...
//                 "selectors": ""
//             }
//             let _userData = await _mUtils.commonMonogoCall("dk_crush_users", "findOne", _uParams, "", "", "", req.headers.orgkey.toLowerCase());
//             // let _userData = await commonMonogoCall("dk_crush_users", "findOne", _uParams, "", req.body);
...
//                 let _sessionUpdate = await _mUtils.commonMonogoCall("dk_crush_usersessions", "findOneAndUpdate", pLoadResp.payload, "", "", "", req.headers.orgkey.toLowerCase());
...
//             let _sessionData = await _mUtils.commonMonogoCall("dk_crush_usersessions", "findOne", _filter, "", "", "", req.headers.orgkey.toLowerCase());
...
//             mongoMapper('dk_crush_usersessions', 'insertMany', _sessionParams, req.headers.orgkey.toLowerCase()).then(async (result) => {
...
router.post("/auth-user", async (req, res) => {
    try {
        if (req.body && req.body.orgId && req.body.uName && req.body.pwd && req.body.locId) {
            ...
            let _userData = await _mUtils.commonMonogoCall("dk_crush_users", "find", _uParams, "", "", "", req.headers.orgkey.toLowerCase());
            // let _userData = await commonMonogoCall("dk_crush_users", "findOne", _uParams, "", req.body);
            if (!(_userData && _userData.success && _userData.data && Object.keys(_userData.data).length > 0)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', code: 400, desc: "Invalid crendentials / No user found..", data: [] }));
            }
            ...
            let _mResp = await _mUtils.commonMonogoCall("dk_crush_roles", "find", _rParams, "", "", "", req.headers.orgkey.toLowerCase());
            ...
            let _sessionParams = {
                ...
                displayName: _clnUser[0].dispName,
                roleId: _user.roleId,
                roleName: _user.roleName,
                browser: _userAgent,
                token: _tkn,
...
router.use(async function verifyToken(req, res, next) {
    ...
    try {
        ...
        let _sessionData = await commonMonogoCall("dk_crush_usersessions", "findOne", _filter, "", req.body, "", "", req.headers.orgkey.toLowerCase());
        ...
        _token.verifyToken(_sessionData.data.token).then(async (data) => {
            ...
            let _sessionUpdate = await commonMonogoCall("dk_crush_usersessions", "findOneAndUpdate", pLoadResp.payload, "", "", "", "", req.tokenData.dbType)
...
router.post("/get-all-patient-data", async (req, res) => {
    try {
        ...
        mongoMapper("dk_crush_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
    ...
    try {
        ...
        if (req.body && req.body.params && req.body.params.flag && (req.body.params.flag == 'NAME' || req.body.params.flag == 'MOB' || req.body.params.flag == 'ADMN' || req.body.params.flag == 'UMR' || req.body.params.flag == 'DOS') && req.body.params.searchValue && req.body.params.searchValue.length > 0) {
            ...
            if (req.body.params.flag == 'DOS') {
                ...
                mongoMapper("dk_crush_patients", "aggregation", _filter, req.tokenData.dbType).then(async (_resp) => {
                    if (_resp.data.length > 0) {
                        ...
                        _.map(_response, (_o) => { _o['mobile'] = _o.mobile && _o.mobile[0].num ? _o.mobile[0].num : "" })

                        // let _finalResp = []
                        // _.each(_response, (_o,_i)=>{
                        //     let _nameWithSpace = _o.dg.dispName.split(' ')
                        //     _o.dg['umr'] = _o.umr
                        //     _o.dg['fName'] = _nameWithSpace && _nameWithSpace.length > 1 ? _nameWithSpace[0] : _nameWithSpace
                        //     _o.dg['lName'] = _nameWithSpace && _nameWithSpace.length > 1 ? _nameWithSpace[1] : ""
                        //     let data = transform(_o.dg, baseMap)
                        //     _finalResp.push(data)
                        // })


                        return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: ``, data: _response.reverse() || [] }));
                    }
                    else {
                    ...
                    return res.status(400).json(await encryptData({ status: 'FAIL', status: 400, desc: error.desc || error, data: [] }));
                });
                ...
                mongoMapper("dk_crush_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
                    ...
                    // }
                    if (req.clientUrls.patSearch !== true) {
                        return res.status(200).json(await encryptData({ status: "SUCCESS", desc: '', data: result.data.reverse() || [] }));
                    }
                    else {
            ...
            if (_parseData && _parseData.data) {
                for (let _o in _parseData.data) {
                    ...
                    _.filter(_parseData.data[_o].comorbidities, (_c) => {
                        if ((_c && _c.cd && _c.cd == "DIABETES") && _c.value == "Y") {
                            _parseData.data[_o]['firstSurgeryDiab'] = "Yes"
                        }
                    })

                    let _scores = []
                    if ((_parseData.data[_o].scores && _parseData.data[_o].scores.length > 0) && (_parseData.data[_o].surgeries && _parseData.data[_o].surgeries.length > 0)) {
                        _.filter(_parseData.data[_o].scores, (_s) => {
                            if (_s && _s.surgId == _parseData.data[_o].surgeries[0].surgId) {
...
router.post("/insert-patient", async (req, res) => {
    try {
        ...
        // let phone = req.body.params.phone[0].num
        ...
        //     "selectors": "-document -surgeries -scores"
        // }
        let _patientData = await commonMonogoCall("dk_crush_patients", "findOne", _filter, "", req.body, "", "", req.tokenData.dbType)
        if ((_patientData && _patientData.success && _patientData.data && _patientData.data.length > 0)) {
            return res.status(400).json(await encryptData({ patExistCd: 100, status: 'FAIL', desc: "Patient already exists..", data: _patientData }));
        }
        req.body.params["orgId"] = req.tokenData.orgId;
        req.body.params["locId"] = req.tokenData.locId;
        mongoMapper('dk_crush_patients', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
...
router.post("/update-patient-details", async (req, res) => {
    try {
        let _patient = await commonMonogoCall("dk_crush_patients", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData, req.tokenData.dbType)
        ...
        mongoMapper('dk_crush_patients', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
        });
...
router.post("/insert-surgery-data", async (req, res) => {
    try {
        if (req.body.params.patientId && req.body.params.umr) {
            ...
            let _params = {};
            let _surgery = await commonMonogoCall("dk_crush_surgeries", "findOne", _filter, "", req.body, "", "", req.tokenData.dbType)
            ...
            mongoMapper('dk_crush_surgeries', _query, _params, req.tokenData.dbType).then(async (result) => {
                ...
                if (_resp && _resp.admns && _resp.admns.length > 0) {
                    ...
                    if (_admns && _admns.length > 0) {
                        ...
                        if (_surgData && _surgData.length > 0) {
                            let _patObj = {
                                params: {
                                    ...
                                    surgeries:
                                    {
                                        ...
                                        surgeryClosedBy: "",
                                        surgeryClosedDt: "",
                                        surgeryBranch: _surgData[0].patInfo && _surgData[0].patInfo.hospName ? _surgData[0].patInfo.hospName : "",
                                        surgeryIntraOPComp: _surgData[0].intraOpDet && _surgData[0].intraOpDet.intOpComp ? _surgData[0].intraOpDet.intOpComp : "",
                            ...
                            let _surgUpdate = await commonMonogoCall("dk_crush_patients", "updateOne", pLoadResp.payload, "", "", "", "", req.tokenData.dbType)
...
router.post("/get-surgery-data-by-id", async (req, res) => {
    try {
        if (req.body.params.surgId) {           //req.body.params.admNo && 
            let _params = {
                "filter": { 'admns.admnNo': req.body.params.admnNo, "orgId": req.tokenData.orgId, "locId": req.tokenData.locId },
                "selectors": { 'admns.$': 1 }
            }
            mongoMapper("dk_crush_surgeries", "findOne", _params, req.tokenData.dbType).then(async (result) => {
                ...
                if (_surgData && _surgData.length > 0) {
                    ...
                    let _sData = {
                        ...
                        "admns": {
                            ...
                            "admnNo": result.data.admns[0].admnNo || "",
                            "datOfAdmn": result.data.admns[0].datOfAdmn || "",
                            "surgDtls": _surgData
                        }
                    }
    ...
    try {
        if (req.body.params._id && req.body.params.admns && req.body.params.admns[0] && req.body.params.admns[0]._id && req.body.params.admns[0].surgDtls && req.body.params.admns[0].surgDtls[0] && req.body.params.admns[0].surgDtls[0]._id && (req.body.params.admns[0].surgDtls[0].revNo || req.body.params.admns[0].surgDtls[0].revNo == 0)) {
            ...
            */
            ...
            pLoadResp.payload.pData.push(_cloneBw);
            mongoMapper('dk_crush_surgeries', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                let pLoadResp = { payload: {} };
                ...
                if (_cloneBody.flag !== "REVIEW" && _cloneBody.flag !== "FOLLOWUP") {
                    ...
                    let _patient = await commonMonogoCall("dk_crush_patients", "bulkWrite", pLoadResp.payload, "", req.body, "", "", req.tokenData.dbType);
                }
                ...
                if (_cloneBody.flag === "REVIEW") {
                    ...
                    let _reviewData = await commonMonogoCall("dk_crush_reviews", "insertMany", _reviewParams, "", "", "", "", req.tokenData.dbType)
...
router.post("/print-surgery-data", async (req, res) => {
    try {
        ...
        mongoMapper("dk_crush_surgeries", "findOne", _params, req.tokenData.dbType).then(async (result) => {
            ...
            let _patientData = await commonMonogoCall("dk_crush_patients", "findOne", _patParams, "", req.body, "", "", req.tokenData.dbType)
            ...
            let _scores = await commonMonogoCall("dk_crush_scores", "find", _scoresParams, "", req.body, "scores", "", req.tokenData.dbType)
...
router.post("/insert-scores-data-deprecated", async (req, res) => {
    try {
        if (req.body.params.umr) {
            ...
            let _patient = await commonMonogoCall("dk_crush_patients", "findOne", _filter, "", req.body, "", "", req.tokenData.dbType)
            if (!(_patient && _patient.success)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: _patient.desc || "", data: _patient.data || [] }));
            }
            delete req.body.params.umr;
            ...
            mongoMapper('dk_crush_patients', "findOneAndUpdate", pLoadResp.payload).then(async (result) => {
...
router.post("/update-score-details-deprecated", async (req, res) => {
    try {
        if (req.body.params._id && req.body.params.scores && req.body.params.scores[0] && req.body.params.scores[0]._id) {
            ...
            let _patient = await commonMonogoCall("dk_crush_patients", "findOne", _params, "REVNO", { params: req.body.params.scores[0] }, "scores", "", req.tokenData.dbType)
            if (!(_patient && _patient.success)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: _patient.desc || "", data: _patient.data || [] }));
            }
...
router.post("/insert-scores-data", async (req, res) => {
    try {
        if (req.body.params.patientId && req.body.params.umr) {
            let _patient = await commonMonogoCall("dk_crush_patients", "findById", req.body.params.patientId, "", req.body, "", "", req.tokenData.dbType)
            if (!(_patient && _patient.success)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: _patient.desc || "", data: _patient.data || [] }));
            }
            req.body.params["orgId"] = req.tokenData.orgId;
            ...
            let _cloneBody = JSON.parse(JSON.stringify(req.body.params));
            delete req.body.params.supporttedObj;
            mongoMapper("dk_crush_scores", "insertMany", req.body.params, req.tokenData.dbType).then(async (result) => {
                ...
                let _patUpdate = await commonMonogoCall("dk_crush_patients", "bulkWrite", pLoadResp.payload, "", "", "", "", req.tokenData.dbType)
...
router.post("/get-scores-data-by-id", async (req, res) => {
    try {
        if (req.body.params._id) {
            mongoMapper("dk_crush_scores", "findById", req.body.params._id, req.tokenData.dbType).then(async (result) => {
                if (!(result.data && result.data._id)) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No Records are available.." || error, data: [] }));
                }
                return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: result.data || [] }));
...
router.post("/update-score-details", async (req, res) => {
    try {
        if (req.body.params._id) {
            let _patient = await commonMonogoCall("dk_crush_scores", "findById", req.body.params._id, "REVNO", req.body, "scores", "", req.tokenData.dbType)
            if (!(_patient && _patient.success)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: _patient.desc || "", data: _patient.data || [] }));
            }
            ...
            _cloneBw.updateOne.filter = { "_id": pLoadResp.payload._id };
            pLoadResp.payload.pData.push(_cloneBw);
            mongoMapper('dk_crush_scores', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                let _patObj = {
                    params: {
...
router.post("/upload-document", async (req, res) => {
    try {
        if (req.body.params.patientId && req.body.params.umr && req.body.params.docs) {
            ...
            mongoMapper("dk_crush_documents", "insertMany", req.body.params.docs, req.tokenData.dbType).then(async (result) => {
                if (!(result.data && result.data.length > 0)) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No Records are available.." || error, data: [] }));
                }
                        ...
                        if (_rObj.docName == _doc.docName) {
                            try {
                                ...
                                if (_docData.success && _doc.docName == _docData.name) {
                                    ...
                                    let _patUpdate = await commonMonogoCall("dk_crush_patients", "updateOne", pLoadResp.payload, "", "", "", "", req.tokenData.dbType)
                                    if (!(_patUpdate && _patUpdate.success)) {
                                        // return res.status(400).json({ status: 'FAIL', desc: _surgUpdate.desc || "", data: _surgUpdate.data || [] });
                                    }
                                }
...
router.post("/get-upload-documents-by-umr", async (req, res) => {
    try {
        if (req.body.params._id) {
            ...
            mongoMapper("dk_crush_patients", "find", _params, req.tokenData.dbType).then(async (result) => {
...
router.post("/get-upload-documents-by-id", async (req, res) => {
    try {
        if (req.body.params._id) {
            mongoMapper("dk_crush_documents", "findById", req.body.params._id, req.tokenData.dbType).then(async (result) => {
                if (!(result.data)) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No Records are available.." || error, data: [] }));
                }
                let _base64 = "";
            ...
            if (req.body.params.from === 'S') {
                ...
                axios.post(req.clientUrls.apkApiUrl + "/getIpClinicalData", _params).then(async function (response) {
                    ...
                    if (response && response.SQL_ERROR) {
                        ...
                        let _patient = await commonMonogoCall("dk_crush_patients", "find", _params, "", req.body, "", "", req.tokenData.dbType)
                        if (Object.keys(logObj).length > 0 && _patient.success && _patient.data && _patient.data[0] && Object.keys(_patient.data[0]).length > 0) {
                            ...
                            let _patUpdate = await commonMonogoCall("dk_crush_patients", "findOneAndReplace", _payload, "", "", "", "", req.tokenData.dbType)
                            if (!(_patUpdate && _patUpdate.success)) {
                                // return res.status(400).json({ status: 'FAIL', desc: _surgUpdate.desc || "", data: _surgUpdate.data || [] });
                            }
                        }
                ...
                let _clinicalInfo = await commonMonogoCall("dk_crush_patients", "find", _params, "", req.body, "", "", req.tokenData.dbType)
...
router.post("/insert-role", async (req, res) => {
    try {
        ...
        delete req.body.params.audit;
        mongoMapper('dk_crush_roles', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
...
router.post("/get-roles", async (req, res) => {
    try {
        ...
        mongoMapper("dk_crush_roles", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/update-role", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            let _mResp = await _mUtils.commonMonogoCall("dk_crush_roles", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            ...
            let _hResp = await _mUtils.insertHistoryData('dk_crush_roles', _mResp.data.params, _cBody.params, req);
            let pLoadResp = { payload: {} };
            ...
            _cloneBw.updateOne.filter = { "_id": pLoadResp.payload._id };
            pLoadResp.payload.pData.push(_cloneBw);
            mongoMapper('dk_crush_roles', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
            });
...
router.post("/insert-user", async (req, res) => {
    try {
        if (req.body && req.body.params) {
            ...
            let _seqResp = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: 'Users' }, "dk_crush", req);
            ...
            mongoMapper('dk_crush_users', "insertMany", req.body.params, req.tokenData.dbType).then(async (result) => {
                if (!(result && result.data && result.data.length > 0)) {
...
router.post("/get-user", async (req, res) => {
    try {
        ...
        mongoMapper("dk_crush_users", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/update-user", async (req, res) => {
    try {
        let _cBody = JSON.parse(JSON.stringify((req.body)));
        if (req.body.params._id) {
            let _mResp = await _mUtils.commonMonogoCall("dk_crush_users", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] }));
            }
            let _hResp = await _mUtils.insertHistoryData('dk_crush_users', _mResp.data.params, _cBody.params, req);
            let pLoadResp = { payload: {} };
            if (!(_hResp && _hResp.success)) {
            ...
            mongoMapper('dk_crush_users', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
            });
...
router.post("/insert-form", async (req, res) => {
    try {
        req.body.params.orgId = req.tokenData.orgId;
        let _seqResp = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: 'Forms' }, "dk_crush", req);
        if (!(_seqResp && _seqResp.success)) {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: _seqResp.desc || "", data: _seqResp.data || [] }));
        }
        req.body.params["cd"] = _seqResp.data;
        mongoMapper('dk_crush_forms', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/get-forms", async (req, res) => {
    try {
        ...
        mongoMapper("dk_crush_forms", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/update-form", async (req, res) => {
    try {
        let _cBody = JSON.parse(JSON.stringify((req.body)));
        if (req.body.params._id) {
            let _mResp = await _mUtils.commonMonogoCall("dk_crush_forms", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] }));
            }
            let _hResp = await _mUtils.insertHistoryData('dk_crush_forms', _mResp.data.params, _cBody.params, req);
            let pLoadResp = { payload: {} };
            if (!(_hResp && _hResp.success)) {
            ...
            mongoMapper('dk_crush_forms', 'findOneAndUpdate', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
            });
...
router.post("/insert-counter", async (req, res) => {
    try {
        ...
        mongoMapper('dk_crush_counters', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/insert-field-management", async (req, res) => {
    try {
        if (req.body && req.body.params) {
            ...
            mongoMapper('dk_crush_fieldsmanagement', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
            });
...
router.post("/get-field-management", async (req, res) => {
    try {
        if (req.body && req.body.params) {
            ...
            mongoMapper("dk_crush_fieldsmanagement", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
...
router.post("/update-field-management", async (req, res) => {
    try {
        let _cBody = JSON.parse(JSON.stringify((req.body)));
        if (req.body.params._id) {
            let _mResp = await _mUtils.commonMonogoCall("dk_crush_fieldsmanagement", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] }));
            }
            let _hResp = await _mUtils.insertHistoryData('dk_crush_fieldsmanagement', _mResp.data.params, _cBody.params, req);
            let pLoadResp = { payload: {} };
            if (!(_hResp && _hResp.success)) {
            ...
            mongoMapper('dk_crush_fieldsmanagement', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
            });
...
router.post("/get-entity", async (req, res) => {
    try {
        ...
        mongoMapper("dk_crush_entity", req.body.query, _pGData.data, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/logout", async (req, res) => {
    try {
        ...
        //     "selectors": ""
        // }
        // let _sessionData = await commonMonogoCall("dk_crush_usersessions", "findOne", _filter, "", req.body);
        ...
        mongoMapper("dk_crush_usersessions", "findOneAndUpdate", pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
...
router.post("/get-all-surgeries-list", async (req, res) => {
    try {
        ...
        mongoMapper("dk_crush_surgeries", "aggregation", _filter, req.tokenData.dbType).then(async (result) => {
...
router.post("/insert-unlock-requests", async (req, res) => {
    try {
        if (req.body && req.body.params) {
            ...
            mongoMapper('dk_crush_unlockrequests', "insertMany", _cBody.params, req.tokenData.dbType).then(async (_result) => {
                ...
                mongoMapper("dk_crush_surgeries", "find", _filter, req.tokenData.dbType).then(async (result) => {
                    if (!result.data || result.data.length == 0) {
                    ...
                    mongoMapper("dk_crush_surgeries", "bulkWrite", pLoadResp.payload, req.tokenData.dbType).then(async (resp) => {
                        if (!resp) {
                        ...
                        mongoMapper("dk_crush_patients", "bulkWrite", _pLoad.payload, req.tokenData.dbType).then(async (_resp) => {
                            if (!_resp) {
                            ...
                            // mongoMapper('dk_crush_unlockrequests', "insertMany", _cBody.params, req.tokenData.dbType).then(async (result) => {
                            //     return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
                            // }).catch(async (error)=>{
                            //     return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
                            // })
...
router.post("/get-unlock-requests", async (req, res) => {
    try {
        ...
        mongoMapper("dk_crush_unlockrequests", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
        });
        ...
        mongoMapper("dk_crush_surgeries", "find", _filter, req.tokenData.dbType).then(async (result) => {
            ...
            mongoMapper("dk_crush_surgeries", "bulkWrite", pLoadResp.payload, req.tokenData.dbType).then(async (resp) => {
                ...
                mongoMapper("dk_crush_patients", "bulkWrite", _pLoad.payload, req.tokenData.dbType).then(async (_resp) => {
                    ...
                    mongoMapper("dk_crush_unlockrequests", "bulkWrite", _pLoad.payload, req.tokenData.dbType).then(async (_finalResp) => {
...
router.post("/get-filter-by-umr", async (req, res) => {
    try {
        ...
        else if (req.body.flag == "NAME") {
            if (req.body.params.umr && req.body.params.phone && req.body.params.gender && req.body.params.dob) {
...
router.post("/get-dashboard-data", async (req, res) => {
    try {
        ...
        mongoMapper("dk_crush_patients", "aggregation", _filter, req.tokenData.dbType).then(async (result) => {
            if (!result.data || result.data.length == 0) {
...
router.post("/insert-report-request", async (req, res) => {
    try {
        ...
        mongoMapper('dk_crush_reportrequests', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/get-reports-data", async (req, res) => {
    try {
        ...
        //     return res.status(400).json(await encryptData({ success: false, status: 400, desc: _pGData.desc, data: [] }));
        // }
        mongoMapper("dk_crush_reports", "find", _filter, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/insert-template", async (req, res) => {
    try {
        ...
        mongoMapper('dk_crush_templates', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/update-template", async (req, res) => {
    try {
        let _cBody = JSON.parse(JSON.stringify((req.body)));
        if (req.body.params._id) {
            let _mResp = await _mUtils.commonMonogoCall("dk_crush_templates", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] }));
            }
            let _hResp = await _mUtils.insertHistoryData('dk_crush_templates', _mResp.data.params, _cBody.params, req);
            let pLoadResp = { payload: {} };
            if (!(_hResp && _hResp.success)) {
            ...
            mongoMapper('dk_crush_templates', 'findOneAndUpdate', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
            });
...
router.post("/insert-pincodes", async (req, res) => {
    try {
        ...
        mongoMapper('dk_crush_pincodes', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
...
//         mongoMapper(`dk_crush_zipcodes`, "aggregation", _filter, req.tokenData.dbType).then(async (result) => {
//             console.log("result", result.data[0])
//             mongoMapper(`dk_crush_pincodes`, "insertMany", result.data, req.tokenData.dbType).then(async (result)=>{
//                 return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result }));
//             }).catch(async (error) => {
    ...
    try {
        ...
        _.each(_pGData.data.filter, (_v, _k) => {
            if (typeof _v !== "number") {
                ...
                let min = _v * Math.pow(10, 6 - _digits) - 1
                let max = (_v + 1) * Math.pow(10, 6 - _digits) - 1
                _pGData.data.filter[`${_k}`] = { $gte: min, $lte: max }
            }
        })
        mongoMapper('dk_crush_pincodes', req.body.query, _pGData.data, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
function prepareUpdtMany(obj, parentKey = '', filter = {}, update = {}) {
    // let filter = {}
    // let update = {}
        ...
        if (obj.hasOwnProperty(key)) {
            ...
            if (Array.isArray(obj[key]) && obj[key].length > 0) {
                if (typeof obj[key] == 'object' && obj[key] !== null) {
                    prepareUpdtMany(obj[key][0], `${newKey}.0`, filter, update)
                    //const nestedResult = prepareUpdtMany(obj[key][0], `${newKey}.0`, filter, update)
                    // filter = { ...filter, ...nestedResult.filter }
                    // update = { ...update, ...nestedResult.update }
                    // const { filter:nestedFilter, update:nestedUpdate } = prepareUpdtMany(obj[key][0], `${newKey}.0`)
                    // Object.assign(filter, nestedFilter)
                    // Object.assign(update, nestedUpdate)
                }
                else {
                    filter[newKey] = { $exists: false }
                    update[newKey] = obj[key]
                }
            }
            else if (typeof obj[key] == 'object' && obj[key] !== null) {
                prepareUpdtMany(obj[key], newKey, filter, update);
                //const nestedResult = prepareUpdtMany(obj[key], newKey, filter, update);
                // filter = { ...filter, ...nestedResult.filter }
                // update = { ...update, ...nestedResult.update }
                // const {filter: nestedFilter, update:nestedUpdate} = prepareUpdtMany(obj[key], newKey);
                // Object.assign(filter, nestedFilter)
                // Object.assign(update, nestedUpdate)
            }
            else {
...
router.post("/insert-new-field", async (req, res) => {
    try {
        ...
        //let { filter, update } = prepareUpdtMany(cBody.params.newField)
        // let _params = {
        //     "filter": { [newField]: { $exists: false } },
        //     "update": { $set: { [newField]: true } }
        // }
        let _params = {
            "filter": { stateName: { $eq: cBody.params.stateName } },
            "update": { $set: { stateCd: cBody.params.stateCd } }
        }
        //let _params = { filter, update }
        console.log("_params", _params)
        //_params.update = { $set: _params.update }
        mongoMapper(`dk_crush_${_collection}`, cBody.query, _params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            console.log("api-error", error)
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
