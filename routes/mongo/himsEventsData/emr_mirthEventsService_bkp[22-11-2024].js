...
//const insertAndUpdateUser = require('../cm/api')

const loggingTransports = [
    new DailyRotateFile({
        name: 'file',
        datePattern: 'YYYYMMDD',
        filename: `${dirPath}public/logs/winston/sh_hims_events_data/%DATE%.log`,
        maxFiles: '15d',
        maxsize: 10000000
    })
];
...
async function insertAndUpdateUser(type, req, empData) {
    return new Promise(async (resolve, reject) => {
        try {
            if (type === "I") {
                console.log("inside insertAndUpdateUser vvvvvvffffffffffvvvvvvvvvvvv ")
                let userData = {
                ...
                //console.log("userData ff333", userData)
                ...
                if (!(_mResp && _mResp.success)) {
                   ...
                   // console.log("userData success _mResp.data")
...
function prepareEvent184(result) {
    try {
        return {
            ...
            parameters: [],
            audit: {
                documentedById: "6729c846504d4fd7cb5d43e7",
                documentedBy: "sh_himssync",
                documentedDt: new Date().toISOString()
            }
        };
    } catch (error) {
...
function prepareEvent700(result) {
    try {
        return {
            "docId": "671f706b75dcb2b8137a55e1",
            "isVIP": result.is_vip || false, //false,
            "patName": result.display_name || result.first_name || "", //"mahesh  kumar  pippalla",
            "reasonForVisit": "Illness",
            "email": "mahesh@gmail.com",
            "mobile": result.mobile_no2 || "", //"6666666666",
            "apmntType": result.consultation_type || "", //"NORMAL",
            "address": result.address2 || result.address1 || "", //"hsdhdhdjssdhdcdhdhcdchdcdhdchbjk",
            "refBy": "",
            "titleCd": result.titile_cd ? { "mr": "MR", "mrs": "MRS", "dr": "DR", "ms": "MS" }[result.titile_cd.trim().toLowerCase()] || "" : "",//"MR",
            "titleName": result.title_cd || "",//"Mr.",
            "shiftId": "671f706b75dcb2b8137a55ec",
            "locId": "643015a8dcb6ba449bcd9deb",
            "patId": "66f14747c25c05b7c96bd5d8",
            "UMR": result.umr_no, //"MB240900070",
            "age": new Date().getFullYear() - new Date(result.dob).getFullYear() + "Y", //"27Y",
            "gender": { "F": "Female", "M": "Male" }[result.gender_cd] || "", //"Male",
            "visit": result.consultationno, //"OP",
            "apmntDtTime": "29-Oct-2024 17:00"
        };
    } catch (error) {
        console.error(`Error preparing payload for event 701: ${error.message}`);
        return {};
...
async function insertOrUpdateData(collectionName, preparedData, dbName, eventId, matchedMigId) {
    try {
        await logMessage('info', `Starting insertOrUpdateData for collection: ${collectionName}, eventId: ${eventId}`);
        const specialEventIds = ['7011', '9001', '1981', '1811', '184'];
        let existingRecord;
        let eventConfigItem = eventConfig[eventId];
        await logMessage('info', `Special Event IDs: ${(specialEventIds)}, EventConfigItem: ${(eventConfigItem)}`);
        if (specialEventIds.includes(eventId)) {
            await logMessage('info', `Processing special eventId: ${eventId}`);
            let _filter;
            if (eventId === '1981') {
            ...
            await logMessage('info', `_filter created for eventId ${eventId}: ${(_filter),dbName}`);
            existingRecord = await mongoMapper(collectionName, 'find', _filter, dbName);
            await logMessage('info', `Existing record for eventId ${eventId}: ${existingRecord}`);
        }
        if (existingRecord && existingRecord.data.length > 0) {
            await logMessage('info', `Updating existing record in collection: ${collectionName}, eventId: ${eventId}`);
            let _payload = {
                by: { _id: existingRecord.data[0]._id },
                query: { $set: preparedData },
                mode: { $upsert: true }
            }
            await logMessage('info', `Payload for update: ${(_payload)}`);
            let updateResult = await mongoMapper(collectionName, 'findOneAndUpdate', _payload, dbName);
            await logMessage('info', `Update result for eventId ${eventId}: ${(updateResult)}`);
            ...
            await logMessage('info', `Inserting new record into collection: ${collectionName} for special eventId ${eventId}`);
            await logMessage('info', `Event Config Item: ${(eventConfigItem)}`);
            if(eventConfigItem && eventConfigItem.seqName) {
                ...
                await logMessage('info', `Event  matchedMigId: ${JSON.stringify(matchedMigId)}`);
                await logMessage('info', `Event Config matchedMigId: ${JSON.stringify(matchedMigId._id)}`);
                await logMessage('info', `Event : ${JSON.stringify({ locId: matchedMigId._id, seqName: eventConfigItem.seqName, req:req } )}`);
                let _seqResp = await _mUtils.getSequenceNextValue({ locId: matchedMigId._id, seqName: eventConfigItem.seqName }, "cm", req);
                await logMessage('info', `Sequence response for eventId ${eventId}: ${JSON.stringify(_seqResp)}`);
                if (!(_seqResp && _seqResp.success)) {
                    let errorMsg = `Failed to process getSequenceNextValue for eventId ${eventId}`
                    await logMessage('Error', errorMsg)
                }
                else {
                    if (eventId === '1981') {
            ...
            //await logMessage('info', `Prepared data for insert: ${JSON.stringify(preparedData)}`);
            let insertResult = await mongoMapper(collectionName, "insertMany", preparedData, dbName);
            //await logMessage('info', `Insert result for eventId ${eventId}: ${(insertResult)}`);
        ...
        if (getdata && Array.isArray(getdata.data) && getdata.data.length > 0) {
            ...
            if (filteredData.length === 0) {
                ...
                for (let record of filteredData) {
                    ...
                    await logMessage('info', `okkkkk migID ${migID}`);
                    let _filter = {
                        "filter": { 'locations[0].migId': migID  },
                        "selectors": {}
                    }
                    await logMessage('info', `okkkkk _filter ${_filter}`);
                    ...
                    await logMessage('info', `okkkkk matchedMigId okkk ${JSON.stringify(matchedMigId)}`);
                    //console.log("_filter locRecord", locRecord.data[0].locations)
                    //console.log("_filter matchedMigId", matchedMigId)
...
cron.schedule('*/10 * * * * *', () => {
    console.log("starting sh mirthEventsTransaction service")
    getdata('sh');
});
