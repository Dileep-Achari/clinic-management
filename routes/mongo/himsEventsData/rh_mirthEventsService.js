...
let req = {
    tokenData: {
        dbType: "rh",
        dbName: "rh",
        userId: "6716207a63bfefeff4418687",
        displayName: "System S",
        userName: "system"
    },
    body: {
        params: {
            ...
            locations: [
                {
                    ...
                    locKey: matchedMigId ? matchedMigId.locKey : "rh",
...
function prepareEvent184(result) {
    try {
        return {
            ...
            serviceGroupCd: result.SERVICE_GROUP_CD || "", 
...
async function insertOrUpdateData(collectionName, preparedData, dbName, eventId, matchedMigId, _docLoc) {
    try {
        //const specialEventIds = ['9001', '1981', '1811', '1841', '7001'];
                    ...
                    if (eventId === '700') {
                        ...
                        if ((_mResp && _mResp.success && _mResp.data.length === 0)) {
                            ...
                            if ((!(_mResp && _mResp.success)) || (_mResp && _mResp.success && _mResp.data.length === 0)) {
                                let errorMsg = `There is no Doctor available in this hospital against to ${record.data.rsrc_cd}`
                                await logMessage('Error', errorMsg)
                                //continue;
