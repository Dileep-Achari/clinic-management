...
async function insertOrUpdateData(collectionName, preparedData, dbName, eventId, matchedMigId) {
    try {
        //console.log("=========inside insertOrUpdateData========");
        ...
        if (existingRecord && existingRecord.data.length > 0) {
            ...
            console.log("updating existingRecord using findOneAndUpdate");
            let _payload = {
            ...
            console.log("inserting newRecord using insertmany", preparedData);
            console.log("eventConfigItem", eventConfigItem.seqName);
        ...
        let errorMsg = `Error inserting or updating data into ${collectionName}: ${error.desc}`;
        await logMessage('error', errorMsg);
        return { success: false, desc: errorMsg };
    }
}
