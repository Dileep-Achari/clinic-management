...
router.post('/update-snomedct-data', async (req, res) => {
    try {
        let _filter = {
            filter: [
                {
                    $match: {
                        recStatus: { $eq: true },
                        DISEASE_CD: { $regex: "E", $options: "i" },
                        DISEASE_MASTER_TYPE: { $regex: "snomed", $options: "i" }
                    },
                }
            ]
        }
        let _mRespDataProduct = await _mUtils.commonMonogoCall(`${_dbWihtColl}_disease_source_masters_with_umls_dd`, "aggregation", _filter, "", "", "", "")
        ...
        let _data = 0;
        while (_mRespDataProduct.data.length > _data) {
            console.log("_data", _data)
            let _filterObj = {
                filter: [
                    {
                        $match: {
                            CUI: _mRespDataProduct.data[_data].DISEASE_CUI,
                            SAB: "SNOMEDCT_US"
                        }
                    },
                    {
            ...
            let _mRespDataProduct1 = await _mUtils.commonMonogoCall(`${_dbWihtColl}_umls_dd`, "aggregation", _filterObj, "", "", "", "");
            if (!(_mRespDataProduct1 && _mRespDataProduct1.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mRespDataProduct1.desc || "", data: _mRespDataProduct1.data || [] });
            }
            if (_mRespDataProduct1.data.length > 0) {
                let _para = {
                    params: {
                        _id: _mRespDataProduct.data[_data]._id,
                        DISEASE_CD: _mRespDataProduct1.data[0].CODE
                    }
                }

                let pLoadResp = { payload: {} };
                pLoadResp = await _mUtils.preparePayload("BW", _para);
                if (!pLoadResp.success) {
                    resolve({ success: false, status: 400, desc: pLoadResp.desc || "", data: [] });
                }

                let _uResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_disease_source_masters_with_umls_dd`, 'bulkWrite', pLoadResp.payload, "", "", "", "");
...
router.post('/pharmaClgGetData', async (req, res) => {

    try {
        let _filter = {
            "filter": [
                {
                    $match: { "BRAND_PRODUCT_MAP_CD": { $ne: "" }, HIMS_STATUS: "N", COMPANYCD: req.body.params.COMPANYCD, LOCATIONCD: req.body.params.LOCATIONCD }
                },
                {
                    $project: { COMPANYCD: 1, COMPANYNAME: 1, ITEMCD: 1, ITEMDESC: 1, LOCATIONCD: 1, BRAND_PRODUCT_MAP_CD: 1, BRAND_STRING2: 1, revNo: 1, _id: 1 }
                },
                {
                    $limit: 50
                }
        ...
        let _mRespDataProduct = await _mUtils.commonMonogoCall(`${_dbWihtColl}_pharmacology_master`, "aggregation", _filter, "", "", "", "")
        if (!(_mRespDataProduct && _mRespDataProduct.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _mRespDataProduct.desc || "", data: _mRespDataProduct.data || [] });
        } else {
            // let isMongographData=JSON.parse(JSON.stringify(_mRespDataProduct.data))
            let response = JSON.parse(JSON.stringify(_mRespDataProduct.data))
            if (_mRespDataProduct.data.length > 0) {
                let k = 0;
                while (_mRespDataProduct.data.length > k) {
                    //isMonograph
                    let _obj = { isMonograph: undefined }
                    let _matchingKet = []

                    if (_mRespDataProduct.data[k].BRAND_PRODUCT_MAP_CD.startsWith("DBPI")) {
                        _matchingKet.push(_mRespDataProduct.data[k].BRAND_PRODUCT_MAP_CD)
                        let _getData = await getBerandSubstanceNames(_matchingKet, 0, [], req, _filter);
                        _matchingKet.pop()
                        if (_getData.data.length > 0) {
                            let _filter = {
                                "filter": [
                                    {
                                        $match: { recStatus: true, DD_SUBSTANCE_CD: _getData.data[0].DD_SUBSTANCE_CD }
                                    }
                                ]
                            }
                            let _mResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_sections`, "aggregation", _filter, "", "", "", "")
                            if (_mResp.data.length > 0) {
                                response[k]["IS_MONOGRAPH"] = "Y"


                            } else {
                                response[k]["IS_MONOGRAPH"] = "N"
                            }

                        }
                    } else {
                        _matchingKet.push(_mRespDataProduct.data[k].BRAND_PRODUCT_MAP_CD)
                        let _getData = await getBrandSubstanceNamesThroughDGPI(_matchingKet, 0, [], req, _filter);
                        _matchingKet.pop()
                        if (_getData.data.length > 0) {
                            let _filter = {
                                "filter": [
                    ...
                    let updateResponse = await _mUtils.commonMonogoCall(`${_dbWihtColl}_pharmacology_master`, 'bulkWrite', pLoadResp.payload, "", "", "", "")
                    k++
                }
            }
            return res.status(200).json({ success: true, status: 200, desc: '', data: response });
        }
...
router.post("/update-cui-to-who-atc", async (req, res) => {
    try {
        let _filter = {
            filter: [
                {
        ...
        let _mRespDataProduct = await _mUtils.commonMonogoCall(`${_dbWihtColl}_snowmeds`, "aggregation", _filter, "", "", "", "")
        if (!(_mRespDataProduct && _mRespDataProduct.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _mRespDataProduct.desc || "", data: _mRespDataProduct.data || [] });
        }

        let _indx = 0;
        while (_mRespDataProduct.data.length > _indx) {
            let _filterObj = {
                "filter": [
                    {
                        $match: {
                            CODE: _mRespDataProduct.data[_indx].IDENTIFIER_SUBSTANCE
                        }
                    },
                    {
                        $group: {
            ...
            let _substanceData = await _mUtils.commonMonogoCall(`${_dbWihtColl}_umls_dd`, "aggregation", _filterObj, "", "", "", "")
            if (!(_substanceData && _substanceData.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _substanceData.desc || "", data: _substanceData.data || [] });
            } else {
                if (_substanceData.data.length > 0) {
                    let _patObj = {
                        params: {
                            _id: mongoose.Types.ObjectId(`${_mRespDataProduct.data[_indx]._id}`),
                            CUI: _substanceData.data[0].CUI,
                        }
                    }
                    ...
                    let _uResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_snowmeds`, 'bulkWrite', pLoadResp.payload, "", "", "", "");
                    if (!(_uResp && _uResp.success && _uResp.data)) {
                        _output.push({ success: false, status: 400, desc: _uResp.desc || 'Error occurred while insert User ..', data: [] });
                    }
                }
        ...
        return res.status(200).json({ success: true, status: 200, desc: '', data: _mRespDataProduct.data });
...
router.post("/update-str-name-update", async (req, res) => {
    try {
        let _filter = {
            filter: [
                {
        ...
        let _mRespDataProduct = await _mUtils.commonMonogoCall(`${_dbWihtColl}_disease_map_with_substance`, "aggregation", _filter, "", "", "", "")
        if (!(_mRespDataProduct && _mRespDataProduct.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _mRespDataProduct.desc || "", data: _mRespDataProduct.data || [] });
        }
        ...
        while (_mRespDataProduct.data.length > _indx) {
            let _filterObj = {
                "filter": [
                    {
                        $match: {
                            CUI: _mRespDataProduct.data[_indx].SUBSTANCE_CUI
                        }
                    },
                    {
                        $group: {
            ...
            let _substanceData = await _mUtils.commonMonogoCall(`${_dbWihtColl}_substance`, "aggregation", _filterObj, "", "", "", "")
            if (!(_substanceData && _substanceData.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _substanceData.desc || "", data: _substanceData.data || [] });
            } else {
                if (_substanceData.data.length > 0) {
                    let _patObj = {
                        params: {
                            _id: mongoose.Types.ObjectId(`${_mRespDataProduct.data[_indx]._id}`),
                            DD_SUBSTANCE_CD: _substanceData.data[0].DD_SUBSTANCE_CD,
                            DD_SUBSTANCE_NAME: _substanceData.data[0].DD_SUBSTANCE_NAME,
                        }
                    }
                    ...
                    let _uResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_disease_map_with_substance`, 'bulkWrite', pLoadResp.payload, "", "", "", "");
                    if (!(_uResp && _uResp.success && _uResp.data)) {
                        _output.push({ success: false, status: 400, desc: _uResp.desc || 'Error occurred while insert User ..', data: [] });
                    }
                }
...
router.post("/substance-with-class-mapping-mrconsorela", async (req, res) => {
    try {
        let _filter = {
            filter: [
                {
        ...
        let _mRespDataProduct = await _mUtils.commonMonogoCall(`${_dbWihtColl}_substance_cui`, "aggregation", _filter, "", "", "", "")
        if (!(_mRespDataProduct && _mRespDataProduct.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _mRespDataProduct.desc || "", data: _mRespDataProduct.data || [] });
        }
        let _indx = 0;
        ...
        while (_mRespDataProduct.data.length > _indx) {
            if (_mRespDataProduct.data[_indx].CUI != "0") {
                let _filterObj = {
                    filter: [
                        {
                            $match: {
                                CUI2_RELA: _mRespDataProduct.data[_indx].CUI,
                                RELA: { $in: ["may_treat", "may_prevent", "has_mechanism_of_action", "has_physiologic_effect", "has_therapeutic_class", 'has_structural_class'] }
                            }
                        },
                        {
                            $group: {
                                _id: { CUI1: "$CUI1", RELA: "$RELA" },
                                data: { $first: "$$ROOT" }
                            }
                        },
                        {
                ...
                let _data = [];
                let _mRespDataProduct1 = await _mUtils.commonMonogoCall(`${_dbWihtColl}_dd_mrconso_rela`, "aggregation", _filterObj, "", "", "", "")
                if (!(_mRespDataProduct1 && _mRespDataProduct1.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _mRespDataProduct1.desc || "", data: _mRespDataProduct1.data || [] });
                } else {
                    ...
                    if (_mRespDataProduct1.data.length > 0) {
                        let _indx1 = 0;
                        while (_mRespDataProduct1.data.length > _indx1) {
                            let _filterObj1 = {
                                filter: [
                                    {
                                        $match: {
                                            CUI1: _mRespDataProduct1.data[_indx1].CUI1
                                        }
                                    }
                                ]
                            }
                            let _mRespDataProduct2 = await _mUtils.commonMonogoCall(`${_dbWihtColl}_class_name_cui`, "aggregation", _filterObj1, "", "", "", "")
                            if (!(_mRespDataProduct2 && _mRespDataProduct2.success)) {
                                return res.status(400).json({ success: false, status: 400, desc: _mRespDataProduct2.desc || "", data: _mRespDataProduct2.data || [] });
                            }
                            _data = []
                            _data.push({
                                CUI1: _mRespDataProduct1.data[_indx1].CUI1,
                                STR: _mRespDataProduct2.data[0].STR,
                                RELA: _mRespDataProduct1.data[_indx1].RELA,
                                CUI2_RELA: _mRespDataProduct1.data[_indx1].CUI2_RELA,
                                SAB: _mRespDataProduct1.data[_indx1].SAB,
                                CUI: _mRespDataProduct1.data[_indx1].CUI,
                                CODE: _mRespDataProduct1.data[_indx1].CODE,
                                RELA: _mRespDataProduct1.data[_indx1].RELA
                            })
                            let _mRespDataProduct3 = await _mUtils.commonMonogoCall(`${_dbWihtColl}_dd_substance_classifications_cui`, "insertMany", _data, "", "", "", "")
                            if (!(_mRespDataProduct3 && _mRespDataProduct3.success)) {
                                return res.status(400).json({ success: false, status: 400, desc: _mRespDataProduct3.desc || "", data: _mRespDataProduct3.data || [] });
                            }
                            _indx1++;
                        }
                        _par = {
                            params: {
                                _id: _mRespDataProduct.data[_indx]._id,
                                CLASS_CHECK: "Y"
                            }
                        }
                ...
                let pLoadResp = { payload: {} };
                pLoadResp = await _mUtils.preparePayload("BW", _par);
                if (!pLoadResp.success) {
                    resolve({ success: false, status: 400, desc: pLoadResp.desc || "", data: [] });
                }

                let _uResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_substance_cui`, 'bulkWrite', pLoadResp.payload, "", "", "", "");
                if (!(_uResp && _uResp.success && _uResp.data)) {
                    _output.push({ success: false, status: 400, desc: _uResp.desc || 'Error occurred while insert User ..', data: [] });
                }
            }
...
router.post("/substance-with-disease-mapping-mrconsorela", async (req, res) => {
    try {
        let _filter = {
            filter: [
                {
            ...
            if (_mRespDataProduct.data[_indx].CUI != "0") {
                let _filterObj = {
                    filter: [
                        {
                            $match: {
                                CUI2_RELA: _mRespDataProduct.data[_indx].DISEASE_CUI,
                                RELA: { $in: ["may_be_treated_by", "may_be_prevented_by"] }
                            }
                        },
                        // {
                        //   $addFields: {
                        ...
                        //       $switch:{
                        //         branches:[
                        //           {case:{$eq:["$SAB","MSH"]},then :1},
                        //           {case:{$eq:["$SAB","ICD10CM"]},then :2},
                        //           {case:{$eq:["$SAB","SNOMEDCT_US"]},then :3}
                        //           ],
                        //           default:4
                        //       }
                        //     }
                ...
                if (!(_mRespDataProduct1 && _mRespDataProduct1.success)) {
                    ...
                    if (_mRespDataProduct1.data.length > 0) {
                        ...
                        while (_mRespDataProduct1.data.length > _indx1) {
                            ...
                            _data.push({
                                CUI1: _mRespDataProduct1.data[_indx1].CUI1,
                                STR: _mRespDataProduct1.data[_indx1].STR,
                                RELA: _mRespDataProduct1.data[_indx1].RELA,
                                CUI2_RELA: _mRespDataProduct1.data[_indx1].CUI2_RELA,
                                SAB: _mRespDataProduct1.data[_indx1].SAB,
                                CUI: _mRespDataProduct1.data[_indx1].CUI,
                            ...
                            let _mRespDataProduct3 = await _mUtils.commonMonogoCall(`${_dbWihtColl}_disease_map_with_substance`, "insertMany", _data, "", "", "", "")
                            if (!(_mRespDataProduct3 && _mRespDataProduct3.success)) {
                                return res.status(400).json({ success: false, status: 400, desc: _mRespDataProduct3.desc || "", data: _mRespDataProduct3.data || [] });
                            }
                            _indx1++;
...
router.post("/dd-substance-classification-match-substance-name", async (req, res) => {
    try {
        let _filter = {
            filter: [
                {
        ...
        let _mRespDataProduct = await _mUtils.commonMonogoCall(`${_dbWihtColl}_dd_substance_classifications_cui`, "aggregation", _filter, "", "", "", "")
        if (!(_mRespDataProduct && _mRespDataProduct.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _mRespDataProduct.desc || "", data: _mRespDataProduct.data || [] });
        }
        let _indx = 0;
        while (_mRespDataProduct.data.length > _indx) {
            let _filterObj = {
                "filter": [
                    {
                        $match: {
                            CUI: _mRespDataProduct.data[_indx].CUI2_RELA
                        }
                    }
                ]
            }
            let _substanceData = await _mUtils.commonMonogoCall(`${_dbWihtColl}_substance_cui`, "aggregation", _filterObj, "", "", "", "")
            if (!(_substanceData && _substanceData.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _substanceData.desc || "", data: _substanceData.data || [] });
            } else {
                if (_substanceData.data.length > 0) {
                    ...
                    let _uResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_dd_substance_classifications_cui`, 'bulkWrite', pLoadResp.payload, "", "", "", "");
                    if (!(_uResp && _uResp.success && _uResp.data)) {
                        _output.push({ success: false, status: 400, desc: _uResp.desc || 'Error occurred while insert User ..', data: [] });
                    }
                }
...
router.post("/generateSequenceValue", async (req, res) => {
    try {
        let _filter = {
            filter: [
                {
        ...
        while (_mRespDataProduct.data.length > _indx) {
            ...
            let _seqParenBrandResp = await _mUtils.getSequenceNextValue({ locId: "", seqName: 'DdsubstanceClassifications_cui' }, _dbWihtColl, req);
            ...
            let _patObj = {
                params: {
                    _id: mongoose.Types.ObjectId(`${_mRespDataProduct.data[_indx]._id}`),
                    DD_SUBSTANCE_CLS_CD: _seqParenBrandResp.data
                }
            }
...
// //pharmaCollege get data against conditions 
// router.post('/pharmaClgGetData', async (req, res) => {
...
//                 },
//                 {
//                     $project: { COMPANYCD: 1, COMPANYNAME: 1, ITEMCD: 1, ITEMDESC: 1, LOCATIONCD: 1, BRAND_PRODUCT_MAP_CD: 1, revNo: 1, _id: 1 }
//                 },
//                 {
//                     $limit: 1
//                 }
...
router.post("/update-brand-product-data-with-bdn-code", async (req, res) => {
    try {
        const excelDta = excelToJson({
            sourceFile: path.join(__dirname, `/Brand Strength_IT_May.2024.xlsx`),
            header: {
                rows: 1
            },
            columnToKey: {
        ...
        //     _excelData.push(o)
        // })
        req.body.params = excelDta.changing_bdn
        let _data = 0;
        while (excelDta.changing_bdn.length > _data) {
            let _filter = {
                filter: [
                    {
                        $match: {
                            BRAND_DISPLAY_CD: excelDta.changing_bdn[0].revisedBDNCD
                        }
                    }
                ]
            }
            let _mRespDataProduct = await _mUtils.commonMonogoCall(`${_dbWihtColl}_brand_product_mapping`, "aggregation", _filter, "", "", "", "")
            if (!(_mRespDataProduct && _mRespDataProduct.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mRespDataProduct.desc || "", data: _mRespDataProduct.data || [] });
            }
            if (_mRespDataProduct.data.length > 0) {
                let _data1 = 0;
                while (_mRespDataProduct.data.length > _data1) {
                    let _filterObj = {
                        filter: [
                            {
                                $match: {
                                    BRAND_DISPLAY_CD: _mRespDataProduct.data[0].BRAND_DISPLAY_CD
                                }
                            }
                        ]
                    }

                    let _mRespDataProduct1 = await _mUtils.commonMonogoCall(`${_dbWihtColl}_brand`, "aggregation", _filterObj, "", "", "", "")
                    if (!(_mRespDataProduct1 && _mRespDataProduct1.success)) {
                        return res.status(400).json({ success: false, status: 400, desc: _mRespDataProduct1.desc || "", data: _mRespDataProduct1.data || [] });
                    }
                    if (_mRespDataProduct1.data.length > 0) {
                        let _par = {
                            "params": {
                                _id: _mRespDataProduct.data[0]._id,
                                BRAND_CD: _mRespDataProduct1.data[0].BRAND_CD,
                                BRAND_NAME: _mRespDataProduct1.data[0].BRAND_NAME,
                                PARENT_BRAND_CD: _mRespDataProduct1.data[0].PARENT_BRAND_CD,
                                PARENT_BRAND: _mRespDataProduct1.data[0].PARENT_BRAND,
                                BRAND_EXTENSION_CD: _mRespDataProduct1.data[0].BRAND_EXTENSION_CD,
                                BRAND_EXTENSION_NAME: _mRespDataProduct1.data[0].BRAND_EXTENSION_NAME,
                            }
                        }
                        let pLoadResp = { payload: {} };
                        pLoadResp = await _mUtils.preparePayload("BW", _par);
                        ...
                        let _uResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_brand_product_mapping`, 'bulkWrite', pLoadResp.payload, "", "", "", "");
                        if (!(_uResp && _uResp.success && _uResp.data)) {
                            _output.push({ success: false, status: 400, desc: _uResp.desc || 'Error occurred while insert User ..', data: [] });
                        }
                    }
        ...
        return res.status(200).send({ status: "200", data: excelDta.changing_bdn })
...
router.post("/excelToJson_umls", (req, res) => {
    try {
        const excelDta = excelToJson({
            sourceFile: path.join(__dirname, `/ICD10CM_modified_1.xlsx`),
            header: {
                rows: 1
            },
            columnToKey: {
        ...
        //     _excelData.push(o)
        // })
        req.body.params = excelDta.Sheet1
        mongoMapper(`${_dbWihtColl}_disease_source_masters_with_umls_dd`, "insertMany", req.body.params).then(async (result) => {
...
router.post("/disease-mapping-excel-to-json", async (req, res) => {
    try {
        const excelDta = excelToJson({
            sourceFile: path.join(__dirname, `/Final Disease Mapping from UMLS.xlsx`),
            header: {
                rows: 1
            },
            columnToKey: {
        ...
        let _checkResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_umls_dd`, "aggregation", _filter, "", "", "", "")
        if (!(_checkResp && _checkResp.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _checkResp.desc || "", data: _checkResp.data || [] });
        }
        return res.status(200).send({ status: "200", data: excelDta })
...
router.post("/update-cui-to-substance", async (req, res) => {
    try {
        let _filter = {
            filter: [
                {
                    $match: {
                        recStatus: { $eq: true },
                        DD_WHO_REF: { $in: ["NRCeS-SNOMED", "WHO-ATC"] }
                    }
                }
            ]
        }
        let _sResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_substance`, "aggregation", _filter, "", "", "", "")
        if (!(_sResp && _sResp.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _sResp.desc || "", data: _sResp.data || [] });
        }
        //return res.status(200).json({success: true, status: 200, desc: "", data: _sResp.data})

        let count = 0

        while (_sResp.data.length > count) {
            console.log("count", count)
            let _patObj = {}
            if (_sResp.data[count].DD_WHO_REF == "WHO-ATC" && _sResp.data[count].UNIQUE_CD_ATC != "") {
                let _filterObj = {
                    "filter": [
                        {
                            $match: {
                                UNIQUE_CODE_ATC: _sResp.data[count].UNIQUE_CD_ATC
                            }
                        }
                    ]
                }
                let _sResp1 = await _mUtils.commonMonogoCall(`${_dbWihtColl}_whoatcs`, "aggregation", _filterObj, "", "", "", "");
                if (!(_sResp1 && _sResp1.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _sResp1.desc || "", data: _sResp1.data || [] });
                }
                ...
                let filter = {
                    "filter": [
                        {
                            $match: {
                                "SAB": "ATC", CODE: _sResp1.data[0].ATC_CODE
                            }
                        }
                    ]
                }
                let _sResp2 = await _mUtils.commonMonogoCall(`${_dbWihtColl}_umls_dd`, "aggregation", filter, "", "", "", "");
                if (!(_sResp2 && _sResp2.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _sResp2.desc || "", data: _sResp2.data || [] });
                }
                _patObj = {
                    "params": {
                        _id: _sResp.data[count]._id,
                        CUI: _sResp2.data && _sResp2.data.length > 0 && _sResp2.data[0].CUI ? _sResp2.data[0].CUI : "",
                        revNo: _sResp.data[count].revNo
                    }
                }
            } else if (_sResp.data[count].DD_WHO_REF == "NRCeS-SNOMED" && _sResp.data[count].UNIQUE_CD_ATC != "") {
                let filter = {
                    "filter": [
                        {
                            $match: {
                                "SAB": "SNOMEDCT_US", CODE: _sResp.data[count].UNIQUE_CD_ATC
                            }
                        }
                    ]
                }
            ...
            if (_sResp.data[count].UNIQUE_CD_ATC != "" && _patObj.params.CUI.length > 0) {
                let pLoadResp = { payload: {} };
                let _mResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_substance`, "findById", _sResp.data[count]._id, "REVNO", _patObj, "", "")
                if (!(_mResp && _mResp.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
                }
                _patObj.params['revNo'] = _mResp.data.params.revNo;
                pLoadResp = await _mUtils.preparePayload("BW", _patObj);
                if (!pLoadResp.success) {
                    return res.status(400).json({ success: false, status: 400, desc: pLoadResp.desc || "", data: [] });
                }
                let _mResp3 = await _mUtils.commonMonogoCall(`${_dbWihtColl}_substance`, "bulkWrite", pLoadResp.payload, "", "", "", "")
                console.log("_mResp3", _mResp3)
            }

            count++
        }
        return res.status(200).json({ success: false, status: 200, desc: "", data: _sResp.data });
    } catch (error) {
        console.log("error", error);
...
//     level: "info",
//     filename: (__dirname + "/public/logs/winston/rxseed/%DATE%.log"),
...
// const loggingData = winston.createLogger({
//     format: winston.format.json(),
//     transports: [transportInfo1]
// })

// function logging(message) {
//     loggingData.info(`${message}`)
// }
...
//             payload: req.body,
//             clientName: req.headers.clientName ? req.headers.clientName : ""
//         }
//         logging(`-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------`)
//         next()
//     } catch (error) {
    ...
    try {
        if (req.body && req.body.params.uName && req.body.params.pwd) {
            ...
            mongoMapper(`${_dbWihtColl}_users`, "find", _filter, "").then(async (result) => {
                ...
                res.cookie('x-token', _tkn, { maxAge: 9000000, httpOnly: true });
                _user['x-token'] = _tkn;
                // req.logging['response'] = _user
                // logging(`${JSON.stringify(req.logging)}`)
...
//slice the patInfo of last one 
const slicePatInfo = (obj, start, end) => Object.fromEntries(Object.entries(obj).slice(start, end))

//dataInformation about clinet,Doctor...etc
router.use(async function (req, res, next) {

    // let slicePat = slicePatInfo(req.body.JSON[0].PatInfo, 0, 3)
    try {
        req.dataInfo = {
            METHOD: req.url.split('/')[req.url.split('/').length - 1],
            IP: req.ip ? req.ip : "",
            USER_AGENT: req.get('User-Agent') ? req.get('User-Agent') : "",
            PAYLOAD: req.body.JSON ? req.body.JSON[0] : "",
            ORG_ID: req.body.dataInfo && req.body.dataInfo.ORG_ID ? req.body.dataInfo.ORG_ID : 0,
            ORG_NAME: req.body.dataInfo && req.body.dataInfo.ORG_NAME ? req.body.dataInfo.ORG_NAME : 0,
            EMAIL: req.body.dataInfo && req.body.dataInfo.EMAIL ? req.body.dataInfo.EMAIL : "",
            PAGE_TYPE: req.body.dataInfo && req.body.dataInfo.PAGE_TYPE ? req.body.dataInfo.PAGE_TYPE : "",
            CONTACT_NUMBER: req.body.dataInfo && req.body.dataInfo.CONTACT_NUMBER ? req.body.dataInfo.CONTACT_NUMBER : "",
            USER_DESIGNATION: req.body.dataInfo && req.body.dataInfo.USER_DESIGNATION ? req.body.dataInfo.USER_DESIGNATION : "",
            USER_SPECIALIZATION: req.body.dataInfo && req.body.dataInfo.USER_SPECIALIZATION ? req.body.dataInfo.USER_SPECIALIZATION : "",
            USER_DEPARTMENT: req.body.dataInfo && req.body.dataInfo.USER_DEPARTMENT ? req.body.dataInfo.USER_DEPARTMENT : "",
            LOC_ID: req.body.dataInfo && req.body.dataInfo.LOC_ID ? req.body.dataInfo.LOC_ID : 0,
            LOC_NAME: req.body.dataInfo && req.body.dataInfo.LOC_NAME ? req.body.dataInfo.LOC_NAME : "",
            STOCK_POINT: req.body.dataInfo && req.body.dataInfo.STOCK_POINT ? req.body.dataInfo.STOCK_POINT : "",
            HOST: req.body.dataInfo && req.body.dataInfo.HOST ? req.body.dataInfo.HOST : "",
            SOURCE: req.body.dataInfo && req.body.dataInfo.SOURCE ? req.body.dataInfo.SOURCE : "",
            PAT_NAME: req.body.dataInfo && req.body.dataInfo.PAT_NAME ? req.body.dataInfo.PAT_NAME : "",
            UMR_NO: req.body.dataInfo && req.body.dataInfo.UMR_NO ? req.body.dataInfo.UMR_NO : "",
            AGE: req.body.dataInfo && req.body.dataInfo.AGE ? req.body.dataInfo.AGE : "",
            GENDER: req.body.dataInfo && req.body.dataInfo.GENDER ? req.body.dataInfo.GENDER : "",
            VISIT_TYPE: req.body.dataInfo && req.body.dataInfo.VISIT_TYPE ? req.body.dataInfo.VISIT_TYPE : "",
            VISIT_NO: req.body.dataInfo && req.body.dataInfo.VISIT_NO ? req.body.dataInfo.VISIT_NO : "",
            ORDER_NO: req.body.dataInfo && req.body.dataInfo.ORDER_NO ? req.body.dataInfo.ORDER_NO : 0,
            APPT_ID: req.body.dataInfo && req.body.dataInfo.APPT_ID ? req.body.dataInfo.APPT_ID : "",
            PRIMARY_DOCTOR: req.body.dataInfo && req.body.dataInfo.PRIMARY_DOCTOR ? req.body.dataInfo.PRIMARY_DOCTOR : "",
            patientInfo: req.body.JSON && req.body.JSON.length > 0 ? req.body.JSON[0].PatInfo : "",
            audit: {
                documentedById: req.body.dataInfo && req.body.dataInfo.CREATE_BY_CD ? req.body.dataInfo.CREATE_BY_CD : "",
                documentedBy: req.body.dataInfo && req.body.dataInfo.CREATE_BY ? req.body.dataInfo.CREATE_BY : "",
            }
        }
        // dataInformation(req.dataInfo)
        next()
    } catch (error) {
...
//getAllertsData
async function monographGetAlert(_permuteData, req, res, _filterData) {
    let _finalData = []
    if (_filterData.length > 0) {
        let _indx = 0;
        while (_permuteData.length > _indx) {
            let _filterAggregation = {
                "filter": [
                    {
                        $match: { DD_SUBSTANCE_CD: _permuteData[_indx] }
                    }
                ]
            }
            let concatinate = "";
            ...
            _classInput.data.push({ DD_SUBSTANCE_CD: _permuteData[_indx] })
            let classAganistSubstances = await getDataBasedOnLooping(_classInput.data, 0, [], req, _filterAggregation, "dd_substance_classifications_cui", "SUBSTANCE_AGANIST_CLASS");
            if (classAganistSubstances.data.length > 0) {
                _.each(classAganistSubstances.data, (_o, _i) => {
                    concatinate += _o.CLASS_NAME + ","
                })
            }
            let _sResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_sections`, "find", _filterAggregation, "", "", "", "");
            if (!(_sResp && _sResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _sResp.desc || "", data: _sResp.data || [] });
            } else {
                if (JSON.parse(JSON.stringify(_sResp.data)).length > 0) {
                    _.each(JSON.parse(JSON.stringify(_sResp.data)), (_obj, _i) => {
                        _obj['DD_SUBSTANCE_NAME'] = _filterData[0].DD_SUBSTANCE_NAME
                        _obj['CLASS_NAME'] = concatinate.length > 0 ? concatinate.substring(0, concatinate.length - 1) : ""
...
router.post("/get-allerts_1", async (req, res) => {
    try {
        ...
        if (req.body.JSON[0].DRUG_CD && req.body.JSON[0].DRUG_CD.length > 0) {
            let _indx = 0
            while (req.body.JSON[0].DRUG_CD.split(",").length > _indx) {
                let _matchingKet = []
                let _getData = []
                if (req.body.JSON[0].DRUG_CD.split(",")[_indx].startsWith("DBPI")) {
                    _matchingKet.push(req.body.JSON[0].DRUG_CD.split(",")[_indx])
                    _getData = await getBerandSubstanceNames(_matchingKet, 0, [], req, _filter);
                } else if (req.body.JSON[0].DRUG_CD.split(",")[_indx].startsWith("DGPI")) {
                    _matchingKet.push(req.body.JSON[0].DRUG_CD.split(",")[_indx])
                    _getData = await getBrandSubstanceNamesThroughDGPI(_matchingKet, 0, [], req, _filter);
                }
            ...
            _.each(_o.DD_SUBSTANCE_CD.split('+'), (_o1, _i1) => {
                ...
                _.each(_o, (_val, _key) => {
                    if (_key == "DD_SUBSTANCE_CD" || _key == "DD_SUBSTANCE_NAME") {
                        ...
                        _substanceObject['DD_SUBSTANCE_NAME'] = _o.DD_SUBSTANCE_NAME
                        //_o.DD_SUBSTANCE_NAME.split("+")[_i1]
                    } else {
        ...
        if (req.body.JSON[0].TYPE == "ALL" || req.body.JSON[0].TYPE == "") {
            ...
            if ((req.body.JSON[0].BRAND_PRODUCT_CD.length == 0) || (req.body.JSON[0].BRAND_PRODUCT_CD.length != 0)) {
                if (req.body.JSON[0].BRAND_PRODUCT_CD != "" && req.body.JSON[0].DRUG_CD == "" && req.body.JSON[0].CLS_CD == "" && req.body.JSON[0].FOOD_CD == "" && req.body.JSON[0].DIS_CD == "" && req.body.JSON[0].LAB_TEST_CD == "" && req.body.JSON[0].ALLERGY.length == 0 && req.body.JSON[0].PREG_CD == "" && req.body.JSON[0].LACT_CD == "") {
                    if (_getBerandSubstanceNames.data.length > 1) {
                        ...
                        let selectedKeys = ['SRC_DRUG_CD', 'INT_TYPE_ID', 'recStatus','isApproved']
                        let _filterObj = {
                            filter: [
                                {
                                    $match: {
                                        $or: _.map(_comparisonData, o => _.pick(o, selectedKeys)),
                                        recStatus: { $eq: true }
                                    },
                                }
                            ]
                        }
                        let _testResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_drugchildinteraction`, "aggregation", _filterObj, "", "", "", "")
                        ...
                        let selectedKeysWithClass = ['SRC_DRUG_CD', 'INT_TYPE_ID', 'recStatus', 'CLASS_NAME_CD','isApproved']
                        let _filterObjWthCls = {
                            filter: [
                                {
                                    $match: {
                                        $or: _.map(_duplicateData, o => _.pick(o, selectedKeysWithClass)),
                                        recStatus: { $eq: true }
                                    },
                                }
                            ]
                        ...
                        if (_duplicateData.length > 0) {
                            ...
                            let _testResp1 = await _mUtils.commonMonogoCall(`${_dbWihtColl}_drugchildinteraction`, "aggregation", _filterObjWthCls, "", "", "", "")
                    ...
                    let clonedObject = _.omit(_.clone(req.body.JSON[0]), ["AGE", 'GENDER', 'ISPREGNANCY', 'ISLACTATION', 'PatInfo'])
                                        ...
                                        _comparisonData.push(
                                            {
                                                ...
                                                "SRC_SUBSTANCE_NAME": _o.DD_SUBSTANCE_NAME,
                                                "INT_SUBSTANCE_NAME": _o1.DD_SUBSTANCE_NAME,
                                                "BRAND_PRODUCT_NAME": _o.BRAND_NAME.length > 0 && _o1.BRAND_NAME.length > 0 ? _o.BRAND_NAME + "<-->" + _o1.BRAND_NAME : _o.BRAND_NAME,
                                                "isApproved": true
                                            }
                                        )
                                    }
                    ...
                    //     })
                    // }
                    req.dataInfo['TYPE'] = "ALLERTS"
                    req.dataInfo['RESPONSE'] = _finalData.alertsData
                    let dataInformation = await _mUtils.commonMonogoCall(`${_dbWihtColl}_logsInfo`, "insertMany", req.dataInfo, "", "", "", "");
                    return res.status(200).json({ success: true, status: 200, desc: '', data: _finalData, config: config });
                }
            } else {
                let filterKeyData = []
            ...
            // let _getBerandSubstanceNames = await getBerandSubstanceNames(req.body.params.BRAND_PRODUCT_CD.split(","), 0, [], req, _filter)
            let _permuteData = await permuteData(_cloneMonographData[0].DD_SUBSTANCE_CD.split(" + ")).map(str => str.join(" + "))
            if (_permuteData.length > 0) {
                let _filter = {
                    "filter": [
                        { $match: { DD_SUBSTANCE_CD: { $in: _permuteData } } },
                        {
                            $group: {
                                _id: "$DD_SUBSTANCE_CD",
                                data: { $first: "$$ROOT" }
                ...
                //return _mBrandProductResp
                let _filterData = _.filter(_mBrandProductResp.data, (_o, _i) => { return _o.IS_ASSIGNED == "PUBLISHED" });
                if (_filterData.length > 0) {
                    responseData = await monographGetAlert(_permuteData, req, res, _filterData)
                    if (!(responseData)) {
                        return res.status(400).json({ success: false, status: 400, desc: "", data: responseData || [] });
                    } else {
                        req.dataInfo['TYPE'] = "MONOGRAPHS"
                        req.dataInfo['RESPONSE'] = JSON.parse(JSON.stringify(responseData))
                        let dataInformation = await _mUtils.commonMonogoCall(`${_dbWihtColl}_logsInfo`, "insertMany", req.dataInfo, "", "", "", "")
                        return res.status(200).json({ success: true, status: 200, desc: '', data: responseData });
                    }
                } else {
                    req.dataInfo['TYPE'] = "MONOGRAPHS"
                    req.dataInfo['RESPONSE'] = responseData
                    let dataInformation = await _mUtils.commonMonogoCall(`${_dbWihtColl}_logsInfo`, "insertMany", req.dataInfo, "", "", "", "")
                    return res.status(200).json({ success: true, status: 200, desc: '', data: responseData })
                }
        ...
        req.dataInfo['ERROR'] = error
        let dataInformation = await _mUtils.commonMonogoCall(`${_dbWihtColl}_logsInfo`, "insertMany", req.dataInfo, "", "", "", "");
        return res.status(500).json({ success: false, status: 500, desc: '', data: error.message })
    }
});

router.post("/get-allerts-v1", async (req, res) => {
    try {
        let _filter = { "filter": { "recStatus": { $eq: true } } }
        let _brandProductName = "";
        let _comparisonData = [];
        ...
        if (req.body.JSON[0].BRAND_PRODUCT_CD) {
            let _indx = 0
            while (req.body.JSON[0].BRAND_PRODUCT_CD.split(",").length > _indx) {
                let _matchingKet = []
                let _getData = []
                if (req.body.JSON[0].BRAND_PRODUCT_CD.split(",")[_indx].startsWith("DBPI")) {
                    _matchingKet.push(req.body.JSON[0].BRAND_PRODUCT_CD.split(",")[_indx])
                    _getData = await getBerandSubstanceNames(_matchingKet, 0, [], req, _filter);
                } else if (req.body.JSON[0].BRAND_PRODUCT_CD.split(",")[_indx].startsWith("DGPI")) {
                    _matchingKet.push(req.body.JSON[0].BRAND_PRODUCT_CD.split(",")[_indx])
                    _getData = await getBrandSubstanceNamesThroughDGPI(_matchingKet, 0, [], req, _filter);
                }
            ...
            if ((req.body.JSON[0].BRAND_PRODUCT_CD.length == 0) || (req.body.JSON[0].BRAND_PRODUCT_CD.length != 0)) {
                if (req.body.JSON[0].BRAND_PRODUCT_CD != "" && req.body.JSON[0].DRUG_CD == "" && req.body.JSON[0].CLS_CD == "" && req.body.JSON[0].FOOD_CD == "" && req.body.JSON[0].DIS_CD == "" && req.body.JSON[0].LAB_TEST_CD == "" && req.body.JSON[0].ALLERGY.length == 0 && req.body.JSON[0].PREG_CD == "" && req.body.JSON[0].LACT_CD == "") {
                    if (_getBerandSubstanceNames.data.length > 1) {
                                    ...
                                    _comparisonData.push(
                                        {
                                            ...
                                            "INT_TYPE_ID": _o1.DD_SUBSTANCE_CD,
                                            "recStatus": true,
                                            "SRC_BRAND_PRODUCT_CD": _o.BRAND_PRODUCT_MAP_CD,
                                            "INT_BRAND_PRODUCT_CD": _o1.BRAND_PRODUCT_MAP_CD,
                                            "BRAND_NAME_1": _o.BRAND_NAME,
                                            "BRAND_NAME_2": _o1.BRAND_NAME,
                                            "SRC_SUBSTANCE_NAME": _o.DD_SUBSTANCE_NAME,
                                            "INT_SUBSTANCE_NAME": _o1.DD_SUBSTANCE_NAME,
                        ...
                        let selectedKeys = ['SRC_DRUG_CD', 'INT_TYPE_ID', 'recStatus']
                        let _filterObj = {
                            filter: [
                                {
                                    $match: {
                                ...
                                },
                                {
                                    $project:
                                    {
                                        _id: 0, SRC_DRUG_CD: 1, SRC_DRUG_NAME: 1, INT_TYPE_ID: 1, INT_TYPE_NAME: 1, SEVERITY_ID: 1, SEVERITY_NAME: 1, INTERACTIONS: 1, CLASS_NAME_CD: 1, CLASS_NAME: 1
                                    }
                                }
                            ]
                        }
                        ...
                        if (!(_testResp && _testResp.success)) {
                            ...
                            if (_testResp.data.length > 0) {
                                _.each(JSON.parse(JSON.stringify(_testResp.data)), (_to, _ti) => {
                                    ...
                                    _to['BRAND_PRODUCT_NAME'] = _compareFilter[0].BRAND_PRODUCT_NAME
                                    _to['SRC_BRAND_PRODUCT_CD'] = _compareFilter[0].SRC_BRAND_PRODUCT_CD
                                    _to['SRC_BRAND_PRODUCT_NAME'] = _compareFilter[0].BRAND_NAME_1
                                    _to['INT_BRAND_PRODUCT_CD'] = _compareFilter[0].INT_BRAND_PRODUCT_CD
                                    _to['INT_BRAND_PRODUCT_NAME'] = _compareFilter[0].BRAND_NAME_2
                                    _to['INT_TYPE'] = "DRUG_TO_DRUG"
                                    // delete _to.INT_TYPE_ID
                                    // delete _to.INT_TYPE_NAME
                                    _UsingOrQueryData.push(_to)
                                    // _finalData.alertsData.push(_to)
                                })
                            }
                            let _findDuplicatesAndCombinedData = await findDuplicatesAndCombinedData(JSON.parse(JSON.stringify(_UsingOrQueryData)), 'SRC_DRUG_CD', "INT_TYPE_ID");
                            _.each(_findDuplicatesAndCombinedData, (_o, _i) => {
                                delete _o.INT_TYPE_ID
                                delete _o.INT_TYPE_NAME
                                delete SRC_DRUG_CD
                                delete SRC_DRUG_NAME
                                _finalData.alertsData.push(_o)
                            })
                        }


                        //GET DATA ONLY DRUG TO DRUG (AGANIST SUBSTANCE TO CLASS) START.
                        _comparisonData = []
                        _.each(givenDataagainstAddingClassData.data, (_o, _i) => {
                            _.each(givenDataagainstAddingClassData.data, (_o1, _i1) => {
                                if (_o._id != _o1._id) {
                        ...
                        if (_duplicateData.length > 0) {
                            ...
                            if (!(_testResp1 && _testResp1.success)) {
                                ...
                                if (_testResp1.data.length > 0) {
                                    ...
                                    while (_indx < _testResp1Data.length) {
                                        ...
                                        while (_indx1 < _findDuplicatesAndCombinedData.length) {
                                            _manuaLfinalData.push({
                                                ...
                                                "BRAND_PRODUCT_NAME": _findDuplicatesAndCombinedData[_indx1].BRAND_PRODUCT_NAME,
                                                "SRC_BRAND_PRODUCT_CD": _findDuplicatesAndCombinedData[_indx1].SRC_BRAND_PRODUCT_CD,
                                                "SRC_BRAND_PRODUCT_NAME": _findDuplicatesAndCombinedData[_indx1].BRAND_NAME_1,
                                                "INT_BRAND_PRODUCT_CD": _findDuplicatesAndCombinedData[_indx1].INT_BRAND_PRODUCT_CD,
                                                "INT_BRAND_PRODUCT_NAME": _findDuplicatesAndCombinedData[_indx1].BRAND_NAME_2
                                            })
                                            _indx1++
                                        }
                                        _indx++;
                                    ...
                                    let _findDuplicatesAndCombinedData_1 = await findDuplicatesAndCombinedData(JSON.parse(JSON.stringify(_manuaLfinalData)), 'SRC_BRAND_PRODUCT_CD', "INT_BRAND_PRODUCT_CD");
                                    _.each(_findDuplicatesAndCombinedData_1, (_fo, _fi) => {
                                        delete _fo.INT_TYPE_ID
                                        delete _fo.INT_TYPE_NAME
                                        delete _fo.SRC_DRUG_CD
                                        delete _fo.SRC_DRUG_NAME
                                        _finalData.alertsData.push(_fo)
                                    })
                                }
                            }
                        ...
                        //GET DATA ONLY DRUG TO DRUG (AGANIST SUBSTANCE TO CLASS) END.
                    }
                } else if ((req.body.JSON[0].BRAND_PRODUCT_CD == "" && req.body.JSON[0].DRUG_CD == "" && req.body.JSON[0].CLS_CD == "" && (req.body.JSON[0].FOOD_CD != "" || req.body.JSON[0].DIS_CD != "" || req.body.JSON[0].LAB_TEST_CD != "" || req.body.JSON[0].PREG_CD != "" || req.body.JSON[0].LACT_CD != "" || req.body.JSON[0].ALLERGY.length != ""))) {
                    let filterKeyData = []
                    let _fil = _.filter(req.body.JSON[0], (_O, _I) => {
                    ...
                    if (_BrandOrClassOrDrugCds.length > 0) {
                            ...
                            if (!(_testResp && _testResp.success)) {
                                ...
                                if (_testResp.data.length > 0) {
                                    _.each(JSON.parse(JSON.stringify(_testResp.data)), (_to, _ti) => {
                                        ...
                                        _to['SRC_BRAND_PRODUCT_NAME'] = _compareFilter[0].BRAND_NAME_1
                                        _to['INT_TYPE'] = "DRUG_TO_DRUG"
                                        // _to['INT_DRUG_CD'] = _compareFilter[0].INT_TYPE_ID
                                        // _to['INT_DRUG_NAME'] = _to.INT_TYPE_NAME
                                        // _to['INT_TYPE'] = "drug to drug"
                                        _to['INT_BRAND_PRODUCT_CD'] = _compareFilter[0].INT_BRAND_PRODUCT_CD
                                        _to['INT_BRAND_PRODUCT_NAME'] = _compareFilter[0].BRAND_NAME_2
                                        // _to['SRC_SUBSTANCE_NAME'] = _compareFilter[0].SRC_SUBSTANCE_NAME
                                        // _to['INT_SUBSTANCE_NAME'] = _compareFilter[0].INT_SUBSTANCE_NAME
                                        ...
                                        while (_indx < _testResp1Data.length) {
                                            ...
                                            while (_indx1 < _findDuplicatesAndCombinedData.length) {
                                                _manuaLfinalData.push({
                                                    "SRC_DRUG_CD": _testResp1Data[_indx].SRC_DRUG_CD,
                                                    "SRC_DRUG_NAME": _testResp1Data[_indx].SRC_DRUG_NAME,
                                                    // "INT_DRUG_ID": _testResp1Data[_indx].INT_TYPE_ID,
                                                    // "INT_DRUG_NAME": _testResp1Data[_indx].INT_TYPE_NAME,
                                                    "INT_TYPE": "DRUG_TO_DRUG",
                                                    "SEVERITY_NAME": _testResp1Data[_indx].SEVERITY_NAME,
                                                    "INTERACTIONS": _testResp1Data[_indx].INTERACTIONS,
                                                    "CLASS_NAME_CD": _testResp1Data[_indx].CLASS_NAME_CD,
                        ...
                        if (_findDuplicatesAndCombinedDatawithAllData.length > 0) {
                            ...
                            _.each(_findDuplicatesAndCombinedDatawithAllData, (_o, _i) => {
                                let _pickData = _.pick(_o, 'BRAND_PRODUCT_NAME', 'INT_BRAND_PRODUCT_CD', 'INT_BRAND_PRODUCT_NAME', 'SRC_BRAND_PRODUCT_NAME', 'INTERACTIONS', "SRC_BRAND_PRODUCT_CD", 'SRC_DRUG_CD', 'SRC_DRUG_NAME', 'INT_TYPE_NAME', 'INT_TYPE_ID', 'INT_ID', 'CLASS_NAME', 'CLASS_NAME_CD', 'SEVERITY_ID', 'SEVERITY_NAME')
                                if (_pickData.INT_ID == 10) {
                                    _pickData['INT_TYPE'] = "DRUG_TO_DISEASE"
                                } else if (_pickData.INT_ID == 13) {
                                    _pickData['INT_TYPE'] = "DRUG_TO_PREGNANCY"
                                    _pickData['INT_BRAND_PRODUCT_NAME'] = _pickData.INT_TYPE_NAME
                                    _pickData['INT_BRAND_PRODUCT_CD'] = _pickData.INT_TYPE_ID

                                } else if (_pickData.INT_ID == 14) {
                                    _pickData['INT_TYPE'] = "DRUG_TO_LACTATION"
                                    _pickData['INT_BRAND_PRODUCT_NAME'] = _pickData.INT_TYPE_NAME
                                    _pickData['INT_BRAND_PRODUCT_CD'] = _pickData.INT_TYPE_ID
                                }
                                delete _pickData.INT_TYPE_NAME
                                delete _pickData.INT_TYPE_ID
                                delete _pickData.INT_ID
                                _finalData.alertsData.push(_pickData)
                            })
                        }
                    }
                    req.dataInfo['TYPE'] = "ALLERTS"
                    req.dataInfo['RESPONSE'] = _finalData.alertsData
                    let dataInformation = await _mUtils.commonMonogoCall(`${_dbWihtColl}_logsInfo`, "insertMany", req.dataInfo, "", "", "", "");
                    return res.status(200).json({ success: true, status: 200, desc: '', data: _finalData.alertsData });
                }
            }
            req.dataInfo['TYPE'] = "ALLERTS"
            req.dataInfo['RESPONSE'] = _finalData.alertsData
...
async function prepareLogsPayload(_params, _payload) {
    try {
        _.each(_params, (_val, _key) => {
            _payload.filter[0].$match[_key] = _val
        })
        return { success: true, data: _payload };;
    } catch (error) {
        return res.status(500).json({ success: false, status: 500, desc: '', data: error.message })
...
router.post('/get-logs-data', async (req, res) => {
    try {
        let _filter = {
            "filter": [
                {
        ...
        let _prepareLogsPayload = await prepareLogsPayload(req.body.params, _filter)
        if (!_prepareLogsPayload.success) {
            return res.status(400).json({ success: false, status: 400, desc: _pGData.desc, data: [] });
        }
        let _mResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_logsInfo`, "aggregation", _filter, "", "", "", "")
        if (!(_mResp && _mResp.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
        }
...
router.post("/insert-pharmacology-data", async (req, res) => {
    try {
        ...
        while (_index < req.body.params.length) {
            ...
            // _filter.filter["COMPANYCD"] = req.body.params[_index].COMPANYCD
            // _filter.filter["LOCATIONCD"] = req.body.params[_index].LOCATIONCD
            _filter.filter["COMPANYCD"] = req.body.params[_index].COMPANYCD
            _filter.filter["LOCATIONCD"] = req.body.params[_index].LOCATIONCD
    ...
    try {
        ...
        if (_flag === "EXIST") {
            ...
            if (_method == "find") {
                ...
                if (req.body.params.SEARCH_TYPE && req.body.params.SEARCH_TYPE != "ALL") {
                    if (req.body.params.SEARCH_COL && req.body.params.SEARCH_COL != "" && req.body.params.SEARCH_COL != "^") {
                        // let searchItem=
                        ...
                        const nameExp=new RegExp(req.body.params.SEARCH_COL.startsWith('^') ?`^${req.body.params.SEARCH_COL.slice(1).replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}`:req.body.params.SEARCH_COL.startsWith('.*') && req.body.params.SEARCH_COL.endsWith('.*') ? `.*${req.body.params.SEARCH_COL.slice(2,-2).replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}.*`: req.body.params.SEARCH_COL.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'), 'i' )
                    ...
                    if (req.body.params.SEARCH_COL && req.body.params.SEARCH_COL != "" && req.body.params.SEARCH_COL != "^") {
                        ...
                        _.each(_columnNames, (_obj, _indx) => {
                            ...
                            if (req.body.params.hasOwnProperty("isApproved")) {
                                _filterObj.filter["isApproved"] = { $eq: req.body.params.isApproved }
                            }
...
async function dropdownMasterWiseData(req, _output, _filterObj, res) {
    try {
        ...
        let _collName = ""
        const nameExp = req.body.params.searchValue && req.body.params.searchValue.length > 0 ? new RegExp(req.body.params.searchValue.startsWith('^') ? `^${req.body.params.searchValue.slice(1).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}` : req.body.params.searchValue.startsWith('.*') && req.body.params.searchValue.endsWith('.*') ? `.*${req.body.params.searchValue.slice(2, -2).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}.*` : req.body.params.searchValue.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), "i") : ""
        //let nameExp = req.body.params.searchValue && req.body.params.searchValue.length > 8 ? { $regex: new RegExp("\\b" + req.body.params.searchValue.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&") + "\\b", "i") } : { $regex: req.body.params.searchValue, $options: "i" };
        if (req.body.params.MASTER_NAME == "UNII") {
            ...
            col2 = "STRING_3_CD"
            val2 = "STRING_3_CD"
            col4 = "CDCI_IDENTIFIER"
            val3 = "CDCI_IDENTIFIER"
            _filterObj.skipData = 0
            _filterObj.limit = 50
            let _mRespData = await _mUtils.commonMonogoCall(`${_dbWihtColl}_${_collName}`, "find", _filterObj, "", "", "", "")
            if (!(_mRespData && _mRespData.success)) {
...
router.post("/insert-company-master", async (req, res) => {
    try {
        ...
        if (req.body.params.flag == "COMPANY") {
            ...
            req.body.params = _.omit(_.clone(req.body.params), ["DIVISION_ALTERNATIVE_NAME", 'COMPANY_ALTERNATIVE_NAME', 'EMAIL', 'WEBSITE', 'MOBILE', 'PHONE', 'ADDRESS2', 'DIVISION_NAME', 'ADDRESS1',
                'DESIGNATION', 'CONTACT_PERSON', 'PIN_CODE', 'COUNTRY', 'STATE', 'CITY', 'COMPANY_CATEGORY', "COMPANY_TYPE"])
            _filter.filter['COMPANY_NAME'] = req.body.params.COMPANY_NAME;
            let _existData = await commonCheckExitOrNot(_collectionName, req, _filter, "EXIST");
            if (_existData.code == 1) {
                ...
                let clonedKeys = _.pick(cloneData, ['DIVISION_NAME', 'ADDRESS1'])
                _filter.filter['COMPANY_NAME'] = req.body.params.COMPANY_NAME;
                _filter.filter['DIVISION_NAME'] = clonedKeys.DIVISION_NAME;
                _filter.filter['ADDRESS1'] = clonedKeys.ADDRESS1;
                let _divisionData = await commonCheckExitOrNot(_collectionName, req, _filter, "EXIST");
                if (_divisionData.code == 1) {
                    req.body.params = cloneData;
                    res.status(200).json({ success: true, status: 200, desc: "", data: "Not Exists" })
                }
                else {
...
async function addKeyToNestedObjects(data, _key) {
    try {
        if (Array.isArray(data)) {
            data.forEach(item => addKeyToNestedObjects(item))
            ...
            for (let prop in data) {
                if (data.hasOwnProperty(prop)) {
                    addKeyToNestedObjects(data[prop])
                }
            }
        }
    } catch (error) {
...
router.post("/insert-master-data", async (req, res) => {
    try {
        let _seqResp;
        let _filter = { "filter": { "recStatus": { $eq: true } } };
        if (req.body.params.flag != "INN" && req.body.params.flag !== "COUNTRY" && req.body.params.flag != "UNII" && req.body.params.flag != "SNOWMED" && req.body.params.flag !== "DMF" && req.body.params.flag !== undefined && req.body.params.flag !== "ENTITY" &&
            req.body.params.flag !== "DD_SUBSTANCE_CLASSIFICATIONS" && req.body.params.flag !== "DRUG_INTERACTIONS" && req.body.params.flag !== "VALIDATE_BRAND_PRODUCT" && req.body.params.flag !== "FREQUENT" && req.body.params.flag !== "COMPANY" && req.body.params.flag != "USER_DOCUMENTS" &&
            req.body.params.flag != "USER_MODULES" && req.body.params.flag != "ROLE_DOC_ACCESS" && req.body.params.flag != "SYNONYMY_MASTER" && req.body.params.flag != "PARENT_BRAND_MASTER" && req.body.params.flag != "BRANDMASTER" && req.body.params.flag != "PARENT_BRAND_MAPPING" && req.body.params.flag != "BRAND_EXTENSION" && req.body.params.flag != "BRAND_DISPLAY_NAME" && req.body.params.flag != "DISEASE_SOURCE_DATA" && req.body.params.flag != "DD_HEALTH_CONDITION" ) {
            _seqResp = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: `${req.body.params.sequenceName}` }, _dbWihtColl, req);
            if (!(_seqResp && _seqResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _seqResp.desc || "", data: _seqResp.data || [] });
            }
        ...
        if (req.body.params.flag == "UOM") {
        ...
        } else if (req.body.params.flag == "USER_MODULES") {
            let totalData = [];
            ...
            if (req.body.params.JSON && req.body.params.JSON.length > 0) {
                ...
                while (req.body.params.JSON.length > _indx) {
                    let _loopData = await addKeyToNestedObjects(req.body.params.JSON[_indx], "_id")
                    totalData.push(_loopData)
                    _indx++;
                }
            } else {
                let _loopData = await addKeyToNestedObjects(req.body.params, "_id")
            }
            req.body.params = req.body.params.JSON && req.body.params.JSON.length > 0 ? totalData : req.body.params 
            ...
            if (_existData.code == 1) {
                ...
                let _filter = {
                    "filter": {
                        ...
                        "BRAND_DISPLAY_CD": req.body.params.BRAND_DISPLAY_CD
                    }
                }
                //let _mRespDatabrand = await _mUtils.commonMonogoCall(`${_dbWihtColl}_brand`, "find", _filter, "", "", "", "")
                let _mRespDatabrand = await _mUtils.commonMonogoCall(`${_dbWihtColl}_brand_display_name`, "find", _filter, "", "", "", "")
            ...
            _collectionName = "class_name_cui"
            delete req.body.params.flag
            let _existData = await commonCheckExitOrNot(_collectionName, req, _filter, "EXIST")
            if (_existData.code == 1) {
            ...
            _collectionName = "dd_substance_classifications_cui"
            delete req.body.params.flag
            ...
            let interactionData = [
                {
                    search: "Drug-Drug-Parent",
                    type: 8,
                    key: "P"
                },
                {
                    search: "Drug-Food-Parent",
                    type: 9,
                    key: "P"
                },
                {
                    search: "Drug-Disease-Parent",
                    type: 10,
                    key: "P"
                },
                {
                    search: "Drug-Lab-Parent",
                    type: 11,
                    key: "P"
                },
                {
                    search: "Drug-Pregnancy-Parent",
                    type: 13,
                    key: "P"
                },
                {
                    search: "Drug-Laction-Parent",
                    type: 14,
                    key: "P"
                },
                {
                    search: "Drug-Drug-Children",
                    type: 8,
                    key: "C"
                },
                {
                    search: "Drug-Food-Children",
                    type: 9,
                    key: "C"
                },
                {
                    search: "Drug-Disease-Children",
                    type: 10,
                    key: "C"
                },
                {
                    search: "Drug-Lab-Children",
                    type: 11,
                    key: "C"
                },
                {
                    search: "Drug-Pregnancy-Children",
                    type: 13,
                    key: "C"
                },
                {
                    search: "Drug-Laction-Children",
                    type: 14,
                    key: "C"
                }
            ...
            if (req.body.params.JSON1 && req.body.params.JSON1.length > 0 && req.body.params.JSON1[0].isApproved == false) {
                ...
                //add the InteractionCd 
                let sequenceData = interactionData.filter(item => item.type === req.body.params.JSON1[0].INT_ID && item.key === "P");
                req.body.params.JSON1[0]["INTERACTION_CD"] = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: sequenceData[0].search }, _dbWihtColl, req).then((_obj) => { return _obj.data })
                ...
                if (parent_data.data.length > 0 && req.body.params.JSON.length > 0) {
                    //add the InteractionCd
                    let index = 0;
                    while (req.body.params.JSON.length > index) {
                        req.body.params.JSON[index]['DRUG_IN_PARENT_ID'] = parent_data.data[0].DRUG_IN_PARENT_ID
                        req.body.params.JSON[index]['DRUG_IN_PARENT_MONGO_ID'] = parent_data.data[0]._id
                        req.body.params.JSON[index]['audit'] = req.body.params.audit
                        let sequenceData = interactionData.filter(item => item.type === req.body.params.JSON[index].INT_ID && item.key === "C");
                        req.body.params.JSON[index]["INTERACTION_CD"] = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: sequenceData[0].search }, _dbWihtColl, req).then((_obj) => { return _obj.data })
                        _finalJsonData.push(req.body.params.JSON[index])
                        index++
                    }
            ...
            _collectionName = "disease_source_masters_with_umls_dd"
            delete req.body.params.flag;
            let audit = req.body.params.audit;
            req.body.params = req.body.params.JSON[0];
            req.body.params.audit = audit;
            let _cloneData = _.clone(req.body.params)
            delete req.body.params.sequenceName
            ...
            //     $or: [
            //         {
            //             "DISEASE_CD": req.body.params.JSON[0].DISEASE_CD
            //         },
            //         {
            //             "DISEASE_NAME": req.body.params.JSON[0].DISEASE_NAME
            //         },
            //         {
            //             "DISEASE_CUI": req.body.params.JSON[0].DISEASE_CUI
            //         }    
            //     ]
            // }
            _filter.filter = {
                "DISEASE_MASTER_TYPE": req.body.params.DISEASE_MASTER_TYPE,
                "DISEASE_CD": req.body.params.DISEASE_CD,
                "DISEASE_NAME": req.body.params.DISEASE_NAME,
                "DISEASE_CUI": req.body.params.DISEASE_CUI
            }

            let _existData = await commonCheckExitOrNot(_collectionName, req, _filter, "EXIST","","","DOSE_FORM")
            if (_existData.code == 1) {
                req.body.params['DISEASE_MASTER_CD'] = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: `${_cloneData.sequenceName}` }, _dbWihtColl, req).then((_obj) => { return _obj.data })
            } else {
                return res.status(200).json({ success: true, status: 200, desc: '', data: "already exist" });
            }
            ...
            let _existData = await commonCheckExitOrNot(_collectionName, req, _filter, "EXIST")
            if (_existData.code == 1) {
                req.body.params["BRAND_CD"] = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: 'Brand' }, "monography", req).then((_obj) => { return _obj.data });
            } else {
                return res.status(200).json({ success: true, status: 200, desc: '', data: "already exist" })
            }
            ...
            let _existData = await commonCheckExitOrNot(_collectionName, req, _filter, "EXIST")
            if (_existData.code == 1) {
                req.body.params["PARENT_BRAND_CD"] = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: 'ParentBrand' }, "monography", req).then((_obj) => { return _obj.data });
            } else {
                return res.status(200).json({ success: true, status: 200, desc: '', data: "already exist" })
            }
            ...
            while (req.body.params.JSON.length > _indx) {
                ...
                req.body.params.JSON[_indx]['PARENT_BRAND_MAPPING_CD'] = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: `${req.body.params.sequenceName}` }, "monography", req).then((_obj) => { return _obj.data });
            ...
            let _existData = await commonCheckExitOrNot(_collectionName, req, _filter, "EXIST")
            if (_existData.code == 1) {
                req.body.params["BRAND_EXTENSION_CD"] = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: 'BrandExtension' }, "monography", req).then((_obj) => { return _obj.data });
            } else {
                return res.status(200).json({ success: true, status: 200, desc: '', data: "already exist" })
            }
        } else if (req.body.params.flag == "DD_HEALTH_CONDITION") {
            try {
                _collectionName = 'diseasemapping_cui';
                let audit = req.body.params.audit;
                delete req.body.params.flag;
                let index = 0;
                while (req.body.params.JSON.length > index) {
                    ...
                    data.body.params = _.clone(req.body.params.JSON[index])
                    _filter.filter['$or'] = [
                        {
                            DISEASE_CUI: data.body.params.DISEASE_CUI
                        },
                        {
                            DISEASE_NAME: data.body.params.DISEASE_NAME
                        }
                    ]
                    let _exitData = await commonCheckExitOrNot(_collectionName, data, _filter, "EXIST", "", "", "")
                    if (_exitData.code == 1) {
                        req.body.params.JSON[index]["DISEASE_MAP_CD"] = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: 'disease_mapping_cui' }, _dbWihtColl, req).then((_obj) => { return _obj.data })
                    } else {
                        return res.status(200).json({ success: true, status: 200, desc: '', data: "already exist" });
                    }
                    // req.body.params.JSON[index]["DISEASE_MAP_CD"] = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: 'disease_mapping_cui' }, _dbWihtColl, req).then((_obj) => { return _obj.data })
            ...
            if (_existData.code == 1) {
                req.body.params = cloneData
                req.body.params["BRAND_DISPLAY_CD"] = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: 'BrandDisplaycode' }, "monography", req).then((_obj) => { return _obj.data });
                // _filter.filter = {}
                // let _exitData = await commonCheckExitOrNot(_collectionName, req, _filter, "GETALL", "findCount");
                // req.body.params["BRAND_MASTER_ID"] = _exitData.data[0].TOTAL_COUNT + 1
            } else {
            ...
            while (_compData.length > _indx) {
                ...
                if (!(_mRespDataProduct && _mRespDataProduct.success)) {
                    ...
                    if (_mRespDataProduct.data.length > 0) {
                        ...
                        _data[_idx][`${_compData[_indx]._assignKey}`] = concatinate.substring(0, concatinate.length - 1)
                        if (_compData[_indx].hasOwnProperty('_combName')) {
                            if (_data[_idx].hasOwnProperty(`${_compData[_indx]._combName}`)) {
                                _data[_idx][`${_compData[_indx]._combName}`] = _data[_idx][_compData[_indx]._comb1] + " " + _data[_idx][_compData[_indx]._comb2]
                            }
                        }
...
async function aggregationGetData(_collName, _matchObj, _groupKey, _projectWthSliceObject, _deleteKeys) {
    try {
        let _filterObj = {
            "filter": [
                {
                ...
                {
                    "$project": _projectWthSliceObject
                },
                {
                    "$unwind": "$records"
                },
        ...
        let _msubclsData = await _mUtils.commonMonogoCall(`${_dbWihtColl}_${_collName}`, "aggregation", _filterObj, "", "", "", "");
        if (!(_msubclsData && _msubclsData.success)) {
            ...
            if (_msubclsData.data.length > 0) {
                return _msubclsData.data[0].allRecords
            } else {
                return []
            }
        }
...
router.post("/get-master-data", async (req, res) => {
    try {
        ...
        if (req.body.params.flag == "UOM") {
            ...
            } else if (req.body.params.flag1 == "DROPDOWN") {
                _filter.filter = [
                    {
                        $match: { recStatus: { $eq: true }, "DOSE_FORM_CD": { $ne: "" } }
                    },
                    {
                        $project: { _id: 0, DOSE_DEFAULT_NAME: 1, DOSE_DISPLAY_NAME: 1, DOSE_FORM_CD: 1, }
                    }
                ]

                let _mResp = await _mUtils.commonMonogoCall(`monography_${_collectionName}`, "aggregation", _filter, "", "", "", "");
                let _totalData = []
                let _distinctData = _.chain(_mResp.data).groupBy('DOSE_DISPLAY_NAME')
                    .map((_d, _k) => {
                        if (_k != "") {
                            _totalData.push({
                                DOSE_FORM_NAME: _d[0].DOSE_DISPLAY_NAME && _d[0].DOSE_DISPLAY_NAME.length > 0 ? _d[0].DOSE_DISPLAY_NAME : _d[0].DOSE_DEFAULT_NAME,
                                DOSE_FORM_CD: _d[0].DOSE_FORM_CD
                            })
                        }
                    }).value()
            ...
            if (req.body.params.flag1 == "DRPDATA") {
                ...
                let nameExp = { $regex: req.body.params.searchValue, $options: "i" }
                _filter.filter['BRAND_DISPLAY_CD'] = nameExp
            ...
            if (req.body.params.flag1 == "GETALL") {
                ...
                return res.status(200).json({ success: true, status: 200, desc: '', data: _finalCountData });
            } else if (req.body.params.flag1 == "SEARCH_RELA") {
                let nameExp = { $regex: req.body.params.searchValue, $options: "i" }
                let _filterObj = {
                    filter: [
                        {
                ...
                let _mResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_dd_mrconso_rela`, "aggregation", _filterObj, "", "", "", "")
                if (!(_mResp && _mResp.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
                }
                return res.status(200).json({ success: true, status: 200, desc: '', data: _mResp.data });
            ...
            let _finalCountData = {};
            if (req.body.params.flag1 == "GETALL") {
                if (req.body.params.DD_SUBSTANCE_CD && req.body.params.DD_SUBSTANCE_CD != '') {
                    _filter.filter['DD_SUBSTANCE_CD'] = req.body.params.DD_SUBSTANCE_CD
            ...
            if (req.body.params.flag1 == "DRPDATA") {
            ...
            } else if (req.body.params.flag1 == "DRUGANDCLASSDATA") {
                let nameExp = { $regex: req.body.params.searchValue, $options: 'i' };
                let _matchObject = {};
                let _projectObject = {
                    _id: 0,
                ...
                let _deleteKeys = {}
                if (req.body.params.type == "SUBSTANCE") {
                    let _collName = "substance"
                    _matchObject = {
                        "recStatus": { $eq: true },
                        "DD_SUBSTANCE_NAME": nameExp,
                        "DD_SUBSTANCE_CD": { $nin: [`${req.body.params.DD_SUBSTANCE_CD}`] }
                    }
                    _deleteKeys = {
                        _id: 0,
                        "DD_SUBSTANCE_NAME": 1,
                    ...
                    let useAggregationData = await aggregationGetData(_collName, _matchObject, 'DD_SUBSTANCE_CD', _projectObject, _deleteKeys);
                    let _indx = 0
                    while (useAggregationData.length > _indx) {
                        let concatinate = "";
                        let _filterObject = {
                            "filter": [
                                {
                                    $match: {
                                        recStatus: { $eq: true },
                                        DD_SUBSTANCE_CD: useAggregationData[_indx].DD_SUBSTANCE_CD
                                    }
                                }
                            ]
                        };

                        let classAganistSubstances = await _mUtils.commonMonogoCall(`${_dbWihtColl}_dd_substance_classifications`, "aggregation", _filterObject, "", "", "", "");
                        if (!(classAganistSubstances && classAganistSubstances.success)) {
                            return res.status(400).json({ success: false, status: 400, desc: classAganistSubstances.desc || "", data: classAganistSubstances.data || [] });
                        } else {
                            if (classAganistSubstances.data.length > 0) {
                                _.each(classAganistSubstances.data, (_o, _i) => {
                                    concatinate += _o.CLASS_NAME + ","
                        ...
                        useAggregationData[_indx]['CLASS_NAME'] = concatinate.length > 0 ? concatinate.substring(0, concatinate.length - 1) : ""
                        _indx++;
                    }
                    return res.status(200).json({ success: true, status: 200, desc: '', data: useAggregationData });
                } else if (req.body.params.type == "CLASS") {
                    let _collName = "dd_substance_classifications";
                    _matchObject = {
                        "recStatus": { $eq: true },
                        "CLASS_NAME": nameExp,
                        "DD_SUBSTANCE_CD": { $nin: [`${req.body.params.DD_SUBSTANCE_CD}`] }
            ...
            try {
                ...
                if (req.body.params.flag1 === "DRPDATA") {
                    ...
                    let _pharmocolyData = await _mUtils.commonMonogoCall(`monography_pharmacology_master`, "find", _filter, "", "", "", "")
                    ...
                    delete req.body.params.flag1;
                    req.body.params.recStatus
                    _filter.filter['COMPANYCD'] = req.body.params.ORG_ID
                    _filter.filter['LOCATIONCD'] = req.body.params.LOC_CD


                    const globalFilter = {
                        "COMPANYCD": { $eq: req.body.params.ORG_ID },
                        "LOCATIONCD": { $eq: req.body.params.LOC_CD }
                    }


                    const { types, type, Match_Type } = req.body.params;


                    const addiFilter = (
                        types === "Required" && type === "mapped" && Match_Type === 'DBPI' ?
                            { "BRAND_PRODUCT_MAP_CD": { $ne: "" }, IS_REQUIRED: true, Match_Type: 'DBPI' } : types === "Required" && type === "mapped" && Match_Type === 'DGPI' ? { "BRAND_PRODUCT_MAP_CD": { $ne: "" }, IS_REQUIRED: true, Match_Type: 'DGPI' } : types === "Not Required" && type === "mapped" && Match_Type === 'DBPI' ?
                                { "BRAND_PRODUCT_MAP_CD": { $ne: "" }, IS_REQUIRED: false, Match_Type: 'DBPI' } : types === "Not Required" && type === "mapped" && Match_Type === 'DGPI' ?
                                    { "BRAND_PRODUCT_MAP_CD": { $ne: "" }, IS_REQUIRED: false, Match_Type: 'DGPI' } : types === "Required" && type === "mapped" ?
                                        { "BRAND_PRODUCT_MAP_CD": { $eq: "" }, IS_REQUIRED: false } : types === "Not Required" && type === "unmapped" ?
                                            { "BRAND_PRODUCT_MAP_CD": { $eq: "" }, IS_REQUIRED: false } : types === "All" && type === "all" ? { COMPANYCD: req.body.params.ORG_ID, LOCATIONCD: req.body.params.LOC_CD } : types === "Required" && type === "unmapped" ? { "BRAND_PRODUCT_MAP_CD": { $eq: "" }, IS_REQUIRED: true } : {}
                    )


                    _filter.filter = { ...globalFilter, ...addiFilter }
                }
                _filter['selectors'] = { COMPANYCD: 1, COMPANYNAME: 1, revNo: 1, ITEMCD: 1, ITEMDESC: 1, ITEMLEVEL0: 1, ITEMLEVEL1CD: 1, ITEMLEVEL1DESC: 1, ITEMLEVEL2CD: 1, ITEMLEVEL2DESC: 1, ITEMLEVEL3CD: 1, ITEMLEVEL3DESC: 1, MANUFACTURERCD: 1, MANUFACTURERNAME: 1, ISACTIVE: 1, CATEGORYNAME: 1, CLASSIFICATION1: 1, CLASSIFICATION2: 1, CLASSIFICATION3: 1, CLASSIFICATION4: 1, CLASSIFICATION5: 1, BRAND_PRODUCT_MAP_CD: 1, STRING_3: 1, IS_REQUIRED: 1, Match_Type: 1, BRAND_STRING2: 1, HIMS_STATUS: 1, LOCATIONNAME: 1, LOCATIONCD: 1, audit: 1 }
                //_filter['selectors'] = { COMPANYCD: 1, COMPANYNAME: 1, revNo: 1, ITEMCD: 1, ITEMDESC: 1, ITEMLEVEL0: 1, ITEMLEVEL1CD: 1, ITEMLEVEL1DESC: 1, ITEMLEVEL2CD: 1, ITEMLEVEL2DESC: 1, ITEMLEVEL3CD: 1, ITEMLEVEL3DESC: 1, MANUFACTURERCD: 1, MANUFACTURERNAME: 1, ISACTIVE: 1, CATEGORYNAME: 1, CLASSIFICATION1: 1, CLASSIFICATION2: 1, CLASSIFICATION3: 1, CLASSIFICATION4: 1, CLASSIFICATION5: 1, BRAND_PRODUCT_MAP_CD: 1, STRING_3: 1, IS_REQUIRED: 1, Match_Type: 1, BRAND_STRING2: 1, HIMS_STATUS: 1, audit: 1 }
                //let _columnNames = ["COMPANYCD", "COMPANYNAME", "ITEMCD", "ITEMDESC", "ITEMLEVEL0", "ITEMLEVEL1CD", "ITEMLEVEL1DESC", "ITEMLEVEL2CD", "ITEMLEVEL2DESC", "ITEMLEVEL3CD", "ITEMLEVEL3DESC", "MANUFACTURERCD", "MANUFACTURERNAME", "ISACTIVE", "CATEGORYNAME", "CLASSIFICATION1", "CLASSIFICATION2", "CLASSIFICATION3", "CLASSIFICATION4", "CLASSIFICATION5", "BRAND_STRING2","BRAND_PRODUCT_MAP_CD","HIMS_STATUS","LOCATIONNAME"]
                let _columnNames = ["COMPANYCD", "COMPANYNAME", "ITEMCD", "ITEMDESC", "ITEMLEVEL0", "ITEMLEVEL1CD", "ITEMLEVEL1DESC", "ITEMLEVEL2CD", "ITEMLEVEL2DESC", "ITEMLEVEL3CD", "ITEMLEVEL3DESC", "MANUFACTURERCD", "MANUFACTURERNAME", "ISACTIVE", "CATEGORYNAME", "CLASSIFICATION1", "CLASSIFICATION2", "CLASSIFICATION3", "CLASSIFICATION4", "CLASSIFICATION5", "BRAND_STRING2", "LOCATIONNAME", "HIMS_STATUS", "BRAND_PRODUCT_MAP_CD", "LOCATIONCD"]
                let _totaldata = await commonCheckExitOrNot(_collectionName, req, _filter, "GETALL", "find", _columnNames)
                let _exitData = await commonCheckExitOrNot(_collectionName, req, _filter, "GETALL", "findCount");
                if (_totaldata.data.length > 0 && _exitData.data.length > 0) {
                    _finalCountData = {
            ...
            if (req.body.params.flag1 == "GETALL") {
                ...
                delete req.body.params.flag1;
                let _columnNames = []
                if (req.body.params.TYPE != "MAPPING") {
                    _columnNames = ["DISEASE_CD", "DISEASE_NAME", "DISEASE_DESC", "STY", "TTY", "NCI_ID", "DISEASE_MESH_CUI", "Version", "DISEASE_DEFINATION", "DISEASE_CUI", "DISEASE_DUI", "DISEASE_PARENT_CD", "DISEASE_MASTER_CD", "DISEASE_MASTER_ID"]
                    // _columnNames = ["DISEASE_CD", "DISEASE_NAME", "DISEASE_DESC", "NCI_ID", "DISEASE_DEFINATION", "DISEASE_CUI", "DISEASE_DUI", "DISEASE_PARENT_CD", "DISEASE_MASTER_CD", "DISEASE_MASTER_ID"]
                    _filter.filter["DISEASE_MASTER_TYPE"] = req.body.params.TYPE
                    _collectionName = "disease_source_masters_with_umls_dd";
                } else {
                    _columnNames = ["DISEASE_MAP_CD", "DISEASE_NAME", "DISEASE_CUI", "ICD_CODE", "DISEASE_PARENT_CD", "REFERENCE_ID", "REFERENCE_SOURCE", "SNOMED_CD", "SNOMED_NAME", "MESHDUI", "DF_CODE"]
                   // _columnNames = ["DISEASE_MAP_CD", "DISEASE_NAME", "DISEASE_CUI", "NCI_ID", "DISEASE_PREFERRED_TERM", "ICD_CODE", "DISEASE_PARENT_CD", "REFERENCE_ID", "REFERENCE_SOURCE"]
                    _collectionName = "diseasemapping_cui";
                }
            ...
            _collectionName = "company"
            delete req.body.params.flag;
            const searchKey = req.body.params.Search_key
            if (req.body.params.flag1 == "GETALL") {
                delete req.body.params.flag1;
                let _columnNames = ["COMPANY_CD", "COMPANY_NAME", "SEL_DIVISION", "DIVISION_NAME", "ADDRESS1", "ADDRESS2", "CITY", "STATE", "COUNTRY", "PIN_CODE", "CONTACT_PERSON", "DESIGNATION", "PHONE", "MOBILE", "WEBSITE", "EMAIL", "COMPANY_CATEGORY", "COMPANY_TYPE", "FILES", "WEBSITE_NAME"]
                let _totaldata = await commonCheckExitOrNot(_collectionName, req, _filter, "GETALL", "find", _columnNames)
                let _exitData = await commonCheckExitOrNot(_collectionName, req, _filter, "GETALL", "findCount");
                if (_totaldata.data.length > 0 && _exitData.data.length > 0) {
                    _finalCountData = {
                ...
                let _filterObj = {
                    filter: [
                        {
                            $match: {
                                [req.body.params.Search_key]: { $regex: req.body.params.Search_Val, $options: 'i' }
                            }
                        },
                        {
                            $group: {
                                _id: `$${searchKey}`, data: { $first: "$$ROOT" }
                            }
                        },
                        {
                            $replaceRoot: {
                ...
                let _mRespData = await _mUtils.commonMonogoCall(`monography_${_collectionName}`, "aggregation", _filterObj, "", "", "", "")
                if (!(_mRespData && _mRespData.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _mRespData.desc || "", data: _mRespData.data || [] });
                } else {
                    return res.status(200).json({ success: true, status: 200, desc: '', data: _mRespData.data });
            ...
            else if (req.body.params.flag1 == "CODEDATA") {
                _filter.filter = [
                    { $match: { COMPANY_CD: { $eq: req.body.params.COMPANY_CD } } },
                ]
                let _mRespData = await _mUtils.commonMonogoCall(`monography_${_collectionName}`, "aggregation", _filter, "", "", "", "")
                if (!(_mRespData && _mRespData.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _mRespData.desc || "", data: _mRespData.data || [] });
                } else {
                    return res.status(200).json({ success: true, status: 200, desc: '', data: _mRespData.data });
            ...
            if (req.body.params.flag1 == "GETALL") {
                ...
                let _mResp = await _mUtils.commonMonogoCall(`monography_parent_brand_mapping`, "aggregation", _filter, "", "", "", "");
                let _customArrey =
                    [
                        {
                            _collName: "parent_brand_master",
                ...
                let _getAllMasterRelationalData = await getAllMasterRelationalData1(JSON.parse(JSON.stringify(_mResp.data)), 0, [], req, _customArrey)
                if (_getAllMasterRelationalData.data.length > 0) {
                    return res.status(200).json({ success: true, status: 200, desc: '', data: _getAllMasterRelationalData.data });
                }
            ...
            if (req.body.params.flag1 == "GETALL") {
                ...
                let nameExp = { $regex: req.body.params.searchValue, $options: 'i' };
                if (req.body.params.searchValue && req.body.params.searchValue != "") {
                    _filter.filter["BRAND_EXTENSION_NAME"] = nameExp;
                }
                let _mRespData = await _mUtils.commonMonogoCall(`monography_${_collectionName}`, "find", _filter, "", "", "", "")
                if (!(_mRespData && _mRespData.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _mRespData.desc || "", data: _mRespData.data || [] });
                ...
                let _finalCountData = {};
                if (_mRespData.data.length > 0) {
                    let _uomData = _.chain(_mRespData.data).groupBy('BRAND_EXTENSION_NAME')
                        .map((_v, _k) => {
                            if (_k != "") {
                                _data.push({
                                    BRAND_EXTENSION_NAME: _k,
                                    BRAND_EXTENSION_CD: _v[0].BRAND_EXTENSION_CD
                                })
                            }
                        }).value();
                    return res.status(200).json({ success: true, status: 200, desc: '', data: _data });
            ...
            delete req.body.params.flag;
            try {
                const nameExp = new RegExp(req.body.params.searchValue.startsWith('^') ? `^${req.body.params.searchValue.slice(1).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}` : req.body.params.searchValue.startsWith('.*') && req.body.params.searchValue.endsWith('.*') ? `.*${req.body.params.searchValue.slice(2, -2).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}.*` : req.body.params.searchValue.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), "i");
                let _filter = {
                    "filter": [
                        {
                            $match: { DISEASE_NAME: nameExp }
                        },
                        {
                            $project: { DISEASE_CUI: 1, DISEASE_CD: 1, DISEASE_DEFINATION: 1, DISEASE_DUI: 1, DISEASE_PARENT_CD: 1, DISEASE_MASTER_CD: 1, DISEASE_MASTER_ID: 1, DISEASE_MASTER_TYPE: 1, DISEASE_MESH_CUI: 1, DISEASE_NAME: 1 }
                        }
                    ]
                }
                let dd_helthOutput = await _mUtils.commonMonogoCall(`${_dbWihtColl}_${_collectionName}`, "aggregation", _filter, "", "", "", "");
                if (!(dd_helthOutput && dd_helthOutput.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: dd_helthOutput.desc || "", data: dd_helthOutput.data || [] });
                }
                else {
                    let _distinctCol = 'DISEASE_MASTER_CD'
                    if (dd_helthOutput.data.length > 0) {
                        let _distResp = _.chain(dd_helthOutput.data).groupBy(`${_distinctCol}`)
                            .map((_v, _k) => {
                                if (_k != "") {
                                    _output1.push({
                                        DISEASE_CUI: _v[0].DISEASE_CUI ? _v[0].DISEASE_CUI : "",
                                        DISEASE_NAME: _v[0].DISEASE_NAME ? _v[0].DISEASE_NAME : "",
                                        REFERENCE_ID: _v[0].DISEASE_MASTER_CD ? _v[0].DISEASE_MASTER_CD : "",
                                        REFERENCE_SOURCE: _v[0].DISEASE_MASTER_TYPE ? _v[0].DISEASE_MASTER_TYPE : "",
                                    })
                                }
                            }).value();
                    }
                    return res.status(200).json({ success: true, status: 200, desc: '', data: _output1 });
                }
        ...
        } else if (req.body.params.flag == "DD_HEALTH_CONDITION_REMAINS_DATA") {
            _collectionName = "disease_source_masters_with_umls_dd";
            delete req.body.params.flag;
            try {
                let _filter = {
                    "filter": [
                        {
                            $match: { DISEASE_CUI: req.body.params.DISEASE_CUI }
                        },
                        {
                            $project: { DISEASE_MASTER_TYPE: 1, DISEASE_CD: 1, DISEASE_NAME: 1, DISEASE_DUI: 1, DISEASE_MESH_CUI: 1, DISEASE_DEFINATION: 1, DISEASE_PARENT_CD: 1 }
                        }
                    ]
                }
                let _outputData = await _mUtils.commonMonogoCall(`${_dbWihtColl}_${_collectionName}`, "aggregation", _filter, "", "", "", "");
                let obj = {}
                let data = [
                    {
                        DISEASE_MASTER_TYPE: "ICD10CM",
                        ICD_CODE: "DISEASE_CD",
                        ICD_NAME: "DISEASE_NAME",
                        DISEASE_PARENT_CD: "DISEASE_PARENT_CD"
                    },
                    {
                        DISEASE_MASTER_TYPE: "MSH",
                        MESHDUI: "DISEASE_DUI",
                        MESH_CUI: "DISEASE_MESH_CUI",
                        MESHDUINAME: "DISEASE_NAME",
                        DF_CODE: "DISEASE_DEFINATION"

                    },
                    {
                        DISEASE_MASTER_TYPE: "SNOMEDCT_US",
                        SNOMED_CD: "DISEASE_CD",
                        SNOMED_NAME: "DISEASE_NAME"
                    }
                ];

                let diffrence = _outputData.data.length > 0 ? _.differenceBy(data, _outputData.data, 'DISEASE_MASTER_TYPE') : data
                if (diffrence.length > 0) {
                    _.each(diffrence, (_o, _i) => {
                        _.each(_o, (_v, _k) => {
                            if (_k != "DISEASE_MASTER_TYPE") {
                                obj[`${_k}`] = ""
                            }
                ...
                if (_outputData.data.length > 0) {
                    _.each(_outputData.data, (_o, _i) => {
                        let _filterData = _.filter(data, (_obj) => { return _obj.DISEASE_MASTER_TYPE == _o.DISEASE_MASTER_TYPE })
                        _.each(_filterData[0], (_v, _k) => {
                            if (_k != "DISEASE_MASTER_TYPE") {
                                _.each(_o, (_v1, _k1) => {
                                    if (_v == _k1) {
                                        obj[`${_k}`] = _v1
                ...
                return res.status(200).json({ success: true, status: 200, desc: '', data: obj });
                    ...
                    [
                        {
                            ...
                            _project1: "BRAND_NAME",
                            _project2: "BRAND_CD",
                            _combName: "BRAND_DISPLAY_NAME",
                            _comb1: "BRAND_NAME",
                            _comb2: "BRAND_EXTENSION_NAME"
                        }, {
    ...
    try {
        ...
        if (req.body.params._id || req.body.params[0]._id) {
            ...
            if (req.body.params.flag == "UOM") {
                ...
                //     }
                // }
                // let _mRespDatabrand = await _mUtils.commonMonogoCall(`monography_brand_display_name`, "find", _filter, "", "", "", "")
                // if (!(_mRespDatabrand && _mRespDatabrand.success)) {
                //     // return res.status(400).json({ success: false, status: 400, desc: _mRespDatabrand.desc || "", data: _mRespDatabrand.data || [] });
                // } else {
                //     req.body.params.BRAND_CD = _mRespDatabrand.data[0].BRAND_CD ? _mRespDatabrand.data[0].BRAND_CD : "";
                ...
                //     }
                //     let _mResp;
                //     if (req.body.params.JSON && req.body.params.JSON.length > 0 && req.body.params.JSON1[0].isApproved === true) {
                ...
                _arrayObj = [
                    {
                        "flag1": "brand-master",
                        "_searchKey": "BRAND_CD",
                        "col1": "parent_brand_mapping",
                        "col1_Name": "Brand Mapping Form",
                        "col2": "brand_display_name",
                        "col2_Name": "brand display name",
                        "master_col_Name": _collectionName
                    }
                ]
                if (req.body.params.hasOwnProperty('recStatus') && req.body.params.recStatus == false) {
                ...
                if (req.body.params.hasOwnProperty('recStatus') && req.body.params.recStatus == false) {
                    ...
                    _filter.filter = {
                        $or: [
                            ...
                            {
                                "BRAND_CD": req.body.params.BRAND_CD,
                                "PARENT_BRAND_CD": req.body.params.PARENT_BRAND_CD
                            }
                        ]
                    }
                    _filter.filter['recStatus'] = { $eq: true }
            ...
            // }
                ...
                _arrayObj = [
                    {
                        "flag1": "brand-extension-master",
                        "_searchKey": "BRAND_EXTENSION_CD",
                        "col1": "brand_display_name",
                        "col1_Name": "brand display name",
                        "master_col_Name": _collectionName
                    }
                ]
                if (req.body.params.hasOwnProperty('recStatus') && req.body.params.recStatus == false) {
                ...
                if (req.body.params.hasOwnProperty('recStatus') && req.body.params.recStatus == false) {
                    ...
                    _filter.filter = {
                        $or: [
                            ...
                            {
                                "BRAND_CD": req.body.params.BRAND_CD,
                                "BRAND_EXTENSION_CD": req.body.params.BRAND_EXTENSION_CD == undefined ? "" : req.body.params.BRAND_EXTENSION_CD,
            ...
            if (req.body.params.recStatus == false) {
                pLoadResp = { payload: {} };
                if (req.body.params.hasOwnProperty('flag1')) {
                    let j = 0;
                    while (_arrayObj.length > j) {
                        if (_arrayObj[j].flag1 === req.body.params.flag1) {
                            let _filterObj = _arrayObj.filter(obj => obj.flag1 == _arrayObj[j].flag1)
                            let _getAllMappedData = await getAllMappedData(_filterObj, 0, [], req)
                            if (_getAllMappedData.data.length > 0) {
                                return res.status(200).json({ success: true, status: 200, desc: '', data: _getAllMappedData.data });
                            } else {
                                return res.status(200).json({ success: true, status: 200, desc: '', data: _getAllMappedData.data || [] });
                            }
                        }
...
//get-all mapped data from brand
async function getAllMappedData(filter, _idx, _output, req) {
    if (filter.length > _idx) {
        let obj = {}
        let _filter = { "filter": [] }
        let _matchedKey;
        if (req.body.params.recStatus == false) {
        ...
        _filter = {
            "filter": [
                {
                    $match: { recStatus: { $eq: _matchedKey }, [`${filter[_idx]._searchKey}`]: { $ne: "" } }
                }
            ]
        }
        if (filter[_idx].hasOwnProperty('col1')) {
            let _mRespData = await _mUtils.commonMonogoCall(`monography_${filter[_idx].col1}`, "aggregation", _filter, "", "", "", "")
            if (!(_mRespData && _mRespData.success)) {

            } else {
                obj['master_col_Name'] = filter[_idx].master_col_Name;
                let _index = 0;
                while (_mRespData.data.length > _index) {
                    if (_mRespData.data[_index][filter[_idx]._searchKey] === req.body.params[filter[_idx]._searchKey]) {
                        _output.push(_mRespData.data[_index])
                    }
                    _index++;
                }
                let i = 0;
                while (_output.length > i) {
                    _output[i].Screen_Name = filter[_idx].col1_Name;
                    _output[i].Collection_Name = filter[_idx].col1;
                    i++;
                }
            }
        }
        if (filter[_idx].hasOwnProperty('col2')) {
            let _mRespData = await _mUtils.commonMonogoCall(`monography_${filter[_idx].col2}`, "aggregation", _filter, "", "", "", "")
            if (!(_mRespData && _mRespData.success)) {

            } else {
                let _index = 0;
                let i = _output.length - 1 + 1;
                while (_mRespData.data.length > _index) {
                    if (_mRespData.data[_index][filter[_idx]._searchKey] === req.body.params[filter[_idx]._searchKey]) {
                        _output.push(_mRespData.data[_index])
                    }
                ...
                while (_output.length > i) {
                    _output[i].Screen_Name = filter[_idx].col2_Name;
                    _output[i].Collection_Name = filter[_idx].col2;
                    i++;
                }

            }
        }
        if (filter[_idx].hasOwnProperty('col3')) {
            let _mRespData = await _mUtils.commonMonogoCall(`monography_${filter[_idx].col3}`, "aggregation", _filter, "", "", "", "")
            if (!(_mRespData && _mRespData.success)) {
                ...
                while (_output.length > i) {
                    _output[i].Screen_Name = filter[_idx].col3_Name;
                    _output[i].Collection_Name = filter[_idx].col3;
                    i++;
        ...
        _output.push(obj);
        _idx = _idx + 1
        await getAllMappedData(filter, _idx, _output, req)
    } else {
        return { success: true, data: _output }
    }

    return {
        "success": _output.length > 0 ? true : false,
        "data": _output
    }
        ...
        if (_data.length > _idx) {
            let _filter = {
                "filter": {
                    ...
                    "SRC_DRUG_CD": _data[_idx].SRC_DRUG_CD,
                    "INT_TYPE_ID": _data[_idx].INT_TYPE_ID,
                    "isApproved": _data[_idx].isApproved,
                }
            }
            ...
            if (_mResp.data.length > 0) {
                _.each(JSON.parse(JSON.stringify(_mResp.data)).splice(0, 1), (_obj, _ind) => {
                    _obj['BRAND_PRODUCT_NAME'] = _data[_idx].BRAND_NAME_1 && _data[_idx].BRAND_NAME_1.length > 0 && _data[_idx].BRAND_NAME_2 && _data[_idx].BRAND_NAME_2.length > 0 ? _data[_idx].BRAND_NAME_1 + "<-->" + _data[_idx].BRAND_NAME_2 : _data[_idx].BRAND_NAME_1
                    _obj['SRC_BRAND_PRODUCT_CD'] = _data[_idx].SRC_BRAND_PRODUCT_CD
                    _obj['SRC_BRAND_PRODUCT_NAME'] = _data[_idx].BRAND_NAME_1
                    _obj['INT_BRAND_PRODUCT_CD'] = _data[_idx].INT_BRAND_PRODUCT_CD
                    _obj['INT_BRAND_PRODUCT_NAME'] = _data[_idx].BRAND_NAME_2
        ...
        if (_data.length > _idx) {
            let _filter = {
                "filter": [
                    ...
                    { $project: { STRING_3_CD: "$STRING_3_CD", BRAND_STRING2_CD: "$BRAND_STRING2_CD", BRAND_STRING2: "$BRAND_STRING2", BRAND_PRODUCT_MAP_CD: "$BRAND_PRODUCT_MAP_CD" } }
                ]
            }
            // _filterObj.filter = {}
            // _filterObj.filter['recStatus'] = { $eq: true }
            ...
            if (!(_mBrandProductResp && _mBrandProductResp.success)) {
                ...
                if (!(_mClinicalProductResp && _mClinicalProductResp.success)) {
                    ...
                    if (_mMedicinalProductResp.data.length > 0) {
                        _.each(JSON.parse(JSON.stringify(_mMedicinalProductResp.data)), (_obj, _ind) => {
                            _obj['BRAND_CD'] = _mBrandProductResp.data[0].BRAND_STRING2_CD
                            _obj['BRAND_PRODUCT_MAP_CD'] = _mBrandProductResp.data[0].BRAND_PRODUCT_MAP_CD
                            _obj['BRAND_NAME'] = _mBrandProductResp.data[0].BRAND_STRING2
        ...
        if (_data.length > _idx) {
            ...
            if (_mResp.data.length > 0) {
                _.each(JSON.parse(JSON.stringify(_mResp.data)), (_obj, _ind) => {
                    ...
                    _obj["SRC_SUBSTANCE_NAME"]= _data[_idx].DD_SUBSTANCE_NAME,
                    _obj["INT_SUBSTANCE_NAME"]= "",
...
//                             referenceIdAgnaistClassCd = await getDataBasedOnLooping(_data[_idx].ALLERGY[index].ALLERGY_CD.split(","), 0, [], req, _filter, "class_name_cui", "REFERENCE")
//                             let index1 = 0;
...
//                     //     _output.push(_Bo)
//                     // })
//                 } else if ((_data[_idx].BRAND_PRODUCT_CD && _data[_idx].PREG_CD) &&  req.body.JSON[0].GENDER == "Female" && req.body.JSON[0].ISPREGNANCY == "Y") {
...
//                         })
//                     }
//                 } else if ((_data[_idx].BRAND_PRODUCT_CD && _data[_idx].LACT_CD) &&  req.body.JSON[0].GENDER == "Female" && req.body.JSON[0].ISLACTATION == "Y") {
        ...
        if (_data.length > _idx) {
            ...
            if (_data[_idx].BRAND_PRODUCT_CD || _data[_idx].CLS_CD || _data[_idx].DRUG_CD) {
                ...
                if ((_data[_idx].BRAND_PRODUCT_CD && _data[_idx].CLS_CD) || (_data[_idx].BRAND_PRODUCT_CD && _data[_idx].DRUG_CD)) {
                    ...
                    let classAganistSubstances = await getDataAgainstClass(_getBerandSubstanceNames.data, 0, [], req, _filter, "dd_substance_classifications", "SUBSTANCE_AGANIST_CLASS");
                    // let classAganistSubstances = await getDataBasedOnLooping(_getBerandSubstanceNames.data, 0, [], req, _filter, "dd_substance_classifications", "SUBSTANCE_AGANIST_CLASS");
                    ...
                    while (index < _data[_idx].ALLERGY.length) {
                        let substanceAganistClass;
                        let referenceIdAgnaistClassCd = await getDataBasedOnLooping(_data[_idx].ALLERGY[index].ALLERGY_CD.split(","), 0, [], req, _filter, "dd_substance_classifications", "REFERENCE")
                        if (_data[_idx].ALLERGY[index].TYPE == "DRUG" || _data[_idx].ALLERGY[index].TYPE == "Substance") {
                            substanceAganistClass = await getDataBasedOnLooping(_data[_idx].ALLERGY[index].ALLERGY_CD.split(","), 0, [], req, _filter, "dd_substance_classifications", "CLASS_NAME");
                            allergyAgainstData = await getDataBasedOnLooping(_data[_idx].ALLERGY[index].ALLERGY_CD.split(","), 0, [], req, _filter, "allergy_master", "ALLERGY_NAME");

                            _.each(_getBerandSubstanceNames.data, (_o, _i) => {
                                let _filterClassData = _.filter(classAganistSubstances.data, (_filClassData, _filClassIndx) => { return _filClassData.DD_SUBSTANCE_CD === _o.DD_SUBSTANCE_CD })
                                _.each(JSON.parse(JSON.stringify(allergyAgainstData.data)), (_o1, _i1) => {
                                    if (_o.DD_SUBSTANCE_CD === _o1.ALLERGY_CD) {
                                        let _filterSubstanceAgnstClassData = _.filter(classAganistSubstances.data, (_obj, _indx) => { return _obj.DD_SUBSTANCE_CD === _o.DD_SUBSTANCE_CD })
                                        _output.push(
                                            {
                                                "INT_NAME": "Drug To Allergy",
                                                "INT_TYPE": "DRUG_TO_ALLERGY",
                                                "SRC_BRAND_PRODUCT_CD": _o.BRAND_PRODUCT_MAP_CD,
                                                "SRC_BRAND_PRODUCT_NAME": _o.BRAND_NAME,
                                                "INT_BRAND_PRODUCT_CD": _o1.ALLERGY_CD,
                                                "INT_BRAND_PRODUCT_NAME": _o1.ALLERGY_NAME,
                                                "SRC_SUBSTANCE_NAME": _o.DD_SUBSTANCE_NAME,
                                                "INT_SUBSTANCE_NAME": "",
                                                "SRC_DRUG_CD": _o.DD_SUBSTANCE_CD,
                                                "INT_TYPE_ID": _o.BRAND_NAME + "<-->" + _o1.ALLERGY_NAME,
                                                "CLASS_NAME_CD": "",
                                                "CLASS_NAME": "",
                                                "BRAND_PRODUCT_NAME": _o.BRAND_NAME + "<-->" + _o1.ALLERGY_NAME,
                                                "INTERACTIONS": `Drug allergy identified in response to the prescribed medication the patient has a documented history of drug allergy to <b>${_o['DD_SUBSTANCE_NAME']}</b>. The prescribed medication <b>${_o.BRAND_NAME}</b> includes <b>${_o1['ALLERGY_NAME']}</b>, Which belongs to the class of medications known as <b>${_filterSubstanceAgnstClassData[0].CLASS_NAME}</b>`
                                            }
                                        )
                                    } else {
                                        _.each(_filterClassData, (_filDta, _filindx) => {
                                            _.each(referenceIdAgnaistClassCd.data, (_referObj, _referIndx) => {
                                                if (_filDta.CLASS_NAME_CD === _referObj.CLASS_NAME_CD) {
                                                    _output.push(
                                                        {
                                                            "INT_NAME": "Drug To Allergy",
                                                            "INT_TYPE": "DRUG_TO_ALLERGY",
                                                            ...
                                                            "SRC_DRUG_CD": _o.DD_SUBSTANCE_CD,
                                                            "INT_TYPE_ID": _o.BRAND_NAME + "<-->" + _o1.ALLERGY_NAME,
                                                            "CLASS_NAME_CD": _filDta.CLASS_NAME_CD,
                                                            "CLASS_NAME": _filDta.CLASS_NAME,
                                                            "BRAND_PRODUCT_NAME": _o.BRAND_NAME + "<-->" + _o1.ALLERGY_NAME,
                                                            "INTERACTIONS": `Drug allergy identified in response to the prescribed medication the patient has a documented history of drug allergy to <b>${_o1['ALLERGY_NAME']}</b>. The prescribed medication <b>${_o.BRAND_NAME}</b> includes <b>${_o['DD_SUBSTANCE_NAME']}</b>, Which belongs to the class of medications known as <b>${_filDta.CLASS_NAME}</b>`
                                                        }
                                                    )
                                                }
                                            })
                    ...
                    //                             "INT_BRAND_PRODUCT_NAME": _o1.ALLERGY_NAME,
                    //                             "SRC_DRUG_CD": _o.DD_SUBSTANCE_CD,
                    //                             "SRC_DRUG_NAME": _o.DD_SUBSTANCE_NAME,
                    //                             // "INT_DRUG_CD": _o1.ALLERGY_CD,
                    //                             // "INT_DRUG_NAME": _o1.ALLERGY_NAME,
                    //                             "INT_TYPE_ID": _o.BRAND_CD + "<-->" + _o1.ALLERGY_NAME,
                    //                             "BRAND_PRODUCT_NAME": _o.BRAND_NAME + "<-->" + _o1.ALLERGY_NAME,
                    //                             "INTERACTIONS": `Drug allergy identified in response to the prescribed medication the patient has a documented history of drug allergy to ${_o['DD_SUBSTANCE_NAME']}.the prescribed medication ${_o.BRAND_NAME} includes ${_o1['ALLERGY_NAME']},which belongs to the class of medications known as ${_filterSubstanceAgnstClassData[0].CLASS_NAME}`
                    //                         }
                    ...
                    let _BrandWithDifferentTypes = await BrandWithDifferentTypes(req.body.JSON[0], 0, [], req, _filter, "BRAND_WITH_FOOD_OR_LAB", "", _getBerandSubstanceNames, _data[_idx].PREG_CD.split(","))
                    if (_BrandWithDifferentTypes && _BrandWithDifferentTypes.data && _BrandWithDifferentTypes.data.length > 0) {
                        _.each(_BrandWithDifferentTypes.data, (_o, _i) => {
                            _output.push(_o)
                        })
                    }
                            ...
                            _comparisonData.push(
                                {
                                    ...
                                    "SRC_BRAND_PRODUCT_CD": _o.BRAND_CD ? _o.BRAND_CD : "",
                                    "INT_BRAND_PRODUCT_CD": _o1.BRAND_CD ? _o1.BRAND_CD : "",
                                    "SRC_SUBSTANCE_NAME": _o.DD_SUBSTANCE_NAME && _o.DD_SUBSTANCE_NAME.length >0 ? _o.DD_SUBSTANCE_NAME :"",
                                    "INT_SUBSTANCE_NAME": _o1.DD_SUBSTANCE_NAME && _o1.DD_SUBSTANCE_NAME.length > 0 ? _o1.DD_SUBSTANCE_NAME:"",
                                    "BRAND_NAME_1": _o.BRAND_NAME ? _o.BRAND_NAME : "",
                                    "BRAND_NAME_2": _o1.BRAND_NAME ? _o1.BRAND_NAME : "",
                                    "isApproved": true
                                }
                            ...
                            _comparisonData.push(
                                {
                                    "SRC_DRUG_CD": _o.DD_SUBSTANCE_CD,
                                    "INT_TYPE_ID": _o1,
                                    "SRC_BRAND_PRODUCT_CD": _o.BRAND_PRODUCT_MAP_CD ? _o.BRAND_PRODUCT_MAP_CD : "",
                                    "INT_BRAND_PRODUCT_CD": _o1.BRAND_CD ? _o1.BRAND_CD : "",
                                    "SRC_SUBSTANCE_NAME": _o.DD_SUBSTANCE_NAME && _o.DD_SUBSTANCE_NAME.length >0 ? _o.DD_SUBSTANCE_NAME :"",
                                    "INT_SUBSTANCE_NAME": _o1.DD_SUBSTANCE_NAME && _o1.DD_SUBSTANCE_NAME.length > 0 ? _o1.DD_SUBSTANCE_NAME:"",
                                    "BRAND_NAME_1": _o.BRAND_NAME ? _o.BRAND_NAME : "",
...
async function getDiseaseCodesAganistICDS(_data, _idx, _output, req, _filterObj) {
    try {
        if (_data.length > _idx) {
            ...
            let _mRespData = await _mUtils.commonMonogoCall(`${_dbWihtColl}_diseasemapping_cui`, "find", _filter, "", "", "", "")
...
router.post("/insert-document", async (req, res) => {
    try {
        mongoMapper(`${_dbWihtColl}_production_document`, req.body.query, req.body.params).then((result) => {
            return res.status(200).json({ status: 'SUCCESS', desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ status: 'FAIL', desc: error.desc || error, data: [] });
        });
    ...
    try {
        ...
        if (req.body.params.fileUploads && req.body.params.fileUploads.length > 0 && req.body.params.reference) {
            ...
            while (_reqData.length > _indx) {
                ...
                } else if (_reqData[_indx].format.toLowerCase() == "vnd.openxmlformats-officedocument.spreadsheetml.sheet" || _reqData[_indx].format.toLowerCase() == "csv" || _reqData[_indx].format.toLowerCase() == "pdf" || _reqData[_indx].format.toLowerCase() == "docx") {
                    let _extension = _reqData[_indx].imageData.split(';')[0].split('/')[1];
                    if (_extension == 'vnd.openxmlformats-officedocument.spreadsheetml.sheet' || _extension == 'csv' || _extension == 'kswps' || _extension == 'pdf' || _extension == 'docx') {
                        // let _docData = await compressDocument(_cBody.params.fileUploads[_indx].imageData, _reqData[_indx].fileName);
                        _reqData[_indx]['docMimeType'] = await _reqData[_indx].imageData.split(',')[0];
...
async function generateCombination(data, _json) {
    let _keys = Object.keys(data)
    const values = Object.keys(data).map(key => data[key].split(','))
    if (values.length < 2) throw new Error("at least two keys are required")
    const combination = [];
    const createCombinations = (_keys, values) => {
        for (let i = 0; i < values[0].length; i++) {
            for (let j = 0; j < values[1].length; j++) {
                if (values.length === 3) {
                    for (let k = 0; k < values[2].length; k++) {
                        combination.push({ [_keys[0]]: values[0][i], [_keys[1]]: values[1][j], [_keys[2]]: values[2][k], recStatus: true, INT_ID: _json.INT_ID })
                        combination.push({ [_keys[1]]: values[0][i], [_keys[0]]: values[1][j], [_keys[2]]: values[2][k], recStatus: true, INT_ID: _json.INT_ID })
                    }
                } else {
                    combination.push({ [_keys[0]]: values[0][i], [_keys[1]]: values[1][j], recStatus: true, INT_ID: _json.INT_ID, IS_CD: _json.IS_CD })
                    combination.push({ [_keys[1]]: values[0][i], [_keys[0]]: values[1][j], recStatus: true, INT_ID: _json.INT_ID, IS_CD: _json.IS_CD })
                }
            }
        }
    }
    createCombinations(_keys, values)
    return combination
}
/////checking in integration is thrie or not
router.post('/checking-interaction', async (req, res) => {
    try {
        let _indx = 0
        let _data = []
        while (req.body.params.JSON1.length > _indx) {
            let _reqireKeys = _.pick(req.body.params.JSON1[_indx], ['SRC_DRUG_CD', 'INT_TYPE_ID', 'CLASS_NAME_CD'])
            let _combData = await generateCombination(_reqireKeys, req.body.params.JSON1[_indx])
            ...
            let _filter = {
                "filter": [
                    ...
                    {
                        $group: {
                            _id: { SRC_DRUG_CD: "$SRC_DRUG_CD", INT_TYPE_ID: "$INT_TYPE_ID" },
                            data: { $first: "$$ROOT" }
                        }
                    },
                    {
            ...
            let _checkResp = await _mUtils.commonMonogoCall(`${_dbWihtColl}_drugchildinteraction`, "aggregation", _filter, "", "", "", "");
            if (!(_checkResp && _checkResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _checkResp.desc || "", data: _checkResp.data || [] });
            } else {
                if (_checkResp.data.length > 0) {
                    _.each(_checkResp.data[0].allRecords, (_o, _i) => {
                        _data.push(_o)
                    })
                }
            }
...
async function _batchingData(req, batchSize = 122757) {
    try {
        ...
        let _exitData = await commonCheckExitOrNot("umls_dd", req, _filterObj, "GETALL", "findCount");
        let totalDocuments = _exitData.data[0].TOTAL_COUNT
        let skip = 0
...
router.post("/checking-large-data", async (req, res) => {
    try {
        let batchFunction = await _batchingData(req)
        console.log("batchFunction", batchFunction)
    } catch (error) {
        console.log("error", error)
    }
})
...
router.post('/inactive-mapped-data', async (req, res) => {
    try {

        let _getAllinactiveAllMappedData = await inactiveAllMappedData(req, 0, [])
        return res.status(200).json({ success: true, status: 200, desc: 'inactive Succesfully' })
...
async function inactiveAllMappedData(req, _idx, _output) {
    while (req.body.params.json.length > _idx) {
        // if (req.body.params.json[_idx].recStatus == false) {
        let _filter = {
            "filter": [
                {
                    $match: { "_id": { $eq: mongoose.Types.ObjectId(req.body.params.json[_idx]._id) } }
                }
            ]
        }
        let _mResp = await _mUtils.commonMonogoCall(`monography_${req.body.params.json[_idx].Collection_Name}`, "aggregation", _filter, "", "", "", "")
        if (!(_mResp && _mResp.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
        }
        if (_mResp.data.length > 0) {
            ...
            let patData = {
                "params": {
                    "_id": JSON.parse(JSON.stringify(_mResp.data[0]._id)),
                    "recStatus": req.body.params.json[_idx].recStatus
                }
            }
            let pLoadResp = { payload: {} };
            patData.params.revNo = JSON.parse(JSON.stringify(_mResp.data[0])).revNo + 1;
            pLoadResp = await _mUtils.preparePayload("BW", patData);
            if (!pLoadResp.success) {
            }
            let updateResponse = await _mUtils.commonMonogoCall(`monography_${req.body.params.json[_idx].Collection_Name}`, 'bulkWrite', pLoadResp.payload, "", "", "", "")
            // if (!(updateResponse && updateResponse.success)) {
            //     return res.status(400).json({ success: false, status: 400, desc: updateResponse.desc || "", data: updateResponse.data || [] });
            // }
        }
        // }
        _idx++;
