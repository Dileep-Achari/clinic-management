...
router.post("/get-allerts_1", async (req, res) => {
    try {
        ...
        //return res.status(200).json({ success: true, status: 200, desc: '', data: givenDataagainstAddingClassData.data });
                        ...
                        if (!(_testResp1 && _testResp1.success)) {
                            ...
                            if (_testResp1.data.length > 0) {
                                ...
                                while (_indx < _testResp1Data.length) {
                                    let _compareFilter = _.filter(_comparisonData, (_co, _ci) => { return _testResp1.data[_indx].SRC_DRUG_CD == _co.SRC_DRUG_CD && _testResp1.data[_indx].CLASS_NAME_CD == _co.CLASS_NAME_CD })
                                    ...
                                    _.each(referenceIdAgnaistClassCd.data, (_secondClass, _secondIndx) => {
                                        if (_firstClass.CLASS_NAME_CD === _secondClass.CLASS_NAME_CD) {
                                            _output.push({
                                                ...
                                                "INTERACTIONS": `<p> Drug allergy identified in response to the prescribed medication. The patient has a documented history of <strong> drug allergy to ${_firstClass.DD_SUBSTANCE_NAME} </strong> .the prescribed medication <strong> ${_firstClass.BRAND_NAME} </strong> includes  <strong> ${_secondClass['CLASS_NAME']} </strong> ,which belongs to the class of medications known as <strong> ${_firstClass.CLASS_NAME} </strong> </p>`
                                            })
                                        }
                                    })
                                })
