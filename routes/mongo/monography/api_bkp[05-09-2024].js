...
router.use(async function (req, res, next) {
    ...
    try {
        req.dataInfo = {
            ...
            ORG_ID: req.body.dataInfo && req.body.dataInfo.ORG_ID ? req.body.dataInfo.ORG_ID : "",
            ORG_NAME: req.body.dataInfo && req.body.dataInfo.ORG_NAME ? req.body.dataInfo.ORG_NAME : "",
            ...
            LOC_ID: req.body.dataInfo && req.body.dataInfo.LOC_ID ? req.body.dataInfo.LOC_ID : "",
    ...
    try {
        ...
        if (req.body.JSON[0].TYPE == "ALL" || req.body.JSON[0].TYPE == "") {
            ...
            if (_cloneMonographData && _cloneMonographData.length > 0 && _cloneMonographData[0].IS_ASSIGNED == "PUBLISHED") {
                ...
                req.dataInfo['RESPONSE'] = _finalData.mongraphData
                let dataInformation = await _mUtils.commonMonogoCall(`${_dbWihtColl}_logsInfo`, "insertMany", req.dataInfo, "", "", "", "");
                return res.status(200).json({ success: true, status: 200, desc: '', data: _finalData, config: config });
            } else {
                return res.status(200).json({ success: true, status: 200, desc: '', data: [] });
    ...
    try {
        let _filter = {
            "filter": [
                ...
                {
                    $project: {
                        COMPANYCD: 1, LOCATIONCD: 1, BRAND_PRODUCT_MAP_CD: 1, BRAND_STRING2: 1, ITEMCD: 1, ITEMDESC: 1
                    }
                },
                {
                    $limit: 1
        ...
        if (!(_mRespDataProduct && _mRespDataProduct.success)) {
            ...
            if (_mRespDataProduct.data.length > 0) {
                ...
                while (_mRespDataProduct.data.length > k) {
                    ...
                    // if(updateResponse){
                    //     return res.status(200).json({ success: true, status: 200, desc: '', data: updateResponse.data });
                    // }else{
            ...
            for (let j = 0; j < values[1].length; j++) {
                if (values.length === 3) {
                    for (let k = 0; k < values[2].length; k++) {
                        combination.push({ [_keys[0]]: values[0][i], [_keys[1]]: values[1][j], [_keys[2]]: values[2][k], recStatus: true, INT_ID:_json[0].INT_ID})
                        combination.push({ [_keys[1]]: values[0][i], [_keys[0]]: values[1][j], [_keys[2]]: values[2][k], recStatus: true, INT_ID:_json[0].INT_ID })
                    }
                } else {
                    combination.push({ [_keys[0]]: values[0][i], [_keys[1]]: values[1][j], recStatus: true, INT_ID:_json[0].INT_ID })
                    combination.push({ [_keys[1]]: values[0][i], [_keys[0]]: values[1][j], recStatus: true, INT_ID:_json[0].INT_ID })
                }
            }
        }
    }
...
router.post('/checking-interaction', async (req, res) => {
    try {
        // let data={
        //    "SRC_DRUG_CD":"SS0000002,SS0000760",
        //    "INT_TYPE_ID":"SS0000285,SS0000571"
        // }

        let _reqireKeys = _.map(req.body.params.JSON1, _obj => _.pick(_obj, ['SRC_DRUG_CD', 'INT_TYPE_ID', 'CLASS_NAME_CD']))
        let _combData = await generateCombination(_reqireKeys[0],req.body.params.JSON1)
        let _filter = {
            "filter": [
                {
                    $match: {
        ...
        if (!(_checkResp && _checkResp.success)) {
            ...
            if (_checkResp.data.length > 0) {
                return res.status(200).json({ success: true, status: 200, desc: '', data: _checkResp.data[0].allRecords });
            } else {
                return res.status(200).json({ success: true, status: 200, desc: "", data: [] });
            }
...
router.post("/checking-large-data", async (req, res) => {
 ...
 router.post('/take-str-from-umls-dd',async(req,res)=>{
    try {
