...
async function monographGetAlert(_permuteData, req, res, _filterData) {
    ...
    if (_filterData.length > 0) {
        ...
        while (_permuteData.length > _indx) {
            ...
            let classAganistSubstances = await getDataBasedOnLooping(_classInput.data, 0, [], req, _filterAggregation, "dd_substance_classifications", "SUBSTANCE_AGANIST_CLASS");
    ...
    try {
        ...
        if (req.body.params.flag == "UOM") {
            ...
            try {
                ...
                //         _filter.filter = { "BRAND_PRODUCT_MAP_CD": { $exists: false }, IS_REQUIRED: true }
                                                    ...
                                                    _output.push(
                                                        {
                                                            ...
                                                            "BRAND_PRODUCT_NAME": _o.BRAND_NAME + "<-->" + _o1.ALLERGY_NAME,
                                                            "INTERACTIONS": `Drug allergy identified in response to the prescribed medication the patient has a documented history of drug allergy to ${_o1['ALLERGY_NAME']}. The prescribed medication ${_o.BRAND_NAME} includes ${_o['DD_SUBSTANCE_NAME']}, Which belongs to the class of medications known as ${_filDta.CLASS_NAME}`
                                                        }
                                                    )
                                                }
                                            })
