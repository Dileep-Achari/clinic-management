    ...
    try {
        ...
        if (_flag === "EXIST") {
            _.each(req.body.params, (_v, _k) => {
                if (_k != "_id" && _k != "audit" && _k != "history" && _k != "revNo" && _k != "REMARKS" && _k != "BRAND_PRODUCT_MAP_ID" && _k != "BRAND_PRODUCT_MAP_CD" && _k != "SOURCE_DATA" && _k != "recStatus") {
            ...
            _.each(_brandData.data, async (_o, _i) => {
                _.each(_itemType.data, async (_o1, _i1) => {
                    if (_o.DD_SUBSTANCE_CD === _o1.ALLERGY_CD) {
                        ...
                        console.log("_filterSubstanceAgnstClassData",_filterSubstanceAgnstClassData[0])
                        _output.push(
                            {
                                "INT_NAME": "Drug To Allergy",
                                "BRAND_PRODUCT_NAME": _o.BRAND_NAME + "<-->" + _o1.ALLERGY_NAME,
...
router.post("/update-assigned_data", async (req, res) => {
    try {
        ...
        let _data = []
        let _mRespData = await _mUtils.commonMonogoCall(`monography_drugworkflow`, "find", _filter, "", "", "", "")
        if (!(_mRespData && _mRespData.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _mRespData.desc || "", data: _mRespData.data || [] });
        }
        let _mapData = _.chain(_mRespData.data).groupBy('DD_SUBSTANCE_CD')
            .map((_d, _k) => {
                _data.push(_k)
            }).value();
        console.log("ffffffffffff", _data)
        let _indx = 0;
        while (_data.length > _indx) {
            _filter.filter['DD_SUBSTANCE_CD'] = _data[_indx]
            let _mRespData = await _mUtils.commonMonogoCall(`monography_dd_drug_master`, "find", _filter, "", "", "", "")
            if (!(_mRespData && _mRespData.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mRespData.desc || "", data: _mRespData.data || [] });
            }
            ...
            if (_mRespData.data.length > 0) {
                while (_mRespData.data.length > _Indx1) {
                    if (_mRespData.data[_Indx1].IS_ASSIGNED != "ASSIGNED") {
                        let patData = {
                            "params": {
                        ...
                        let _uResp = await _mUtils.commonMonogoCall("monography_dd_drug_master", 'bulkWrite', pLoadResp.payload, "", "", "", req.tokenData.dbType);
            ...
            // if(_mRespData.data.length > 0 && _mRespData.data[0].IS_ASSIGNED !="ASSIGNED"){
            //     let patData={
            //         "params":{
            //             "_id":_mRespData.data[0]._id,
            //             "IS_ASSIGNED":"ASSIGNED",
