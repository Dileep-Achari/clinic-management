...
router.post("/colectData-and-convert-json-to-excel_only_monographs", async (req, res) => {
    try {
        ...
        let _mBrandProductResp = await _mUtils.commonMonogoCall("monography_brand_product_mapping", "aggregation", _filter, "", "", "", "")
        if (!(_mBrandProductResp && _mBrandProductResp.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _mBrandProductResp.desc || "", data: _mBrandProductResp.data || [] });
        } else {
            let _indx = 0;
            while (_mBrandProductResp.data.length > _indx) {
                ...
                // _filter.filter['recStatus'] = { $eq: true }
                // _filter.filter['STRING_3_CD'] = _mBrandProductResp.data[_indx].STRING_3_CD
                let _mClinicalProductResp = await _mUtils.commonMonogoCall("monography_dd_product_master", "aggregation", _filter, "", "", "", "")
                if (!(_mClinicalProductResp && _mClinicalProductResp.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _mClinicalProductResp.desc || "", data: _mClinicalProductResp.data || [] });
                } else {
                    // _filter.filter = {}
                    ...
                    if (_mClinicalProductResp.data.length > 0 && _mClinicalProductResp.data[0].DD_DRUG_MASTER_CD) {
                        ...
                        // _filter.filter['DD_DRUG_MASTER_CD'] = _mClinicalProductResp.data[0].DD_DRUG_MASTER_CD
                        let _mMedicinalProductResp = await _mUtils.commonMonogoCall("monography_dd_drug_master", "aggregation", _filter, "", "", "", "")
                        if (!(_mMedicinalProductResp && _mMedicinalProductResp.success)) {
                            ...
                            if (_mMedicinalProductResp.data.length > 0 && _mMedicinalProductResp.data[0].DD_SUBSTANCE_CD) {
                                ...
                                // _filter.filter['DD_SUBSTANCE_CD'] = _mMedicinalProductResp.data[0].DD_SUBSTANCE_CD
                                let _mectionResp = await _mUtils.commonMonogoCall("monography_sections", "aggregation", _filter, "", "", "", "")
                                _exactRespObject['IS_MONOGRAPH'] = _mectionResp.data.length > 0 ? true : false;
                                finalJsonData.push(_exactRespObject)
                            }
                        }
            ...
            fs.writeFile(path.join(__dirname, "/monograph_data.json"), JSON.stringify(finalJsonData), () => {
                return res.status(200).json({ success: false, status: 200, desc: "", data: [] });
            })
...
async function _givenDataagainstAddingClassData(_data, _idx, _output, req, _filterObj) {
    try {
        if (_data.length > _idx) {
            ...
            let _dd_substance_classification_resp = await _mUtils.commonMonogoCall("monography_dd_substance_classifications", "aggregation", _filter, "", "", "", "");
            ...
            if ((req.body.JSON[0].BRAND_PRODUCT_CD.length == 0) || (req.body.JSON[0].BRAND_PRODUCT_CD.length != 0)) {
                if (req.body.JSON[0].BRAND_PRODUCT_CD != "" && req.body.JSON[0].DRUG_CD == "" && req.body.JSON[0].CLS_CD == "" && req.body.JSON[0].FOOD_CD == "" && req.body.JSON[0].DIS_CD == "" && req.body.JSON[0].LAB_TEST_CD == "" && req.body.JSON[0].ALLERGY.length == 0 && req.body.JSON[0].PREG_CD == "" && req.body.JSON[0].LACT_CD == "") {
                    if (_getBerandSubstanceNames.data.length > 1) {
                        ...
                        let _testResp = await _mUtils.commonMonogoCall("monography_drugchildinteraction", "find", _filter, "", "", "", "")
                        ...
                        let _testResp1 = await _mUtils.commonMonogoCall("monography_drugchildinteraction", "find", _filter, "", "", "", "")
                    ...
                    //     _finalData.alertsData.push(_o)
                    // })
                } else if ((req.body.JSON[0].BRAND_PRODUCT_CD == "" && req.bodyJSON[0].DRUG_CD == "" && req.body.JSON[0].CLS_CD == "" && (req.bodyJSON[0].FOOD_CD != "" || req.body.JSON[0].DIS_CD != "" || req.body.JSON[0].LAB_TEST_CD != "" || req.body.JSON[0].PREG_CD != "" || req.body.JSON[0].LACT_CD != "" || req.body.JSON[0].ALLERGY.length != ""))) {
                    let filterKeyData = []
...
router.post("/insert-pharmacology-data", async (req, res) => {
    try {
        ...
        while (_index < req.body.params.length) {
            ...
            let _mResp = await _mUtils.commonMonogoCall("monography_pharmacology_master", "find", _filter, "", "", "", "")
            ...
            if (_mResp.data.length > 0) {
                ...
                let _mResp3 = await _mUtils.commonMonogoCall("monography_pharmacology_master", "bulkWrite", pLoadResp.payload, "", "", "", "")
                ...
                let _mResp2 = await _mUtils.commonMonogoCall("monography_pharmacology_master", "insertMany", req.body.params[_index], "", "", "", "")
...
async function getDataAgainstClass(_data, _idx, _output, req, _filterObj, _coll, _type) {
    try {
        if (_data.length > _idx) {
            ...
            let _mRespData = await _mUtils.commonMonogoCall(`monography_${_coll}`, "aggregation", _filter, "", "", "", "")
            if (!(_mRespData && _mRespData.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mRespData.desc || "", data: _mRespData.data || [] });
            }
            if (_mRespData.data.constructor.name == "Array" && _mRespData.data.length > 0) {
...
router.post("/insert-master-data", async (req, res) => {
    try {
        let _seqResp;
        let _filter = { "filter": { "recStatus": { $eq: true } } };
        if (req.body.params.flag != "INN" && req.body.params.flag != "UNII" && req.body.params.flag != "SNOWMED" && req.body.params.flag !== "DMF" && req.body.params.flag !== undefined && req.body.params.flag !== "ENTITY" && req.body.params.flag !== "DD_SUBSTANCE_CLASSIFICATIONS" && req.body.params.flag !== "DRUG_INTERACTIONS" && req.body.params.flag !== "VALIDATE_BRAND_PRODUCT" && req.body.params.flag !== "FREQUENT" && req.body.params.flag !== "COMPANY") {
            _seqResp = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: `${req.body.params.sequenceName}` }, "monography", req);
            if (!(_seqResp && _seqResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _seqResp.desc || "", data: _seqResp.data || [] });
            }
        ...
        if (req.body.params.flag == "UOM") {
            ...
            if (_existData.code == 1) {
                ...
                req.body.params['DOSE_FORM_CD'] = req.body.params.DOSE_FORM_CD == "" ? await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: 'brandDoseFormCode' }, "monography", req).then((_obj) => { return _obj.data }) : req.body.params.DOSE_FORM_CD
            ...
            while (req.body.params.JSON.length > _indx) {
                _seqResp = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: "Company" }, "monography", req);
                if (!(_seqResp && _seqResp.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _seqResp.desc || "", data: _seqResp.data || [] });
                }
            ...
            let _mResp = await _mUtils.commonMonogoCall(`monography_company`, "insertMany", req.body.params.JSON, "", "", "", "");
            if (_mResp.data.length > 0) {
                return res.status(200).json({ success: true, status: 200, desc: '', data: _mResp });
            } else {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        ...
        //     let _existData = await commonCheckExitOrNot(_collectionName,req,_filter,"EXIST");
        //     if (_existData.code == 1) {
        //       req.body.params["COMPANY_CD"] =Object.keys(_seqResp.data).length > 0 ? _seqResp.data : "";
        //     } else {
        //       return res.status(200).json({success: true,status: 200,desc: "",data: "already exist",});
        //     }
        // } 
...
router.post("/update-dd_drug_master", async (req, res) => {
    try {
        ...
        while (_indx < _jsonData.length) {
            ...
            let _uResp = await _mUtils.commonMonogoCall("monography_dd_drug_master", 'bulkWrite', pLoadResp.payload, "", "", "", "");
...
async function addionbranddisplaycd(_data, _idx, _output, req) {
    try {
        if (_data.length > _idx) {
            ...
            let _mRespDataProduct = await _mUtils.commonMonogoCall(`monography_dd_product_master`, "aggregation", _filter, "", "", "", "")
    ...
    try {
        ...
        if (req.body.params.flag == "UOM") {
            ...
            if (req.body.params.flag1 == "GETALL") {
                ...
                let _totalUomData = []
                let _mRespData = await _mUtils.commonMonogoCall(`monography_releases`, "find", {}, "", "", "", "")
                if (!(_mRespData && _mRespData.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _mRespData.desc || "", data: _mRespData.data || [] });
                } else {
            ...
            if (req.body.params.flag1 === "DRPDATA") {
                ...
                let _mRespData = await _mUtils.commonMonogoCall(`monography_routeofadministrations`, "find", _filter, "", "", "", "")
                let _totalUomData = []
            ...
            if (req.body.params.flag1 == "DRPDATA") {
                ...
                let _mRespData = await _mUtils.commonMonogoCall(`monography_dose_form_map`, "find", _filter, "", "", "", "")
                if (!(_mRespData && _mRespData.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _mRespData.desc || "", data: _mRespData.data || [] });
                } else {
            ...
            if (req.body.params.flag1 == "DRPDATA") {
                ...
                if (req.body.params.MASTER_NAME == "IS") {
                    ...
                    let _msubclsData = await _mUtils.commonMonogoCall(`monography_drugintendedsite`, "aggregation", _filterObj, "", "", "", "");
                    if (!(_msubclsData && _msubclsData.success)) {
                        return res.status(400).json({ success: false, status: 400, desc: _msubclsData.desc || "", data: _msubclsData.data || [] });
                    }
                    let _frequentData = []
        ...
        // } 
            ...
            if (req.body.params.flag1 == "GETALL") {
                let _totalIsAddingData = []
                delete req.body.params.flag1;
                let _columnNames = ["DISEASE_CD", "DISEASE_NAME", "DISEASE_DESC", "NCI_ID", "DISEASE_DEFINATION", "DISEASE_CUI", "DISEASE_DUI", "DISEASE_PARENT_CD", "DISEASE_MASTER_CD", "DISEASE_MASTER_ID"]
            ...
            if (_data[_idx].BRAND_PRODUCT_CD || _data[_idx].CLS_CD || _data[_idx].DRUG_CD) {
                ...
                if ((_data[_idx].BRAND_PRODUCT_CD && _data[_idx].CLS_CD) || (_data[_idx].BRAND_PRODUCT_CD && _data[_idx].DRUG_CD)) {
                    ...
                    if (_data[_idx].DRUG_CD) {
                        ...
                        let _getFoodAndLabData = _.drop(_.clone(_getBerandSubstanceNames.data))
                        let _commonGetData = await commonGetData(_getFoodAndLabData, 0, [], req, _filter, "GIVEN_ONLY_BRAND_PRODUCT")
                        let _findDuplicatesAndCombinedData_1 = await findDuplicatesAndCombinedData(JSON.parse(JSON.stringify(_commonGetData.data)), 'SRC_DRUG_CD', "INT_TYPE_ID");
                        _.each(_findDuplicatesAndCombinedData_1, (_o, _i) => {
                            _output.push(_o)
                        })
                        /////drug with class data  start
                        let _splitFinalData = [];
                        ...
                        if (_getBerandSubstanceNames.data.length > 1) {
                            ...
                            if (!(_testResp && _testResp.success)) {
                                ...
                                let _findDuplicatesAndCombinedData_2 = await findDuplicatesAndCombinedData(JSON.parse(JSON.stringify(_UsingOrQueryData)), 'SRC_DRUG_CD', "INT_TYPE_ID");
                                _.each(_findDuplicatesAndCombinedData_2, (_o, _i) => {
                                    _output.push(_o)
                                })
                            }
                            ...
                            if (!(_testResp1 && _testResp1.success)) {
                                ...
                                if (_testResp1.data.length > 0) {
                                    ...
                                    while (_indx < _testResp1Data.length) {
                                        _compareFilter = _.filter(_comparisonData, (_co, _ci) => { return _testResp1.data[_indx].SRC_DRUG_CD == _co.SRC_DRUG_CD && _testResp1.data[_indx].CLASS_NAME_CD == _co.CLASS_NAME_CD })
                                        let _findDuplicatesAndCombinedData_3 = await findDuplicatesAndCombinedData(JSON.parse(JSON.stringify(_compareFilter)), 'SRC_BRAND_PRODUCT_CD', "INT_BRAND_PRODUCT_CD");
                                        let _indx1 = 0;
                                        while (_indx1 < _findDuplicatesAndCombinedData_3.length) {
                                            _manuaLfinalData.push({
                                                "SRC_DRUG_CD": _testResp1Data[_indx].SRC_DRUG_CD,
                                                "SRC_DRUG_NAME": _testResp1Data[_indx].SRC_DRUG_NAME,
                                                "INT_TYPE_ID": _testResp1Data[_indx].INT_TYPE_ID,
                                                ...
                                                "CLASS_NAME_CD": _testResp1Data[_indx].CLASS_NAME_CD,
                                                "CLASS_NAME": _testResp1Data[_indx].CLASS_NAME,
                                                "BRAND_PRODUCT_NAME": _findDuplicatesAndCombinedData_3[_indx1].BRAND_PRODUCT_NAME,
                                                "SRC_BRAND_PRODUCT_CD": _findDuplicatesAndCombinedData_3[_indx1].SRC_BRAND_PRODUCT_CD,
                                                "INT_BRAND_PRODUCT_CD": _findDuplicatesAndCombinedData_3[_indx1].INT_BRAND_PRODUCT_CD,
                                                "SRC_SUBSTANCE_NAME": _findDuplicatesAndCombinedData_3[_indx1].SRC_SUBSTANCE_NAME,
                                                "INT_SUBSTANCE_NAME": _findDuplicatesAndCombinedData_3[_indx1].INT_SUBSTANCE_NAME
                                            })
                                            _indx1++
                                        }
                                        _indx++;
                                        ...
                                        _output.push(
                                            {
                                                ...
                                                "CLASS_NAME": "",
                                                "BRAND_PRODUCT_NAME": _o.BRAND_NAME + "<-->" + _o1.ALLERGY_NAME,
                                                "INTERACTIONS": `<p>Drug allergy identified in response to the prescribed medication. The patient has a documented history of <strong>drug allergy to ${_o['DD_SUBSTANCE_NAME']}</strong>. The prescribed medication <strong>${_o.BRAND_NAME}</strong> includes <strong>${_o1['ALLERGY_NAME']}</strong>, Which belongs to the class of medications known as <strong>${_filterSubstanceAgnstClassData[0].CLASS_NAME}</strong></p>`
                                            }
                                        )
                                    } else {
                                                    ...
                                                    _output.push(
                                                        {
                                                            ...
                                                            "BRAND_PRODUCT_NAME": _o.BRAND_NAME + "<-->" + _o1.ALLERGY_NAME,
                                                            "INTERACTIONS": `<p>Drug allergy identified in response to the prescribed medication. The patient has a documented history of <strong>drug allergy to ${_o1['ALLERGY_NAME']}</strong>. The prescribed medication <strong>${_o.BRAND_NAME}</strong> includes <strong>${_o['DD_SUBSTANCE_NAME']}</strong>, Which belongs to the class of medications known as <strong>${_filDta.CLASS_NAME}</strong></p>`
                                                        }
                                                    )
                                                }
                                            })
                                    ...
                                    _.each(referenceIdAgnaistClassCd.data, (_secondClass, _secondIndx) => {
                                        if (_firstClass.CLASS_NAME_CD === _secondClass.CLASS_NAME_CD) {
                                            _output.push({
                                                ...
                                                "INTERACTIONS": `<p> Drug allergy identified in response to the prescribed medication. The patient has a documented history of <strong> drug allergy to ${_firstClass.DD_SUBSTANCE_NAME} </strong>. The prescribed medication <strong> ${_firstClass.BRAND_NAME} includes </strong> ${_secondClass['CLASS_NAME']}</strong>,which belongs to the class of medications known as <strong> ${_firstClass.CLASS_NAME} <strong></p>`
                                            })
                                        }
                                    })
                                })
...
async function BrandWithDifferentTypes(_keys, _indx, _output, req, _filterObj, _type, res, _brandData, _itemType, _classData, _classData1, _clneData) {
    let _comparisonData = []
    let _filter = {
        "filter": {
            "recStatus": { $eq: true }
    ...
    if (_type === "BRAND_WITH_CLASS") {
        ...
        if (_brandData.data.length > 1) {
            ...
            // else if(req.body.JSON[0].TYPE === ""){
            //     _.each(_clneData.data, (_o, _i) => {
            //         _.each(_brandData.data, (_o1, _i1) => {
            //             if (_o._id != _o1._id) {
            //                 _comparisonData.push(
            //                     {
            ...
            //                         "SRC_DRUG_CD": _o1.DD_SUBSTANCE_CD,
            //                         "INT_TYPE_ID": _o.DD_SUBSTANCE_CD,
            //                         "SRC_BRAND_PRODUCT_CD": _o1.BRAND_CD ? _o.BRAND_CD : "",
            //                         "INT_BRAND_PRODUCT_CD": _o.BRAND_CD ? _o1.BRAND_CD : "",
            //                         "BRAND_NAME_1": _o1.BRAND_NAME ? _o.BRAND_NAME : "",
            //                         "BRAND_NAME_2": _o.BRAND_NAME ? _o1.BRAND_NAME : "",
            //                         "SRC_SUBSTANCE_NAME": _o1.DD_SUBSTANCE_NAME,
            //                         "INT_SUBSTANCE_NAME": _o.DD_SUBSTANCE_NAME
            //                     }
            //                 )
...
router.post("/get-brand-data-with-recStatus", async (req, res) => {
    try {
        ...
        let _mResp = await _mUtils.commonMonogoCall("monography_brand", "aggregation", _filter, "", "", "", "")
        if (!(_mResp && _mResp.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
        }
...
router.post("/update-brand-data_using_xlsx", async (req, res) => {
    try {
        ...
        while (excelDta.Dataforchanges.length > _indx) {
            ...
            let _mRespData1 = await _mUtils.commonMonogoCall(`monography_brand`, "find", _filter, "", "", "", "")
            if (_mRespData1.data.length > 0) {
                let _indexJ = 0;
                while (_mRespData1.data.length > _indexJ) {
                    ...
                    let _uResp = await _mUtils.commonMonogoCall("monography_brand", 'bulkWrite', pLoadResp.payload, "", "", "", req.tokenData.dbType);
                    if (!(_uResp && _uResp.success && _uResp.data)) {
...
router.post("/update-brand-product-mapping-data", async (req, res) => {
    try {
        ...
        while (excelDta.BDNUpdationnew.length > _indx) {
            ...
            let _mRespData1 = await _mUtils.commonMonogoCall(`monography_brand_product_mapping`, "find", _filter, "", "", "", "")
            if (_mRespData1.data.length > 0) {
                let _indexJ = 0;
                while (_mRespData1.data.length > _indexJ) {
...
router.post("/update-drug-histories", async (req, res) => {
    try {
        ...
        let _mBrandProductResp = await _mUtils.commonMonogoCall("monography_substance", "aggregation", _filter, "", "", "", "")
        if (!(_mBrandProductResp && _mBrandProductResp.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _mBrandProductResp.desc || "", data: _mBrandProductResp.data || [] });
        } else {
            let i = 0;
            while (_mBrandProductResp.data.length > i) {
                ...
                let _uResp = await _mUtils.commonMonogoCall("monography_substance", 'bulkWrite', pLoadResp.payload, "", "", "", req.tokenData.dbType);
                if (!(_uResp && _uResp.success && _uResp.data)) {
                    return res.status(400).json({ status: 'FAIL', desc: _uResp.data });
                } else {
                    _filter.filter = []
                    ...
                    let _mBrandProductResp1 = await _mUtils.commonMonogoCall("monography_dd_substance_combination", "aggregation", _filter, "", "", "", "")
