...
let _approvalStatus = [
    ...
    { flag: 'CHC_APPROVED', status: ['Pending for State Approval', 'Approved', 'Reject at State Level', 'Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer', 'Ready To Dispatch', 'Dispatched to Patient', 'Rejected at PHC User' ] },
    { flag: 'CHC_REJECTED', status: ['Reject at CHC Level'] },
    { flag: 'CHC_ALL', status: ['Pending for CHC Approval', 'Pending for State Approval', 'Reject at CHC Level', 'Approved', 'Reject at State Level', 'Sent to Manufacturer', 'Received from Manufacturer','Reject at Manufacturer', 'Ready To Dispatch', 'Dispatched to Patient', 'Rejected at PHC User' ] },
    { flag: 'STATE_PENDING', status: ['Pending for State Approval'] },
    { flag: 'STATE_APPROVED', status: ['Approved', 'Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer', 'Ready To Dispatch', 'Dispatched to Patient', 'Rejected at PHC User' ] },
    ...
    { flag: 'READY_TO_DISPATCH', status: ['Ready to Dispatch'] },
    { flag: 'REJECTED_FROM_MANUFACTURER', status: ['Reject at Manufacturer'] },
    { flag: 'REJECTED_FROM_PHC', status: ['Rejected at PHC User'] },
    { flag: 'DISPATCHED_TO_PATIENT', status: ['Dispatched to Patient'] },

    { flag: 'CHC_SUMMARIZED', status: ['Pending for CHC Approval', 'Reject at CHC Level', 'Pending for State Approval', 'Approved', 'Reject at State Level', 'Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer', 'Ready To Dispatch', 'Dispatched to Patient', 'Rejected at PHC User'] },
    { flag: 'STATE_SUMMARIZED', status: ['Pending for State Approval', 'Approved', 'Reject at State Level', 'Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer', 'Ready To Dispatch', 'Dispatched to Patient', 'Rejected at PHC User'] },
    { flag: 'MANUFACTURER_SUMMARIZED', status: ['Approved', 'Sent to Manufacturer','Received from Manufacturer', 'Reject at Manufacturer', 'Ready To Dispatch', 'Dispatched to Patient', 'Rejected at PHC User'] },
    { flag: 'MANUFACTURER_DISPATCH_SUMMARIZED', status: ['Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer', 'Ready To Dispatch', 'Dispatched to Patient', 'Rejected at PHC User'] },
...
const loggingTransports = [
    new DailyRotateFile({
        ...
        filename: `${dirPath}public/logs/winston/ophtha/%DATE%.log`,
        maxFiles: '15d',
        maxsize: 10000000
    })
];
...
async function insertMedia_bkp(_type, coll, media, req, _data) {
    try {
        let _output = []
...
function logApiActivity(data) {
    ...
    const startTime = moment().format("DD-MMM-YYYY HH:mm");
    //const originalJson = res.json;

        try {
            let endTime = moment().format("DD-MMM-YYYY HH:mm");
            const logEntry = {
                data
            };
            ...
            return{ success:true, desc: "Logging Success", data: data } ;
        } catch (err) {
            return{ success:false, desc: "Logging failed", error: err }
        }
}
        ...
        if (req.query && req.query.orgKey) {
            let _filter = {
                ...
                "populate": [
                    ...
                    { 'path': 'favIcon', 'match': { referenceType: req.query.orgKey }, 'select': '' }
                ]
            }
            mongoMapper("ophthamology_ecg_Organization", "find", _filter, req.query.orgKey.toLowerCase()).then(async (result) => {
                let _parseData = JSON.parse(JSON.stringify(result.data))
                ...
                if (_parseData[0].orglogo && typeof _parseData[0].orglogo == "object" && Object.keys(_parseData[0].orglogo).length > 0 && _parseData[0].orglogo.orglogo != "" && _parseData[0].orglogo.orglogoMimeType && _parseData[0].orglogo.orglogoMimeType.length > 0) {
                    ...
                    _base64org = await orgbufferData.toString('base64');
                    _parseData[0]['orglogoMediaId'] = result.data[0].orglogo._id
                    _parseData[0]['orglogo'] = `${_parseData[0].orglogo.orglogoMimeType},${_base64org}`
                }
                else {
                    _parseData[0]['orglogo'] = ""
                }
                if (_parseData[0].favIcon && typeof _parseData[0].favIcon == "object" && Object.keys(_parseData[0].favIcon).length > 0 && _parseData[0].favIcon.favIcon != "" && _parseData[0].favIcon.favIconMimeType && _parseData[0].favIcon.favIconMimeType.length > 0) {
                    var favIconbufferData = Buffer.from(_parseData[0].favIcon.favIcon)
                    _base64favIcon = await favIconbufferData.toString('base64');
                    _parseData[0]['favIconMediaId'] = result.data[0].favIcon._id
                    _parseData[0]['favIcon'] = `${_parseData[0].favIcon.favIconMimeType},${_base64favIcon}`
                }
                else {
                    _parseData[0]['favIcon'] = ""
                }
...
router.use(async function verifyToken(req, res, next) {
    if (req.url === '/auth-user' || req.url === '/validate-user' || req.url === '/forgot-password' || req.url === '/update-password' || req.url === '/logout' || req.headers.exclude || req.headers.authtoken == 'BHAVYA') {
        next();
    }
    else {
        try {
...
router.use(function paramValidation(req, res, next) {
    try {
        ...
        if (req.headers.authtoken == 'BHAVYA') {
            req.tokenData = { "userId": 12345, "userName": "Bhavya", "displayName": "Bhavya", "dbType": "emr" };
            if(req.url === '/postPrescription'){
            ...
            req.body['query'] = req.url == '/postPrescription' ? 'insertMany' : 'find'
    ...
    try {
        ...
        if (req.body.params._id) {
            ...
            if ((req.body.params.orglogo && req.body.params.orglogo.length > 0) || (req.body.params.favIcon && req.body.params.favIcon.length > 0)) {
                ...
                if (_cBody.params.orglogoMediaId) {
                    _media["orglogoMediaId"] = _cBody.params.orglogoMediaId
                }
                if (req.body.params.favIcon) {
                    _media['favIcon'] = req.body.params.favIcon
                }
                if (_cBody.params.favIconMediaId) {
                    _media["favIconMediaId"] = _cBody.params.favIconMediaId
                }
...
router.post("/insert-employee", async (req, res) => {
    try {
        if (req.body && req.body.params && req.body.params.locations && req.body.params.locations.length > 0 && !req.body.params.locations[0]._id && req.body.params.contact && req.body.params.contact.mobile && req.body.params.contact.mobile.length > 0 && req.body.params.contact.email && req.body.params.contact.email.length > 0) {
            ...
            let _eBody = JSON.parse(JSON.stringify(req.body))
            if (_eBody.params.photo && _eBody.params.photo.length > 0) {
                delete _eBody.params['photo']
            }
            if (_eBody.params.signature && _eBody.params.signature.length > 0) {
                delete _eBody.params['signature']
            }
            if (typeof _eBody.params.photo === "string" && _eBody.params.photo.trim() === "") {
                delete _eBody.params['photo']
            }
            if (typeof _eBody.params.signature === "string" && _eBody.params.signature.trim() === "") {
                delete _eBody.params['signature']
            }
            mongoMapper('ophthamology_ecg_Employees', req.body.query, _eBody.params, req.tokenData.dbType).then(async (result) => {
                if (!(result && result.data && result.data.length > 0)) {
                    return res.status(400).json({ success: false, status: 400, desc: `Error occurred while insert Employee`, data: [] });
                }
                if (req.body.params.photo || req.body.params.signature) {
                    let _media = {};
                    ...
                    if (req.body.params.signature && req.body.params.signature.length > 0) {
                        _media["signature"] = req.body.params.signature
                    }

                    let _mediaResp = await insertMedia("I", "ophthamology_ecg_Employees", _media, req, result.data)
...
router.post("/get-employee", async (req, res) => {
    try {
        ...
        mongoMapper("ophthamology_ecg_Employees", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
            if (result.data && result.data.length > 0) {
                // _.each(result.data, (_data) => {
                //     if (_data.dispName && _data.dispName.length > 0) {
                //         _data['dispName'] = convertToPascalCase(_data.dispName)
                ...
                for (let resp of _finalResp) {
                    ...
                    if (resp.signature && resp.signature.signature && resp.signature.signature !== "" && resp.signature.signatureMimeType && resp.signature.signatureMimeType.length > 0) {
                        ...
                        let data = await getBase64Data(resp.signature.signatureMimeType, resp.signature.signature)
                        resp['signatureMediaId'] = resp.signature._id
                        resp['signature'] = data
                    }
                    else {
                        resp['signature'] = ""
    ...
    try {
        ...
        if (req.body.params._id) {
            ...
            if (!(_hResp && _hResp.success)) {
                ...
                if (_cBody.params.photo || _cBody.params.signature) {
                    let _media = {};
                    ...
                    if (_cBody.params.signature) {
                        _media["signature"] = _cBody.params.signature
                    }
                    if (_cBody.params.signatureMediaId) {
                        _media["signatureMediaId"] = _cBody.params.signatureMediaId
                    }
                    let _mediaResp = await insertMedia("U", "ophthamology_ecg_Employees", _media, req, _mResp.data);
                    if (!_mediaResp.success) {
                        return res.status(400).json({ success: false, status: 400, desc: _mediaResp.desc, data: _mediaResp.data || [] });
                    }
                    delete _cBody.params['photo'];
                    delete _cBody.params['signature']
                }
...
router.post("/insert-doctor", async (req, res) => {
    try {
        if (req.body && req.body.params && req.body.params.locations && req.body.params.locations.length > 0 && !req.body.params.locations[0]._id && req.body.params.contact && req.body.params.contact.mobile && req.body.params.contact.mobile.length > 0 && req.body.params.contact.email && req.body.params.contact.email.length > 0) {
            ...
            if (_cBody.photo && _cBody.photo.length > 0) {
                delete _cBody['photo']
            }
            if (_cBody.signature && _cBody.signature.length > 0) {
                delete _cBody['signature']
            }
            if (typeof _cBody.photo === "string" && _cBody.photo.trim() === "") {
                delete _cBody['photo']
            }
            if (typeof _cBody.signature === "string" && _cBody.signature.trim() === "") {
                delete _cBody['signature']
            }
            mongoMapper('ophthamology_ecg_Doctors', "insertMany", _cBody, req.tokenData.dbType).then(async (result) => {
                if (!(result && result.data && result.data.length > 0)) {
                ...
                if (req.body.params.photo || req.body.params.signature) {
                    ...
                    let _mediaResp = await insertMedia("I", "ophthamology_ecg_Doctors", _media, req, result.data)
    ...
    try {
        ...
        if (req.body.params._id) {
            ...
            if (_cBody.params.photo || _cBody.params.signature) {
                ...
                let _mediaResp = await insertMedia("U", "ophthamology_ecg_Doctors", _media, req, _mResp.data);
                if (!_mediaResp.success) {
                    return res.status(400).json({ success: false, status: 400, desc: _mediaResp.desc, data: _mediaResp.data || [] });
                }
                delete _cBody.params['photo'];
...
router.post("/insert-patient", async (req, res) => {
    try {
        ...
        if (_cBody.params.abhaJson && _cBody.params.abhaJson.length > 0) {
            ...
            let _mResp = await _mUtils.commonMonogoCall("ophthamology_ecg_AbhaJson", "insertMany", _cBody.params.abhaJson, "", "", "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            else {
        ...
        mongoMapper('ophthamology_ecg_Patients', req.body.query, _cBody.params, req.tokenData.dbType).then(async (result) => {
            req.body.params["patId"] = result.data[0]._id;
            req.body.params['reasonForVisit'] = "Ophthamology Prescription";
            ...
            if (req.body.params.photo || req.body.params.signature) {
                ...
                let _mediaResp = await insertMedia("I", "ophthamology_ecg_Patients", _media, req, result.data)
        ...
        if (req.body && req.body.params && req.body.params.flag && (req.body.params.flag === 'NAME' || req.body.params.flag == 'MOB' || req.body.params.flag == 'UMR' || req.body.params.flag == 'VSTID' || req.body.params.flag === 'VSTNO') && req.body.params.searchValue && req.body.params.searchValue.length > 2) {
            let _filter = {
                ...
                "populate": [
                    { 'path': 'gender', 'select': '_id code name' },
                    { 'path': 'title', 'select': '_id code name' },
                    // { 'path': 'photo', 'select': '_id code name' },
                    // { 'path': 'signature', 'select': '_id code name' },
                    // { 'path': 'abhaCard', 'select': '_id code name' },
                ]
            };
    ...
    try {
        ...
        let _filter = {
            ...
            "populate": [
                ...
                { 'path': 'prefferedLanquage', 'select': '_id code name' },//PreferLanguage
                ...
                //{ 'path': 'abhaJson', 'select': '' },//
            ]
        }

        if(req.body.params && req.body.params.flag && req.body.params.flag.length > 0){
            _.each(req.body.params.flag.split('|'), (item)=>{
                _filter.populate.push({ 'path': item, 'select': ''})
            })
            if(!(req.body.params.flag.includes("abhaJson")) && !_filter.selectors.includes("abhaJson")){
                _filter.selectors += ` -abhaJson`
            }
        }
    ...
    try {
        ...
        if (req.body.params._id) {
            ...
            if (!(_hResp && _hResp.success)) {
                ...
                _cBody.params.revNo = _mResp.data.params.revNo;
                if (_cBody.params.photo || _cBody.params.signature || _cBody.params.abhaCard) {
                    let _media = {};
                    if (_cBody.params.photo) {
                        _media["photo"] = _cBody.params.photo
                    }
                    ...
                    let _mediaResp = await insertMedia("U", "ophthamology_ecg_Patients", _media, req, _mResp.data);
                    if (!_mediaResp.success) {
                        return res.status(400).json({ success: false, status: 400, desc: _mediaResp.desc, data: _mediaResp.data || [] });
                    }
                    delete _cBody.params['photo'];
                ...
                if (_cBody.params.abhaJson && _cBody.params.abhaJson.length > 0) {
                    ...
                    let _aResp = await _mUtils.commonMonogoCall("ophthamology_ecg_AbhaJson", "bulkWrite", _abhapLoadResp.payload, "", "", "", req.tokenData.dbType)
                    if (!(_aResp && _aResp.success)) {
                        return res.status(400).json({ success: false, status: 400, desc: _aResp.desc || "", data: _aResp.data || [] });
                    }
                    else {
                        //_cBody.params.abhaJson = _aResp.data[0]._id
                        delete _cBody.params['abhaJson'];
                    }
                }
                pLoadResp = await _mUtils.preparePayload("BW", _cBody);
                if (!pLoadResp.success) {
    ...
    try {
        if (req.body.params && req.body.params.docId && req.body.params.locId) {
            let _filter = {
                ...
                "selectors": "-history",
                "populate": [
                    { 'path': 'patient', 'select': '_id dispName dateOfBirth', 'populate': [{ 'path': 'title', 'select': '' }, { 'path': 'gender', 'select': '_id code name' }] },
                    { 'path': 'doctor', 'select': '_id dispName' },
                    { 'path': 'visitTransaction', 'select': '', 'populate': [{ 'path': 'prescription', 'select': 'approvals' }] },
...
/**Get Appointment */
router.post("/get-dashboard-old", async (req, res) => {
    try {
...
router.post("/get-glass-prescriptions", async (req, res) => {
    try {
        if (req.body && req.body.params && req.body.params.flag) {
            ...
            let _params = {};
            if (_userLocData && _userLocData[0] && (_userLocData[0].role === "PHC User" || _userLocData[0].role === "CHC User" || _userLocData[0].role === "PHC Doctor" || _userLocData[0].role === "CHC Doctor")) {
            ...
            if (req.body.params.flag == "STATE_APPROVED" && _userLocData[0].role === "State Nodal Officer") {
                _status[0].status = ['Approved', 'Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer']
            }
            if (req.body.params.flag == "STATE_APPROVED" && _userLocData[0].role === "Manufacturer User") {
                _status[0].status = ['Approved']
            }
            ...
            if (req.body.params.fromDt && req.body.params.fromDt.length > 0) {
                ...
                let fromDt = req.body.params.fromDt.split('T')[0]
                let toDt = req.body.params.toDt && req.body.params.toDt.length > 0 ? req.body.params.toDt.split('T')[0] : fromDt
                if (req.tokenData.roleName.toLowerCase().split(" ").join("") == 'phcuser' && req.body.params.flag != "CHC_SUMMARIZED") {
                    _params['audit.documentedDt'] = { $gte: `${fromDt}T00:00:00.000Z`, $lte: `${toDt}T23:59:59.999Z` }
                }
                if (req.tokenData.roleName.toLowerCase().split(" ").join("") == 'phcuser' && req.body.params.flag == "CHC_SUMMARIZED") {
                    _params['approvals.manufacturer.approvedDateTime'] = { $gte: `${fromDt}T00:00:00.000Z`, $lte: `${toDt}T23:59:59.999Z` }
                }//|| req.tokenData.roleName.toLowerCase().split(" ").join("") == 'chcuser' || req.tokenData.roleName.toLowerCase().split(" ").join("") == 'chcdoctor'
                else if (req.tokenData.roleName.toLowerCase().split(" ").join("") == 'chcnodalofficer' || req.tokenData.roleName.toLowerCase().split(" ").join("") == 'chcuser' || req.tokenData.roleName.toLowerCase().split(" ").join("") == 'chcdoctor' ) {
                    _params['approvals.phc.approvedDateTime'] = { $gte: `${fromDt}T00:00:00.000Z`, $lte: `${toDt}T23:59:59.999Z` }
                }
                else if (req.tokenData.roleName.toLowerCase().split(" ").join("") == 'statenodalofficer') {
                    _params['approvals.chc.approvedDateTime'] = { $gte: `${fromDt}T00:00:00.000Z`, $lte: `${toDt}T23:59:59.999Z` }
                }
                else if (req.tokenData.roleName.toLowerCase().split(" ").join("") == 'manufactureruser' && req.body.params.flag != "MANUFACTURER_DISPATCH_SUMMARIZED") {
                    _params['approvals.state.approvedDateTime'] = { $gte: `${fromDt}T00:00:00.000Z`, $lte: `${toDt}T23:59:59.999Z` }
                }
                else if (req.tokenData.roleName.toLowerCase().split(" ").join("") == 'manufactureruser' && req.body.params.flag == "MANUFACTURER_DISPATCH_SUMMARIZED") {
                    _params['approvals.manufacturer.approvedDateTime'] = { $gte: `${fromDt}T00:00:00.000Z`, $lte: `${toDt}T23:59:59.999Z` }
                }
            ...
            if (req.body.params.glassType && req.body.params.glassType.length > 0) {
                _params['glassType'] = { $eq: req.body.params.glassType }
            }
            let _filter = {
                "filter": _params,
                "selectors": '-manufacturerActions',
                "populate": [
                    { 'path': 'patient', 'select': '_id UMR dispName dateOfBirth contact.mobile homeAddress abhaNo', 'populate': [{ 'path': 'gender', 'select': '' }, { 'path': 'title', 'select': '' }] },
                    {
                    ...
                    { 'path': 'approvals.phc.approvedBy', 'select': '' },
                    { 'path': 'approvals.phc.rejectedBy', 'select': '' },
                    { 'path': 'approvals.chc.approvedBy', 'select': '' },
                    { 'path': 'approvals.chc.rejectedBy', 'select': '' },
                    { 'path': 'approvals.state.approvedBy', 'select': '' },
                    { 'path': 'approvals.state.rejectedBy', 'select': '' },
                    { 'path': 'approvals.manufacturer.approvedBy', 'select': '' },
                    { 'path': 'approvals.manufacturer.rejectedBy', 'select': '' },
                ]
            }
            ...
            mongoMapper('ophthamology_ecg_GlassPrescriptions', "find", _pGData.data, req.tokenData.dbType).then((result) => {
                ...
                let _approvalKey1 = ""
                let _approvalKey2 = ""
                let _role = req.tokenData.roleName.toLowerCase().split(" ").join("")
                //_approvalKey = _role == "phcuser" || _role == "phcdoctor" ? "phc" : _role == "chcuser" || _role == "chcdoctor" || _role == "chcnodalofficer" ? "chc" : _role == "statenodalofficer" ? "state" : ""
                if (_role == "phcuser" || _role == "phcdoctor") {
                    _approvalKey = "phc"
                    _dateTime = "receivals"
                }
                else if (_role == "chcuser" || _role == "chcdoctor" || _role == "chcnodalofficer") {
                    _approvalKey = "chc"
                    _dateTime = "phc"
                }
                else if (_role == "statenodalofficer") {
                    _approvalKey = "state"
                    _dateTime = "chc"
                }
                else if (_role == "manufactureruser" && req.body.params.flag != "MANUFACTURER_DISPATCH_SUMMARIZED") {
                    _approvalKey = "manufacturer"
                    _dateTime = "state"
                }
                else if (_role == "manufactureruser" && req.body.params.flag == "MANUFACTURER_DISPATCH_SUMMARIZED") {
                    _approvalKey1 = "receivals"
                    _approvalKey2 = "dispatch"
                    _dateTime = "manufacturer"
                }
                let _finalData = JSON.parse(JSON.stringify(result.data))
                let _totalData = [
                    {
                        "overview": {
                            ...
                            "singleNearVisionGlasses": 0,
                            "prescriptionGlasses": 0,
                            "planoBioFocalGlasses": 0,
                            "pendingApproval": 0,
                ...
                // if(req.body.params.flag.split("_")[1] == "SUMMARIZED" && req.body.params.flag != "MANUFACTURER_SUMMARIZED"){
                //     let _groupedData = _.groupBy(_finalData, (_dt)=> 
                //         _dt.prescribedLocation.locName 
                //     )
                //     //console.log(_groupedData)
                //     _totalData[0].prescriptions = _.map(_groupedData, (_items, locName)=>{
                //         let facilityType = _items[0].prescribedLocation.locType;
                //         let prescriptionsAdded = _items.length
                //         let pending = _.filter(_items, (_item)=> _item.approvals[_approvalKey].approved == false && _item.approvals[_approvalKey].rejected == false)
                //         let approved = _.filter(_items, (_item)=> _item.approvals[_approvalKey].approved == true && _item.approvals[_approvalKey].rejected == false)
                //         let rejected = _.filter(_items, (_item)=> _item.approvals[_approvalKey].rejected == true && _item.approvals[_approvalKey].approved == false)
                //         let singleNearVisionGlasses = _.filter(_items, (_item)=> _item.glassType == "SINGLENEARVISIONGLASSES")
                //         let prescriptionGlasses = _.filter(_items, (_item)=> _item.glassType == "PRESCRIPTIONGLASSES")
                //         let planoBioFocalGlasses = _.filter(_items, (_item)=> _item.glassType == "PLANOBIFOCAL")

                //         _totalData[0].overview["totalPrescriptions"] += prescriptionsAdded,
                //         _totalData[0].overview["pendingApproval"] += pending.length,
                //         _totalData[0].overview["approvedPrescriptions"] += approved.length,
                //         _totalData[0].overview["rejectedPrescriptions"] += rejected.length
                ...
                //             singleNearVisionGlasses: singleNearVisionGlasses.length,
                //             prescriptionGlasses: prescriptionGlasses.length,
                //             planoBioFocalGlasses: planoBioFocalGlasses.length,
                //             totalPrescriptions:(singleNearVisionGlasses.length+prescriptionGlasses.length+planoBioFocalGlasses.length),
                //             prescriptionsApproved: approved.length,
                //             prescriptionsRejected: rejected.length,
                //             data: {
                //                 all: _items
                //             }
                //         }
                ...
                // }
                if (req.body.params.flag.split("_")[1] == "SUMMARIZED" && req.body.params.flag != "MANUFACTURER_SUMMARIZED") {
                    if (req.body.params.facilityName && req.body.params.facilityName == true) {
                        let glassTypes = ["SINGLENEARVISIONGLASSES", "PLANOBIFOCAL", "PRESCRIPTIONGLASSES"]
                        let prescriptionTypes = req.body.params.prescriptionType && req.body.params.prescriptionType.length > 0 && req.body.params.prescriptionType != "ALL" ? [`${req.body.params.prescriptionType}`] : req.body.params.prescriptionType && req.body.params.prescriptionType.length > 0 && req.body.params.prescriptionType && req.body.params.prescriptionType == "ALL" ? glassTypes : glassTypes

                        if(req.body.params.facilityId && req.body.params.facilityId.length > 0){
                            _finalData = _.filter(_finalData, (_rec)=> { return _rec.prescribedLocation._id == req.body.params.facilityId })
                        }

                        let facilities = _(_finalData).groupBy(item => `${item.prescribedLocation.locName}-${item.prescribedLocation._id}`)
                            .map((records, key) => {

                                let [ facilityName, facilityId ] = key.split("-")
                                let pendingData = []
                                let _groupGlassType = _.groupBy(records, 'glassType');
                                let _glassTypeData = {}
                                prescriptionTypes.forEach(glassType => {

                                    let items = records.filter(_item => _item.glassType == glassType)

                                    let totalData = items.length
                                    let pending = _.filter(items, (_item) => _item.approvals[_approvalKey].approved == false && _item.approvals[_approvalKey].rejected == false)
                                    let approvedData = _.filter(items, (_item) => _item.approvals[_approvalKey].approved == true && _item.approvals[_approvalKey].rejected == false)
                                    let rejectedData = _.filter(items, (_item) => _item.approvals[_approvalKey].rejected == true && _item.approvals[_approvalKey].approved == false)

                                    let pendingDataCount = pending.length
                                    let approvedDataCount = approvedData.length
                                    let rejectedDataCount = rejectedData.length
                                    if(req.body.params.pendingData && req.body.params.pendingData == true){
                                        // pendingData.push(pending)
                                        pendingData.push(items)
                                    }
                                    //pendingData = req.body.params.pendingData && req.body.params.pendingData == true ? [...pending] : pendingData

                                    _glassTypeData[glassType] = {  totalData, pendingDataCount, approvedDataCount, rejectedDataCount }
                                })
                                if(pendingData && pendingData.length > 0){
                                    pendingData = _.flatten(_.compact(pendingData))
                                }
                                return { facilityName, facilityId, pendingData, ..._glassTypeData }
                            }).value()
                        console.log(facilities)
                        _totalData = facilities
                    }
                    else {
                        let glassTypes = ["SINGLENEARVISIONGLASSES", "PLANOBIFOCAL", "PRESCRIPTIONGLASSES"]
                        let prescriptionTypes = req.body.params.prescriptionType && req.body.params.prescriptionType.length > 0 && req.body.params.prescriptionType != "ALL" ? [`${req.body.params.prescriptionType}`] : req.body.params.prescriptionType && req.body.params.prescriptionType.length > 0 && req.body.params.prescriptionType && req.body.params.prescriptionType == "ALL" ? glassTypes : glassTypes

                        let _dateFilterData = _.uniqBy(_finalData.map(_record => _record.approvals[_dateTime].approvedDateTime.split("T")[0]));

                        let _result = _dateFilterData.flatMap(dateObj => {
                            return prescriptionTypes.map(type => {
                                let _totalData = _.filter(_finalData, (record) => { return record.approvals[_dateTime].approvedDateTime.split("T")[0] == dateObj && record.glassType == type })

                                let pending = _.filter(_totalData, (_item) => _item.approvals[_approvalKey].approved == false && _item.approvals[_approvalKey].rejected == false)
                                let approved = _.filter(_totalData, (_item) => _item.approvals[_approvalKey].approved == true && _item.approvals[_approvalKey].rejected == false)
                                let rejected = _.filter(_totalData, (_item) => _item.approvals[_approvalKey].rejected == true && _item.approvals[_approvalKey].approved == false)
                                if(_totalData.length > 0){
                                return {
                                    date: new Date(dateObj), //new Date(dateObj),
                                    prescriptionType: type,
                                    totalCount: _totalData.length,
                                    pendingCount: pending.length,
                                    approvedCount: approved.length,
                                    rejectedCount: rejected.length,
                                    // pendingData: req.body.params.pendingData && req.body.params.pendingData == true ? pending : []
                                    pendingData: req.body.params.pendingData && req.body.params.pendingData == true ? _totalData : []
                                }
                             }
                            }).filter(Boolean)
                        })
                ...
                else if (req.body.params.flag == "MANUFACTURER_SUMMARIZED") {
                    //let result = {}
                    let glassTypes = ["SINGLENEARVISIONGLASSES", "PLANOBIFOCAL", "PRESCRIPTIONGLASSES"]
                    let prescriptionTypes = req.body.params.prescriptionType && req.body.params.prescriptionType.length > 0 && req.body.params.prescriptionType != "ALL" ? [`${req.body.params.prescriptionType}`] : req.body.params.prescriptionType && req.body.params.prescriptionType.length > 0 && req.body.params.prescriptionType && req.body.params.prescriptionType == "ALL" ? glassTypes : glassTypes
                    ...
                    let _result = _dateFilterData.flatMap(dateObj => {
                        return prescriptionTypes.map(type => {
                            ...
                            //let DispatchPending = _.filter(_totalData, (_item) =>  _item.status == "Sent to Manufacturer")
                            let DispatchPending = _.filter(_totalData, (_item) => _item.status == "Approved" || _item.status == "Sent to Manufacturer")

                            let pending = _.filter(_totalData, (_item) => _item.approvals[_approvalKey].approved == false && _item.approvals[_approvalKey].rejected == false)
                            let approved = _.filter(_totalData, (_item) => _item.approvals[_approvalKey].approved == true && _item.approvals[_approvalKey].rejected == false)
                            let rejected = _.filter(_totalData, (_item) => _item.approvals[_approvalKey].rejected == true && _item.approvals[_approvalKey].approved == false)

                            let _batchNumber = _totalData && _totalData.length > 0 ? _totalData[0].batchNumber : ""
                            if(_totalData.length > 0) {
                                for(let _data of _totalData){
                                    if(_data.status && _data.status == 'Approved'){
                                        _data['viewingStatus'] = 'Need To Approve'
                                    }
                                    else if(_data.status && _data.status == 'Sent to Manufacturer'){
                                        _data['viewingStatus'] = 'Approved'
                                    }
                                    else if(_data.status && _data.status == 'Received from Manufacturer' || _data.status && _data.status == 'Ready To Dispatch' || _data.status && _data.status == 'Dispatched to Patient' || _data.status && _data.status == 'Rejected at PHC User'){
                                        _data['viewingStatus'] = 'Dispatched'
                                    }
                                    else if(_data.status && _data.status == 'Reject at Manufacturer'){
                                        _data['viewingStatus'] = 'Reject at Manufacturer'
                                    }
                                }
                            return {
                                date: new Date(dateObj), //dateObj,
                                prescriptionType: type,
                                totalCount: _totalData.length,
                ...
                else if (req.body.params.flag == "MANUFACTURER_DISPATCH_SUMMARIZED") {
                    let glassTypes = ["SINGLENEARVISIONGLASSES", "PLANOBIFOCAL", "PRESCRIPTIONGLASSES"]
                    let prescriptionTypes = req.body.params.prescriptionType && req.body.params.prescriptionType.length > 0 && req.body.params.prescriptionType != "ALL" ? [`${req.body.params.prescriptionType}`] : req.body.params.prescriptionType && req.body.params.prescriptionType.length > 0 && req.body.params.prescriptionType && req.body.params.prescriptionType == "ALL" ? glassTypes : glassTypes
                    ...
                    let _result = _dateFilterData.flatMap(dateObj => {
                        return prescriptionTypes.map(type => {
                            ...
                            let total = _.filter(_totalData, (_item) => _item.status == "Received from Manufacturer" || _item.status == "Rejected at PHC User" || _item.status == "Ready To Dispatch" || _item.status == "Dispatched to Patient")  //  || _item.status == "Sent to Manufacturer"

                            let _batchNumber = _totalData && _totalData.length > 0 ? _totalData[0].batchNumber : ""

                            // let userPending = _.filter(_totalData, (_item) => _item.approvals[_approvalKey1].approved == false && _item.approvals[_approvalKey1].rejected == false && _item.status == "Received from Manufacturer")
                            // let userApproved = _.filter(_totalData, (_item) => _item.approvals[_approvalKey1].approved == true && _item.approvals[_approvalKey1].rejected == false && _item.status == "Ready to Dispatch")
                            // let userRejected = _.filter(_totalData, (_item) => _item.approvals[_approvalKey1].rejected == true && _item.approvals[_approvalKey1].approved == false && _item.status == "Rejected at PHC User")

                            // let pendingForDispatch = _.filter(_totalData, (_item) => _item.approvals[_approvalKey2].approved == false && _item.approvals[_approvalKey2].rejected == false && _item.status == "Ready to Dispatch")
                            // let dispatched = _.filter(_totalData, (_item) => _item.approvals[_approvalKey2].approved == true && _item.approvals[_approvalKey2].rejected == false && _item.status == "Dispatched to Patient")

                            return {
                                date: new Date(dateObj), //new Date(dateObj),
                                prescriptionType: type,
                                dispatchedCount: total.length,
                                batchNumber: _batchNumber,
                                dispatchedData: req.body.params.dispatched && req.body.params.dispatched == true ? total : []
                                //dispatchedData: req.body.params.dispatchedData && req.body.params.dispatchedData == true ? total : []
                                // totalCount: _totalData.length,
                                // userPendingCount: userPending.length,
                                // userApprovedCount: userApproved.length,
                                // userRejectedCount: userRejected.length,
                                // pendingForDispatchCount: pendingForDispatch.length,
                                // dispatchedCount: dispatched.length,
                                // totalData: req.body.params.totalData && req.body.params.totalData == true ? _totalData : []
                            }
                        })
                    })
                ...
                else if (req.body.params.flag == "DETAILED") {
                    _totalData = result.data
                }
                else {
                    _totalData = result.data
                }
...
router.post("/get-dispatched-photo", async (req, res) => {
    try {
        let _filter = {
            "filter": { "recStatus": { $eq: true } },
            "selectors": "-history",
        ...
        mongoMapper("ophthamology_ecg_GlassPrescriptions", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {

            if(result.data.length > 0){
                let _finalResp = JSON.parse(JSON.stringify(result.data))
                for (let resp of _finalResp) {
                    if (resp.dispatchIdentity && resp.dispatchIdentity.dispatchIdentity && resp.dispatchIdentity.dispatchIdentity !== "" && resp.dispatchIdentity.dispatchIdentityMimeType && resp.dispatchIdentity.dispatchIdentityMimeType.length > 0) {
                        let data = await getBase64Data(resp.dispatchIdentity.dispatchIdentityMimeType, resp.dispatchIdentity.dispatchIdentity)
                        resp['dispatchIdentityMediaId'] = resp.dispatchIdentity._id
                        resp['dispatchIdentity'] = data
                    }
                    else {
                        resp['dispatchIdentity'] = ""
                    }
                }
                    ...
                    if (_userResp && _userLocDataResp) {
                        ...
                        if (_data[_idx].approvals && typeof _data[_idx].approvals == "object" && Object.keys(_data[_idx].approvals).length > 0) {
                            _.each(_data[_idx].approvals, (_aVal, _aKey) => {
                                if (_aKey !== "_id") {
                                    if ((_aKey == "chc" && req.tokenData.roleName.toLowerCase().split(" ").join("") == "chcnodalofficer") || (_aKey == "state" && req.tokenData.roleName.toLowerCase().split(" ").join("") == "statenodalofficer") || (_aKey == "manufacturer" && req.tokenData.roleName.toLowerCase().split(" ").join("") == "manufactureruser")) {
                                        if (_aKey != "_id" && typeof _aVal == "object") {
                                            _aVal['location'] = req.tokenData.locId;
                                            _aVal['requestReceivedDateTime'] = new Date().toISOString();
                                            if (_aVal.approved) {
                ...
                if (_data[_idx]._id) {
                    ...
                    if (req.tokenData.roleName.toLowerCase().split(" ").join("") == "chcnodalofficer") {
                        ...
                        if (!(_uResp && _uResp.success && _uResp.data && _uResp.data.length > 0)) {
                            ...
                            let _userLocData = JSON.parse(JSON.stringify(_uResp.data[0].linkedFacilities)).filter(_f => _f.locId === _data[_idx].prescribedLocation);
                            if (_userLocData.length == 0) {
                                _userLocDataResp = false;
                            }
                            if (_data[_idx].prescribedLocation == req.tokenData.locId) {
                                _userLocDataResp = true;
                            }
                        }
                    ...
                    if (_userResp && _userLocDataResp) {
                        ...
                        if (_data[_idx].approvals && typeof _data[_idx].approvals == "object" && Object.keys(_data[_idx].approvals).length > 0) {
                            _.each(_data[_idx].approvals, (_aVal, _aKey) => {
                                if (_aKey !== "_id") {
                                    if ((_aKey == "chc" && req.tokenData.roleName.toLowerCase().split(" ").join("") == "chcnodalofficer") || (_aKey == "state" && req.tokenData.roleName.toLowerCase().split(" ").join("") == "statenodalofficer") || (_aKey == "manufacturer" && req.tokenData.roleName.toLowerCase().split(" ").join("") == "manufactureruser") || (_aKey == "receivals" && (req.tokenData.roleName.toLowerCase().split(" ").join("") == "phcuser" || req.tokenData.roleName.toLowerCase().split(" ").join("") == "chcuser")) || (_aKey == "dispatch" && (req.tokenData.roleName.toLowerCase().split(" ").join("") == "phcuser" || req.tokenData.roleName.toLowerCase().split(" ").join("") == "chcuser"))) {
                                        if (_aKey != "_id" && typeof _aVal == "object") {
                                            _aVal['location'] = req.tokenData.locId;
                                            _aVal['requestReceivedDateTime'] = new Date().toISOString();
                                            if (_aVal.approved) {
                                                ...
                                                else if ((_aKey == "receivals" && (req.tokenData.roleName.toLowerCase().split(" ").join("") == "phcuser" || req.tokenData.roleName.toLowerCase().split(" ").join("") == "chcuser"))) {
                                                    _status = "Ready to Dispatch";
                                                }
                                                else if ((_aKey == "dispatch" && (req.tokenData.roleName.toLowerCase().split(" ").join("") == "phcuser" || req.tokenData.roleName.toLowerCase().split(" ").join("") == "chcuser"))) {
                                                    _status = "Dispatched to Patient";
                                                }
                                                // _status = req.tokenData.roleName.toLowerCase().split(" ").join("") == "statenodalofficer" && _aKey == "state" ? "Approved" : req.tokenData.roleName.toLowerCase().split(" ").join("") == "statenodalofficer" ? "Pending for State Approval" : "Pending for CHC Approval"
                                                _aVal['approvedDateTime'] = new Date().toISOString();
                                                _aVal['approvedBy'] = req.tokenData.userId;
                        ...
                        if (_role == true) {
                            _data[_idx]['status'] = _status;
                            if (req.tokenData.roleName.toLowerCase().split(" ").join("") == "statenodalofficer") {
                                _data[_idx]['batchNumber'] = _batchNo;
                            }
                            let _updtData = JSON.parse(JSON.stringify(_data[_idx]))
                            if (_updtData.hasOwnProperty('dispatchIdentity')) {
                                delete _updtData['dispatchIdentity']
                            }
                            let _updtprms = {
                                "params": _updtData
                            }
...
async function updateGlassPrescriptionsV1(data, _flag, req, _output, _idx) {
    try {
        const roleName = req.tokenData.roleName;
        let approveStatus, rejectStatus, approvalKey;

        if (roleName === "CHC Nodal Officer") {
            approveStatus = "Pending for State Approval";
            rejectStatus = "Reject at CHC Level";
            approvalKey = "chc"
        }
        else if (roleName === "State Nodal Officer") {
            approveStatus = "Approved";
            rejectStatus = "Reject at State Level";
            approvalKey = "state"
        }

        for (const prescription of data) {
            let approvalFieldId = prescription.approvals[approvalKey]._id;
            console.log("approvalFieldId", approvalFieldId)
            let status, approvalField

            //if (_flag == "APPROVE_ALL" || "APPROVE") {
            if ((_flag == "APPROVE_ALL") || (_flag == "APPROVE")) {
                status = approveStatus,
                    approvalField = {
                        _id: approvalFieldId,
                        approved: true,
                        location: mongoose.Types.ObjectId(req.tokenData.locId),
                        requestReceivedDateTime: new Date().toISOString(),
                        approvedDateTime: new Date().toISOString(),
                        approvedBy: req.tokenData.userId
                    }
            }
            //else if (_flag == "REJECT_ALL" || "REJECT") {
              else if ((_flag == "REJECT_ALL") || (_flag == "REJECT")) {
                status = rejectStatus,
                    approvalField = {
                        _id: approvalFieldId,
                        rejected: true,
                        ...
                        rejectedBy: req.tokenData.userId
                    }
            };
            ...
            if (roleName === "CHC Nodal Officer") {
                updatedData = {
                    params: {
                        ...
                        "status": status,
                        "approvals.chc": approvalField
                    }
                };
            }
            if (roleName === "State Nodal Officer") {
                let _batchNo = `${req.tokenData.orgKey.toUpperCase()}${new Date().toISOString().split("T")[0].split("-").join("")}`
                updatedData = {
                    params: {
                        _id: prescription._id,
                        "status": status,
                        "approvals.state": approvalField,
                        "batchNumber": _batchNo
                    }
                };
            }
            let pLoadResp = await _mUtils.preparePayload('BW', updatedData);
            if (!pLoadResp.success) {
                _output.push({ success: false, status: 500, desc: pLoadResp.desc || "", data: [] });
            }
            let _mResp = await _mUtils.commonMonogoCall("ophthamology_ecg_GlassPrescriptions", "bulkWrite", pLoadResp.payload, "", "", "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success && _mResp.data)) {
                _output.push({ success: false, status: 400, desc: _mResp.desc || "error while updating", data: [] });
            } else {
                _output.push({ success: true, status: 200, desc: '', data: _mResp.data });
            }
        }
        let _final = _.filter(_output, (_r) => { return !_r.success });
        return {
        ...
        return { success: false, status: 500, desc: error.message || error };
    }
}
...
router.post("/update-glass-prescriptions", async (req, res) => {
    try {
        ...
        if (req.body.flag == "UPLOAD_PHOTO" && _updtResp.success && _cBody.params[0].dispatchIdentity && _cBody.params[0].dispatchIdentity.length > 0) {
            let _media = {
                "dispatchIdentity": _cBody.params[0].dispatchIdentity
            }

            let _mediaResp = await insertMedia("I", "ophthamology_ecg_GlassPrescriptions", _media, req, _cBody.params)

            if (!_mediaResp.success) {
                return res.status(400).json({ success: false, status: 400, desc: "Error While Inserting Media", data: _mediaResp.data || [] });
            }
...
router.post("/update-glass-prescriptions-v1", async (req, res) => {
    try {
        if (req.body && req.body.params && req.body.params.flag && (req.body.flag == "FACILITY_WISE" ||  req.body.flag == "DATE_WISE")) {
            let fromDt = req.body.params.fromDt ? req.body.params.fromDt.split('T')[0] : ""
            let toDt = req.body.params.toDt ? req.body.params.toDt.split("T")[0] : fromDt
            let _preFilter;
            let mappedlocations = [];
            let _params = {};
            if ((req.body.params._id).length == 0 && req.body.flag == "FACILITY_WISE" && (req.body.params.flag == "APPROVE_ALL" || req.body.params.flag == "REJECT_ALL")) {
                req.body.flag = "DATE_WISE"
            }
            //if (!((req.body.params._id).length > 0 && req.body.flag == "FACILITY_WISE")) {
            if (((req.body.params._id).length == 0 && req.body.flag == "DATE_WISE")) {
                let _userFilter = {
                    "filter": {
                        "recStatus": true,
                        "_id": req.tokenData.userId
                ...
                if (_userLocData && _userLocData[0] && (_userLocData[0].role === "State Nodal Officer")) {
                    let _locFilter = {
                        "filter": {
                            "recStatus": true,
                            "linkedFacilities": { $ne: [] }
                    ...
                    let _locResp = await _mUtils.commonMonogoCall("ophthamology_ecg_Locations", "find", _locFilter, "", "", "", req.tokenData.dbType)
                    if (!(_locResp && _locResp.success && _locResp.data && _locResp.data.length > 0)) {
                        return res.status(400).json({ success: false, status: 400, desc: _locResp.desc || "No locations found for state login", data: [] });
                    }
                    _locResp.data.forEach(location => {
                        if (location.linkedFacilities && Array.isArray(location.linkedFacilities)) {
                            location.linkedFacilities.forEach(facility => {
                                if (facility.locId) {
                                    mappedlocations.push(facility.locId)
                                }
                            })
                        }
                    });
                }
                if (_userLocData && _userLocData[0] && (_userLocData[0].role === "CHC Nodal Officer")) {
                    const flag = req.body.params.flag
                    let _locFilter = {
                        "filter": {
                            "recStatus": true,
                            "_id": req.tokenData.locId
                    ...
                    let _mResp = await _mUtils.commonMonogoCall("ophthamology_ecg_Locations", "find", _locFilter, "", "", "", req.tokenData.dbType)
                    if (!(_mResp && _mResp.success && _mResp.data && _mResp.data.length > 0)) {
                        return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "No locations found for CHC login", data: [] });
                    }

                    let _locations = JSON.parse(JSON.stringify(_mResp.data[0].linkedFacilities));
                    let _mappedLoc = _locations.map(_l => _l.locId);
                    _mappedLoc.push(req.tokenData.locId);
                    _params["prescribedLocation"] = { _mappedLoc };

                }
            }
            let mappedLocIDs = mappedlocations.map(id => mongoose.Types.ObjectId(id));
            if (req.tokenData.roleName === "CHC Nodal Officer") {
                _preFilter = {
                    "filter": {
                        "recStatus": true,
                        "prescribedLocation": req.body.flag == "FACILITY_WISE" ? mongoose.Types.ObjectId(req.body.params._id) : { $in: _params.prescribedLocation._mappedLoc.map(id => mongoose.Types.ObjectId(id)) },
                        "status": "Pending for CHC Approval"
                    },
                    "selectors": ""
                }
                 if (req.body.params.fromDt) {
                _preFilter.filter["audit.documentedDt"] = {
                    $gte: `${fromDt}T00:00:00Z`,
                    $lte: `${toDt}T23:59:59Z`
                   }
                }
            }
            else if (req.tokenData.roleName === "State Nodal Officer") {
                _preFilter = {
                    "filter": {
                        "recStatus": true,
                        "prescribedLocation": req.body.flag == "FACILITY_WISE" ? mongoose.Types.ObjectId(req.body.params._id) : { $in: mappedLocIDs },
                        "status": "Pending for State Approval"
                    },
                    "selectors": "status prescribedLocation approvals"
                }
                if (req.body.params.fromDt) {
                    _preFilter.filter["approvals.chc.approvedDateTime"] = {
                        $gte: `${fromDt}T00:00:00Z`,
                        $lte: `${toDt}T23:59:59Z`
                    }
                }
            ...
            if (req.body.params.glassType) {
                _preFilter.filter["glassType"] = req.body.params.glassType
            }
            let _preResp = await _mUtils.commonMonogoCall("ophthamology_ecg_GlassPrescriptions", "find", _preFilter, "", "", "", req.tokenData.dbType)
            if (!(_preResp && _preResp.success && _preResp.data && _preResp.data.length > 0)) {
                return res.status(400).json({ success: false, status: 400, desc: _preResp.desc || "No pending records found", data: [] });
            }
            else {
                let updateGlassPresResult = await updateGlassPrescriptionsV1(_preResp.data, req.body.params.flag, req, [], 0)
                if (updateGlassPresResult.success) {
                    return res.status(200).json({ success: true, status: 200, desc: '', data: updateGlassPresResult.data });
                }
                else {
                    return res.status(400).json({ success: false, status: 400, desc: updateGlassPresResult.data[0].desc || error, data: [] });
                }
            }
        }
        else {
...
router.post("/get-manufacturer-view", async (req, res) => {
    try {
        if (req.body && req.body.params) {
            let date = new Date()
            const formattedDate = date.toISOString().split('T')[0];
            let fromDt = req.body.params.fromDt ? req.body.params.fromDt.split('T')[0] : formattedDate
            let toDt = req.body.params.toDt ? req.body.params.toDt.split("T")[0] : fromDt

            _preFilter = {
                "filter": {
                    "recStatus": true,
                    "approvals.state.approvedDateTime": {
                        $gte: `${fromDt}T00:00:00Z`,
                        $lte: `${toDt}T23:59:59Z`
                    },
                    "status": { $in: ['Approved', 'Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer', 'Ready To Dispatch', 'Dispatched to Patient', 'Rejected at PHC User'] }
                },
                "selectors": '-manufacturerActions',
                "populate": [
                    { 'path': 'patient', 'select': '_id UMR dispName dateOfBirth contact.mobile homeAddress abhaNo', 'populate': [{ 'path': 'gender', 'select': '' }, { 'path': 'title', 'select': '' }] },
            ...
            let _preResp = await _mUtils.commonMonogoCall("ophthamology_ecg_GlassPrescriptions", "find", _preFilter, "", "", "", req.tokenData.dbType)
            if (!(_preResp && _preResp.success && _preResp.data && _preResp.data.length > 0)) {
                return res.status(400).json({ success: false, status: 400, desc: _preResp.desc || "No records found", data: [] });
            }
            let _finalData = JSON.parse(JSON.stringify(_preResp.data))
            if(_finalData && _finalData.length > 0){
        ...
        if (req.body && req.body.params && req.body.params.patient) {
            if (req.body.params.prescription && typeof req.body.params.prescription == "object" && Object.keys(req.body.params.prescription > 0)) {
                if (req.body.params.prescription.consultation && req.body.params.prescription.consultation.length > 0 && req.body.params.prescription.prescribedLocation && req.body.params.prescription.prescribedLocation.length > 0) {
                    ...
                    let _seqResp = await _mUtils.getSequenceNextValue({ orgId: req.tokenData.orgId, seqName: 'OrderId' }, "ophthamology_ecg", req);
                    if (!(_seqResp && _seqResp.success)) {
                        return res.status(400).json({ success: false, status: 400, desc: _seqResp.desc || "", data: _seqResp.data || [] });
                    }
                    const _locFilter = {
                        "filter": { "_id": mongoose.Types.ObjectId(req.body.params.prescription.prescribedLocation), "recStatus": true },
                        "selectors": "facilityNum"
                    };

                    let _locResp = await _mUtils.commonMonogoCall("ophthamology_ecg_Locations", "find", _locFilter, "", "", "", req.tokenData.dbType)
                    if (!(_locResp && _locResp.success && _locResp.data && _locResp.data.length > 0)) {
                        return res.status(400).json({ success: false, status: 400, desc: _locResp.desc || "location miss matched", data: [] });
                    }
                    let orderId = `${formattedDate}-${_locResp.data[0].facilityNum}-${_seqResp.data}`
                    req.body.params.prescription['orderId'] = orderId
                    //orderId chnages
                    req.body.params.prescription['approvals'] = {}
                    req.body.params.prescription['approvals']['phc'] = {}
                    req.body.params.prescription['approvals']['phc']['approved'] = true
                    req.body.params.prescription['approvals']['phc']['location'] = req.body.params.prescription.prescribedLocation
                    req.body.params.prescription['approvals']['phc']['approvedDateTime'] = new Date().toISOString()
                    req.body.params.prescription['approvals']['phc']['approvedBy'] = req.tokenData.userId;
                    req.body.params.prescription['approvals']['phc']['requestReceivedDateTime'] = new Date().toISOString();
    ...
    try {
        if (req.body && req.body.params) {
            let _filter = {
                ...
                "populate": [
                    {
                        'path': 'patient', 'select': '-abhaJson -photo -signature -abhaCard',
                        "populate": [
                            { 'path': 'gender', 'select': '_id code name' },
                            { 'path': 'title', 'select': '_id code name' },
                            { 'path': 'bloodGroup', 'select': '_id code name' },
                    ...
                    { 'path': 'prescription', 'select': '-patient', 'populate': [{ 'path': 'consultation', 'select': '' }] },
                ]
            }
    ...
    try {
        if (req.body && req.body.params && req.body.params.length > 0) {
            ...
            for (let _data of req.body.params) {
                ...
                let _gResp = await _mUtils.commonMonogoCall("ophthamology_ecg_Manufacturers", "find", _filter, "", "", "", req.tokenData.dbType)
                if (_gResp && _gResp.success && _gResp.data.length > 0) {
                    _data['manufacturerId'] = _gResp.data[0]._id
                }
    ...
    try {
        if (req.body && req.body.params) {
            ...
            if (req.body.params.fromDt && req.body.params.fromDt.length > 0) {
                ...
                let toDt = req.body.params.toDt && req.body.params.toDt.length > 0 ? req.body.params.toDt.split('T')[0] : fromDt
                _filter.filter['receivedDateTime'] = { $gte: `${fromDt}T00:00:00.000Z`, $lte: `${toDt}T23:59:59.999Z` }
            }
            let _pGData = await prepareGetPayload(_filter);
            if (!_pGData.success) {
                return res.status(400).json({ success: false, status: 400, desc: _pGData.desc, data: [] });
...
router.post("/get-dashboard", async (req, res) => {
    try {
        ...
        let _userData = await _mUtils.commonMonogoCall("ophthamology_ecg_Users", "find", _userFilter, "", "", "", req.tokenData.dbType)
        if (!(_userData && _userData.success && _userData.data && _userData.data.length > 0)) {
            return res.status(400).json({ success: false, status: 400, desc: _userData.desc || "User does not exist", data: [] });
        }
        _userData = JSON.parse(JSON.stringify(_userData))
        const _locations = _userData.data[0].locations;
        let _dashboardFilter;
        let _collectionName;
        if (req.tokenData.roleName == "PHC Doctor" || req.tokenData.roleName == "CHC Doctor") {
            _collectionName = "ophthamology_ecg_Consultations"
            _dashboardFilter = {
                filter: [
                    {
                        $match: {
                            doctor: mongoose.Types.ObjectId(_userData.data[0].empId)
                        },
                    },
                    {
                        $match: {
                    ...
                    },
                    {
                        $project: {
                            "summary": {
                                totalVisits: {
                                    $arrayElemAt: ["$totalVisits.count", 0],
                                },
                                prescriptionsPending: {
                                    $arrayElemAt: ["$pendingVisits.count", 0],
                                },
                                prescriptionsAdded: {
                                    $arrayElemAt: ["$completedVisits.count", 0,],
                                },
                            }
                        },
                    },
        ...
        else if (req.tokenData.roleName == "CHC Nodal Officer") {
            _collectionName = "ophthamology_ecg_Locations"
            _dashboardFilter_bkp = {
                filter: [
                    {
                        $match: {
                            _id: mongoose.Types.ObjectId(_locations[0].locId)
                        }
                    },
                    {
                        $lookup: {
                    ...
                    {
                        $lookup: {
                            from: "glassprescriptions",
                            let: {
                                consultationIds: "$chcConsultations._id"
                            },
                            pipeline: [
                                {
                                    $match: {
                    ...
                    },
                    {
                        $addFields: {
                            chcOverview: {
                                chcName: "$locName",
                                totalVisits: {
                                    $size: "$chcConsultations"
                                },
                                prescriptionsAdded: {
                                    $size: {
                                        $filter: {
                                            input: "$chcConsultations",
                                            as: "consultation",
                                            cond: {
                                                $ne: [
                                                    {
                                                        $ifNull: [
                                                            "$$consultation.visitTransaction",
                                                            null
                                                        ]
                                                    },
                                                    null
                                ...
                                // prescriptionsPending: {
                                //   $subtract: [
                                //     { $ifNull: ["$prescriptionsReceived", 0] },
                                //     { $ifNull: ["$prescriptionsDispatched", 0] }
                                //   ]
                                // }
                            }
                        }
                    ...
                    {
                        $unwind: "$linkedFacilities"
                    },
                    {
                        $lookup: {
                            from: "consultations",
                            let: {
                                phcId: "$linkedFacilities.locId"
                            },
                            pipeline: [
                                {
                                    $match: {
                    ...
                    },
                    {
                        $lookup: {
                            from: "glassprescriptions",
                            let: {
                                consultationIds: "$phcConsultations._id"
                            },
                            pipeline: [
                                {
                                    $match: {
                            ...
                            as: "phcGlassPrescriptions"
                        }
                    },
                    {
                        $addFields: {
                            "linkedFacilities.totalVisits": {
                                $size: "$phcConsultations"
                            },
                            "linkedFacilities.prescriptionsAdded": {
                                $size: {
                                    $filter: {
                                        input: "$phcConsultations",
                                        as: "consultation",
                                        cond: {
                                            $ne: [
                                                {
                            ...
                            "linkedFacilities.prescriptionsReceived": {
                                $size: {
                                    $filter: {
                                        input: "$phcGlassPrescriptions",
                                        as: "prescription",
                                        cond: {
                                            $eq: [
                                                "$$prescription.status",
                            ...
                            "linkedFacilities.prescriptionsDispatchedToPatient":
                            {
                                $size: {
                                    $filter: {
                                        input: "$phcGlassPrescriptions",
                            ...
                            "linkedFacilities.prescriptionsPending":  {
                                $size: {
                                    $filter: {
                                        input: "$phcGlassPrescriptions",
                                        as: "prescription",
                    ...
                    },
                    {
                        $group: {
                            ...
                            totalVisits: {
                                $sum: "$linkedFacilities.totalVisits"
                            },
                            totalPrescriptionsAdded: {
                                $sum: "$linkedFacilities.prescriptionsAdded"
                            },
                            totalPrescriptionsReceived: {
                                $sum: "$linkedFacilities.prescriptionsReceived"
                            },
                            totalPrescriptionsDispatched: {
                                $sum: "$linkedFacilities.prescriptionsDispatchedToPatient"
                            },
                            totalPrescriptionsPending: {
                                $sum: "$linkedFacilities.prescriptionsPending"
                            },
                            approvalPending: {
                                $sum: {
                                    $size: {
                                        $filter: {
                                            input: {
                                                $concatArrays: [
                                                    "$phcGlassPrescriptions",
                                                    "$chcGlassPrescriptions"
                                                ]
                                            },
                                            as: "prescription",
                                            cond: {
                    ...
                    },
                    {
                        $addFields: {
                            summary: {
                                totalVisits: {
                                    $add: [
                                        "$chcOverview.totalVisits",
                                        "$totalVisits"
                                    ]
                                },
                                prescriptionsAdded: {
                                    $add: [
                                        "$chcOverview.prescriptionsAdded",
                                        "$totalPrescriptionsAdded"
                                    ]
                                },
                                prescriptionsReceived: {
                                    $add: [
                                        "$chcOverview.prescriptionsReceived",
                                        "$totalPrescriptionsReceived"
                                    ]
                                },
                                prescriptionsDispatched: {
                                    $add: [
                                        "$chcOverview.prescriptionsDispatched",
                                        "$totalPrescriptionsDispatched"
                                    ]
                                },
                                prescriptionsPending: {
                                    $add: [
                                        "$chcOverview.prescriptionsPending",
                                        "$totalPrescriptionsPending"
                                    ]
                                }
                            },
                      ...
                      $addFields: {
                        approvedPrescriptionsAtChc: {
                          $cond: {
                            ...
                            else: "$approvedPrescriptionsAtChc"
                          }
                        }
                      }
                    },
            ...
            _dashboardFilter = {
                filter: [
                    { $match: { "_id": mongoose.Types.ObjectId(_locations[0].locId) } },//_id: ObjectId("6772c90d9f407d83fba09816")
                    { $unwind: "$linkedFacilities" },
                    { $match: { "linkedFacilities.locId": { $ne: null } } },
                    {
                      $lookup: {
                        from: "locations",
                        let: { locId: "$linkedFacilities.locId" },
                        pipeline: [
                          { $match: { $expr: { $eq: ["$_id", { $toObjectId: "$$locId" }] } } },
                          { $unwind: "$linkedFacilities" },
                          { $project: { locId: "$linkedFacilities.locId", facilityName: "$name" } }
                        ],
                        as: "relatedLocIds"
                      }
                    },
                    {
                      $addFields: {
                        allLocIds: {
                          $concatArrays: [
                            ["$linkedFacilities.locId"],
                            { $map: { input: "$relatedLocIds", as: "loc", in: "$$loc.locId" } }
                          ]
                        },
                        chcName: "$linkedFacilities.name"
                      }
                    },
                    {
                      $lookup: {
                        from: "consultations",
                        localField: "allLocIds",
                        foreignField: "locId",
                        as: "consultations"
                      }
                    },
                    { $unwind: { path: "$consultations", preserveNullAndEmptyArrays: true } },
                    {
                      $lookup: {
                        from: "glassprescriptions",
                        let: { consultationId: "$consultations._id" },
                        pipeline: [{ $match: { $expr: { $eq: ["$consultation", "$$consultationId"] } } }],
                        as: "prescriptions"
                      }
                    },
                    {
                      $addFields: {
                        withinDateRange: {
                          $and: [
                            { $gte: ["$consultations.dateTime",`${fromDt}T00:00:00.000Z` ] },//"2025-01-01T00:00:00Z"
                            { $lte: ["$consultations.dateTime", `${toDt}T23:59:59:999Z`] }//"2025-01-08T23:59:59Z"
                          ]
                        },
                        prescriptionsInRange: {
                          $filter: {
                            ...
                            cond: {
                              $and: [
                                { $gte: ["$$prescription.audit.documentedDt", `${fromDt}T00:00:00.000Z`] },//"2025-01-01T00:00:00Z"
                                { $lte: ["$$prescription.audit.documentedDt", `${toDt}T23:59:59:999Z`] }//"2025-01-08T23:59:59Z"
                              ]
                            }
                          }
                        }
                    ...
                    },
                    {
                      $group: {
                        _id: "$allLocIds",
                        chcName: { $first: "$chcName" },
                        totalVisitsInRange: { $sum: { $cond: ["$withinDateRange", 1, 0] } },
                        prescriptionsAddedInRange: { $sum: { $size: "$prescriptionsInRange" } },
                        pendingApprovalSHC: { $sum: { $size: { $filter: { input: "$prescriptionsInRange", as: "p", cond: { $eq: ["$$p.status", "Pending for State Approval"] } } } } },
                        approvedPrescriptionsSHC: { $sum: { $size: { $filter: { input: "$prescriptionsInRange", as: "p", cond: { $in: ["$$p.status", ["Approved", "Sent to Manufacturer", "Received from Manufacturer", "Reject at Manufacturer", "Ready to Dispatch", "Rejected at PHC User", "Dispatched to Patient"]] } } } } },
                        rejectedPrescriptionsSHC: { $sum: { $size: { $filter: { input: "$prescriptionsInRange", as: "p", cond: { $eq: ["$$p.status", "Reject at State Level"] } } } } },
                        prescriptionsReceivedInRange: { $sum: { $size: { $filter: { input: "$prescriptionsInRange", as: "p", cond: { $in: ["$$p.status", ["Received from Manufacturer", "Ready to Dispatch", "Rejected at PHC User", "Dispatched to Patient"]] } } } } },
                        prescriptionsDispatchedInRange: { $sum: { $size: { $filter: { input: "$prescriptionsInRange", as: "p", cond: { $eq: ["$$p.status", "Dispatched to Patient"] } } } } },
                        prescriptionsPendingInRange: { $sum: { $size: { $filter: { input: "$prescriptionsInRange", as: "p", cond: { $eq: ["$$p.status", "Ready to Dispatch"] } } } } },
                        totalVisits: { $sum: { $cond: [{ $ifNull: ["$consultations._id", false] }, 1, 0] } },
                        totalPrescriptionsAdded: { $sum: { $size: "$prescriptions" } },
                        totalPrescriptionsReceived: { $sum: { $size: { $filter: { input: "$prescriptions", as: "p", cond: { $in: ["$$p.status", ["Received from Manufacturer", "Ready to Dispatch", "Rejected at PHC User", "Dispatched to Patient"]] } } } } },
                        totalPrescriptionsDispatched: { $sum: { $size: { $filter: { input: "$prescriptions", as: "p", cond: { $eq: ["$$p.status", "Dispatched to Patient"] } } } } },
                        totalPrescriptionsPending: { $sum: { $size: { $filter: { input: "$prescriptions", as: "p", cond: { $eq: ["$$p.status", "Ready to Dispatch"] } } } } }
                      }
                    },
                    {
                      $group: {
                        ...
                        chcOverview: {
                          $push: {
                            ...
                            prescriptionsAdded: "$prescriptionsAddedInRange",
                            prescriptionsReceived: "$prescriptionsReceivedInRange",
                            prescriptionsDispatched: "$prescriptionsDispatchedInRange",
                            prescriptionsPending: "$prescriptionsPendingInRange"
                          }
                        },
                        totalVisits: { $sum: "$totalVisits" },
                        totalPrescriptionsAdded: { $sum: "$totalPrescriptionsAdded" },
                        totalPrescriptionsReceived: { $sum: "$totalPrescriptionsReceived" },
                        totalPrescriptionsPending: { $sum: "$totalPrescriptionsPending" },
                        totalPrescriptionsDispatched: { $sum: "$totalPrescriptionsDispatched" },
                        prescriptionsAddedInRange: { $sum: "$prescriptionsAddedInRange" },
                        prescriptionsReceivedInRange: { $sum: "$prescriptionsReceivedInRange" },
                        prescriptionsDispatchedInRange: { $sum: "$prescriptionsDispatchedInRange" },
                        prescriptionsPendingInRange: { $sum: "$prescriptionsPendingInRange" },
                        pendingApprovalSHC: { $sum: "$pendingApprovalSHC" },
                        approvedPrescriptionsSHC: { $sum: "$approvedPrescriptionsSHC" },
                        rejectedPrescriptionsSHC: { $sum: "$rejectedPrescriptionsSHC" }
                      }
                    },
                    {
                      $addFields: {
                        overView: {
                          totalPrescriptions: "$prescriptionsAddedInRange",
                          pendingApproval: "$pendingApprovalSHC",
                          approvedPrescriptions: "$approvedPrescriptionsSHC",
                          rejectedPrescriptions: "$rejectedPrescriptionsSHC"
                        },
                        summary: {
                          totalVisits: "$totalVisits",
                          prescriptionsAdded: "$totalPrescriptionsAdded",
                          prescriptionsReceived: "$totalPrescriptionsReceived",
                          prescriptionsDispatched: "$totalPrescriptionsDispatched",
                          prescriptionsPending: "$totalPrescriptionsPending"
                        }
                      }
                    },
                    { $project: { _id: 0, overView: 1, chcOverview: 1, summary: 1 } }
                  ]
            };
        }
        else {
        ...
        const _dashboardData = await _mUtils.commonMonogoCall(_collectionName, "aggregation", _dashboardFilter, "", "", "", req.tokenData.dbType)
        if (!(_dashboardData && _dashboardData.success)) {
            return res.status(400).json({ success: false, status: 400, desc: "Error fetching data" });
        };
        if(req.tokenData.roleName == "PHC Doctor" || req.tokenData.roleName == "CHC Doctor"){
            _dashboardData.data[0].summary['totalVisits'] = _dashboardData.data && _dashboardData.data.length > 0 && _dashboardData.data[0].summary && _dashboardData.data[0].summary.totalVisits ? _dashboardData.data[0].summary.totalVisits : 0
            _dashboardData.data[0].summary['prescriptionsPending'] = _dashboardData.data && _dashboardData.data.length > 0 && _dashboardData.data[0].summary && _dashboardData.data[0].summary.prescriptionsPending ? _dashboardData.data[0].summary.prescriptionsPending : 0
            _dashboardData.data[0].summary['prescriptionsAdded'] = _dashboardData.data && _dashboardData.data.length > 0 && _dashboardData.data[0].summary && _dashboardData.data[0].summary.prescriptionsAdded ? _dashboardData.data[0].summary.prescriptionsAdded : 0
        }
        return res.status(200).json({ success: true, status: 200, desc: '', data: _dashboardData.data });
    } catch (error) {
        return res.status(500).json({ success: false, status: 500, desc: error.message || error });
    }
})
...
router.post("/postPrescription", async (req, res) => {
    try {
        let _cBody = JSON.parse(JSON.stringify(req.body))
        if(_cBody.params){
            _.each(_cBody.params, (_p)=>{
                _p['audit'] = req.body.params['audit']
            })
            delete _cBody.params['audit']
        }
        mongoMapper('ophthamology_ecg_Prescriptions', "insertMany", _cBody.params, req.tokenData.dbType).then(async (result) => {
            await logMessage('info', `---REQUEST TIME:-${moment().format("DD-MMM-YYYY HH:mm")}, URL:-${req.url}, PAYLOAD:-${JSON.stringify(req.body.params)}, RESPONSE:-${JSON.stringify(result.data)}---`);
            await logMessage('info', `----------------------------------------------------------------------------------------------------------`);
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch(async(error) => {
            await logMessage('error', `---REQUEST TIME:-${moment().format("DD-MMM-YYYY HH:mm")}, URL:-${req.url}, ERROR:- ${JSON.stringify(error.desc)}---`);
            await logMessage('info', `----------------------------------------------------------------------------------------------------------`);
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
    } catch (error) {
        await logMessage('error', `---REQUEST TIME:-${moment().format("DD-MMM-YYYY HH:mm")},  URL:-${req.url}, ERROR:- ${error.message}---`);
        await logMessage('info', `----------------------------------------------------------------------------------------------------------`);
        return res.status(500).json({ success: false, status: 500, desc: error.message || error });
    }
});

router.post("/postPrescriptionDispatchCb", async (req, res) => {
    try {
        let _filter = { 
            "filter": {
                "recStatus": true
        ...
        if(Array.isArray(req.body) && req.body.length > 0){
            for(let _data of req.body){
                if(_data.patientID && _data.patientID.length > 0){
                    _patParams.push(_data.patientID)
                }
                if(_data.orderID && _data.orderID.length > 0){
                    _ordparams.push(_data.orderID)
                }
            }
            if(_patParams.length > 0){
                _filter.filter['patient.patientID'] = { $in: _patParams }
            }
            if(_ordparams.length > 0){
                _filter.filter['orderID'] = { $in: _ordparams }
            }
        ...
        mongoMapper('ophthamology_ecg_Prescriptions', "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
            let _finalData = []
            let _resp = JSON.parse(JSON.stringify(result.data))
            if(_resp && _resp.length > 0){
                for(let _data of _resp){
                    if(_data.dispatched && _data.dispatched == true){
                        _finalData.push({ 'patientID': _data.patient.patientID, 'orderID': _data.orderID, 'acknowledged': true })
                    }
                    else{
                        _finalData.push({ 'patientID': _data.patient.patientID, 'orderID': _data.orderID, 'acknowledged': false })
                    }
                }
            }
            await logMessage('info', `---REQUEST TIME:-${moment().format("DD-MMM-YYYY HH:mm")},  URL:-${req.url}, PAYLOAD:-${JSON.stringify(req.body.params)}, RESPONSE:-${JSON.stringify(_finalData)}---`);
            await logMessage('info', `----------------------------------------------------------------------------------------------------------`);
            return res.status(200).json({ success: true, status: 200, desc: '', data: _finalData });
        }).catch(async(error) => {
            await logMessage('error', `---REQUEST TIME:-${moment().format("DD-MMM-YYYY HH:mm")}, URL:-${req.url}, ERROR:- ${error.desc}---`);
            await logMessage('info', `----------------------------------------------------------------------------------------------------------`);
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
    } catch (error) {
...
router.post("/postPrescriptionReceipt", async (req, res) => {
    try {
        let _filter = { 
            "filter": {
                "recStatus": true
        ...
        mongoMapper('ophthamology_ecg_Prescriptions', "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
            let _finalData = []
            if(result.data && result.data.length > 0){
                for(let _data of result.data){
                    _finalData.push({ 'patientID': _data.patient.patientID, 'orderID': _data.orderID, 'userDetails': _data.userDetails })
                }
            }
            await logMessage('info', `---REQUEST TIME:-${moment().format("DD-MMM-YYYY HH:mm")},  URL:-${req.url}, PAYLOAD:-${JSON.stringify(req.body.params)}, RESPONSE:-${JSON.stringify(_finalData)}---`);
            await logMessage('info', `----------------------------------------------------------------------------------------------------------`);
...
router.post("/postPrescriptionDispense", async (req, res) => {
    try {
        let _filter = { 
            "filter": {
                "recStatus": true
        ...
        mongoMapper('ophthamology_ecg_Prescriptions', "find", _pGData.data, req.tokenData.dbType).then(async(result) => {
            ...
            if(_resp && _resp.length > 0){
                for(let _data of _resp){
                    if(_data.dispensed && _data.dispensed == true){
                        _finalData.push({ 'patientID': _data.patient.patientID, 'orderID': _data.orderID, 'acknowledged': true })
                    }
                    else{
                        _finalData.push({ 'patientID': _data.patient.patientID, 'orderID': _data.orderID, 'acknowledged': false })
