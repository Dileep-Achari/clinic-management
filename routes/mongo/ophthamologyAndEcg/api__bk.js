...
router.post("/auth-user", async (req, res) => {
    try {
        ...
        let _user = {
            ...
            "displayName": _clnUser[0].displayName, "roleId": _clnUser[0].locations[0].roleId, "roleName": _clnUser[0].locations[0].role, docAccess: _docString, "i_orgCd": locationDetails.i_orgCd, "i_locCd": locationDetails.i_locCd, "emrMigId": locationDetails.emrMigId, "syncApiUrl": locationDetails.syncServUrl
        };
        //syncchanges
        // let _user = {
    ...
    try {
        if (req.body.params._id) {
            ...
            if (_dResp.success == true && _dResp.data.length > 0) {
                ...
                let _drResp = await _mUtils.commonMonogoCall("cm_drafts", "findOneAndUpdate", pLoadResp.payload, "", req.body, "", req.tokenData.dbType)
                if (!(_drResp && _drResp.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _drResp.desc || "", data: _drResp.data || [] });
                }
            }
        ...
        if (req.body.params.flag == "HIS") {
            ...
            commonAxiosCall('GET', url).then(async (resp) => {
                ...
                if (req.body.params.isForceLoad == true) {
                    ...
                    let _finalResp = transformLabResults(jsonString)
                ...
                mongoMapper("cm_hims_lab_results", "find", _filter, _dbType).then(async (result) => {
                    ...
                    if (result.data && result.data.length > 0) {
                        ...
                        let _finalResp2 = transformLabResults(JSON.parse(JSON.stringify(_mResp.data.data)))
                        ...
                        mongoMapper("cm_hims_lab_results", "insertMany", _insrtPrms, req.tokenData.dbType.toLowerCase()).then(async (result) => {
                            ...
                            finalResp = transformLabResults(JSON.parse(JSON.stringify(result.data[0].data)))
...
router.post("/generate-report", async (req, res) => {
    try {
        if (req.body.params && req.body.params.toDt && req.body.params.type && req.body.params.fromDt) {
            ...
            await mongoMapper("cm_patients", "aggregation", _patRegReport, req.tokenData.dbType).then(async (_resp) => {
                if (!_resp.data || _resp.data.length == 0) {
...
router.post("/get-dashboard", async (req, res) => {
    try {
        ...
        if (req.tokenData.roleName == "PHC Doctor" || req.tokenData.roleName == "CHC Doctor") {
                    ...
                    },
                    {
                        $match: {
                            "consultations.dateTime": {
                                $gte: `${fromDt}T00:00:00.000Z`,
                                $lte: `${toDt}T23:59:59:999Z`
                                // $gte: "2025-01-06T00:00:00Z",
                                // $lte: "2025-01-06T23:59:59Z"
                            }
                        }
                    },
                    {
                                  ...
                                  $cond: [
                                    {
                                      $in: [
                                        ...
                                        [
                                          ...
                                          "Received from Manufacturer",
                                          "Reject at Manufacturer",
                                          "Ready to Dispatch",
                                          "Received from Manufacturer",
                                          "Rejected at PHC User",
            ...
            if (!(_dashboardData && _dashboardData.success)) {
            ...
            else if (_dashboardData && _dashboardData.success){
                _summaryFilter = {
                    filter: [
                        {
                          $match: {
                ...
                const _summaryData = await _mUtils.commonMonogoCall(_collectionName, "aggregation", _summaryFilter, "", "", "", req.tokenData.dbType)
                if (!(_summaryData && _summaryData.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: "Error fetching data" });
                }
                // else if (_summaryData && _summaryData.success){
                //     //_dashboardData[summary]= _summaryData.summary
                //     const finalData = {
                //         overView: _dashboardData.data[0].overView,
                //         chcOverview: _dashboardData.data[0].chcOverview,
                //         summary: _summaryData.data[0].summary
                //     };
                //     return res.status(200).json({ success: true, status: 200, desc: '', data: finalData });
                // } else {
                //     return res.status(400).json({ success: false, status: 400, desc: "Error Fetching Data" });
                // }
                if (_dashboardData && _dashboardData.success && _dashboardData.data && _dashboardData.data.length > 0) {
                    const finalData = {
                        overView: _dashboardData.data[0].overView,
                        chcOverview: _dashboardData.data[0].chcOverview,
                        summary: _summaryData && _summaryData.success && _summaryData.data.length > 0  ? _summaryData.data[0].summary : null 
                    };
                    return res.status(200).json({  success: true, status: 200, desc: '', data: finalData });
                } else if (_summaryData && _summaryData.success && _summaryData.data.length > 0) {
                    return res.status(200).json({
                        success: true, status: 200, desc: '', data: { summary: _summaryData.data[0].summary }  });
                } else {
                    return res.status(400).json({ success: false,status: 400,  desc: "No data available"  });
                } 
            }
        }
        else {
