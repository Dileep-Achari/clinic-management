...
async function bookAppointment_new(_type, req, _flag) {
    try {
        let _output = [];
        let _doctorId;
        let duration;
        ...
        if (req.body && req.body.params && req.body.params.doctor) {
            ...
            let _filter = {
                "filter": { "locations.locId": { $eq: req.body.params.locId } },
                "selectors": "-audit -history"
            }
...
async function updateGlassPrescriptions(_data, _idx, _output, req) {
    try {
        if (_data.length > _idx) {
            ...
            if (_data[_idx]._id) {
                ...
                if (_userResp && _userLocDataResp) {
                    ...
                    pLoadResp = await _mUtils.preparePayload('BW', _updtprms);
                    if (!pLoadResp.success) {
                        _output.push({ success: false, _id: _data[_idx]._id, status: 400, desc: pLoadResp.desc || "", data: [] });
                    }

                    let _gResp = await _mUtils.commonMonogoCall("ophthamology_ecg_GlassPrescriptions", "bulkWrite", pLoadResp.payload, "", "", "", req.tokenData.dbType)
                ...
                pLoadResp = await _mUtils.preparePayload('BW', _manfctUpdtprms);
                if (!pLoadResp.success) {
                    _output.push({ success: false, _id: _data[_idx]._id, status: 400, desc: pLoadResp.desc || "", data: [] });
                }
            ...
            _idx = _idx + 1;
            await updateGlassPrescriptions(_data, _idx, _output, req);
        }
        else {
            return { success: true, data: _output }
        }
...
router.post("/update-glass-prescriptions", async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, status: 200, desc: "", data: _updtResp.data });
    ...
    try {
        if (req.body && req.body.params && req.body.params.length > 0) {
            ...
            for(let _data of req.body.params){
                _data['batchNo'] = `${new Date().toISOString().split("T")[0].split("-").join("")}`
