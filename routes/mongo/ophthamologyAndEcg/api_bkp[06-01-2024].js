...
router.post("/get-glass-prescriptions", async (req, res) => {
    try {
        if (req.body && req.body.params && req.body.params.flag) {
            ...
            // console.log("req.body.params.flag", req.body.params.flag)
            // console.log("Role", _userLocData[0].role)
            if(req.body.params.flag == "STATE_APPROVED" && _userLocData[0].role.toLowerCase().split(" ").join("") == "manufactureruser"){  
                _status[0].status = ['Approved']
            }
            if(req.body.params.flag == "STATE_APPROVED" && _userLocData[0].role.toLowerCase().split(" ").join("") == "statenodalofficer"){  
...
router.post("/get-dashboard", async (req, res) => {
    try {
        let _filter = {
            "filter": { "_id": req.tokenData.userId },
            "selectors": "-audit -history"
        }
         let _dbType = req.headers.orgkey.toLowerCase() || req.tokenData.dbType;
        let _userData = await _mUtils.commonMonogoCall("ophthamology_ecg_Users", "find", _filter, "", "", "", _dbType)
        if (!(_userData && _userData.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _userData.desc || "", data: _userData.data || [] });
        }
        ...
        //                 _id: null,
        //                 totalVisits: { $sum: 1 },
        //                 withVisitTransaction: {
        //                     $sum: {
        //                         $cond: [{ $ne: ["$visitTransaction", null] }, 1, 0]
        //                         // $cond: [{ $and: [{ $ne: ["$visitTransaction", null]}, {$exists: true}]}, 1, 0]
        //                     }
        //                 }
        //             }
        //         }
        ...
        const _visitData = await _mUtils.commonMonogoCall("ophthamology_ecg_Consultations", "aggregation", consultationFilter, "", "", "", req.tokenData.dbType)
        if (!(_visitData && _visitData.success)) {
            return res.status(400).json({ success: false, status: 400, desc: "Error fetching appointments" });
        };

        // const aggregationresult = _visitData.data[0] || {
        //     totalVisits: 0,
        //     withVisitTransaction: 0
        // }


        // const { totalVisits, withVisitTransaction } = aggregationresult
        ...
        return res.status(200).json({ success: true, status: 200, desc: '', data: _visitData.data });
    } catch (error) {
        return res.status(500).json({ success: false, status: 500, desc: error.message || error });
    }
})
