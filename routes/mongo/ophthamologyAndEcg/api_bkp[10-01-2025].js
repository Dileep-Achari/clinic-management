...
let _approvalStatus = [
    ...
    { flag: 'CHC_APPROVED', status: ['Pending for State Approval', 'Approved', 'Reject at State Level', 'Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer' ] },
    ...
    { flag: 'STATE_APPROVED', status: ['Approved', 'Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer'] },
    ...
    { flag: 'CHC_SUMMARIZED', status: ['Pending for CHC Approval', 'Pending for State Approval', 'Approved', 'Reject at State Level', 'Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer' ] },
    { flag: 'STATE_SUMMARIZED', status: ['Pending for State Approval', 'Approved', 'Reject at State Level', 'Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer' ] },
    ...
    try {
        if (req.body && req.body.params && req.body.params.flag) {
            ...
            if(req.body.params.fromDt && req.body.params.fromDt.length > 0){
                // if (req.body.params.toDt) {
                //     _params["audit.documentedDt"] = { $gte: new Date(new Date(req.body.params.fromDt).setHours(0, 0, 0, 0)).toISOString(), $lt: new Date(new Date(req.body.params.toDt).setHours(23, 59, 59, 999)).toISOString() }
                // } 
                // else {
                //     _params["audit.documentedDt"] = { $gte: new Date(new Date(req.body.params.fromDt).setHours(0, 0, 0, 0)).toISOString(), $lt: new Date(new Date(req.body.params.fromDt).setHours(23, 59, 59, 999)).toISOString() }
                // }
            ...
            mongoMapper('ophthamology_ecg_GlassPrescriptions', "find", _pGData.data, req.tokenData.dbType).then((result) => {
                ...
                else if(_role == "manufactureruser"){
                ...
                if(req.body.params.flag.split("_")[1] == "SUMMARIZED"){
...
router.post("/get-dashboard/oldd", async (req, res) => {
    try {
        let date = new Date()
            ...
            if (!(_dashboardData && _dashboardData.success)) {
                ...
                if (_dashboardData && _dashboardData.success && _dashboardData.data && _dashboardData.data.length > 0) {
                    ...
                    return res.status(200).json({
                        success: true, status: 200, desc: '', data: finalData });
                } else {
