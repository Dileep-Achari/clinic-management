...
let _approvalStatus = [
    ...
    { flag: 'CHC_SUMMARIZED', status: ['Pending for CHC Approval', 'Pending for State Approval', 'Approved', 'Reject at State Level', 'Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer', 'Ready To Dispatch', 'Dispatched to Patient', 'Rejected at PHC User'] },
    ...
    { flag: 'MANUFACTURER_SUMMARIZED', status: ['Approved', 'Sent to Manufacturer', 'Received from Manufacturer', 'Reject at Manufacturer'] },
    ...
    try {
        if (req.body && req.body.params && req.body.params.flag) {
            ...
            if (req.body.params.fromDt && req.body.params.fromDt.length > 0) {
                ...
                else if (req.tokenData.roleName.toLowerCase().split(" ").join("") == 'chcnodalofficer') {
                ...
                if (req.body.params.flag.split("_")[1] == "SUMMARIZED" && req.body.params.flag != "MANUFACTURER_SUMMARIZED") {
                    ...
                    let _result = _dateFilterData.flatMap(dateObj => {
                        return prescriptionTypes.map(type => {
                            ...
                            return {
                                ...
                                approvedCount: approved.length,
                                rejectedCount: rejected.length,
                                pendingData: req.body.params.pendingData && req.body.params.pendingData == true ? DispatchPending : []
                            }
                        })
                    })
                    _totalData = _result
                    ...
                    let _result = _dateFilterData.flatMap(dateObj => {
                        return prescriptionTypes.map(type => {
                            ...
                            let total = _.filter(_totalData, (_item) => _item.status == "Received from Manufacturer" || _item.status == "Rejected at PHC User" || _item.status == "Ready To Dispatch" || _item.status == "Dispatched to Patient")

                            let _batchNumber = total && total.length > 0 ? total[0].batchNumber : ""
        ...
        if (req.body && req.body.params && req.body.params.flag && (req.body.flag == "FACILITY_WISE" ||  req.body.flag == "DATE_WISE")) {
            ...
            if (req.tokenData.roleName === "CHC Nodal Officer") {
                _preFilter = {
                    "filter": {
                        "recStatus": true,
                        "prescribedLocation": req.body.params._id || { $in: _params.prescribedLocation._mappedLoc.map(id => mongoose.Types.ObjectId(id)) },
        ...
        if (req.tokenData.roleName == "PHC Doctor" || req.tokenData.roleName == "CHC Doctor") {
            ...
            _dashboardFilter = {
                filter: [
                    {
                        "$match": {
                            "_id": mongoose.Types.ObjectId(_locations[0].locId)//ObjectId("6772c8809f407d83fba091dd") 
                        }
                    },
                    {
                    ...
                    },
                    {
                        "$lookup": {
                            "from": "consultations",
                            "localField": "linkedFacilities.locId",
                            "foreignField": "locId",
                            "as": "consultations"
                        }
                    },
                            ...
                            "linkedFacilities.prescriptionsAdded": {
                                "$size": {
                                    "$filter": {
                                        ...
                                        "as": "consultation",
                                        "cond": {
                                            "$ne": [{ "$ifNull": ["$$consultation.visitTransaction", null] }, null]
                                        }
                                    }
                                }
                            }
                    ...
                    },
                    {
                        "$lookup": {
                            ...
                            "localField": "linkedFacilities.locId",
                            "foreignField": "prescribedLocation",
                            "as": "associatedPrescriptions"
                        }
                    },
                    {
                        "$addFields": {
                            "linkedFacilities.prescriptionsReceived": {
                                "$size": {
                                    "$filter": {
                                        "input": "$associatedPrescriptions",
                                        "as": "prescription",
                                        "cond": {
                                            "$and": [
                                                { "$eq": ["$$prescription.status", "Sent to Manufacturer"] },
                                                { "$eq": ["$$prescription.prescribedLocation", "$linkedFacilities.locId"] }
                                            ]
                                        }
                                    }
                                }
                            ...
                            "linkedFacilities.prescriptionsDispatchedToPatient": {
                                "$size": {
                                    "$filter": {
                                        ...
                                        "cond": {
                                            "$and": [
                                                { "$eq": ["$$prescription.status", "Dispatched to Patient"] },
                                                { "$eq": ["$$prescription.prescribedLocation", "$linkedFacilities.locId"] }
                                            ]
                                        }
                                    }
                            ...
                            "linkedFacilities.prescriptionsPending": {
                                "$size": {
                                    "$filter": {
                                        ...
                                        "cond": {
                                            "$and": [
                                                { "$eq": ["$$prescription.status", "Received from Manufacturer"] },
                                                { "$eq": ["$$prescription.prescribedLocation", "$linkedFacilities.locId"] }
                                            ]
                                        }
                                    }
                    ...
                    {
                        "$addFields": {
                            "associatedPrescriptionsToday": {
                                "$filter": {
                                    "input": "$associatedPrescriptions",
                                    "as": "prescription",
                                    "cond": {
                                        "$eq": [
                                            { "$dateToString": { "format": "%Y-%m-%d", "date": "$$prescription.createdAt" } },
                    ...
                    },
                    {
                        "$addFields": {
                            "todaySummary": {
                                "totalPrescriptions": { "$size": "$associatedPrescriptionsToday" },
                                "pendingApprovals": {
                                    "$size": {
                                        "$filter": {
                                            "input": "$associatedPrescriptionsToday",
                                            "as": "prescription",
                                            "cond": { "$eq": ["$$prescription.status", "Pending for CHC Approval"] }
                                        }
                                    }
                                },
                                "approvedPrescriptions": {
                                    "$size": {
                                        "$filter": {
                                            "input": "$associatedPrescriptionsToday",
                                            "as": "prescription",
                                            "cond": { "$eq": ["$$prescription.status", "Pending for State Approval"] }
                                        }
                                    }
                                },
                                "rejectedPrescriptions": {
                                    "$size": {
                                        "$filter": {
                                            "input": "$associatedPrescriptionsToday",
                                            "as": "prescription",
                                            "cond": { "$eq": ["$$prescription.status", "Reject at CHC Level"] }
                                        }
                                    }
                                }
                            }
                    ...
                    },
                    {
                        "$group": {
                            "_id": "$_id",
                            "chcName": { "$first": "$locName" },
                            "phcOverview": {
                                "$push": {
                                    "phcName": "$linkedFacilities.name",
                                    "totalVisits": "$linkedFacilities.totalVisits",
                                    "prescriptionsAdded": "$linkedFacilities.prescriptionsAdded",
                                    "prescriptionsReceived": "$linkedFacilities.prescriptionsReceived",
                                    "prescriptionsDispatched": "$linkedFacilities.prescriptionsDispatchedToPatient",
                                    "prescriptionsPending": "$linkedFacilities.prescriptionsPending"
                                }
                            },
                            "chcTotalVisits": { "$sum": "$linkedFacilities.totalVisits" },
                            "chcPrescriptionsAdded": { "$sum": "$linkedFacilities.prescriptionsAdded" },
                            "chcPrescriptionsReceived": { "$sum": "$linkedFacilities.prescriptionsReceived" },
                            "chcPrescriptionsDispatchedToPatient": { "$sum": "$linkedFacilities.prescriptionsDispatchedToPatient" },
                            "chcPrescriptionsPending": { "$sum": "$linkedFacilities.prescriptionsPending" },
                            "pendingApproval": {
                                "$sum": {
                                    "$size": {
                                        "$filter": {
                                            ...
                                            "as": "prescription",
                                            "cond": {
                                                "$eq": ["$$prescription.status", "Pending for CHC Approval"]
                                            }
                                        }
                                    }
                                }
                                ...
                                "$sum": {
                                    "$size": {
                                        "$filter": {
                                            ...
                                            "as": "prescription",
                                            "cond": {
                                                "$eq": ["$$prescription.status", "Pending for State Approval"]
                                            }
                                        }
                                    }
                                }
                                ...
                                "$sum": {
                                    "$size": {
                                        "$filter": {
                                            ...
                                            "as": "prescription",
                                            "cond": {
                                                "$eq": ["$$prescription.status", "Reject at CHC Level"]
                                            }
                                        }
                                    }
                                }
                            },
                            "overView": { "$first": "$todaySummary" }
                        }
                    },
                    {
                        "$project": {
                            ...
                            "chcOverview": [
                                {
                                    "chcName": "$chcName",
                                    "totalVisits": "$chcTotalVisits",
                                    "prescriptionsAdded": "$chcPrescriptionsAdded",
                                    "prescriptionsReceived": "$chcPrescriptionsReceived",
                                    "prescriptionsDispatched": "$chcPrescriptionsDispatchedToPatient",
                                    "prescriptionsPending": "$chcPrescriptionsPending"
                                }
                            ],
                            "summary": {
                                "totalVisits": "$chcTotalVisits",
            ...
            _dashboardFilter = {
                filter: [
                    {
                        $match: {
                            _id: mongoose.Types.ObjectId("67750ec69341cdfe3bb6b348")
                        }
                    },
                    {
                        $project: {
                            "linkedFacilities.locId": 1,
                            "linkedFacilities.name": 1
                        }
                    },
                    {
                        $unwind: {
                            path: "$linkedFacilities"
                        }
                    },
                    {
                        $match: {
                    ...
                    },
                    {
                        $group: {
                            ...
                            pendingApprovalAtSHC: {
                                $sum: {
                                    $cond: [
                                        {
                                            $eq: [
                                                "$prescriptions.status",
                                                "Pending for State Approval"
                                            ]
                                        },
                                        1,
                                        0
                    ...
                    },
                    {
                        $group: {
                            _id: null,
                            totalPrescriptions: {
                                $sum: "$prescriptionsAdded"
                            },
                            pendingApprovalAtSHC: {
                                $sum: "$pendingApprovalAtSHC"
                            },
                            approvedPrescriptionsAtSHC: {
                                $sum: "$approvedAtSHC"
                            },
                            rejectedPrescriptionsAtSHC: {
                                $sum: "$rejectedAtSHC"
                            },
                            pendingApprovalAtCHC: {
                                $sum: "$pendingApprovalAtCHC"
                            },
                            approvedPrescriptionsAtCHC: {
                                $sum: "$approvedAtCHC"
                            },
                            rejectedPrescriptionsAtCHC: {
                                $sum: "$rejectedAtCHC"
                            },
                            totalVisits: {
                                $sum: "$totalVisits"
                            ...
                            totalPending: {
                                $sum: {
                                    $subtract: ["$received", "$dispatched"]
                                }
                            },
                            chcOverview: {
                                $push: {
                                    chcName: "$facilityName",
                                    totalVisits: "$totalVisits",
                                    prescriptionsAdded: "$prescriptionsAdded",
                                    prescriptionsReceived: "$received",
                                    prescriptionsDispatched: "$dispatched",
                                    prescriptionsPending: {
                                        $subtract: ["$received", "$dispatched"]
                                    },
                                    pendingApproval: "$pendingApprovalAtCHC",
                                    approvedPrescriptions: "$approvedAtCHC",
                                    rejectedPrescriptions: "$rejectedAtCHC"
                                }
                            }
                        }
                    },
                    {
                        $project: {
                            overView: {
                                ...
                                pendingApproval: "$pendingApprovalAtSHC",
                                approvedPrescriptions: "$approvedPrescriptionsAtSHC",
                                rejectedPrescriptions: "$rejectedPrescriptionsAtSHC"
                            },
                            chcOverview: "$chcOverview",
                            summary: {
                                totalVisits: "$totalVisits",
                                prescriptionsAdded: "$totalPrescriptions",
                                prescriptionsReceived: "$totalReceived",
                                prescriptionsDispatched: "$totalDispatched",
                                prescriptionsPending: "$totalPending",
                                pendingApproval: "$pendingApprovalAtSHC",
                                approvedPrescriptions: "$approvedPrescriptionsAtSHC",
                                rejectedPrescriptions: "$rejectedPrescriptionsAtSHC"
                            }
