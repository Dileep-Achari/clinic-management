        ...
        if (req.body && req.body.params) {
            ...
            _preFilter = {
                "filter": {
                    ...
                    "status": { $in: ["Sent to Manufacturer", "Approved"] }
                },
                "selectors": '-manufacturerActions',
                "populate": [
                    { 'path': 'patient', 'select': '_id UMR dispName dateOfBirth contact.mobile homeAddress abhaNo', 'populate': [{ 'path': 'gender', 'select': '' }, { 'path': 'title', 'select': '' }] },
            ...
            return res.status(200).json({ success: true, status: 200, desc: '', data: _preResp.data });
        }
        else {
            return res.status(400).json({ success: false, status: 400, desc: "Required parameters are missing ..", data: [] });
        }
...
router.post("/get-dashboard_bkp", async (req, res) => {
    try {
        let date = new Date()
        const formattedDate = date.toISOString().split('T')[0];
        let fromDt = req.body.params.fromDt ? req.body.params.fromDt.split('T')[0] : formattedDate
                ...
                filter: [
                    {
                        $match: {
                            ...
                            //_id: ObjectId(_locations[0].locId)
                        }
                    },
                    {
                        $unwind: {
                    ...
                    },
                    {
                        $addFields: {
                            ...
                            chcName: "$linkedFacilities.name" // Adding chcName directly
                        }
                    },
                    {
                        $group: {
                            ...
                            // Grouping chcName along with IDs
                            facilityNames: {
                                $push: "$chcName"
                            },
                            totalVisits: {
...
router.post("/get-dashboard", async (req, res) => {
    try {
        ...
        if (req.tokenData.roleName == "PHC Doctor" || req.tokenData.roleName == "CHC Doctor") {
            ...
            _dashboardFilter = {
                filter: [
                    { $match: { "_id": mongoose.Types.ObjectId("677be0fa25b81ad915edc10b") } },//_id: ObjectId("6772c90d9f407d83fba09816")
                    { $unwind: "$linkedFacilities" },
