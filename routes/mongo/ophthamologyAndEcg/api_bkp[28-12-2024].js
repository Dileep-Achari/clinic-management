...
async function bookAppointment(_type, req, _flag) {
    try {
        ...
        let _doctorId = _mResp.data[0]._id;
...
async function checkIfSlotBooked(locId, docId, _apmntTime, req) {
    ...
    let _mResp = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", 'findOne', _cParams, "", "", "", req.tokenData.orgKey.toLowerCase());
    ...
    try {
        return new Promise(async (resolve, reject) => {
            ...
            if (_type && _type === "BILLS") {
                ...
                let getApmtAgstCode = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", "find", _filter, "", "", "", req.tokenData.dbType)
                patData = {
                    params: {
    ...
    try {
        return new Promise(async (resolve, reject) => {
            ...
            if (consultServiceData.length > 0) {
                ...
                let consultAmt = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", "find", _filter, "", "", "", req.tokenData.dbType)
                ...
                let _uResp = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", 'findOneAndUpdate', pLoadResp.payload, "", "", "", req.tokenData.dbType);
...
async function _findDocAganistLocationWithAppointment(_data, _idx, _output, req, _key) {
    try {
        if (_data.length > _idx) {
            ...
            let _mResp1 = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", "aggregation", _filter, "", req.body, "", _key)
...
router.post("/auth-user", async (req, res) => {
    try {
        ...
        let _mOrgResp = await _mUtils.commonMonogoCall("ophthamology_ecg_Organization", "findById", req.body.params.orgId, "", "", "", req.body.params.orgKey.toLowerCase());
...
router.post("/insert-role", async (req, res) => {
    try {
        ...
        let _tabs = ["patientslist", "settings", "signoff"]
        if (req.body.params.defaultTabName && req.body.params.defaultTabName.length > 0) {
...
router.post("/update-employee", async (req, res) => {
    try {
        let _cBody = JSON.parse(JSON.stringify((req.body)));
        if (req.body.params._id) {
            let _mResp = await _mUtils.commonMonogoCall("ophthamology_ecg_employee", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            let _hResp = await _mUtils.insertHistoryData('ophthamology_ecg_employee', _mResp.data.params, _cBody.params, req);
            let pLoadResp = { payload: {} };
            if (!(_hResp && _hResp.success)) {
            ...
            mongoMapper('ophthamology_ecg_employee', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                // let _fLoc = _.filter(req.body.params.locations, (_l) => { return !_l._id });
...
router.post("/insert-patient", async (req, res) => {
    try {
        ...
        let _seqResp = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: 'UMR' }, "ophthamology_ecg", req);
        if (!(_seqResp && _seqResp.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _seqResp.desc || "", data: _seqResp.data || [] });
        }
        ...
        if (req.body.params && (req.body.params.apmntId || (req.body.params.locId && req.body.params.docId && req.body.params.shiftId)) && req.body.params.patName && req.body.params.apmntType) {
            ...
            if (!req.body.params.apmntId) {
                ...
                if (!req.body.params.apmntDtTime) {
                    let _mApmntResp = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", "find", _filter, "", "", "", req.tokenData.dbType)
                    if (!(_mApmntResp && _mApmntResp.success)) {
                ...
                _query = "findOneAndUpdate";
                let _cBody = JSON.parse(JSON.stringify((req.body)));
                let _mResp = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", "findById", req.body.params.apmntId, "REVNO", req.body, "", req.tokenData.dbType);
                if (!(_mResp && _mResp.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
                }
                let _hResp = await _mUtils.insertHistoryData('ophthamology_ecg_appointments', _mResp.data.params, _cBody.params, req);
                let pLoadResp = { payload: {} };
                if (!(_hResp && _hResp.success)) {
                }
                else {
            ...
            mongoMapper('ophthamology_ecg_appointments', _query, _finalResp, req.tokenData.dbType).then(async (result) => {
    ...
    try {
        if (req.body.params && req.body.params.docId && req.body.params.locId) {
            let _filter = {
                ...
                "selectors": "-history",
                "populate": [
                    { 'path': 'patient', 'select': '_id dispName' },
                    { 'path': 'doctor', 'select': '_id dispName' },
                    { 'path': 'visitTransaction', 'select': '_id' }]
            }
            let _finalDataResp = []
...
router.post("/get-dashboard", async (req, res) => {
    try {
        if (req.body.params.flag == "DOCTOR" && req.body.params && req.body.params.docId && req.body.params.locId) {
            ...
            let _mResp1 = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", req.body.query, _filter, "", req.body, "", req.tokenData.dbType);
            _filter.filter = {}
            _filter.filter["docDetails.docId"] = req.body.params.docId;
            let docAgnstApp = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", req.body.query, _filter, "", req.body, "", req.tokenData.dbType);
            let splitObj = {}
...
async function updateQueNoData(req, res, _mongoId, _mResp1) {
    try {
        return new Promise(async (resolve, reject) => {
            ...
            let _mRespOfAppt = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", "find", _filter, "", req.body, "", req.tokenData.dbType)
            _mRespOfAppt["data"] = _.orderBy(_mRespOfAppt.data, ['queueNo'], ['asc'])
            _filter.filter["_id"] = _mongoId;
            let _reResp = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", "find", _filter, "", req.body, "", req.tokenData.dbType)
            ...
            if (req.body.params.flag === "QNO") {
                ...
                if (_reResp.data.length > 0 && _reResp.data[0].queueNo) {
                    ...
                    _cBody = { params: { lst } }
                    let _updateResp = await insertUpdateInMultiData1(_cBody, 0, [], req.tokenData.dbType, 'ophthamology_ecg_appointments', 'findOneAndUpdate', 'U');
            ...
            let _uResp = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", 'bulkWrite', pLoadResp.payload, "", "", "", req.tokenData.dbType);
            if (!(_uResp && _uResp.success && _uResp.data)) {
                resolve({ success: false, status: 400, desc: _uResp.desc || 'Error occurred while insert User ..', data: [] });
            } else {
                resolve({ success: true, status: 200, desc: '', data: _uResp.data });
...
router.post("/update-appointment", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            ...
            let _mResp = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            let _hResp = await _mUtils.insertHistoryData('ophthamology_ecg_appointments', _mResp.data.params, _cBody.params, req);
            ...
            if (!(_hResp && _hResp.success)) {
                ...
                let _mResp1 = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", "findById", req.body.params._id, "", req.body, "", req.tokenData.dbType)
                if (!(_mResp1 && _mResp1.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
                }
            ...
            mongoMapper('ophthamology_ecg_appointments', 'findOneAndUpdate', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
...
router.post("/create-slots", async (req, res) => {
    try {
        if (req.body.params && req.body.params.locId && req.body.params.docId && req.body.params.noOfSlots) {
            ...
            mongoMapper('ophthamology_ecg_appointments', "insertMany", _finalResp, req.tokenData.dbType).then((result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
            ...
            if (Array.isArray(_parseResultData) && _parseResultData.length > 0) {
                for (let data of _parseResultData) {
                    ...
                    if (data.visitNo && data.visitNo.length > 0) {
                        ...
                        let _resp = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", "aggregation", _aptfilter, "", "", "", req.tokenData.dbType)
        ...
        if (req.body.params && req.body.params.docId && req.body.params.fromDt) {
            ...
            mongoMapper("ophthamology_ecg_doctors", "aggregation", _docFilter, req.tokenData.dbType).then(async (result) => {
                if (!result.data || result.data.length == 0) {
                    ...
                    mongoMapper("ophthamology_ecg_appointments", "aggregation", _aptFilter, req.tokenData.dbType).then(async (_resp) => {
        ...
        if (req.body.params && req.body.params.docId && req.body.params.locId && req.body.params.fromDt) {
            ...
            await mongoMapper("ophthamology_ecg_doctors", "aggregation", _docFilter, req.tokenData.dbType).then(async (result) => {
                if (!result.data || result.data.length == 0) {
                    ...
                    await mongoMapper("ophthamology_ecg_appointments", "aggregation", _aptFilter, req.tokenData.dbType).then(async (_resp) => {
                        if (!_resp.data || _resp.data.length == 0) {
                            return res.status(400).json({ status: 'FAIL', desc: 'Error While Getting Appointments', data: [] });
                        }
                        else {
            ...
            if (req.body.params.flag) {
                ...
                const appointmentData = await _mUtils.commonMonogoCall("ophthamology_ecg_appointments", "aggregation", _dayWiseFilter, "", "", "", req.tokenData.dbType)
...
router.post("/insert-doctor", async (req, res) => {
    try {
        if (req.body && req.body.params) {
            mongoMapper('ophthamology_ecg_Doctors', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
        ...
        if (req.body && req.body.params) {
            let _filter = {
                ...
                "populate": [
                    ...
                    { 'path': 'bloodGroup', 'select': 'name -_id' },
                    { 'path': 'speciality', 'select': 'name -_id' },
                    { 'path': 'specializations', 'select': 'name -_id' }
                ]
            }
            let _pGData = await prepareGetPayload(_filter, req.body.params);
            if (!_pGData.success) {
            ...
            mongoMapper("ophthamology_ecg_Doctors", "find", _pGData.data, req.tokenData.dbType).then((result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
                      ...
                      'populate':[
                        {
                            ...
                            'populate': [
                                ...
                                { 'path':'rejectedBy', 'select': '' },
                                { 'path':'approvedBy', 'select': '' }
                            ]
                        },
                        {
                            'path': 'chc',
...
router.post("/insert-visit-transactions", async (req, res) => {
    try {
        if (req.body && req.body.params) {
            mongoMapper('ophthamology_ecg_VisitTransactions', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
