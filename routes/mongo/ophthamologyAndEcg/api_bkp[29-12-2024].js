    ...
    try {
        if (req.body.params && req.body.params.UMR && req.body.params.locId && req.body.params.patId) {
            ...
            if (resp.data && resp.data.length > 0 && !req.body.params.isForceBook) {
                ...
                return res.status(200).json({ success: false, status: 200, desc: "Appointment already exists for the day", data: resp.data[0] });
            }
            if(req.body.params.isForceBook && ( req.body.params.existLocId && req.body.params.existLocId !== req.body.params.locId)){
                return res.status(400).json({ success: false, status: 400, desc: "Appointment already exists in another location for the day", data: [] });
            }
    ...
    try {
        if (req.body && req.body.params && req.body.params.flag) {
            ...
            if (_userLocData && _userLocData[0] && (_userLocData[0].role === "PHC User" || _userLocData[0].role === "CHC User")) {
                ...
                _params["prescribedLocation"] = { $in: _locations.map(_l => _l._id) };
            ...
            let _filter = {
                "filter": _params,
                "selectors": '-manufacturerActions -approvals',
                "populate": [
                    { 'path': 'patient', 'select': '_id dispName dateOfBirth contact.mobile homeAddress' },
                    {
    ...
    try {
        ...
        if (req.body.params._id) {
            ...
            if(_cBody.params.approvals && typeof _cBody.params.approvals == "object" && Object.keys(_cBody.params.approvals).length > 0){
               _cBody.params.approvals
               _.each(_cBody.params.approvals, (_aVal, _aKey)=>{
                     if(_aKey != "_id" && typeof _aVal == "object"){
                        _aVal['location'] = req.tokenData.locId
                        _aVal['requestReceivedDateTime'] = new Date()
                        if(_aVal.approved){
                            _aVal['approvedId'] = req.tokenData.userId
                            _aVal['approvedDateTime'] = new Date()
                            _aVal['approvedBy'] = req.tokenData.userId
                        }
                        if(_aVal.rejected){
                            _aVal['rejectedDateTime'] = new Date()
