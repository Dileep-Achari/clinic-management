...
async function generateToken(_data) {
    return await _token.createTokenWithExpire(_data, "14400000ms");
};
        ...
        try {
            ...
            _.each(_data.document, async (_doc) => {
                ...
                let _resp = {
                    ...
                    biltrlFrstKnee: _doc.biltrlFrstKnee,
                    surgDt: _doc.surgDt,
...
async function encryptData(_data) {
    return new Promise(async (resolve, reject) => {
        try {
            // _data = await CryptoJS.AES.encrypt(JSON.stringify(_data), _secretKey).toString();
            resolve(_data);
        }
        catch (err) {
            resolve(_data)
    ...
    try {
        if (req.headers && req.headers.orgkey) {
            ...
            if (!(_orgDtls && _orgDtls.length > 0)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', data: [], desc: "No Client found.." }));
            }
            ...
            return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: _orgObj }));
        }
        else {
            return res.status(400).json(await encryptData({ status: 'FAIL', data: [], desc: "Invalid Parameters" }));
        }
    } catch (error) {
        return res.status(500).json(await encryptData({ status: 'FAIL', desc: error.message || error }));
    }
});
...
router.post("/validate-doc-access", async (req, res) => {
    try {
        if (req.body && req.body.params.orgId && req.body.params.userId && req.body.params.routeUrl) {
            ...
            if (!_access) {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: "", data: [{ "access": _access }] }));
            }
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: [{ "access": _access }] }));
        }
        else {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: "Required Missing Parameters ..", data: [] }));
        }
    } catch (error) {
    ...
    try {
        if (req.body && req.body.encryptedData) {
            ...
            if (decrypted) {
                ...
                return res.status(498).json(await encryptData({ "ERROR": "Invalid key", "MESSAGE": "Invalid Key.." }));
                // req.body = req.body.encryptedData;
            }
        }
        next();
    } catch (ex) {
        return res.status(400).json(await encryptData({ "ERROR": "ERROR_WHILE_PREPARECPARAMS", "MESSAGE": ex.message }));
    }
});
...
router.post("/validate-user", async (req, res) => {
    try {
        if (req.body.params && req.body.params.userName && req.body.params.userName.length > 0) {
            ...
            mongoMapper("patient_care_users", "find", _filter, _dbType).then(async (result) => {
                if (result.data.length > 0 && !req.body.params.flag) {
                    return res.status(200).json(await encryptData({ success: false, status: 200, desc: `${req.body.params.userName} user is already exist`, data: [] }));
                }
                else if (result.data.length > 0 && req.body.params.flag == 'L') {
                    return res.status(200).json(await encryptData({ success: false, status: 200, desc: ``, data: result.data[0] || {} }));
                }
                else {
                    return res.status(200).json(await encryptData({ success: true, status: 200, desc: 'user not exist', data: [] }));
                }

            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
            });
        } else {
        ...
        return res.status(500).json(await encryptData({ success: false, status: 500, desc: error }));
    }
});
...
//                 "selectors": ""
//             }
//             let _userData = await _mUtils.commonMonogoCall("patient_care_users", "findOne", _uParams, "", "", "", req.headers.orgkey.toLowerCase());
...
//                 return res.status(400).json(await encryptData({ status: 'FAIL', code: 400, desc: "Invalid crendentials / No user found..", data: [] }));
//             }
//             let _clnUser = JSON.parse(JSON.stringify(_userData.data));
//             // let _orgDtls = _.filter(_users, (o) => { return o.orgId == req.body.orgId });
//             // if (!(_orgDtls && _orgDtls.length > 0)) {
//             //     return res.status(400).json({ status: 'FAIL', data: [], desc: "No Client found.." });
//             // }
...
//             //     return res.status(400).json({ status: 'FAIL', data: [], desc: "No User found.." });
//             // }
...
//                 let _sessionUpdate = await _mUtils.commonMonogoCall("patient_care_usersessions", "findOneAndUpdate", pLoadResp.payload, "", "", "", req.headers.orgkey.toLowerCase());
//                 if (!(_sessionUpdate && _sessionUpdate.success && _sessionUpdate.data && Object.keys(_sessionUpdate.data).length > 0)) {
//                     return res.status(401).json(await encryptData({ status: 'FAIL', desc: "The user session de-activation is failed..", data: [] }));
//                 }
//             }
//             //else {
//             let _filter = {
...
//             let _sessionData = await _mUtils.commonMonogoCall("patient_care_usersessions", "findOne", _filter, "", "", "", req.headers.orgkey.toLowerCase());
...
//                 return res.status(403).json(await encryptData({ status: 'FAIL', code: 403, desc: "This user is currently logged in on another device..", data: _sessionData.data || [] }));
//             }
//             // }

//             //find organisation against orgId;
//             let _orgDta = await getClientUrls(_filter.filter)
//             //
//             let _locDtls = _.filter(_userData.data.locations, (o) => { return o.locId == req.body.locId });
//             if (!(_locDtls && _locDtls.length > 0)) {
//                 return res.status(400).json(await encryptData({ status: 'FAIL', data: [], desc: "Provided Location was not mapped.." }));
//             }
//             let _user = {
//                 "orgId": _clnUser.orgId, "locId": _locDtls[0].locId, "userId": _clnUser._id, "userName": _clnUser.userName, "dbType": _orgDta.orgKey,
...
//             mongoMapper('patient_care_usersessions', 'insertMany', _sessionParams, req.headers.orgkey.toLowerCase()).then(async (result) => {
//                 // return res.status(200).json({ status: 'SUCCESS', desc: '', data: result.data });
//                 //console.log("result", result);
//                 if (!(result.status == 'SUCCESS' && result.data && result.data.length > 0)) {
//                     return res.status(400).json(await encryptData({ status: 'FAIL', desc: "Login failed / Failed to generate session", data: [] }));
//                 }
//                 _user['sessionId'] = result.data[0]._id;
//                 let _tkn = await generateToken(_user);
//                 _user['x-token'] = _tkn;
//                 _user['logInTime'] = result && result.data && result.data[0].logInTime ? result.data[0].logInTime : ""
//                 res.setHeader('sessionId', result.data[0]._id);
//                 return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: _user }));
//             }).catch(async (error) => {
//                 return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
//             });
//         }
//         else {
//             return res.status(400).json(await encryptData({ status: 'FAIL', data: [], desc: "Invalid Parameters" }));
...
router.post("/auth-user", async (req, res) => {
    try {
        if (req.body && req.body.orgId && req.body.uName && req.body.pwd && req.body.locId) {

            let _uParams = {
                "filter": {
                    'userName': { $eq: req.body.uName },
                    'password': { $eq: req.body.pwd },
                    "orgId": req.body.orgId,
                    "locations.locId": req.body.locId,
                    "recStatus": true
                },
                "selectors": ""
            }
            let _userData = await _mUtils.commonMonogoCall("patient_care_users", "find", _uParams, "", "", "", req.headers.orgkey.toLowerCase());
            // let _userData = await commonMonogoCall("patient_care_users", "findOne", _uParams, "", req.body);
            if (!(_userData && _userData.success && _userData.data && Object.keys(_userData.data).length > 0)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', code: 400, desc: "Invalid crendentials / No user found..", data: [] }));
            }
            ...
            if (req.body.forceLogout && req.body.sessionId) {
                ...
                _tknObj.params["audit.modifiedBy"] = _clnUser[0].dispName;
            ...
            // }
            let _rParams = {
                "filter": { "orgId": req.body.orgId },
                "selectors": "-audit -history"
            }
            let _mResp = await _mUtils.commonMonogoCall("patient_care_roles", "find", _rParams, "", "", "", req.headers.orgkey.toLowerCase());
            ...
            let _orgDta = await getClientUrls(_filter.filter)
            //
            let _locDtls = _.filter(_userData.data[0].locations, (o) => { return o.locId == req.body.locId });
            if (!(_locDtls && _locDtls.length > 0)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', data: [], desc: "Provided Location was not mapped.." }));
            }
            ...
            let _user = {
                "orgId": _clnUser[0].orgId, "locId": _locDtls[0].locId, "userId": _clnUser[0]._id, "userName": _clnUser[0].userName, "dbType": _orgDta.orgKey,
                "displayName": _clnUser[0].dispName, "roleId": _locDtls[0].roleId, "roleName": _locDtls[0].roleName, docAccess: _docString
            };
            let _tkn = await generateToken(_user);
            res.cookie('x-token', _tkn, { maxAge: 9000000, httpOnly: true });
            _user['x-token'] = _tkn;
            ...
            let _sessionParams = {
                orgId: _clnUser[0].orgId,
                locId: _user.locId,
                userId: _clnUser[0]._id,
                userName: _clnUser[0].userName,
                displayName: `${_clnUser[0].titleName}${_clnUser[0].dispName}`,
                roleId: _user.roleId,
                roleName: _user.roleName,
                browser: _userAgent,
                token: _tkn,
            ...
            mongoMapper('patient_care_usersessions', 'insertMany', _sessionParams, req.headers.orgkey.toLowerCase()).then(async (result) => {
                ...
                _user['x-token'] = _tkn;
                _user['sessionId'] = result.data[0]._id;
                _user['logInTime'] = result.data[0].logInTime;
                res.setHeader('sessionId', result.data[0]._id);
                return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: _user }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
...
router.use(async function verifyToken(req, res, next) {
    ...
    //     return res.status(400).json({ status: 'FAIL', data: [], desc: "Missing Token ." });
    // }
    ...
    try {
        ...
        let _sessionData = await commonMonogoCall("patient_care_usersessions", "findOne", _filter, "", req.body, "", "", req.headers.orgkey.toLowerCase());
        //  console.log("_sessionData", _sessionData);
        if (!(_sessionData && _sessionData.success && _sessionData.data && Object.keys(_sessionData.data).length > 0 && _sessionData.data.token && _sessionData.data.token.length > 0)) {
            return res.status(401).json(await encryptData({ status: 'FAIL', desc: `Session not found :error -> ${_sessionData.desc}` || "Missing Token..", data: [] }));
        }
        _token.verifyToken(_sessionData.data.token).then(async (data) => {
            ...
            if (!pLoadResp.success) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: pLoadResp.desc || "", data: [] }));
            }
            let _sessionUpdate = await commonMonogoCall("patient_care_usersessions", "findOneAndUpdate", pLoadResp.payload, "", "", "", "", req.tokenData.dbType)
            if (!(_sessionUpdate && _sessionUpdate.success)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: _sessionUpdate.desc || "Failed to update token", data: [] }));
            }
            ...
            if (error.name && error.name == 'TokenExpiredError') {
                return res.status(401).json(await encryptData({
                    "code": 401,
        ...
        return res.status(500).json(await encryptData({ status: 'FAIL', desc: err.message || err, data: [] }));
    }
});
...
async function commonMonogoCall(_method, _query, _params, _flag, _body, _filter, _tknData, _key) {
    try {
        return new Promise((resolve, reject) => {
...
router.post("/get-all-patient-data", async (req, res) => {
    try {
        ...
        mongoMapper("patient_care_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
            ...
            return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: _filteredPatients || [] }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
        });
    } catch (error) {
        return res.status(500).json(await encryptData({ status: 'FAIL', desc: error }));
    }
...
router.post('/get-patient-by-umr', async function (req, res) {
    try {
        ...
        if (req.body && req.body.params && req.body.params.flag && (req.body.params.flag == 'NAME' || req.body.params.flag == 'MOB' || req.body.params.flag == 'ADMN' || req.body.params.flag == 'UMR' || req.body.params.flag == 'DOS') && req.body.params.searchValue && req.body.params.searchValue.length > 0) {
            ...
            //     req.body.params.searchValue = moment(req.body.params.searchValue).format('YYYY-MM-DD')
            //     _params = {
            //         "orgId": req.tokenData.orgId, "locId": req.tokenData.locId,
            //         //"surgeries.$.documentedDt": { $eq: new Date(new Date(req.body.params.searchValue).setUTCHours(0, 0, 0, 0)).toISOString() }
            //         // "surgeries.documentedDt": { $gte: new Date(new Date(req.body.params.searchValue).setUTCHours(0, 0, 0, 0)).toISOString(), 
            //         //                             $lte: new Date(new Date(req.body.params.searchValue).setUTCHours(23, 59, 59, 999)).toISOString() }
            //         "surgeries.documentedDt": {$regex:`^${req.body.params.searchValue}`}
            //     }
            // }
            else {
                _params = {
            ...
            if (req.body.params.flag == 'DOS') {
                req.body.params.searchValue = moment(req.body.params.searchValue, 'DD-MMM-YYYY').format('YYYY-MM-DD')
                _filter = {
                    "filter": [
                        {
                            $match: {
                                "surgeries.surgDtTm": { $regex: `^${req.body.params.searchValue}` }
                            }
                        },
                        {
                            $sort: {
                                "surgeries.surgDtTm": 1
                            }
                        }
                    ]
                }
                mongoMapper("patient_care_patients", "aggregation", _filter, req.tokenData.dbType).then(async (_resp) => {
                    if (_resp.data.length > 0) {
                        let _response = _resp.data;
                        let baseMap = fetchTransformColumns("getPatientByIdWithDOS");

                        _.each(_response, (_o, _i) => {
                            let _nameWithSpace = _o.disName.split(' ');
                            _o.fName = _nameWithSpace && _nameWithSpace.length > 1 ? _nameWithSpace[0] : _nameWithSpace;
                            _o.lName = _nameWithSpace && _nameWithSpace.length > 1 ? _nameWithSpace[1] : "";
                            _o['phone'] = _o.phone && _o.phone[0].num ? _o.phone[0].num : ""
                            if (!_o.dob) {
                                _o['dob'] = _o.age ? moment(moment().subtract(_o.age.match(/\d+/g)[0], 'years')
                                    .subtract(_o.age.match(/\d+/g)[1], 'months')
                                    .subtract(_o.age.match(/\d+/g)[2], 'days')) : "";
                            }
                            // _o['dob'] = _o.age ? moment(moment().subtract(_o.age.match(/\d+/g)[0], 'years')
                            //     .subtract(_o.age.match(/\d+/g)[1], 'months')
                            //     .subtract(_o.age.match(/\d+/g)[2], 'days')) : "";
                        ...
                        return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: ``, data: _response || [] }));
                    }
                    else {
                        return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: 'no data', data: [] }));
                    }
                ...
                mongoMapper("patient_care_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
                    ...
                    // if(req.body.params.flag == 'DOS'){
                    //     let _response = JSON.parse(JSON.stringify(result.data));
                    //     let baseMap = fetchTransformColumns("getPatientByIdWithDOS");
                    //     _response = transform(_response, baseMap)
                    //     let _finalResp = _mResp.concat(_response)
                    //     let _findDuplicatesAndCombinedData = await findDuplicatesAndCombinedData(JSON.parse(JSON.stringify(_finalResp)).reverse(), 'umr');
                    //     _.map(_findDuplicatesAndCombinedData, (_o)=>{_o['mobile'] = _o.mobile && _o.mobile[0].num? _o.mobile[0].num: ""})
                    //     return res.status(200).json(await encryptData({ status: "SUCCESS", desc: '', data: _findDuplicatesAndCombinedData.reverse() || [] }));
                    // }
                    _params = {
                        FLAG: req.body.params.flag,
                        SRCH_VAL: req.body.params.searchValue,
                        SRCH_VAL1: req.body.params.searchValue1 || "",
                        ORG_ID: 1003 || req.tokenData.orgId || 1067
                    }
                    /// console.log("req.clientUrls",req.clientUrls)
                    axios.post(req.clientUrls.emrApiUrl + '/uprGetPatDetAutoSrch', _params).then(async (response) => {
                        let _response = response.data;
                        //_response = _.uniqBy(_response, 'UMRNO') 
                        let baseMap = fetchTransformColumns("getPatientById");
                        _response = transform(_response, baseMap);
                        _.each(_response, (_o, _i) => {
                            // let _nameWithSpace = _o.disName.split(' ');
                        ...
                        if (_response.length == 0) {
                            baseMap = fetchTransformColumns("getPatientByIdWithDOS");
                            _finalResp = _mResp.concat(_response)
                            _finalResp = transform(_finalResp, baseMap);
                        }


                        if (req.body.params.flag == "ADMN") {
                            _finalResp = _finalResp.sort((a, b) => {
                                if (a.admnDt < b.admnDt) { return -1 };
                                if (a.admnDt > b.admnDt) { return 1 };
                                return 0;
                            })
                            return res.status(200).json(await encryptData({ status: "SUCCESS", desc: '', data: _finalResp.reverse() || [] }));
                        }
                        let _findDuplicatesAndCombinedData = await findDuplicatesAndCombinedData(JSON.parse(JSON.stringify(_finalResp)).reverse(), 'umr');
                        _findDuplicatesAndCombinedData = _findDuplicatesAndCombinedData.sort((a, b) => {
                            if (a.admnDt < b.admnDt) { return -1 };
                            if (a.admnDt > b.admnDt) { return 1 };
                            return 0;
                        })
                        ...
                        return res.status(400).json(await encryptData({ status: "FAIL", desc: '', data: err }));
                    })
                    // }
                }).catch(async (err) => {
                    return res.status(400).json(await encryptData({ status: 'FAIL', data: [], desc: err }));
                })
            }
        }
        else {
        ...
        return res.status(500).json(await encryptData({ status: 'FAIL', desc: error, data: [], }));
    }
...
router.post("/get-patient-by-id", async (req, res) => {
    try {
        ...
        mongoMapper("patient_care_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
            _parseData = JSON.parse(JSON.stringify(result))
            if (_parseData && _parseData.data) {
                for (let _o in _parseData.data) {
                    _parseData.data[_o]['firstSurgeryBranch'] = _parseData.data[_o] && _parseData.data[_o].surgeries && _parseData.data[_o].surgeries.length > 0 && _parseData.data[_o].surgeries[0].surgeryBranch && _parseData.data[_o].surgeries[0].surgeryBranch.length > 0 ? _parseData.data[_o].surgeries[0].surgeryBranch : '';
                    _parseData.data[_o]['firstSurgeryIntraOPComp'] = _parseData.data[_o] && _parseData.data[_o].surgeries && _parseData.data[_o].surgeries.length > 0 && _parseData.data[_o].surgeries[0].surgeryIntraOPComp && _parseData.data[_o].surgeries[0].surgeryIntraOPComp.length > 0 ? _parseData.data[_o].surgeries[0].surgeryIntraOPComp : ''
                    _parseData.data[_o]['tags'] = _parseData.data[_o] && _parseData.data[_o].surgeries && _parseData.data[_o].surgeries.length > 0 && _parseData.data[_o].surgeries[0].tags && _parseData.data[_o].surgeries[0].tags.length > 0 ? _parseData.data[_o].surgeries[0].tags : ''

                    _.filter(_parseData.data[_o].comorbidities, (_c) => {
                        if ((_c.cd && _c.cd == "DIABETES") && _c.value == "Y") {
                            return _parseData.data[_o]['firstSurgeryDiab'] = "Yes";
                        }
                    })

                    let _scores = []
                    _.filter(_parseData.data[_o].scores, (_s) => {
                        if (_s.surgId && _s.surgId == _parseData.data[_o].surgeries[0].surgId) {
                            return _scores.push({ name: `${_s.scoreName}`, val: `${_s.totalScore}`, percentage: `${_s.percentage}` })
                        }
                    });
                    _parseData.data[_o]['firstSurgeryScores'] = _scores
                    let _surgeryDta = _.filter(_parseData.data[_o].surgeries, (_s) => { return !_s.surgeryClosed });
                    if (!(_surgeryDta && _surgeryDta.length > 0) && _parseData.data[_o].surgeries && _parseData.data[_o].surgeries.length > 0) {
                        _parseData.data[_o].isCompleted = true;
                    }
                    else {
                        _parseData.data[_o].isCompleted = false;
                    }
                };
            }
            return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: _parseData.data || [] }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
        });
    } catch (error) {
...
router.post("/insert-patient", async (req, res) => {
    try {
        ...
        let _patientData = await commonMonogoCall("patient_care_patients", "findOne", _filter, "", req.body, "", "", req.tokenData.dbType)
        if ((_patientData && _patientData.success && _patientData.data && _patientData.data.umr && _patientData.data.umr.length > 0)) {
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: "Patient already exists..", data: [] }));
        }
        req.body.params["orgId"] = req.tokenData.orgId;
        req.body.params["locId"] = req.tokenData.locId;
        mongoMapper('patient_care_patients', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
        });
    } catch (error) {
...
router.post("/update-patient-details", async (req, res) => {
    try {
        let _patient = await commonMonogoCall("patient_care_patients", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData, req.tokenData.dbType)
        if (!(_patient && _patient.success)) {
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: _patient.desc || "", data: _patient.data || [] }));
        }
        ...
        mongoMapper('patient_care_patients', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
        });
...
router.post("/get-patient-all-surgeries", async (req, res) => {
    try {
        mongoMapper("patient_care_patients", req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: result.data.surgeries }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
        });
    } catch (error) {
...
router.post("/insert-surgery-data", async (req, res) => {
    try {
        if (req.body.params.patientId && req.body.params.umr) {
            ...
            let _params = {};
            let _surgery = await commonMonogoCall("patient_care_surgeries", "findOne", _filter, "", req.body, "", "", req.tokenData.dbType)
            if (!(_surgery && _surgery.success)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: _surgery.desc || "", data: _surgery.data || [] }));
            }
            let __admns = _.filter(_surgery.data.admns, function (o) { return o.admNo == req.body.params.admns[0].admnNo });
            if (__admns && __admns.length > 0) {
                let _surgData = _.filter(__admns[0].surgDtls, function (o) { return o.audit.documentedById == req.tokenData.userId && o.surgery == req.body.params.admns[0].surgDtls.surgery && o.type == req.body.params.admns[0].surgDtls.type && o.ps == req.body.params.admns[0].surgDtls.ps && o.patInfo.biltrlFrstKnee == req.body.params.admns[0].surgDtls.patInfo.biltrlFrstKnee });
                if (_surgData && _surgData.length > 0) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: "This Surgery Already Exists For This Admission", data: [] }));
                }
            }
            if (!_surgery.data.umr) {
                ...
                let surgeryAdmns = JSON.parse(JSON.stringify(_surgery.data.admns));
                let _admn = _.filter(surgeryAdmns, function (o) { return o.admNo == req.body.params.admns[0].admNo });

                if (_admn.length > 0) {
                    req.body.params["admns"][0]._id = _admn[0]._id;
                    //_surgery.data.admns[0]._id;
                }
                if (_admn && _admn.length == 0) {
                    delete req.body.params.dg;
                }
                let pLoadResp = { payload: {} };
            ...
            mongoMapper('patient_care_surgeries', _query, _params, req.tokenData.dbType).then(async (result) => {
                ...
                if (_resp && _resp.admns && _resp.admns.length > 0) {
                    ...
                    if (_admns && _admns.length > 0) {
                        let _surgData = _.filter(_admns[0].surgDtls, function (o) { return o.audit.documentedById == req.tokenData.userId && o.surgery == req.sParams.surgDtls.surgery && o.type == req.sParams.surgDtls.type && o.ps == req.sParams.surgDtls.ps && o.patInfo.biltrlFrstKnee == req.sParams.surgDtls.patInfo.biltrlFrstKnee });
                        if (_surgData && _surgData.length > 0) {
                            let _patObj = {
                                params: {
                                    ...
                                    surgeries:
                                    {
                                        ...
                                        disDt: _surgData[0].patInfo && _surgData[0].patInfo.disDt ? _surgData[0].patInfo.disDt : "",
                                        ...
                                        surgeryClosedBy: "",
                                        surgeryClosedDt: "",
                                        surgeryBranch: _surgData[0].patInfo.hospName,
                                        surgeryIntraOPComp: _surgData[0].intraOpDet.intOpComp,
                                        surgDtTm: _surgData[0].patInfo && _surgData[0].patInfo.surgDtTm && _surgData[0].patInfo.surgDtTm,
                                        tags: _surgData[0].tags,
                                        reviewDt: "",
                                        reviewRemarks: "",
                                        ...
                                        radiologicalOutcomes: _surgData[0].mandFilledInRadioOutcomes,
                                        additionalDetails: _surgData[0].mandFilledInAdditDtls,
                                        periOperativePeriod: _surgData[0].mandFilledInPeriop,
                                        progressStatus: 'PENDING',
                                        unilatKnee: req.sParams.surgDtls && req.sParams.surgDtls.patInfo && req.sParams.surgDtls.patInfo.unilatKnee ? req.sParams.surgDtls.patInfo.unilatKnee : "",
                                        biltrlFrstKnee: req.sParams.surgDtls && req.sParams.surgDtls.patInfo && req.sParams.surgDtls.patInfo.biltrlFrstKnee ? req.sParams.surgDtls.patInfo.biltrlFrstKnee : "",
                                        documentedId: req.tokenData.userId || "",
                                        documentedBy: req.tokenData.userName || ""
                                    }
                                }
                            ...
                            let _surgUpdate = await commonMonogoCall("patient_care_patients", "updateOne", pLoadResp.payload, "", "", "", "", req.tokenData.dbType)
                            console.log("_surgUpdate", _surgUpdate);
                    ...
                    return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: _surgId }));
                }
                else {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: 'Record Not Saved ..,', data: [] }));
                }
            ...
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: "Require Parameters ..", data: [] }));
        }
    } catch (error) {
        console.log("error", error);
        return res.status(500).json(await encryptData({ status: 'FAIL', desc: error.message || error }));
...
router.post("/get-surgery-list-by-umr-old", async (req, res) => {
    try {
        if (req.body.params.umr) {
            ...
            mongoMapper("patient_care_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
                if (!(result.data && result.data[0] && result.data[0]._id && result.data[0].surgeries && result.data[0].surgeries.length > 0)) {
                    return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: "No Records are available.." || error, data: [] }));
                }
                ...
                _.each(_data, (_sObj) => {
                    ...
                    _sObj["formattedSurgName"] = _sObj.surgName;
                ...
                return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: _data ? _data.reverse() : [] || [] }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
            });
        }
...
router.post("/get-surgery-list-by-umr", async (req, res) => {
    try {
        if (req.body.params.umr) {
            ...
            let _filter = {
                "filter": req.body.params,
                "selectors": "-comorbidities -scores"
            }
            mongoMapper("patient_care_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
                if (!(result.data && result.data[0] && result.data[0]._id && result.data[0].surgeries && result.data[0].surgeries.length > 0)) {
                    return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: "No Records are available.." || error, data: [] }));
                ...
                let _data = JSON.parse(JSON.stringify(result.data[0].surgeries));
                let _docsData = JSON.parse(JSON.stringify(result.data[0].document));
                let _clnFollowupsData = [];
                _.each(_data, (_sObj) => {
                    _sObj["umr"] = result.data.umr;
                    _sObj["fName"] = result.data.fName;
                    ...
                    _sObj["dob"] = result.data.dob;
                    _sObj["formattedSurgName"] = _sObj.surgName;
                    let _docs = _.filter(_docsData, (_d) => _d.surgId == _sObj.surgId)
                    let _org = _.filter(_orgDetails, (o) => { return o.orgKey == req.headers.orgkey });
                    let uploadedDocuments;
                    if (_org.length > 0 && _org[0].uploadedDocuments && _org[0].uploadedDocuments.length > 0 && Object.keys(_org[0].uploadedDocuments[0]).length > 0) {
                        let _mandDocs = _.keys(_.pickBy(_org[0].uploadedDocuments[0], val => val == true));
                        uploadedDocuments = _mandDocs.reduce((acc, key) => {
                            let _doc = key.toLowerCase()
                            acc[key] = _docs && _docs.length > 0 ? _docs.some(item => item.docType.toLowerCase().split(" ").join("") == key.toLowerCase()) : false;
                            return acc
                        }, {})
                        _sObj['uploadDocuments'] = uploadedDocuments
                    }
                    if (_sObj.followups && _sObj.followups.length > 0) {
                        _clnFollowupsData.push(_sObj);
                    }
...
router.post("/get-surgery-data-by-id", async (req, res) => {
    try {
        if (req.body.params.admNo && req.body.params.surgId) {
            ...
            mongoMapper("patient_care_surgeries", "findOne", _params, req.tokenData.dbType).then(async (result) => {
                if (!(result.data && result.data._id)) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No Records are available.." || error, data: [] }));
                }
                let _surgData = _.filter(result.data.admns[0].surgDtls, (o) => { return o._id == req.body.params.surgId })
                if (_surgData && _surgData.length > 0) {
                    _surgData = JSON.parse(JSON.stringify(_surgData)).map(doc => {
                        if (doc.intraOpDet) {
                            for (const key in doc.intraOpDet) {
                                if (Array.isArray(doc.intraOpDet[key]) && doc.intraOpDet[key].length == 0) {
                                    delete doc.intraOpDet[key];
                                }
                            }
                        }
                        return doc;
                    ...
                    return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: _sData || [] }));
                }
                else {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No Records are available.." || error, data: [] }));
                }
    ...
    try {
        if (req.body.params._id && req.body.params.admns && req.body.params.admns[0] && req.body.params.admns[0]._id && req.body.params.admns[0].surgDtls && req.body.params.admns[0].surgDtls[0] && req.body.params.admns[0].surgDtls[0]._id && (req.body.params.admns[0].surgDtls[0].revNo || req.body.params.admns[0].surgDtls[0].revNo == 0)) {
            ...
            if (!(_sData && _sData.length > 0)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No Records are available..", data: [] }));
            }
            ...
            if (req.body.params.admns[0].surgDtls[0].revNo == _sData[0].revNo) {
                ...
                return res.status(400).json(await encryptData({
                    status: 'FAIL',
                    data: [{
            ...
            pLoadResp = await _mUtils.preparePayload(req.body.flag, req.body);
            if (!pLoadResp.success) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: pLoadResp.desc || "", data: [] }));
            }
            ...
            if (_cloneBody.flag === "FOLLOWUP") {
                ...
                _cloneBw.updateOne.filter = { "_id": pLoadResp.payload._id, "admns._id": req.body.params.admns[0]._id, "admns.surgDtls._id": JSON.parse(JSON.stringify(_sData[0]._id)) };
                pLoadResp.payload.pData.push(_cloneBw);
            }
            ...
            */
            ...
            pLoadResp.payload.pData.push(_cloneBw);
            mongoMapper('patient_care_surgeries', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                let pLoadResp = { payload: {} };
                ...
                if (_cloneBody.flag !== "REVIEW" && _cloneBody.flag !== "FOLLOWUP") {
                    _patObj.params.surgeries =
                        [{
                            ...
                            radiologicalOutcomes: _updSurgData.mandFilledInRadioOutcomes,
                            additionalDetails: _updSurgData.mandFilledInAdditDtls,
                            periOperativePeriod: _updSurgData.mandFilledInPeriop,
                        }];

                    if (_updSurgData && _updSurgData.patInfo && _updSurgData.patInfo.hospName) {
                        _patObj.params.surgeries[0]['surgeryBranch'] = _updSurgData.patInfo.hospName
                    }
                    if (_updSurgData && _updSurgData.intraOpDet && _updSurgData.intraOpDet.intOpComp) {
                        _patObj.params.surgeries[0]['surgeryIntraOPComp'] = _updSurgData.intraOpDet.intOpComp
                    }
                    if (_updSurgData && _updSurgData.tags) {
                        _patObj.params.surgeries[0]['tags'] = _updSurgData.tags
                    }
                    if (_updSurgData && _updSurgData.patInfo && _updSurgData.patInfo.unilatKnee) {
                        _patObj.params.surgeries[0]['unilatKnee'] = _updSurgData.patInfo.unilatKnee
                    }
                    if (_updSurgData && _updSurgData.patInfo && _updSurgData.patInfo.biltrlFrstKnee) {
                        _patObj.params.surgeries[0]['biltrlFrstKnee'] = _updSurgData.patInfo.biltrlFrstKnee
                    }
                    pLoadResp = await _mUtils.preparePayload('BW', _patObj);
                    if (!pLoadResp.success) {
                        return res.status(400).json(await encryptData({ status: 'FAIL', desc: pLoadResp.desc || "", data: [] }));
                    }
                    let _patient = await commonMonogoCall("patient_care_patients", "bulkWrite", pLoadResp.payload, "", req.body, "", "", req.tokenData.dbType);
                }
                ...
                if (_cloneBody.flag === "REVIEW") {
                    ...
                    let _reviewData = await commonMonogoCall("patient_care_reviews", "insertMany", _reviewParams, "", "", "", "", req.tokenData.dbType)
...
router.post("/print-surgery-data", async (req, res) => {
    try {
        ...
        mongoMapper("patient_care_surgeries", "findOne", _params, req.tokenData.dbType).then(async (result) => {
            ...
            let _patientData = await commonMonogoCall("patient_care_patients", "findOne", _patParams, "", req.body, "", "", req.tokenData.dbType)
            if (!(_patientData && _patientData.success && _patientData.data && _patientData.data.umr && _patientData.data.umr.length > 0)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No patient was found", data: [] }));
            }
            let _scoresParams = {
                "filter": { 'umr': req.body.params.umr, "orgId": req.tokenData.orgId, "locId": req.tokenData.locId },
                "selectors": ""
            ...
            let _scores = await commonMonogoCall("patient_care_scores", "find", _scoresParams, "", req.body, "scores", "", req.tokenData.dbType)
...
router.post("/insert-scores-data-deprecated", async (req, res) => {
    try {
        if (req.body.params.umr) {
            ...
            let _patient = await commonMonogoCall("patient_care_patients", "findOne", _filter, "", req.body, "", "", req.tokenData.dbType)
            if (!(_patient && _patient.success)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: _patient.desc || "", data: _patient.data || [] }));
            }
            delete req.body.params.umr;
            ...
            mongoMapper('patient_care_patients', "findOneAndUpdate", pLoadResp.payload).then(async (result) => {
                if (result.data && result.data._id && result.data.scores && result.data.scores.length > 0) {
                    ...
                    if (_scoreData && _scoreData.length > 0) {
                        ...
                        return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: { "_id": _scoreId } }));
                    }
                    else {
                        return res.status(400).json(await encryptData({ status: 'FAIL', desc: 'Record Not Saved ..,', data: [] }));
                    }
...
router.post("/update-score-details-deprecated", async (req, res) => {
    try {
        if (req.body.params._id && req.body.params.scores && req.body.params.scores[0] && req.body.params.scores[0]._id) {
            ...
            let _patient = await commonMonogoCall("patient_care_patients", "findOne", _params, "REVNO", { params: req.body.params.scores[0] }, "scores", "", req.tokenData.dbType)
            if (!(_patient && _patient.success)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: _patient.desc || "", data: _patient.data || [] }));
            }
...
router.post("/insert-scores-data", async (req, res) => {
    try {
        if (req.body.params.patientId && req.body.params.umr) {
            let _patient = await commonMonogoCall("patient_care_patients", "findById", req.body.params.patientId, "", req.body, "", "", req.tokenData.dbType)
            if (!(_patient && _patient.success)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: _patient.desc || "", data: _patient.data || [] }));
            }
            req.body.params["orgId"] = req.tokenData.orgId;
            ...
            let _cloneBody = JSON.parse(JSON.stringify(req.body.params));
            delete req.body.params.supporttedObj;
            mongoMapper("patient_care_scores", "insertMany", req.body.params, req.tokenData.dbType).then(async (result) => {
                    ...
                    params: {
                        ...
                        scores:
                            [{
                                ...
                                biltrlFrstKnee: result.data[0].biltrlFrstKnee,
                                selKnee: result.data[0].selKnee || "",
                ...
                if (_filteredSurgData && _filteredSurgData.length > 0) {
                    if (_filteredSurgData[0].progressStatus != 'COMPLETED') {
                        let _mandStatus = _.pick(_filteredSurgData[0], 'documents', 'intraOperative', 'preOperative', 'prosthesis', 'radiologicalOutcomes', 'additionalDetails', 'periOperativePeriod');
                ...
                let _patUpdate = await commonMonogoCall("patient_care_patients", "bulkWrite", pLoadResp.payload, "", "", "", "", req.tokenData.dbType)
    ...
    try {
        if (req.body.params.umr) {
            ...
            mongoMapper("patient_care_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
                ...
                let mRdata = []
                let _gSurgData = _.groupBy(result.data[0].scores, 'surgId')
                let _sData = _.map(_gSurgData, (_v, _k) => {
                    let gData = _.groupBy(_v, 'scoreCd');
                    let mData = _.map(gData, (gVal, gKey) => {
                        let _gObj = gVal.reverse();
                        if (gVal.length > 0) {
                            _.each(gVal, (_sVal, _sKey) => {
                                mRdata.push({
                                    "_id": _sVal.scoreId,
                                    "patLvlScoreId": _sVal._id,
                                    "admn": _sVal.admn || "",
                                    "admnDt": _sVal.admnDt || "",
                                    "surgId": _sVal.surgId || "",
                                    "surgName": _sVal.surgName || "",
                                    "biltrlFrstKnee": _sVal.biltrlFrstKnee || "",
                                    "selKnee": _sVal.selKnee || "",
                                    "surgDtTm": _sVal.surgDtTm || "",
                                    "scoreCd": gKey,
                                    "scoreName": _sVal.scoreName || "",
                                    "totalScore": _sVal.totalScore || "",
                                    "percentage": _sVal.percentage || "",
                                    "followupId": _sVal.followupId || "",
                                    "followupNo": _sVal.followupNo || "",
                                    "followupOn": _sVal.followupOn || "",
                                    "followupDate": _sVal.followupDate || "",
                                    "documentedId": _sVal.documentedId,
                                    "documentedBy": _sVal.documentedBy,
                                    "documentedDt": _sVal.documentedDt
                                })
                            })
                        }
                ...
                //     }
                // });
                return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: mRdata || [] }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
            });
        }
...
//                     }
//                 });
//                 return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: mData || [] }));
//             }).catch(async (error) => {
//                 return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
//             });
//         }
...
router.post("/get-scores-data-by-id", async (req, res) => {
    try {
        if (req.body.params._id) {
            mongoMapper("patient_care_scores", "findById", req.body.params._id, req.tokenData.dbType).then(async (result) => {
                if (!(result.data && result.data._id)) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No Records are available.." || error, data: [] }));
                }
                return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: result.data || [] }));
...
router.post("/update-score-details", async (req, res) => {
    try {
        if (req.body.params._id) {
            let _patient = await commonMonogoCall("patient_care_scores", "findById", req.body.params._id, "REVNO", req.body, "scores", "", req.tokenData.dbType)
            if (!(_patient && _patient.success)) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: _patient.desc || "", data: _patient.data || [] }));
            }
            ...
            _cloneBw.updateOne.filter = { "_id": pLoadResp.payload._id };
            pLoadResp.payload.pData.push(_cloneBw);
            mongoMapper('patient_care_scores', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                let _patObj = {
                    params: {
                ...
                _.each(req.body.params, (_val, _key)=>{
                    if(_key == "surgName" || _key == "surgId" || _key == "surgDtTm" || _key == "biltrlFrstKnee" || _key == "selKnee" || _key == "admn" || _key == "admnDt"){
                        _patObj.params.scores[0][`${_key}`] = _val
                    }
                })
                let pLoadResp = { payload: {} };
                pLoadResp = await preparePayload('BW', _patObj);
                ...
                // pLoadResp.payload.pData.push(_cloneBw);
                let _patient = await commonMonogoCall("patient_care_patients", "bulkWrite", pLoadResp.payload, "", req.body, "", "", req.tokenData.dbType)
                return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: result }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
            });
        }
    ...
    try {
        if (req.body.params.patientId && req.body.params.umr && req.body.params.docs) {
            ...
            _.each(req.body.params.docs, (_doc) => {
                let _docExtension = _doc.docData.split(';')[0].split('/')[1];
                ...
                if (_docExtension == 'png' || _docExtension == 'jpg' || _docExtension == 'jpeg') {
                    //console.log("_base64data--png", _base64data)
                    _doc["docData"] = new Buffer.from(_base64data, 'base64');
                    //console.log("_doc", _doc.docData)
                }
                else {
                    //console.log("_base64data", _base64data)
                    let _binaryData = Buffer.from(_base64data, 'base64');
                    //console.log("_binaryData", _binaryData)
            ...
            //console.log("req.body.params.docs", req.body.params.docs)
            mongoMapper("patient_care_documents", "insertMany", req.body.params.docs, req.tokenData.dbType).then(async (result) => {
                if (!(result.data && result.data.length > 0)) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No Records are available.." || error, data: [] }));
                }
                //console.log("result", result)
                        ...
                        if (_rObj.docName == _doc.docName) {
                            try {
                                ...
                                if (_extension == 'png' || _extension == 'jpg' || _extension == 'jpeg') {
                                    //console.log("_doc.docData", _doc.docData)
                                ...
                                if (_docData.success && _doc.docName == _docData.name) {
                                    ...
                                    let pLoadResp = { payload: {} };
                                    //console.log("_patObj", _patObj)
                                    pLoadResp = await preparePayload('IAU', _patObj);
                                    if (!pLoadResp.success) {
                                        // return res.status(400).json({ status: 'FAIL', desc: pLoadResp.desc || "", data: [] });
                                    }
                                    //console.log("pLoadResp.payload", pLoadResp.payload)
                                    let _patUpdate = await commonMonogoCall("patient_care_patients", "updateOne", pLoadResp.payload, "", "", "", "", req.tokenData.dbType)
                                    if (!(_patUpdate && _patUpdate.success)) {
                                        // return res.status(400).json({ status: 'FAIL', desc: _surgUpdate.desc || "", data: _surgUpdate.data || [] });
                                    }
                                    //console.log("_patUpdate", _patUpdate)
                                }
                            }
                            catch (err) {
    ...
    try {
        if (req.body.params.patientId && req.body.params.umr && req.body.params.docs) {
            ...
            mongoMapper("patient_care_documents", "insertMany", req.body.params.docs, req.tokenData.dbType).then(async (result) => {
                ...
                for (let _rObj of result.data) {
                    for (let _doc of _clnDocs.docs) {
                        if (_rObj.docName == _doc.docName) {
                            try {
                                let _extension = _doc.docData.split(';')[0].split('/')[1];
                                let _docData = { success: false, data: [] };
                ...
                let _patData = await commonMonogoCall("patient_care_patients", "findById", req.body.params.patientId, "", req.body, "", "", req.tokenData.dbType)
                if (!(_patData && _patData.success)) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: _patData.desc || "", data: _patData.data || [] }));
                }

                let _clnSurg = JSON.parse(JSON.stringify(_patData.data.surgeries));
                let _filteredSurgData = _.filter(_clnSurg, (_o) => { return _o._id == _clnDocs.supporttedObj.selPatSurgId });
                if (_filteredSurgData && _filteredSurgData.length > 0) {
                    if (_filteredSurgData[0].progressStatus != 'COMPLETED') {
                        let _mandStatus = _.pick(_filteredSurgData[0], 'scores', 'intraOperative', 'preOperative', 'prosthesis', 'radiologicalOutcomes', 'additionalDetails', 'periOperativePeriod');
                        let _status = _.filter(_mandStatus, (_o) => { return !_o });
                        let _docStatus = false
                        if (_patData.data.document.length > 0) {
                            let _uploadDoc = _.filter(_patData.data.document, (_uplDoc) => { return _uplDoc.surgId == _filteredSurgData[0].surgId })
                            let _org = _.filter(_orgDetails, (o) => { return o.orgKey == req.headers.orgkey });
                            let _mandDocs = _.keys(_.pickBy(_org[0].uploadedDocuments[0], val => val == true))
                            let _docTypes = []
                            let _orgDocs = _.map(_mandDocs, _d => _d.toLowerCase())
                            _.each(_uploadDoc, (_uplDocs) => {
                                let __doc = _uplDocs.docType.toLowerCase().split(" ").join("");
                                _docTypes.push(__doc)
                            })
                            _docStatus = _.every(_orgDocs, (_doc) => _.includes(_docTypes, _doc))
                        }
                        let _patObj = {
                            params: {
                                _id: _clnDocs.patientId,
                                surgeries: [{
                                    _id: _clnDocs.supporttedObj.selPatSurgId,
                                    documents: _docStatus,
                                    progressStatus: _status.length == 0 && _docStatus ? 'COMPLETED' : 'PENDING'
                                }]
                            }
                        };
...
router.post("/get-upload-documents-by-umr", async (req, res) => {
    try {
        if (req.body.params._id) {
            ...
            mongoMapper("patient_care_patients", "find", _params, req.tokenData.dbType).then(async (result) => {
                if (!(result.data && Object.keys(result.data).length > 0)) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No Records are available.." || error, data: [] }));
                }
                result.data[0].document = _.filter(result.data[0].document, (_doc)=> { return _doc.recStatus == true })
                let _docs = await convertBase64ToBuffer(result.data[0]);
                if (!_docs.success) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: "Failed to convert base64 from buffer..", data: [] }));
                }
                return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: _docs.data || [] }));
...
router.post("/get-upload-documents-by-id", async (req, res) => {
    try {
        if (req.body.params._id) {
            mongoMapper("patient_care_documents", "findById", req.body.params._id, req.tokenData.dbType).then(async (result) => {
                if (!(result.data)) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No Records are available.." || error, data: [] }));
                }
                let _base64 = "";
                ...
                let _resp = {
                    _id: result.data._id,
                    recStatus: result.data.recStatus,
                    revNo: result.data.revNo,
                    umr: result.data.umr,
                    ...
                    biltrlFrstKnee: result.data.biltrlFrstKnee,
                ...
                return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: _resp || [] }));
...
router.post("/update-upload-document", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            let _mResp = await _mUtils.commonMonogoCall("patient_care_documents", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] }));
            }
            let _hResp = await _mUtils.insertHistoryData('patient_care_documents', _mResp.data.params, _cBody.params, req);
            let pLoadResp = { payload: {} };
            if (!(_hResp && _hResp.success)) {
                ...
                if (!pLoadResp.success) {
                    return res.status(400).json(await encryptData({ success: false, status: 400, desc: pLoadResp.desc || "", data: [] }));
                }
            }
            ...
            _cloneBw.updateOne.filter = { "_id": pLoadResp.payload._id };
            pLoadResp.payload.pData.push(_cloneBw);
            mongoMapper('patient_care_documents', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                let _patObj = {
                    params: {
                        _id: req.body.params.patientId,
                        document:
                            [{
                                _id: req.body.params.patLvlDocumentId,
                                //recStatus: req.body.params.recStatus
                            }]
                    }
                }
                _.each(req.body.params, (_val, _key)=>{
                    if(_key == "recStatus" || _key == "surgName" || _key == "surgId" || _key == "surgDt" || _key == "biltrlFrstKnee" || _key == "selKnee" || _key == "admnNo" || _key == "admnDt"){
                        _key = _key == "admnNo" ? "admNo" : _key
                        _patObj.params.document[0][`${_key}`] = _val
                    }
                })
                let pLoadResp = { payload: {} };
                pLoadResp = await preparePayload('BW', _patObj);
                ...
                // pLoadResp.payload.pData.push(_cloneBw);
                let _patient = await commonMonogoCall("patient_care_patients", "bulkWrite", pLoadResp.payload, "", req.body, "", "", req.tokenData.dbType)
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
            });
        }
        else {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: "Require Parameters ..", data: [] }));
        }
    } catch (error) {
        return res.status(500).json(await encryptData({ success: false, status: 500, desc: error.message || error }));
    }
...
router.post('/get-clinical-data', async (req, res) => {
    try {
            ...
            if (req.body.params.from === 'S') {
                ...
                axios.post(req.clientUrls.apkApiUrl + "/getIpClinicalData", _params).then(async function (response) {
                    ...
                    if (response && response.SQL_ERROR) {
                        return res.status(400).json(await encryptData({ status: 'FAIL', desc: response.SQL_ERROR }));
                    }
                    else {
                        ...
                        let _patient = await commonMonogoCall("patient_care_patients", "find", _params, "", req.body, "", "", req.tokenData.dbType)
                        if (Object.keys(logObj).length > 0 && _patient.success && _patient.data && _patient.data[0] && Object.keys(_patient.data[0]).length > 0) {
                            ...
                            let _patUpdate = await commonMonogoCall("patient_care_patients", "findOneAndReplace", _payload, "", "", "", "", req.tokenData.dbType)
                            if (!(_patUpdate && _patUpdate.success)) {
                                // return res.status(400).json({ status: 'FAIL', desc: _surgUpdate.desc || "", data: _surgUpdate.data || [] });
                            }
                        }
                        logObj["lastUpdatedDt"] = new Date().toISOString();
                        return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: logObj || [] }));
                    }

                }).catch(async function (ex) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: ex, data: [] }));
                });
            }
            else {
                let _params = {
                ...
                let _clinicalInfo = await commonMonogoCall("patient_care_patients", "find", _params, "", req.body, "", "", req.tokenData.dbType)
                if (!(_clinicalInfo && _clinicalInfo.success && _clinicalInfo.data && _clinicalInfo.data.length > 0)) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: _clinicalInfo.desc || "", data: _clinicalInfo.data || [] }));
                }
                else {
                    let _resp = {};
        ...
        return res.status(500).json(await encryptData({ status: 'FAIL', desc: ex }));
    }
});
...
router.post('/get-lab-results', async (req, res) => {
    try {
        if (req.body.params && req.body.params._id && req.body.params.umr) {
            if (req.body.params.from === 'S') {
                if (!req.clientUrls.hisSyncUrls && req.clientUrls.hisSyncUrls.length > 0) {
                    return res.status(500).json(await encryptData({ status: 'FAIL', desc: "No API url found to fetch data" }));
                }
                ...
                if (!(_labData.success)) {
                    return res.status(500).json(await encryptData({ status: 'FAIL', desc: "No Data available at HIMS.." }));
                }
                ...
                if (Object.keys(results.gridView).length > 0 || Object.keys(results.trendView).length > 0) {
                    ...
                    return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: _finalResp || [] }));

                } else {
                    return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: "No data available..", data: [] }));
                }
            }
            else {
                let _params = {
        ...
        console.log("err", err);
        return res.status(500).json(await encryptData({ status: 'FAIL', desc: err }));
    }
});
...
router.post("/insert-role", async (req, res) => {
    try {
        ...
        delete req.body.params.audit;
        mongoMapper('patient_care_roles', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/get-roles", async (req, res) => {
    try {
        ...
        if (!_pGData.success) {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: _pGData.desc, data: [] }));
        }
        mongoMapper("patient_care_roles", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/update-role", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            ...
            _cloneBw.updateOne.filter = { "_id": pLoadResp.payload._id };
            pLoadResp.payload.pData.push(_cloneBw);
            mongoMapper('patient_care_roles', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
            });
    ...
    try {
        if (req.body && req.body.params) {
            ...
            if (!(_seqResp && _seqResp.success)) {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: _seqResp.desc || "", data: _seqResp.data || [] }));
            }
            req.body.params["cd"] = _seqResp.data;
            mongoMapper('patient_care_users', "insertMany", req.body.params, req.tokenData.dbType).then(async (result) => {
                if (!(result && result.data && result.data.length > 0)) {
                    return res.status(400).json(await encryptData({ success: false, status: 400, desc: `Error occurred while insert User`, data: [] }));
                }
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
...
router.post("/get-user", async (req, res) => {
    try {
        ...
        mongoMapper("patient_care_users", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/update-user", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            ...
            mongoMapper('patient_care_users', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
            });
...
router.post("/insert-form", async (req, res) => {
    try {
        ...
        req.body.params["cd"] = _seqResp.data;
        mongoMapper('patient_care_forms', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/get-forms", async (req, res) => {
    try {
        ...
        mongoMapper("patient_care_forms", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/update-form", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            ...
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: "Required parameters are missing ..", data: [] }));
        }
    } catch (error) {
        return res.status(500).json(await encryptData({ success: false, status: 500, desc: error.message || error }));
    }
...
router.post("/insert-counter", async (req, res) => {
    try {
        ...
        mongoMapper('patient_care_counters', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/insert-field-management", async (req, res) => {
    try {
        if (req.body && req.body.params) {
            ...
            mongoMapper('patient_care_fieldsmanagement', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
            });
...
router.post("/get-field-management", async (req, res) => {
    try {
        if (req.body && req.body.params) {
            ...
            mongoMapper("patient_care_fieldsmanagement", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
...
router.post("/get-entity", async (req, res) => {
    try {
        ...
        mongoMapper("patient_care_entity", req.body.query, _pGData.data, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/logout", async (req, res) => {
    try {
        ...
        mongoMapper("patient_care_usersessions", "findOneAndUpdate", pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ status: 'SUCCESS', code: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ status: 'FAIL', code: 400, desc: error.desc || error, data: [] }));
        });
    } catch (error) {
        return res.status(500).json(await encryptData({ status: 'FAIL', code: 500, desc: error }));
    }
});
...
router.post("/get-all-surgeries-list", async (req, res) => {
    try {
        ...
        mongoMapper("patient_care_surgeries", "aggregation", _filter, req.tokenData.dbType).then(async (result) => {
            if (result.data.length > 0) {
                ...
                if(_surgeryData.length > 0){
                    for(let _surg of _surgeryData){
                        let _patFilter = {
                            "filter":{
                                "umr": _surg.umr
                            }
                        }
                        let _patDet = await _mUtils.commonMonogoCall("patient_care_patients", "find", _patFilter, "", "", "", req.tokenData.dbType)
                        if (!(_patDet && _patDet.success)) {
                            _output.push({ success: false, status: 400, desc: _patDet.desc || "", data: _patDet.data || [] });
                        }    
                        if(_patDet && _patDet.success && _patDet.data.length > 0){
                            let _patDetails = JSON.parse(JSON.stringify(_patDet.data))
                            if(_patDetails[0].comorbidities && _patDetails[0].comorbidities.length > 0){
                                _.each(_patDetails[0].comorbidities, (_cmbrt) => {
                                    _surg[`comorbidities.${_cmbrt.cd}`] = _cmbrt.value == 'N' || _cmbrt.value == '' || !_cmbrt.value ? "No" : _cmbrt.value == 'Y' ? "Yes" : "No"
                                })
                            }
                            if(_patDetails[0].scores && _patDetails[0].scores.length > 0){
                                _.each(_patDetails[0].scores, (_scr) => {
                                    _surg[`scores.${_scr.scoreCd}`] = `${_scr.percentage}%`
                                    //_surg[`scores.${_scr.scoreCd}.percentage`] = _scr.percentage
                                })
                            }
                        }
                    }
                }
                return res.status(200).json(await encryptData({ success: false, status: 200, desc: ``, data: _surgeryData || [] }));
            }
            else {
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: 'user not exist', data: [] }));
            }
...
router.post("/insert-unlock-requests", async (req, res) => {
    try {
        if (req.body && req.body.params) {

            let _cBody = JSON.parse(JSON.stringify(req.body))
            _cBody.params['reqRaisedDt'] = new Date().toISOString()
            mongoMapper('patient_care_unlockrequests', "insertMany", _cBody.params, req.tokenData.dbType).then(async (_result) => {
                let _surgId = _result.data[0].surgeries[0]._id
                let _filter = {
                    "filter": {
                        "umr": _cBody.params.umr,
                        "admns.surgDtls.surgeryClosed": true
                    },
                    "selectors": ""
                };
                mongoMapper("patient_care_surgeries", "find", _filter, req.tokenData.dbType).then(async (result) => {
                    if (!result.data || result.data.length == 0) {
                        return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No surgeries found", data: [] }));
                    }
                    let _sParams = {
                        "query": "bulkWrite",
                        "flag": "BW",
                        "params": {
                            ...
                            "admns": [
                                {
                                    "_id": result.data[0].admns[0]._id,
                                    "surgDtls": [
                                        {
                                            "_id": result.data[0].admns[0].surgDtls[0]._id,
                                            "reqRaisedBy": _cBody.params.reqRaisedBy,
                                            "reqRaisedId": _cBody.params.reqRaisedId,
                                            "reqRaisedDt": new Date().toISOString(),
                                            "isRequest": true,
                                            "unlockRequest": false,
                                            "unlockReqId": _result.data[0]._id,
                                            "reasonForUnlock": _cBody.params.reasonForUnlock,
                                            "unlockReqSurgId": _surgId,
                                            "reqApprovedDt": ""
                                        }
                                    ]
                                }
                    ...
                    // console.log("pLoadResp", pLoadResp.payload.pData[0])
                    // console.log("pLoadResp", pLoadResp.payload.pData[0].updateOne)
                    if (!pLoadResp.success) {
                        return res.status(400).json(await encryptData({ status: 'FAIL', desc: pLoadResp.desc || "", data: [] }));
                    }
                    mongoMapper("patient_care_surgeries", "bulkWrite", pLoadResp.payload, req.tokenData.dbType).then(async (resp) => {
                        if (!resp) {
                            return res.status(400).json(await encryptData({ success: false, status: 400, desc: "Error while updating in surgeries", data: [] }));
                        }
                        let _params = {
                            "query": "bulkWrite",
                            "flag": "BW",
                            "params": {
                                "_id": _cBody.params.patId,
                                "umr": _cBody.params.umr,
                                "surgeries": [
                                    {
                                        "_id": _cBody.params.supportedObj.selPatSurgId,
                                        "reqRaisedBy": _cBody.params.reqRaisedBy,
                                        "reqRaisedId": _cBody.params.reqRaisedId,
                                        "reqRaisedDt": new Date().toISOString(),
                                        "isRequest": true,
                        ...
                        let _pLoad = await _mUtils.preparePayload("BW", _params)
                        _pLoad.payload.pData[0].updateOne.update.$set['surgeries.$.unlockReqId'] = _result.data[0]._id
                        _pLoad.payload.pData[0].updateOne.update.$set['surgeries.$.unlockReqSurgId'] = _surgId
                        if (!_pLoad.success) {
                            return res.status(400).json(await encryptData({ status: 'FAIL', desc: _pLoad.desc || "", data: [] }));
                        }
                        mongoMapper("patient_care_patients", "bulkWrite", _pLoad.payload, req.tokenData.dbType).then(async (_resp) => {
                            if (!_resp) {
                                return res.status(400).json(await encryptData({ status: 'FAIL', desc: "Error while updating in patients", data: [] }));
                            }
                            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: _result.data[0] }));
                            // mongoMapper('patient_care_unlockrequests', "insertMany", _cBody.params, req.tokenData.dbType).then(async (result) => {
                            //     return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
                            // }).catch(async (error)=>{
                            //     return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
                            // })
...
router.post("/get-unlock-requests", async (req, res) => {
    try {
        let _filter = {
            "filter": {
                "isRequest": { $eq: req.body.params.isRequest },
                "unlockRequest": { $eq: false }
                //"recStatus": { $eq: true } 
            },
            "selectors": "-history -child.audit -child.history"
        }
        if (req.body.flag == "DOR") {
            let fromDt = req.body.params.fromDt.split('T')[0]  //moment(req.body.params.fromDt, 'DD MMM YYYY').format('YYYY-MM-DD')
            let toDt = req.body.params.toDt.split('T')[0] //moment(req.body.params.toDt, 'DD MMM YYYY').format('YYYY-MM-DD')
            _filter = {
                "filter": {
                    "reqRaisedDt": { $gte: `${fromDt}T00:00:00.000Z`, $lte: `${toDt}T23:59:59.999Z` },
                    "unlockRequest": { $eq: false }
                    //"recStatus": { $eq: true } 
                },
                "selectors": "-history"
        ...
        if (req.body.flag == "DOA") {
            let fromDt = req.body.params.fromDt.split('T')[0]  //moment(req.body.params.fromDt, 'DD MMM YYYY').format('YYYY-MM-DD')
            let toDt = req.body.params.toDt.split('T')[0] //moment(req.body.params.toDt, 'DD MMM YYYY').format('YYYY-MM-DD')
            _filter = {
                "filter": {
                    "reqApprovedDt": { $gte: `${fromDt}T00:00:00.000Z`, $lte: `${toDt}T23:59:59.999Z` },
                    "unlockRequest": { $eq: true }
                    //"recStatus": { $eq: true } 
                },
                "selectors": "-history"
            }
        ...
        mongoMapper("patient_care_unlockrequests", "find", _pGData.data, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
        });
...
router.post("/update-unlock-request", async (req, res) => {
    try {
        let _cBody = JSON.parse(JSON.stringify(req.body))
        let _filter = {
            "filter": {
        ...
        mongoMapper("patient_care_surgeries", "find", _filter, req.tokenData.dbType).then(async (result) => {
            if (!result.data || result.data.length == 0) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: "No records found in surgeries", data: [] }));
            }
            let _sParams = {
                "query": "bulkWrite",
                "flag": "BW",
                    ...
                    "admns": [
                        {
                            ...
                            "surgDtls": [
                                {
                                    "_id": result.data[0].admns[0].surgDtls[0]._id,
                                    "reqApprovedBy": _cBody.params.reqApprovedBy,
                                    "reqApprovedId": _cBody.params.reqApprovedId,
                                    "reqApprovedDt": new Date().toISOString(),
                                    "reviewDt": "",
                                    "surgeryClosedBy": "",
                                    "reviewRemarks": "",
                                    "surgeryClosedDt": "",
            ...
            mongoMapper("patient_care_surgeries", "bulkWrite", pLoadResp.payload, req.tokenData.dbType).then(async (resp) => {
                ...
                let _pLoad = await _mUtils.preparePayload("BW", params)
                if (!_pLoad.success) {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: _pLoad.desc || "", data: [] }));
                }
                mongoMapper("patient_care_patients", "bulkWrite", _pLoad.payload, req.tokenData.dbType).then(async (_resp) => {
                    if (!_resp) {
                        return res.status(400).json(await encryptData({ success: false, status: 400, desc: "Error while updating in patients", data: [] }));
                    }
                    _cBody.params['reqApprovedDt'] = new Date().toISOString()
                    _cBody['isRequest'] = false
                    let _pLoad = await _mUtils.preparePayload("BW", _cBody)
                    if (!_pLoad.success) {
                        return res.status(400).json(await encryptData({ status: 'FAIL', desc: _pLoad.desc || "", data: [] }));
                    }
                    mongoMapper("patient_care_unlockrequests", "bulkWrite", _pLoad.payload, req.tokenData.dbType).then(async (_finalResp) => {
                        return res.status(200).json(await encryptData({ success: true, status: 200, desc: ``, data: _finalResp }));
                    }).catch(async (error) => {
                        return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
                    })
                }).catch(async (error) => {
...
function nameSearch(name) {
    return name.toLowerCase().replace(/[^a-z0-9\s]/gi, '').trim().split(/\s+/).sort().join(" ")
}


router.post("/get-filter-by-umr", async (req, res) => {
    try {
        if (req.body.flag == "UMR") {
            let _filter = {
                "filter": {
            ...
            mongoMapper("patient_care_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
                if (result.data && result.data.length > 0) {
                    if (result.data[0].umr && result.data[0].umr.length > 0 && result.data[0].umr == req.body.params.umr) {
                        return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: "Patient already existed with this UMR No", data: result.data }));
                    }
                }
                else {
                    return res.status(400).json(await encryptData({ status: 'FAIL', desc: '', data: "UMR No Created Successfully" }));
                }
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
            });
        }
        else if (req.body.flag == "NAME" && req.body.params.umr) {
            let phone1 = req.body.params.phone[0].num;
            let phone2 = req.body.params.phone && req.body.params.phone.length > 1 ? req.body.params.phone[1].num : "";
            //let Name = nameSearch(req.body.params.disName)
            let _filter = {
                "filter": {
                    $or: [
                        { "phone.num": phone1 },
            ...
            mongoMapper("patient_care_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
                if (result.data && result.data.length > 0) {
                    let _pLoadName = nameSearch(req.body.params.disName)
                    let _patData = []
                    _.each(result.data, (_patient) => {
                        let name = nameSearch(_patient.disName)
                        if (_pLoadName == name) {
                            _patData.push(_patient)
                        }
                    })
                    if (_patData.length > 0) {
                        return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: "Patient already existed with this NAME and Mobile", data: _patData }));
                    }
                    return res.status(400).json(await encryptData({ patExistCd: 100, status: 'FAIL', desc: "No Patients Found with Name", data: [] }));

                }
                else {
                    return res.status(400).json(await encryptData({ patExistCd: 100, status: 'FAIL', desc: "No Patients Found with Existing Mobile and Name and DOB", data: [] }));
                }
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
            });
...
router.post("/get-patient-admns", async (req, res) => {
    try {
        if (req.body.params._id && req.body.params.umr && req.body.flag) {
            let _filter = {
                "filter":
                {
            ...
            mongoMapper("patient_care_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
                if (result && result.data) {
                    ...
                    if (req.body.flag && req.body.flag == "GET_ADMN") {
                        let _admns = []
                        _.each(_resp[0].surgeries, (surgery) => {
                            if (surgery.admn && surgery.admnDt && (!surgery.disDt || surgery.disDt == "")) {
                                admn = { "admnNo": surgery.admn, "admnDt": surgery.admnDt, "umr": _resp[0].umr }
                                _admns.push(admn)
                            }
                        })
                        _admns = _.uniqBy(_admns, 'admnNo')
                        //_admns.push({"umr": _resp[0].umr})
                        if (_admns.length > 0) {
                            return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: '', data: _admns || [] }));
                        }
                        else {
                            return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: 'No admissions found', data: _admns || [] }));
                        }
                    }
                    if (req.body.params.admn && req.body.flag && req.body.flag == "FILTER_ADMN") {
                        let admns = []
                        _.each(_resp[0].surgeries, (surgery) => {
                            if (surgery.admn && surgery.admn == req.body.params.admn) {
                                console.log("surgery.admn", req.body.params.admn, surgery.admn)
                                admns.push({ admnNo: surgery.admn, "umr": _resp[0].umr })
                            }
                        })
                        if (admns.length > 0) {
                            return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: 'Matched admissions found', data: admns || [] }));
                        }
                        else {
                            admns.push({ "umr": _resp[0].umr })
                            return res.status(400).json(await encryptData({ status: 'FAIL', desc: 'No Matched admissions found', data: admns || [] }));
                        }
                    }
                }
                else {
                    return res.status(200).json(await encryptData({ status: 'SUCCESS', desc: 'Patient Not found', data: [] }));
                }
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: error.desc || error, data: [] }));
            });
            ...
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: "Missing Required Parameters" || error, data: [] }));
        }
    } catch (error) {
        return res.status(500).json(await encryptData({ status: 'FAIL', desc: error }));
    }
...
router.post("/get-dashboard-data-old", async (req, res) => {
    try {
        //let _date = req.body.params.date.split('T')[0]
        ...
        //         $or: [
        //             {
        //                 "surgeries.surgDtTm": { $regex: `^${today}` }
        //             },
        //             {
        //                 $and: [
        //                     { "surgeries.surgeryClosedDt": { $regex: `^${today}` } },
        //                     { "surgeries.surgeryClosed": true },
        //                     { "surgeries.systemClosed": false }
        //                 ]
        //             },
        //             {
        //                 $and: [
        //                     { "surgeries.surgeryClosedDt": { $regex: `^${today}` } },
        //                     { "surgeries.surgeryClosed": true },
        //                     { "surgeries.systemClosed": true }

        //                 ]
        //             },
        //             { "surgeries.surgeryClosed": false },
        //         ]
        //     },
        //     "selectors": "-document -comorbidities -scores"
        // }
        ...
        mongoMapper("patient_care_patients", "aggregation", _filter, req.tokenData.dbType).then(async (result) => {
            if (!result.data || result.data.length == 0) {
                return res.status(400).json(await encryptData({ status: 'FAIL', desc: 'no data found', data: [] }));
            }
            let _data = JSON.parse(JSON.stringify(req.clientUrls.reports))
            let _finalResp = []
            _.each(result.data[0], (_val, _key) => {
                if (_val.length == 0) {
                    _val.push({ 'count': 0 })
                }
                let _cd = _.filter(_data, (_c) => { return _c.cd == _key })
...
router.post("/get-dashboard-data", async (req, res) => {
    try {
        if (req.body.params.fromDt && req.body.params.fromDt.length > 0) {
                                        ...
                                        surgeries: {
                                            $filter: {
                                                ...
                                                cond: {
                                                    $and: [
                                                        //{ $regexMatch: { input: "$$surgery.surgDtTm", regex: `^${_date}` } }
                                                        { $gte: ["$$surgery.documentedDt", `${fromDt}T00:00:00.000Z`] },
                                                        { $lte: ["$$surgery.documentedDt", `${toDt}T23:59:59:999Z`] },
                                                    ]
                                                }
                                            }
                                        }
                                        ...
                                        surgeries: {
                                            $filter: {
                                                ...
                                                cond: {
                                                    $and: [
                                                        //{ $regexMatch: { input: "$$surgery.surgeryClosedDt", regex: `^${_date}` } },
                                                        { $gte: ["$$surgery.surgeryClosedDt", `${fromDt}T00:00:00.000Z`] },
                                                        { $lte: ["$$surgery.surgeryClosedDt", `${toDt}T23:59:59:999Z`] },
                                                        { $eq: ["$$surgery.surgeryClosed", true] },
                                                        //{ $eq: ["$$surgery.systemClosed", false]}
                                                    ]
                                                }
                                        ...
                                        surgeries: {
                                            $filter: {
                                                ...
                                                cond: {
                                                    $and: [
                                                        //{ $regexMatch: { input: "$$surgery.reqRaisedDt", regex: `^${_date}` } },
                                                        { $gte: ["$$surgery.reqRaisedDt", `${fromDt}T00:00:00.000Z`] },
                                                        { $lte: ["$$surgery.reqRaisedDt", `${toDt}T23:59:59:999Z`] },
                                                        { $eq: ["$$surgery.isRequest", true] },
                                                        //{ $eq: ["$$surgery.systemClosed", true]}
                                                    ]
                                                }
                                        ...
                                        surgeries: {
                                            $filter: {
                                                ...
                                                cond: {
                                                    $and: [
                                                        //{ $regexMatch: { input: "$$surgery.reqApprovedDt", regex: `^${_date}` } },
                                                        { $gte: ["$$surgery.reqApprovedDt", `${fromDt}T00:00:00.000Z`] },
                                                        { $lte: ["$$surgery.reqApprovedDt", `${toDt}T23:59:59:999Z`] },
                                                        { $eq: ["$$surgery.unlockRequest", true] },
                                                        //{ $eq: ["$$surgery.systemClosed", true]}
                                                    ]
                                                }
            ...
            return res.status(400).json(await encryptData({ status: 'FAIL', desc: "Missing Required Parameters", data: [] }));
        }
    } catch (error) {
        return res.status(500).json(await encryptData({ status: 'FAIL', desc: error }));
    }
...
router.post("/insert-report-request", async (req, res) => {
    try {

        mongoMapper('patient_care_reportrequests', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/get-reports-data", async (req, res) => {
    try {
        let _filter = {
            "filter": { "requestId": req.body.params._id }
        ...
        //     return res.status(400).json(await encryptData({ success: false, status: 400, desc: _pGData.desc, data: [] }));
        // }
        mongoMapper("patient_care_reports", "find", _filter, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/insert-template", async (req, res) => {
    try {
        ...
        mongoMapper('patient_care_templates', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
        }).catch(async (error) => {
            return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
        });
...
router.post("/update-template", async (req, res) => {
    try {
        let _cBody = JSON.parse(JSON.stringify((req.body)));
        if (req.body.params._id) {
            let _mResp = await _mUtils.commonMonogoCall("patient_care_templates", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] }));
            }
            let _hResp = await _mUtils.insertHistoryData('patient_care_templates', _mResp.data.params, _cBody.params, req);
            let pLoadResp = { payload: {} };
            if (!(_hResp && _hResp.success)) {
            ...
            mongoMapper('patient_care_templates', 'findOneAndUpdate', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json(await encryptData({ success: true, status: 200, desc: '', data: result.data }));
            }).catch(async (error) => {
                return res.status(400).json(await encryptData({ success: false, status: 400, desc: error.desc || error, data: [] }));
            });
...
router.post("/insert-pincodes", async (req, res) => {
    try {
        ...
        mongoMapper('patient_care_pincodes', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
...
router.post("/get-pincodes", async (req, res) => {
    try {
        ...
        mongoMapper('patient_care_pincodes', req.body.query, _pGData.data, req.tokenData.dbType).then((result) => {
            if (result.data && result.data.length == 0) {
...
router.post("/get-user-activity", async (req, res) => {
    try {
        ...
        let _filter = {
            filter: {
                "audit.documentedDt" :{ $gte: `${fromDate}T00:00:00Z`,
                                        $lte: `${toDate}T23:59:59Z`}
            }
        }
        if (req.body.params.fromDt && req.body.params.fromDt.length > 0 && req.body.params.toDt && req.body.params.toDt.length > 0) {
        ...
        mongoMapper('patient_care_usersessions', req.body.query, _pGData.data, req.tokenData.dbType).then((result) => {
            let _finalData = JSON.parse(JSON.stringify(result.data))
            if(_finalData.length > 0){
                for(let _data of _finalData){
                    _data['IpAddress'] = req.ip
                    _data['status'] = _data.recStatus == true ? "Login" : "Logout"
                    _data['displayName'] = _data.displayName && _data.displayName.length > 0 ? convertToPascalCase(_data.displayName) : ""
                    if(_data.logInTime && _data.logInTime.length > 0 && _data.logOutTime && _data.logOutTime.length > 0){
                        let logInTime = moment(_data.logInTime);
                        let logOutTime = moment(_data.logOutTime);
                        
                        let diffInMilliSecs = logOutTime.diff(logInTime)

                        let _duration = moment.duration(diffInMilliSecs)
                         
                        let _hours = String(Math.floor(_duration.asHours())).padStart(2,'0')
                        let _minutes = String(_duration.minutes()).padStart(2,'0')
                        let _seconds = String(_duration.seconds()).padStart(2,'0')

                        //_data['sessionDuration']  = `${_hours}h:${_minutes}m:${_seconds}s`
                        _data['sessionDuration']  = `${_hours}:${_minutes}`
                    }
                    else{
                        _data['sessionDuration']  = "--:--"
                    }
                }
            }
