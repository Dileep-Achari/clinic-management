    ...
    try {
        if (req.headers && req.headers.orgkey) {
            ...
            if (!(_orgDtls && _orgDtls.length > 0)) {
                return res.status(400).send(await encryptData({ status: 'FAIL', data: [], desc: "No Client found.." }));
            }
            ...
            return res.status(400).send(await encryptData({ status: 'FAIL', data: [], desc: "Invalid Parameters" }));
        }
    } catch (error) {
...
router.post("/validate-doc-access", async (req, res) => {
    try {
        if (req.body && req.body.params.orgId && req.body.params.userId && req.body.params.routeUrl) {
            ...
            return res.status(400).send(await encryptData({ success: false, status: 400, desc: "Required Missing Parameters ..", data: [] }));
        }
    } catch (error) {
    ...
    try {
        if (req.body && req.body.encryptedData) {
            ...
            if (decrypted) {
                ...
                return res.status(498).send(await encryptData({ "ERROR": "Invalid key", "MESSAGE": "Invalid Key.." }));
                // req.body = req.body.encryptedData;
            }
        }
        next();
    } catch (ex) {
        return res.status(400).send(await encryptData({ "ERROR": "ERROR_WHILE_PREPARECPARAMS", "MESSAGE": ex.message }));
    }
});
    ...
    try {
        if (req.body && req.body.orgId && req.body.uName && req.body.pwd) {
            ...
            if (!(_locDtls && _locDtls.length > 0)) {
                return res.status(400).send(await encryptData({ status: 'FAIL', data: [], desc: "Provided Location was not mapped.." }));
            }
            let _user = {
...
router.post('/get-patient-by-umr', async function (req, res) {
    try {
        ...
        if (req.body && req.body.params && req.body.params.flag && (req.body.params.flag == 'NAME' || req.body.params.flag == 'MOB' || req.body.params.flag == 'ADMN' || req.body.params.flag == 'UMR' || req.body.params.flag == 'DOS') && req.body.params.searchValue && req.body.params.searchValue.length > 0) {
            ...
            mongoMapper("patient_care_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
                ...
                if (req.body.params.flag != "ADMN") {
                    console.log("_orgDta.method ", _orgDta.method);
                    if (_orgDta.method == "get") {
                        ...
                        // })
                        //getPatientByUrls
                        return res.status(200).send(await encryptData({ status: "SUCCESS", desc: '', data: _finalResp || [] }));
                    }
                        ...
                        console.log("req.clientUrls.emrApiUrl", req.clientUrls.emrApiUrl, _params)
                        axios.post(req.clientUrls.emrApiUrl + '/uprGetPatDetAutoSrch', _params).then(async (response) => {
                            ...
                            console.log("_encryptedResp", _encryptedResp, typeof _encryptedResp)
                            return res.status(200).send(_encryptedResp);
                        }).catch(async (err) => {
                            return res.status(400).send(await encryptData({ status: "FAIL", desc: '', data: err }));
                        })
                    }
                } else {
                    let _getDataFromUrl = await getDataAgainstadmns(_orgDta.admnsPayload, 0, [], req);
                    return res.status(200).send(await encryptData({ status: "SUCCESS", desc: '', data: _getDataFromUrl.data || [] }));
                }
                ...
                console.log("err", err)
                return res.status(400).send(await encryptData({ status: 'FAIL', data: [], desc: err }));
            })
        }
        else {
            return res.status(400).send(await encryptData({ status: 'FAIL', data: [], desc: "Invalid Parameters" }));
        ...
        console.log("error", error);
        return res.status(500).send(await encryptData({ status: 'FAIL', desc: error, data: [], }));
    }
