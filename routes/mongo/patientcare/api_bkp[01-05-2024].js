...
router.post("/test-urls", async (req, res) => {
    try {
        console.log("test")
        let url = "http://hisapigcb.kimshospitals.com:8083/Fc_Pat.asmx/patinfo_fc";
        let _params = {
            "APIKEY": "fsd0t6uKticoRaVnaIyIDFL9wZwl7EskgcjDnJzgYvQ=",
            "UMRNO": "UMRG21553",
            "ADMISSIONNO": "",
            "MOBILENO": ""
        }
        let resonseData = [];
        await axios1.get(url, { params: _params }).then((response) => {
            resonseData = response.data
        }).catch((err) => {
            console.log("err", err)
        })

        return res.status(200).json({ status: 'SUCCESS', desc: '', data: resonseData });
    } catch (error) {
        console.log("error")
    }
})
                ...
                if (_orgDta.method == "get") {
                    ...
                    _.each(_response, (_o, _i) => {
                        ...
                        _o['dob'] = moment(moment().subtract(_o.age.match(/\d+/g)[0], 'years')
                            ...
                            .subtract(_o.age.match(/\d+/g)[2], 'days')).format('DD/MM/YYYY')
