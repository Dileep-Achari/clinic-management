...
//                     return res.status(200).json({ status: "SUCCESS", desc: '', data: _finalResp.reverse() || [] });
//                 }).catch((err) => {
//                     return res.status(400).json({ status: "FAIL", desc: '', data: err });
//                 })
//                 // }
//             }).catch((err) => {
//                 return res.status(400).json({ status: 'FAIL', data: [], desc: err });
//             })
//         }
//         else {
//             return res.status(400).json({ status: 'FAIL', data: [], desc: "Invalid Parameters" });
//         }
//     } catch (error) {
...
async function getDataAgainstUrls(_data, _idx, _output, req, _params, _methodName) {
    try {
        if (_data.length > _idx) {
            _params = {
                "APIKEY": _data[_idx].apiKey,
                "UMRNO": req.body.params.searchValue,
                "ADMISSIONNO": req.body.params.ADMISSIONNO && req.body.params.ADMISSIONNO.length > 0 ? req.body.params.ADMISSIONNO : "",
                "MOBILENO": req.body.params.MOBILENO && req.body.params.MOBILENO.length > 0 ? req.body.params.MOBILENO : ""
            }
            let resonseData = [];
            await axios1.get(_data[_idx].url + `/${_methodName}`, { params: _params }).then((response) => {
                console.log("response", response);
                resonseData = response.data
            }).catch((err) => {
                console.log("err", err)
            });
            //{ success: true, desc: "", data: resonseData || [] }
            if (resonseData.length > 0) {
                _.each(resonseData, (_reso) => {
                    _output.push(_reso);
                })
            ...
            _idx = _idx + 1
            await getDataAgainstUrls(_data, _idx, _output, req, _params, _methodName)
        } else {
...
async function getDataAgainstadmns(_data, _idx, _output, req) {
    try {
        if (_data && _data.length > _idx) {
            _data[_idx].UMRNO = req.body.params.searchValue
            let _params = _.omit(_.clone(_data[_idx]), 'url')
            let resonseData = [];
            await axios.post(_data[_idx].url, _params).then((response) => {
                resonseData = response.data
            }).catch((err) => {
                console.log("err", err)
            });
            //{ success: true, desc: "", data: resonseData || [] }
            if (JSON.parse(resonseData.ResponseMsg).length > 0) {
                let baseMap = fetchTransformColumns("getPatientByUrls");
                _response = transform(JSON.parse(resonseData.ResponseMsg), baseMap);
                _.each(_response, (_reso) => {
                    _output.push(_reso);
                })
            ...
            // _output.push({ success: true, desc: "", data: _insertSection.data || [] });
            _idx = _idx + 1
            await getDataAgainstadmns(_data, _idx, _output, req)
        } else {
            return { success: true, data: _output }
        }
        let _final = _.filter(_output, (_r) => { return !_r.success });
...
router.post('/get-patient-by-umr', async function (req, res) {
    try {
        ...
        if (req.body && req.body.params && req.body.params.flag && (req.body.params.flag == 'NAME' || req.body.params.flag == 'MOB' || req.body.params.flag == 'ADMN' || req.body.params.flag == 'UMR' || req.body.params.flag == 'DOS') && req.body.params.searchValue && req.body.params.searchValue.length > 0) {
            ...
            else if (req.body.params.flag == 'ADMN') {
                _params = {
                    "orgId": req.tokenData.orgId, "locId": req.tokenData.locId,
                    "admns.$.admn": new RegExp('.*' + req.body.params.searchValue + '.*')
                }
            }
            else {
                _params = {
            ...
            mongoMapper("patient_care_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
                ...
                let _orgDta = await getClientUrls({ "orgId": req.tokenData.orgId });
                if (req.body.params.flag != "ADMN") {
                    if (_orgDta.method == "get") {
                        let _getDataFromUrl = await getDataAgainstUrls(_orgDta.locationWiseUrls, 0, [], req, _paramsData, 'patinfo_fc');
                        let _response = _getDataFromUrl.data;
                        let baseMap = fetchTransformColumns("getPatientByUrls");
                        _response = transform(_response, baseMap);

                        _.each(_response, (_o, _i) => {
                            _o.fName = _o.disName[0]
                            _o.lName = _o.disName.slice(1).trim();
                            _o['dob'] = moment(moment().subtract(_o.age.match(/\d+/g)[0], 'years')
                                .subtract(_o.age.match(/\d+/g)[1], 'months')
                                .subtract(_o.age.match(/\d+/g)[2], 'days'))

                        })
                        // _.each(_getDataFromUrl.data,(_o)=>{
                        _finalResp = _mResp.concat(_response)
                        // })
                        //getPatientByUrls
                        return res.status(200).json(await encryptData({ status: "SUCCESS", desc: '', data: _finalResp || [] }));
                    }
                    else if (_orgDta.method == "post") {
                        _params = {
                            FLAG: req.body.params.flag,
                            SRCH_VAL: req.body.params.searchValue,
                            SRCH_VAL1: req.body.params.searchValue1 || "",
                        ...
                        axios.post(req.clientUrls.emrApiUrl + '/uprGetPatDetAutoSrch', _params).then(async (response) => {
                            ...
                            let _encryptedResp = await encryptData({ status: "SUCCESS", desc: '', data: _finalResp.reverse() || [] });
                            return res.status(200).json(_encryptedResp);
                        }).catch(async (err) => {
                    ...
                    let _getDataFromUrl = {};
                    if (req.tokenData.orgId === 'ss') {
                        _getDataFromUrl = await getDataAgainstadmns(_orgDta.admnsPayload, 0, [], req);
                        return res.status(200).json(await encryptData({ status: "SUCCESS", desc: '', data: _getDataFromUrl.data || [] }));
                    }
                    else {
                        let _params = {
                            FLAG: req.body.params.flag,
                            SRCH_VAL: req.body.params.searchValue,
                            SRCH_VAL1: req.body.params.searchValue1 || "",
                            ORG_ID: req.tokenData.orgId
                        };
                        _getDataFromUrl = await commonAxiosCall("POST", `${req.clientUrls.emrApiUrl}/uprGetPatDetAutoSrch`, _params);
                        let _response = _getDataFromUrl.data;
                        let baseMap = fetchTransformColumns("getPatientById");
                        _response = transform(_response, baseMap);
                        //let _finalResp = _mResp.concat(_response);
