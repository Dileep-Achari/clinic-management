...
router.use(async function verifyToken(req, res, next) {
    ...
    try {
        ...
        if (!(_sessionData && _sessionData.success && _sessionData.data && Object.keys(_sessionData.data).length > 0 && _sessionData.data.token && _sessionData.data.token.length > 0)) {
            return res.status(401).json(await encryptData({ status: 'FAIL', desc: "Missing Token..", data: [] }));
        }
            ...
            mongoMapper("patient_care_patients", "find", _filter, req.tokenData.dbType).then(async (result) => {
                ...
                if(req.body.params.flag !="ADMN"){
                    if (_orgDta.method == "get") {
                        ...
                        // })
                        //getPatientByUrls
                        return res.status(200).send({ status: "SUCCESS", desc: '', data: _finalResp || [] });
                    ...
                    return res.status(200).send({ status: "SUCCESS", desc: '', data: _getDataFromUrl.data || [] });
                }
