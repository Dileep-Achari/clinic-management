...
//             "selectors": ""
//         }
//         mongoMapper("monography_production_roles", req.body.query, req.body.params, req.tokenData.orgKey).then((result) => {
//             return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
//         }).catch((error) => {
//             return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
//         });
...
//             let _statusResp = await _mUtils.commonMonogoCall("monography_production_drugmonographyworkflowstatuses", "find", {}, "", "", "", req.tokenData.dbType);
         ...
         _.each(resonseData,(o,i)=>{
            _drugData.push({
                ...
                "audit":req.body.params.audit,
                 "assignedTo":[
                       {
                        "roleId":"64ccc6baee65bd288c204a4b",
                        "levelId":"64c34fb9f85a024275132462",
                        "userId":"64ccc9219f93086a900acf59"
                       }
                  ]
...
router.post("/get-total-drugData-with-allUsers", async (req, res) => {
    try {
        ...
        mongoMapper("monography_production_userAssign", "find", _pGData.data, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
        ...
        mongoMapper("monography_production_userAssign", "find", _filter, "").then(async (result) => {
           ...
           //   let num = _sessionRes.data[0].session_id
                ...
                params={
                    ...
                    'orgId':  result.data[0].orgId ? result.data[0].orgId :"",
...
router.post("/insert-section-data", async (req, res) => {
    try {
       ...
       // let _statusResp = await _mUtils.commonMonogoCall("monography_production_drugmonographyworkflowstatuses", "findById",req.body.params.current_status_id, "","", "", req.tokenData.dbType)
      //  req.body.params['current_status']=_statusResp.data.Drug_Mon_Status;
        // delete _filter.filter.session_id
        // if(req.body.params.user_id && req.body.params.drug_id){
        //     _filter.filter['dId']=req.body.params.drug_id;
        //     _filter.filter['assignedTo.userId'] = req.body.params.user_id;
        //     _userAssignResp = await _mUtils.commonMonogoCall("monography_production_userAssign", "find",_filter, "","", "", req.tokenData.dbType)
        // }
        ...
        //         status_id:result.data[0].current_status_id,
        //         status:result.data[0].current_status,
...
router.post("/get-section-data", async (req, res) => {
    try {
        ...
        mongoMapper("monography_production_sections", "find", _pGData.data, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/update-section-data", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            ...
            let _hResp = await _mUtils.insertHistoryData('monography_production_sections', _mResp.data.params, _cBody.params, req, "monography_production");
            if (!(_hResp && _hResp.success)) {
            ...
            mongoMapper('monography_production_sections', 'findOneAndUpdate', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
