...
router.post("/get-section-data", async (req, res) => {
    try {
        let _filter = {
            "filter": {
                "recStatus": { $eq: true },
                "DD_SUBSTANCE_CD": req.body.params.searchCode,
        ...
        if(req.body.params.flag==="patient" || req.body.params.flag==="hcp"){
            ...
            _.each(data[0],(obj,indx)=>{
                if(obj.children.length > 0){
                  _.each(obj.children,(co,ci)=>{
                       if(co.children.length > 0){
                           _.each(co.children,(coo,coi)=>{
                             coo.data="" 
                           })
                       }
                       co.data=""
            ...
            let _sessionRes = await _mUtils.commonMonogoCall("monography_staging_sections", "find", _filter, "", "", "", "");
