...
router.post("/auth-user", async (req, res) => {
    try {
        if (req.body && req.body.params.uName && req.body.params.pwd) {
            ...
            let session_id;
            mongoMapper("monography_staging_users", "find", _filter, "").then(async (result) => {
                ...
                if (result.data && result.data.length > 0) {
                    ...
                    let _sessionRes = await _mUtils.commonMonogoCall("monography_staging_userSession", "find", _filter, "", "", "", "")
                    ...
                    _sessionInsertRes = await _mUtils.commonMonogoCall("monography_staging_userSession", "insertMany", params, "", "", "", "")
...
router.post("/insert-level", async (req, res) => {
    try {
        ...
        //     documentedById:req.tokenData.userId
        // }
        mongoMapper('monography_staging_levels', req.body.query, req.body.params).then((result) => {
...
router.post("/get-levels", async (req, res) => {
    try {
        ...
        //     "selectors": ""
        // }
        mongoMapper("monography_staging_levels", req.body.query, req.body.params, req.tokenData.orgKey).then((result) => {
            return res.status(200).json({ status: 'SUCCESS', desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ status: 'FAIL', desc: error.desc || error, data: [] });
        });
...
router.post("/update-level", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            ...
            let _mResp = await _mUtils.commonMonogoCall("monography_staging_levels", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.orgKey)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            let _hResp = await _mUtils.insertHistoryData('monography_staging_levels', _mResp.data.params, _cBody.params, req, "monography_staging");
            ...
            mongoMapper('monography_staging_levels', 'findOneAndUpdate', pLoadResp.payload, req.tokenData.orgKey).then(async (result) => {
...
router.post("/insert-role", async (req, res) => {
    try {
        mongoMapper('monography_staging_roles', req.body.query, req.body.params).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/get-roles", async (req, res) => {
    try {
        ...
        mongoMapper("monography_staging_roles", "find", _pGData.data, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/update-roles", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            let pLoadResp = { payload: {} };
            let _mResp = await _mUtils.commonMonogoCall("monography_staging_roles", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.orgKey)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            let _hResp = await _mUtils.insertHistoryData('monography_staging_roles', _mResp.data.params, _cBody.params, req, "monography_staging");
            if (!(_hResp && _hResp.success)) {
            }
            else {
                _cBody.params.revNo = _mResp.data.params.revNo;
            ...
            mongoMapper('monography_staging_roles', 'bulkWrite', pLoadResp.payload, req.tokenData.orgKey).then(async (result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
...
router.post("/insert-users", async (req, res) => {
    try {
        if (req.body && req.body.params) {
            ...
            mongoMapper('monography_staging_users', req.body.query, req.body.params).then((result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
...
//             "selectors": ""
//         }
//         mongoMapper("monography_staging_users", req.body.query, req.body.params, req.tokenData.orgKey).then((result) => {
//             return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
//         }).catch((error) => {
//             return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
//         });
...
router.post("/get-users", async (req, res) => {
    try {
        ...
        mongoMapper("monography_staging_users", "find", _pGData.data, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/update-users", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            let pLoadResp = { payload: {} };
            let _mResp = await _mUtils.commonMonogoCall("monography_staging_users", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.orgKey)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            let _hResp = await _mUtils.insertHistoryData('monography_staging_users', _mResp.data.params, _cBody.params, req, "monography_staging");
            if (!(_hResp && _hResp.success)) {
            ...
            mongoMapper('monography_staging_users', 'bulkWrite', pLoadResp.payload, req.tokenData.orgKey).then(async (result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
...
//         mongoMapper('monography_staging_drugcreation', "insertMany", req.body.params).then(async (result) => {
...
//                 }
//             }
//             let _rResp = await _mUtils.commonMonogoCall("monography_staging_roles", "find", _filter, "", "", "", "")
...
//                 let _mResp1 = await _mUtils.commonMonogoCall("monography_staging_userAssign", "find", _filter, "", "", "", "");
//                 if (_mResp1.data.length > 0) {
//                     _mResp  = await _mUtils.commonMonogoCall("monography_staging_userAssign", "insertMany", _assignedTo, "", "", "", "")
//                     if (!(_mResp && _mResp.success)) {
//                         return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
//                     }
//                    }else {
...
async function insertSectionDataAganistContentType(_data, _idx, _output, req) {
    try {
        if (_data.length > _idx) {
            ...
            let resonseData = [];
            let _tResp = await _mUtils.commonMonogoCall("monography_staging_templates", "find", _filter, "", "", "", "");
            ...
            if (resonseData.length > 0) {
                ...
                _insertSection = await _mUtils.commonMonogoCall("monography_staging_sections", "insertMany", _sectionPushData, "", "", "", "");
            ...
            if (resonseData.length > 0) {
                ...
                _insertSection1 = await _mUtils.commonMonogoCall("monography_staging_sections", "insertMany", _sectionPushData, "", "", "", "");
...
router.post("/insert-drug-details", async (req, res) => {
    try {
        ...
        mongoMapper('monography_staging_drugcreation', "insertMany", req.body.params).then(async (result) => {
            ...
            let _statusResp = await _mUtils.commonMonogoCall("monography_staging_drugmonographyworkflowstatuses", "find", {}, "", "", "", "");
            ...
            let _mResp = await _mUtils.commonMonogoCall("monography_staging_userAssign", "insertMany", _assignedTo, "", "", "", "")
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            ...
            if (_getContentWriterData.length > 0) {
                let _cResp = await _mUtils.commonMonogoCall("monography_staging_drugworkflow", "insertMany", _getContentWriterData, "", "", "", req.tokenData.dbType)
            ...
            let _statusflowTracking = await _mUtils.commonMonogoCall("monography_staging_drugmonographyworkflowtracking", "insertMany", statusData, "", "", "", req.tokenData.dbType)
            //tracking logic end
            ...
            if (_data.params[_idx]._id) {
                ...
                if (!(_mResp && _mResp.success)) {
                    ...
                    if (Object.keys(updateDrugFlowData).length > 0) {
                        pLoadResp = await _mUtils.preparePayload(cmf[2], _cBody);
                        let _mResp1 = await _mUtils.commonMonogoCall("monography_staging_drugworkflow", cmf[1], pLoadResp.payload, "", "", "", _dbType)
                    ...
                    let _s = await _mUtils.commonMonogoCall("monography_staging_drugmonographyworkflowtracking", "insertMany", statusData, "", "", "", "")
                    ...
                    let _statusflowTracking = await _mUtils.commonMonogoCall("monography_staging_drugmonographyworkflowtracking", "insertMany", statusData, "", "", "", "")
                ...
                let _pResp = await _mUtils.commonMonogoCall("monography_staging_drugcreation", "insertMany", req.body.params, "", "", "", "")
...
router.post("/update-userAssign-data", async (req, res) => {
    try {
        ...
        let _updateResp = await insertUpdateInMultiData(_cBody, 0, [], req, "", 'monography_staging_userAssign', 'bulkWrite', 'BW')
...
async function InsertCommentdata(_data, _params, _idx, _output, req) {
    try {
        if (_data.length > _idx) {
            ...
            let _insertComment = await _mUtils.commonMonogoCall("monography_staging_comments", "insertMany", _comments, "", "", "", "")
            if (!(_statusflowTracking && _statusflowTracking.success)) {
                //   _output.push({ success: false, desc: _statusflowTracking.desc || "", data: [] });
            }
        ...
        if (_data.params.length > _idx) {
            ...
            if (!(_mResp && _mResp.success)) {
                ...
                if (_cBody.params.assignedTo[0].actionOfRole === "REVIEWER" || _cBody.params.assignedTo[0].actionOfRole === "EXTERNAL REVIEWER" || _cBody.params.assignedTo[0].actionOfRole === "CONTENT WRITER") {
                    ...
                    _sResp = await _mUtils.commonMonogoCall("monography_staging_sections", "find", _filter, "", "", "", "")
                    ...
                    if ((_cBody.params.previousAssigned[0].actionOfRole === "APPROVER" && _cBody.params.current_status === "REVISION OPENED")) {
                        ...
                        let _uResp = await _mUtils.commonMonogoCall("monography_staging_roles", "findById", _cBody.params.assignedTo[0].roleId, "", "", "", "")
                        let _finalData = []
                ...
                let _uSections = await updateSectionData(_sResp.data, 0, [], req, _rParams, "", "monography_staging_sections", "findOneAndUpdate", "U")
                ...
                let _dData = await _mUtils.commonMonogoCall("monography_staging_drugworkflow", "findById", _getId, "", "", "", "")
                statusData.push({
...
router.post("/update-drugFlow-data", async (req, res) => {
    try {
        let _cBody = JSON.parse(JSON.stringify((req.body)));
        let _updateResp = await updateDrugFlowDataWithSections(_cBody, 0, [], req, "", 'monography_staging_drugworkflow', 'bulkWrite', 'BW');
        return res.status(200).json({ success: true, status: 200, desc: '', data: _updateResp.data });
    } catch (error) {
        return res.status(500).json({ success: false, status: 500, desc: error.message || error });
    }
...
router.post("/testing", async (req, res) => {
    try {
        ...
        let _statusResp = []
        _statusResp = await _mUtils.commonMonogoCall("monography_staging_sections", "find", _filter, "", "", "", "");
        let _finalData = []
...
router.post("/get-drugFlow-data", async (req, res) => {
    try {
        ...
        //     return res.status(400).json({ success: false, status: 400, desc: _pGData.desc, data: [] });
        // }
        mongoMapper("monography_staging_drugworkflow", "find",_filter, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/get-history-drugFlow-data", async (req, res) => {
    try {
        ...
        //        return res.status(400).json({ success: false, status: 400, desc: _pGData.desc, data: [] });
        //    }
        mongoMapper("monography_staging_drugmonographyworkflowtracking", "find", _filter, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
//         //     return res.status(400).json({ success: false, status: 400, desc: _pGData.desc, data: [] });
//         // }
//         mongoMapper("monography_staging_userAssign", "find", _filter, "").then(async (result) => {
//             if (!(result && result.data && result.data.length > 0)) {
//                 return res.status(400).json({ success: false, status: 400, desc: `Error occurred while insert Employee`, data: [] });
//             }
...
router.post("/get-user-drug-details", async (req, res) => {
    try {
        ...
        if (req.body.params.flag === "cStatus") {
            ...
            _filter.filter['content_type'] = req.body.params.content_type
            _method = "monography_staging_drugworkflow"
            ...
            _filter.filter['content_type'] = req.body.params.content_type
            _method = "monography_staging_drugmonographyworkflowtracking"
        }
...
router.post("/get-search-drug-details", (req, res) => {
    try {
        if (req.body && req.body.params && req.body.params.searchValue && req.body.params.searchValue.length > 2) {
            ...
            mongoMapper("monography_staging_userAssign", req.body.query, _filter, req.tokenData.dbType).then((result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
...
router.post("/get-total-drugData-with-allUsers", async (req, res) => {
    try {
        ...
        //     return res.status(400).json({ success: false, status: 400, desc: _pGData.desc, data: [] });
        // }
        mongoMapper("monography_staging_userAssign", "find", _filter, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/update-drug-details", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            ...
            let _mResp = await _mUtils.commonMonogoCall("monography_staging_drugcreation", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.orgKey)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }
            let _hResp = await _mUtils.insertHistoryData('monography_staging_drugcreation', _mResp.data.params, _cBody.params, req, "monography_staging");
            if (!(_hResp && _hResp.success)) {
            ...
            mongoMapper('monography_staging_drugcreation', 'findOneAndUpdate', pLoadResp.payload, req.tokenData.orgKey).then(async (result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
...
router.post("/get-drugData", async (req, res) => {
    try {
        ...
        mongoMapper("monography_staging_drugcreation", "find", _filter, req.tokenData.orgKey).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/insert-userAssign", async (req, res) => {
    try {
        mongoMapper('monography_staging_userAssign', req.body.query, req.body.params).then(async (result) => {
            let _filter = {
                "filter": { "_id": result.data[0].assignedTo[0].roleId },
                "selectors": "-audit -history"
            }

            let _mResp = await _mUtils.commonMonogoCall("monography_staging_roles", "find", _filter, "", "", "", req.body.params.dbType)
            if (!_mResp.success) {
...
router.post("/get-userAssignData", async (req, res) => {
    try {
        ...
        mongoMapper("monography_staging_userAssign", req.body.query, req.body.params, req.tokenData.orgKey).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/insert-coreMasterData", async (req, res) => {
    try {
        mongoMapper('monography_staging_coreMasters', req.body.query, req.body.params).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/get-coreData", async (req, res) => {
    try {
        ...
        mongoMapper("monography_staging_coreMasters", req.body.query, _filter, req.tokenData.orgKey).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
async function getRoleParticularData(_flag, _params) {
    ...
    new Promise(async (resolve, reject) => {
        ...
        mongoMapper("monography_staging_roles", "find", _filter, "").then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
async function getAllAssignDrugDetails(_data, _indx, _output, req) {
    try {
        if (_data.length > _indx) {
            ...
            let _mResp = await _mUtils.commonMonogoCall("monography_staging_userAssign", "find", _filter, "", req.body, "", req.tokenData.dbType);
...
router.post("/insert-counter", async (req, res) => {
    try {
        ...
        mongoMapper('monography_staging_counters', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/insert-status", async (req, res) => {
    try {
        let _seqResp = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: 'Status' }, "monography_staging", req);
        ...
        mongoMapper('monography_staging_drugmonographyworkflowstatuses', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/get-status", async (req, res) => {
    try {
        ...
        mongoMapper("monography_staging_drugmonographyworkflowstatuses", req.body.query, _filter, req.tokenData.orgKey).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/insert-section-data", async (req, res) => {
    try {
        let _seqResp = await _mUtils.getSequenceNextValue({ locId: req.tokenData.locId, seqName: 'Section' }, "monography_staging", req);
        if (!(_seqResp && _seqResp.success)) {
            return res.status(400).json({ success: false, status: 400, desc: _seqResp.desc || "", data: _seqResp.data || [] });
        }
        let _filter = {
        ...
        let _userAssignResp;
        let _sessionResp = await _mUtils.commonMonogoCall("monography_staging_userSession", "find", _filter, "", "", "", req.tokenData.dbType)
        ...
        // let _statusResp = await _mUtils.commonMonogoCall("monography_staging_drugmonographyworkflowstatuses", "find", req.body.params.current_status_id, "", "", "", req.tokenData.dbType)
        ...
        if (req.body.params.user_id && req.body.params.drugId) {
            ...
            _userAssignResp = await _mUtils.commonMonogoCall("monography_staging_drugworkflow", "find", _filter, "", "", "", req.tokenData.dbType)
        }
        ...
        mongoMapper('monography_staging_sections', req.body.query, req.body.params, req.tokenData.dbType).then(async (result) => {
            if (result.data.length > 0) {
                ...
                let _statusflowTracking = await _mUtils.commonMonogoCall("monography_staging_drugmonographyworkflowtracking", "insertMany", _trackingFlowdata, "", "", "", req.tokenData.dbType);
                if (!(_statusflowTracking && _statusflowTracking.success)) {
                    return res.status(400).json({ success: false, status: 400, desc: _statusflowTracking.desc || "", data: _statusflowTracking.data || [] });
                }
            }
            let _cResp = await insertCommentsData("monography_staging_comments", result.data[0], req)
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/insert-comment-data", async (req, res) => {
    try {
        mongoMapper('monography_staging_comments', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/get-comments-data", async (req, res) => {
    try {
        ...
        mongoMapper("monography_staging_comments", req.body.query, _filter, req.tokenData.orgKey).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
//         // }
//         req.body.params["drug_monography_Code"] = _seqResp.data;
//         mongoMapper('monography_staging_sections', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
//             return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
//         }).catch((error) => {
//             return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
//         });
...
router.post("/get-section-data", async (req, res) => {
    try {
        ...
        //     return res.status(400).json({ success: false, status: 400, desc: _pGData.desc, data: [] });
        // }
        mongoMapper("monography_staging_sections", "find", _filter, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/update-section-data", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            let pLoadResp = { payload: {} };
            let _mResp = await _mUtils.commonMonogoCall("monography_staging_sections", "findById", req.body.params._id, "REVNO", req.body, "", req.tokenData.dbType)
            if (!(_mResp && _mResp.success)) {
                return res.status(400).json({ success: false, status: 400, desc: _mResp.desc || "", data: _mResp.data || [] });
            }

            // let _cResp = await insertCommentsData("monography_staging_comments", _mResp.data.params, req)
            // if (!(_cResp && _cResp.success)) {
            //     return res.status(400).json({ success: false, status: 400, desc:"comments data not valid", data: [] });
            // }

            let _hResp = await _mUtils.insertHistoryData('monography_staging_sections', _mResp.data.params, _cBody.params, req, "cm");
            if (!(_hResp && _hResp.success)) {
            ...
            mongoMapper('monography_staging_sections', 'bulkWrite', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
...
router.post("/insert-session-data", async (req, res) => {
    try {
        mongoMapper('monography_staging_userSession', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/insert-contentType-data", async (req, res) => {
    try {
        mongoMapper('monography_staging_contenttypes', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/get-contentType-data", async (req, res) => {
    try {
        ...
        mongoMapper("monography_staging_contenttypes", req.body.query, req.body.params, req.tokenData.orgKey).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/insert-sectiontype-data", async (req, res) => {
    try {
        mongoMapper('monography_staging_sectiontypes', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/get-sectiontype-data", async (req, res) => {
    try {
        ...
        mongoMapper("monography_staging_sectiontypes", req.body.query, req.body.params, req.tokenData.orgKey).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/insert-template-data", async (req, res) => {
    try {
        mongoMapper('monography_staging_templates', req.body.query, req.body.params, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/get-template-data", async (req, res) => {
    try {
        if (req.body.params.content_type) {
            ...
            //        return res.status(400).json({ success: false, status: 400, desc: _pGData.desc, data: [] });
            //    }
            mongoMapper("monography_staging_templates", "find", _filter, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
