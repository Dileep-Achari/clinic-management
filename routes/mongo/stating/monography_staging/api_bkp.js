    ...
    try {
        if (req.body && req.body.params.uName && req.body.params.pwd) {
            ...
            mongoMapper("monography_staging_users", "find", _filter, "").then(async (result) => {
                ...
                if(result.data && result.data.length>0){
                    let _filter = {"filter": {recStatus:true},
                       "selectors": "-audit -history"
                    }
...
router.post("/insert-drug-details", async (req, res) => {
    try {
        ...
        mongoMapper('monography_staging_drugcreation', "insertMany", req.body.params).then(async (result) => {
            ...
            let _statusResp = await _mUtils.commonMonogoCall("monography_staging_drugmonographyworkflowstatuses", "find", {}, "", "", "", req.tokenData.dbType);
            _statusResp.data = _.orderBy(_statusResp.data, 'drug_Mongrophy_Status_Code', 'asc')
            if (_statusResp && _statusResp.success) {
                let findIndx = _.findIndex(_statusResp.data, (objects) => { return objects.Drug_Mon_Status === "NOT ACCEPTED" }, 0)
                statusResp_next_status = _statusResp.data[findIndx + 1].Drug_Mon_Status;
            }
...
router.post("/get-user-drug-details", async (req, res) => {
    try {
        ...
        // }
        let _sessionInsertRes;
        let session_id; monography_drugworkflow
        mongoMapper("monography_staging_drugworkflow", "find", _filter, "").then(async (result) => {
            if (!(result && result.data && result.data.length > 0)) {
                return res.status(400).json({ success: false, status: 400, desc: `Error occurred while insert Employee`, data: [] });
            }
...
async function insertUpdateInMultiData(_data, _idx, _output,req, _dbType, ...cmf) {
    try {
        if (_data.params.length > _idx) {
            ...
            _statusResp.data = _.orderBy(_statusResp.data, 'drug_Mongrophy_Status_Code', 'asc')
            if (_statusResp && _statusResp.success) {
                let findIndx = _.findIndex(_statusResp.data, (objects) => { return objects.Drug_Mon_Status === _data.params[_idx].current_status }, 0)
                statusResp_next_status = _statusResp.data[findIndx + 1].Drug_Mon_Status;
            }
            ...
            mongoMapper('monography_staging_drugcreation', "insertMany", req.body.params).then(async (result) => {
             ...
             let _getContentWriterData=[]
             _.each(_assignedTo,(_o,_i)=>{
                 _.each(_o.assignedTo,(_o1,_i1)=>{
                      if(_i1==="actionOfRole" && _o1 === "CONTENT WRITER"){
                         _getContentWriterData.push(_o)
                      }
                 })
             })
...
router.post("/update-section-data", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            ...
            let _hResp = await _mUtils.insertHistoryData('monography_staging_sections', _mResp.data.params, _cBody.params, req, "monography_staging");
            if (!(_hResp && _hResp.success)) {
