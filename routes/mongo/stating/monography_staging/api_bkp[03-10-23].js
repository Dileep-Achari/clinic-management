...
router.post("/get-drugFlow-data", async (req, res) => {
    try {
        ...
        mongoMapper("monography_staging_drugworkflow", "find", _pGData.data, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/get-total-drugData-with-allUsers", async (req, res) => {
    try {
        ...
        mongoMapper("monography_staging_userAssign", "find", _pGData.data, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
async function insertCommentsData(_method, data, req) {
    return new Promise(async (resolve, rejected) => {
        try {
            let params = {
                referenceType: req.body.params.type ? req.body.params.type : "",
...
router.post("/get-section-data", async (req, res) => {
    try {
        ...
        mongoMapper("monography_staging_sections", "find", _pGData.data, req.tokenData.dbType).then((result) => {
            return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
        }).catch((error) => {
            return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
        });
...
router.post("/update-section-data", async (req, res) => {
    try {
        ...
        if (req.body.params._id) {
            ...
            mongoMapper('monography_staging_sections', 'findOneAndUpdate', pLoadResp.payload, req.tokenData.dbType).then(async (result) => {
                return res.status(200).json({ success: true, status: 200, desc: '', data: result.data });
            }).catch((error) => {
                return res.status(400).json({ success: false, status: 400, desc: error.desc || error, data: [] });
            });
    ...
    try {
        if (req.body.params.content_type) {
            ...
            mongoMapper("monography_staging_templates", "find", _filter, req.tokenData.dbType).then(async (result) => {
                ...
                // if (content_type === "650a9d0ad9a18e495d89d223") {
                //     let url = "https://emr.doctor9.com/napi_cmn/pharmacyv1/api/getMasterData"
                //     let resonseData = []
                //     let params = {
                //         "TYPE": "DD_DRUG_MASTER",
