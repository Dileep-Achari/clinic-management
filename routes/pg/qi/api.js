...
router.post('/uploadImageMob', (req, res) => {
    mapper(dbSchema.insUpdImage, req.body, req.cParams).then((response) => {
        res.json(response);
    }).catch((error) => {
        res.status(400).send(error);
...
router.post('/insUpdEleWtrCon', (req, res) => {
	if(req.body.mode!='undefined'&&req.body.mode=='mob')   req.body.elec_water_consumtn=JSON.stringify(req.body.elec_water_consumtn)
    mapper(dbSchema.insUpdEleWtrCon, req.body, req.cParams).then((response) => {
		//console.log("insUpdEleWtrCon",response)
        res.json(response);
    }).catch((error) => {
        res.status(400).send(error);
    });
...
router.post('/insUpdCrtlEquWrkpInfo', (req, res) => {
	if(req.body.mode!='undefined'&&req.body.mode=='mob')   req.body.critical_equipment_info=JSON.stringify(req.body.critical_equipment_info)
    mapper(dbSchema.insUpdCrtlEquWrkpInfo, req.body, req.cParams).then((response) => {
		//console.log("insUpdCrtlEquWrkpInfo",response)
        res.json(response);
    }).catch((error) => {
        res.status(400).send(error);
    });
...
router.post('/insUpdEmpWrkngDignWrkpInfo', (req, res) => {
	if(req.body.mode!='undefined'&&req.body.mode=='mob')   req.body.emp_working_dign_wrkp_info=JSON.stringify(req.body.emp_working_dign_wrkp_info)
    mapper(dbSchema.insUpdEmpWrkngDignWrkpInfo, req.body, req.cParams).then((response) => {
        //console.log("insUpdEmpWrkngDignWrkpInfo",response)
		res.json(response);
    }).catch((error) => {
        res.status(400).send(error);
    });
...
router.post('/insUpdBioMedEngSrvWrkpInfo', (req, res) => {
	if(req.body.mode!='undefined'&&req.body.mode=='mob')   req.body.bio_medical_engg_service_wrkp_info=JSON.stringify(req.body.bio_medical_engg_service_wrkp_info)
    mapper(dbSchema.insUpdBioMedEngSrvWrkpInfo, req.body, req.cParams).then((response) => {
		//console.log("insUpdBioMedEngSrvWrkpInfo",response)
        res.json(response);
    }).catch((error) => {
        res.status(400).send(error);
    });
...
router.post('/insUpdPrcRepCliDiagWrkpInfo', (req, res) => {
	if(req.body.mode!='undefined'&&req.body.mode=='mob')   req.body.percentage_of_rep_clincal_dignosis=JSON.stringify(req.body.percentage_of_rep_clincal_dignosis)
    mapper(dbSchema.insUpdPrcRepCliDiagWrkpInfo, req.body, req.cParams).then((response) => {
		console.log("insUpdPrcRepCliDiagWrkpInfo",response)
        res.json(response);
    }).catch((error) => {
        res.status(400).send(error);
    });
...
router.post('/insUpdIncdntRptng', (req, res) => {
	if(req.body.mode!='undefined'&&req.body.mode=='mob')   req.body.incident_reporting_info=JSON.stringify(req.body.incident_reporting_info)
		//console.log("insUpdIncdntRptng",req.body)
    mapper(dbSchema.insUpdIncdntRptng, req.body, req.cParams).then((response) => {
		//console.log("insUpdIncdntRptng_response",response)
        res.json(response);
    }).catch((error) => {
		//console.log("insUpdIncdntRptng_response",error)
        res.status(400).send(error);
    });
});
