...
router.get('/shSms', (req, res) => {
	var clientName = req.query.CLIENT.toUpperCase();
	//manually this service will be stopped by raja reddy
	clientName=1;
	if( (clientName === 'PSIMS') || (clientName === 'KIMSKON') || (clientName === 'KIMSSEC')|| (clientName === 'KDH') )
	{
		const clientDet = getOrgLoc(clientName);
		if (clientDet.ORG_ID && clientDet.LOC_ID && req.query.MOBILE_NO && req.query.MESSAGE) {
			commonSmsEmail.smsWithTemplate(clientDet.ORG_ID, clientDet.LOC_ID, req.query.MOBILE_NO, req.query.MESSAGE).then((resp) => {
					var emailMESSAGE;
		...
		else return res.status(400).send({status:400, message:"provide valid paramates"});
	}
	else{
		return res.status(400).send({status:400, message:"provide valid paramates"});
	}
});
