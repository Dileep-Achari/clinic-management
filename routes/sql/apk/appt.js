...
//const ips=['::ffff:172.31.31.107']
//router.all(ipfilter(ips,{mode:'allow'}))
...
router.post('/genToken', (req, res) => {
    if (req.body.SESSION_ID && req.body.ORG_ID && req.body.LOC_ID) {
        res.send(token.createToken({ IP_SESSION_ID: req.body.SESSION_ID, ORG_ID: req.body.ORG_ID, LOC_ID: req.body.LOC_ID }));
    }
    else {
        res.send("acepted paramates are SESSION_ID, ORG_ID, LOC_ID");
    }
});
