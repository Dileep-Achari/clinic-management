...
/*async function readAndDelTokensData() {
    ...
    // _.each(_tokenData, (_tkns) => {
    //     let recStatus = await axiosFun('http://localhost', _tkns);
    // });

    let _records = await redisApiCall('TOKENS', 'delKey');
    if (!_records.success) {
        return {
            "status": "FAIL",
            "data": [],
...
function redisApiCall(key, method) {
    return new Promise((resolve, reject) => {
        //   key = 'TOKENS';
        axios.post(appConfig.REDIS_URL + method, { key: key }).then((resp) => {
            if (resp && resp.status == 200 && resp.data)
                resolve({ "success": true, "data": resp.data || [] });
            else resolve({ "success": true, "data": [] });
        }).catch((err) => {
...
router.post('/sign-in', async (req, res) => {
    try {
        if (req.body.userName && req.body.password && req.body.counter && req.body.includes && req.body.includes.length > 0) {
                    ...
                    if (req.body.status !== 'REQUEUE') {
                        ...
                        if (Object.keys(_filteredData).length > 0) {
                            if ((req.body.status === 'SKIPPED' && Object.keys(_activeRecords).length === 0)) {
                                ...
                                //     return item.recId;
                                // });
                               // console.log("_sortByTokns", _sortByTokns);
