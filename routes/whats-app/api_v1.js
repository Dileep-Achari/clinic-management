...
const {appType,getParams} =require("./common");


const filename="D://waData.txt"//D://hosting/prod/node/apk/public/log/whatsapp/waData.txt
let returnOpt = {
    status: 200,
    data: [],
                  ...
                  //  console.log("data-osapi", data);
                    ...
                    if (req.hostData){
                        if(req.body.type&& appType.indexOf(req.body.type)>0){
                            ...
                            return res.status(400).json("type is not a valid")
                        }
                    }
...
router.post('/msgWithTemplate',async (req,res)=>{
    ...
    if(msgData.status==1){
        ...
        let _msgParams= getParams(req.body.type,params,msgData)
        let {gData,gError}=await sendMsg(`${msgData.data.hData.BASE_URL}${msgData.data.tData.url}`,_msgParams,config);
		//console.log(gError.data.requestError.serviceException);
        if(gError){
            ...
            fs.appendFileSync(filename,`\n response Data:- ${gData}`);
            if(gData.messages.status.groupId==5) {
            return res.status(200).json({"message":gData.messages.status.description});
            }
            else{
